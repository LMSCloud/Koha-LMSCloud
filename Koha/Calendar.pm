...
sub single_holidays {
    ...
    unless ($single_holidays) {
        ...
        my $branches_sth = $dbh->prepare('SELECT distinct(branchcode) FROM special_holidays');
        $branches_sth->execute();
        while ( my $br = $branches_sth->fetchrow ) {
            my $single_holidays_sth =
                $dbh->prepare('SELECT day, month, year FROM special_holidays WHERE branchcode = ? AND isexception = 0');
