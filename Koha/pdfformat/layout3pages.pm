...
sub printorders {
    ...
    for my $basket (@$baskets) {
        ...
        my $arrbasket;
        my @keys = (
            'Document',       'Qty', 'RRP tax exc.', 'RRP tax inc.', 'Discount', 'Discount price', 'Tax rate',
            'Total tax exc.', 'Total tax inc.'
        );
        for my $bkey (@keys) {
            push( @$arrbasket, $bkey );
        }
...
sub printbaskets {
    ...
    # header of the table
    my @keys = (
        'Lot', 'Basket (No.)', 'Total RRP tax exc.', 'Total RRP tax inc.', 'Tax rate', 'Tax', 'Total discount',
        'Total tax exc.', 'Total tax inc.'
    );
    for my $bkey (@keys) {
        push( @$arrbasket, $bkey );
    ...
    my (
        $grandtotal_rrp_tax_included, $grandtotal_rrp_tax_excluded, $grandtotal_tax_included,
        $grandtotal_tax_excluded,     $grandtotaltax_value,         $grandtotaldiscount
    );
    ...
    for my $basket (@$hbaskets) {
        ...
        $arrbasket = undef;
        my (
            $total_rrp_tax_excluded, $total_rrp_tax_included, $total_tax_excluded, $total_tax_included,
            $totaltax_value,         $totaldiscount
        );
...
sub printhead {
    ...
    $text->text(
        join( ' ', $billing_library->branchzip, $billing_library->branchcity, $billing_library->branchcountry ) );
    ...
    if ($freedeliveryplace) {
        ...
        $text->text(
            join( ' ', $delivery_library->branchzip, $delivery_library->branchcity, $delivery_library->branchcountry )
        );
    }
