package Koha::pdfformat::layout2pagesdinde2;
...
#
# Adapted to the needs of Stadtbibliothek Karlsruhe
# by Anna Christina Naß <christina.nass@kultur.karlsruhe.de>
# 2024-07-29
#
...
sub printpage1 {
    ...
    my $sender =
          $delivery_library->branchname . " - "
        . $delivery_library->branchaddress1 . " - "
        . $delivery_library->branchzip . " "
        . $delivery_library->branchcity;
    ...
    $text->font( $pdf->corefont( "Arial", -encoding => "utf8" ), 4 / mm );
    $text->translate( 25 / mm, ( $height - 140 ) / mm );
    ...
    if ($freedeliveryplace) {
        ...
        my $start = 150;
        my @fdp   = (
            $delivery_library->branchname,     $delivery_library->branchaddress1,
    ...
    my $start = 150;
    my @pbi   = (
        $billing_library->branchname,     $billing_library->branchaddress1,
    ...
    for my $basket (@$hbaskets) {
        ...
        foreach my $ord (@$ords) {
            ...
            $totaldiscount +=
                ( get_rounded_price( $ord->{rrp_tax_excluded} ) - get_rounded_price( $ord->{ecost_tax_excluded} ) ) *
                $ord->{quantity};
    ...
    ## data output
    $text->font( $pdf->corefont( "Arial", -encoding => "utf8" ), 4 / mm );
    $text->translate( 25 / mm, ( $height - 175 ) / mm );
    $text->text( "Zusammenfassung", -underline => 'auto' );

    #$text->text(($basketgroup->{'name'}."(".$basketgroup->{'id'}."):"),-underline=>'auto') ;

    $text->translate( 75 / mm, ( $height - 175 ) / mm );
    $text->text("Summe");
    $text->translate( ( $width - 40 ) / mm, ( $height - 175 ) / mm );
    ...
    my $content = $page->gfx();
    $content->move( 75 / mm, ( $height - 181 ) / mm );
    ...
    $content->move( 75 / mm, ( $height - 191 ) / mm );
    $content->hline( ( $width - 40 ) / mm );
    $content->stroke;
    ...
    $content->hline( ( $width - 40 ) / mm );
    $content->stroke;
    $content->move( 75 / mm, ( $height - 197 ) / mm );
    $content->hline( ( $width - 40 ) / mm );
    $content->stroke;

    ############################################################################
    ## delivery comment for basketgroup, if not empty:
    if ( $basketgroup->{deliverycomment} ne "" ) {
        $text->font( $pdf->corefont( "Arial", -encoding => "utf8" ), 4 / mm );
        $text->translate( 25 / mm, ( $height - 205 ) / mm );
        $text->lead( 5 / mm );
        $text->paragraph( "Hinweis: " . $basketgroup->{deliverycomment}, ( $width - 45 ) / mm, 30 / mm );
    }
    ...
    $text->translate( 25 / mm, ( $height - 245 ) / mm );
...
sub printorders {
    ...
    for my $basket (@$baskets) {
        ...
        $text->font( $pdf->corefont( "Arial", -encoding => "utf8" ), 6 / mm );
        $text->translate( 25 / mm, ( $height - 15 ) / mm );
        $text->text( "Bestellung: " . $basketgroup->{'name'} . " (" . $basketgroup->{'id'} . ")" );

        ########################################################################
        # print note for basket, if not empty:
        if ( $basket->{booksellernote} ne "" ) {
            $text->font( $pdf->corefont( "Arial", -encoding => "utf8" ), 4 / mm );
            $text->translate( 25 / mm, ( $height - 20 ) / mm );
            $text->lead( 5 / mm );
            $text->paragraph( "Hinweis: " . $basket->{booksellernote}, ( $width - 45 ) / mm, 100 / mm );
        }
        ...
        foreach my $line ( @{ $orders->{ $basket->{basketno} } } ) {
            ...
            push(
                ...
                $line->{quantity},
                Koha::Number::Price->new( $line->{rrp_tax_included} )->format,    # statt excluded
...
sub printpdf {
    ...
    #   my $pdf_template = C4::Context->config('intrahtdocs') . '/' . C4::Context->preference('template') . '/pdf/layout2pagesdin2.pdf';
