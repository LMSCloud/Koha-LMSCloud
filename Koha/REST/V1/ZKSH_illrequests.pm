...
sub handleZKSHRequest {
    ...
    if ($apiparams) {
        ...
        if ( $apiparams->{Art} && $apiparams->{Art} eq "RLV-Bestellung" ) {
            ...
            $args->{'VerfuegbarkeitsSuche'} =
                $apiparams->{VerfuegbarkeitsSuche},    # string containig JSON availability search parameters

                # fields for table illrequests
                $args->{'branchcode'} =
                getBranchcodeFromOrderBranchIsil( C4::Context->preference("ILLDefaultBranch"), undef );
            ...
            for my $attribute (
                'LeserNummer',         'LeserEmail',       'LeserTelNr',  'LeserAnmerkung', 'Serientitel',
                'Medienart',           'Interessenkreis',  'URLzkshBase', 'URLzksh', 'BestelltAm', 'BestelltBeiSigel',
                'BestelltBeiName',     'BestelltVonSigel', 'BestelltVonName', 'LieferungGeplant', 'Lieferweg',
                'VerfuegbarkeitDatum', 'VerfuegbarkeitText'
                )
            {
        ...
        if ( $apiparams->{Art} && $apiparams->{Art} eq "RLV-Bestellinfo" ) {
            ...
            # fields for table illrequests
            $args->{'branchcode'} = getBranchcodeFromOrderBranchIsil(
                C4::Context->preference("ILLDefaultBranch"),
                $apiparams->{'BestelltVonSigel'}
            );
            $args->{'medium'}  = "Book";                    # XXXWH lt. Norbert hat man es immer mit 'Book' zu tun
            $args->{'orderid'} = $apiparams->{BestellID};
            ...
            for my $attribute (
                'LeserNummer',      'LeserEmail',      'LeserTelNr',       'LeserAnmerkung', 'Serientitel',
                'Medienart',        'Interessenkreis', 'VerlagJahrSonst',  'URLzkshBase',    'URLzksh', 'BestelltAm',
                'BestelltBeiSigel', 'BestelltBeiName', 'LieferungGeplant', 'Lieferweg',      'VerfuegbarkeitDatum',
                'VerfuegbarkeitText'
                )
            {
                if ( defined $apiparams->{$attribute} && length( $apiparams->{$attribute} ) ) {
            ...
            # fields for table illrequestattributes
            for my $attribute (
                'Bemerkung',           'BestelltBeiSigel', 'BestelltBeiName', 'LieferungGeplant', 'Lieferweg',
                'VerfuegbarkeitDatum', 'VerfuegbarkeitText'
                )
            {
                if ( defined $apiparams->{$attribute} && length( $apiparams->{$attribute} ) ) {
            ...
            $backend_result =
                $zksh_illbackend->_backend_capability( "confirmed", { request => $zksh_illbackend, other => $args } );
            ...
            $args->{'orderid'} = $apiparams->{BestellID};
            $backend_result =
                $zksh_illbackend->_backend_capability( "erase", { request => $zksh_illbackend, other => $args } );
            ...
            || !$backend_result->{value}->{request}->illrequest_id() )
        {
            ...
            if ( $backend_result->{status} eq "invalid_borrower" ) {
                ...
                if ( $apiparams->{Art} eq "RLV-Bestellung" ) {
                    $cmd->{'err_text'} = "No ordering library found searching for Sigel '"
                        . scalar $apiparams->{BestelltVonSigel} . "'.";
                } else {
                    $cmd->{'err_text'} =
                          "No patron found searching for name '"
                        . scalar $apiparams->{LeserName}
                        . "' or cardnumber '"
                        . scalar $apiparams->{LeserNummer} . "'.";
                }
            } elsif ( $backend_result->{status} eq "illrequest_not_found" ) {
                $cmd->{'err_type'} = 'ILLREQUEST_NOT_FOUND';
                $cmd->{'err_text'} =
                    "No ILL request found searching for orderid '" . scalar $apiparams->{BestellID} . "'.";
            } else {
                $cmd->{'err_type'} = 'ILLREQUEST_NOT_CREATED';
                if ( $apiparams->{Art} eq "RLV-Bestellung" ) {
                    $cmd->{'err_text'} =
                          "The Koha illrequest for the title with BZ-ID '"
                        . scalar $apiparams->{BZID}
                        . "' or ISBN '"
                        . scalar $apiparams->{ISBN}
...
sub getBranchcodeFromOrderBranchIsil {
    ...
    if ($iLLDefaultBranchPreference)
    {    # e.g. for Norderstedt: '1|672:1|673:3|674:2|676:4' or more compact: '1|673:3|674:2|676:4'
