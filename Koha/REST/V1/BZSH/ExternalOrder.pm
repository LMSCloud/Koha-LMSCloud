...
sub handleBZSHExternalOrderRequest {
    ...
    if (   $orderReqest
        && exists( $orderReqest->{external_order_id} )
        && exists( $orderReqest->{order_type} )
        && $orderReqest->{external_order_id}
        && $orderReqest->{order_type} )
    {
    ...
    if ( $orderReqest && exists( $orderReqest->{order_items} ) ) {
        ...
        foreach my $orderItem ( @{ $orderReqest->{order_items} } ) {
            ...
            if ( $orderItem && exists( $orderItem->{biblionumber} ) ) {
                ...
                push @errors,
                    "Bibliographic record with ID " . $orderItem->{biblionumber} . " of order item $i does not exist."
                    if ( !$record );
            } else {
            ...
            if ( $orderItem && exists( $orderItem->{copies} ) ) {
                push @errors, "Count of copies value " . $orderItem->{copies} . " is not correct for order item $i."
                    if ( $orderItem->{copies} !~ /^[0-9]+$/ || $orderItem->{copies} <= 0 );
            } else {
            ...
            if ( $orderItem && exists( $orderItem->{price} ) ) {
                push @errors, "Price value " . $orderItem->{price} . " is not correct for order item $i."
                    if ( $orderItem->{price} !~ /^[0-9]+(\.[0-9]+)?$/ || $orderItem->{price} < 0.0 );
            } else {
    ...
    if ( scalar(@errors) ) {
        ...
        if ($externalOrder) {
            ...
            if ( C4::Context->preference("ExternalOrderProcessingService") ) {
                ...
                if ( !$resp->is_success ) {
                    carp
                        "Error calling external service '$service' to launch external order processing od external order id $externalOrderId. Call returned code "
                        . $resp->code
                        . " with response '"
                        . $resp->as_string . "'";
                }
