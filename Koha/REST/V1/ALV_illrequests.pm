...
sub handleALVRequest {
    ...
    if ($modelparams) {
        ...
        print STDERR
            "Koha::REST::V1::ALV_illrequests::handleZKSHRequest() after alv_illbackend->backend_create() error:"
            . scalar $backend_result->{error}
            . ": status:"
            . scalar $backend_result->{status}
            . ": message:"
            . scalar $backend_result->{message}
            . ": method:"
            . scalar $backend_result->{method}
            . ": stage:"
            . scalar $backend_result->{stage}
            . ": next:"
            . scalar $backend_result->{next} . ":\n";
            ...
            || !$backend_result->{value}->{request}->illrequest_id() )
        {
            ...
            if ( $backend_result->{status} eq "invalid_borrower" ) {
                ...
                $cmd->{'err_text'} =
                    "No ordering library found searching for ISIL '" . scalar $modelparams->{BestelltVonSigel} . "'.";
            } else {
                $cmd->{'err_type'} = 'ILLREQUEST_NOT_CREATED';
                $cmd->{'err_text'} =
                      "The Koha illrequest for the title '"
                    . scalar $modelparams->{Titel}
                    . "' or ISBN '"
                    . scalar $modelparams->{ISBN}
