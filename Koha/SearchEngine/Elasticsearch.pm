...
sub _field_mappings {
    my (
        $_self, $facet, $suggestible, $sort, $search, $filter, $target_name, $target_type, $range, $indicator1,
        $indicator2
    ) = @_;
    my %mapping_defaults = ();
    my @mappings;
    ...
    $self->_foreach_mapping(
        sub {
            ...
            if ( $marc_field =~ $field_spec_regexp ) {
                ...
                my @mappings = $self->_field_mappings(
                    $facet,      $suggestible, $sort, $search, $filter, $name, $type, $range,
