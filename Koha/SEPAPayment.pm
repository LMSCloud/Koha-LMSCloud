...
sub new {
    ...
    $self->{nomail} =
        $nomail;    # for development and debugging only (do not enqueue letter but print its content to STDOUT)
    $self->{sepaSysPrefs} = {};
    $self->{errorMsg}     = $self->checkSepaDirectDebitConfiguration();
    $self->{lettercode}   = '';                                           # default: generate no notification
    $self->{lettercode}   = $self->{sepaSysPrefs}->{SepaDirectDebitBorrowerNoticeLettercode}
        if $self->{sepaSysPrefs}->{SepaDirectDebitBorrowerNoticeLettercode};
    $self->{lettercode} = $lettercode
        if $lettercode
        ; # If function parameter $lettercode is set, it will override the entry read from system preference 'SepaDirectDebitBorrowerNoticeLettercode'.
    ...
    if (   $self->{cashRegisterConfig}->{withoutCashRegisterManagement} == 0
        && $self->{cashRegisterConfig}->{cashRegisterNeedsToBeOpened} )
    {     # required cash register is not opened
        $self->{errorMsg} .= "Invalid cash register configuration for SEPA direct debit. ( cash register name:"
            . (
              $self->{cashRegisterConfig}->{paymentsSepaDirectDebitCashRegisterName}
            ? $self->{cashRegisterConfig}->{paymentsSepaDirectDebitCashRegisterName}
            : 'undef'
            )
            . ": manager cardnumber:"
            . (
              $self->{cashRegisterConfig}->{paymentsSepaDirectDebitCashRegisterManagerCardnumber}
            ? $self->{cashRegisterConfig}->{paymentsSepaDirectDebitCashRegisterManagerCardnumber}
            : 'undef'
            ) . ":)";
    }
    ...
    $self->{paymentInstructionFileNameNoTransactions} =
        $self->{paymentInstructionFileName} . '_no_payment_instructions.xml';
    ...
    if ( -e $self->{paymentInstructionFileName} || -e $self->{paymentInstructionFileNameNoTransactions} ) {
        $self->{errorMsg} .=
            "Payment instruction output file:" . $self->{paymentInstructionFileName} . "* already exists. ";
    }
    ...
    print STDERR "Koha::SEPAPayment::new() self->{sepaSysPrefs}:" . Dumper( $self->{sepaSysPrefs} ) . ":\n"
        if $self->{verbose} > 1;
    print STDERR "Koha::SEPAPayment::new() self->{paymentInstructionFileName}:"
        . $self->{paymentInstructionFileName} . ":\n"
        if $self->{verbose} > 1;
...
sub checkSepaDirectDebitConfiguration {
    ...
    my @sepaSysPrefRecords = (
        ...
        { variable => 'SepaDirectDebitCreditorName', mandatory => 1 }
        ,    # e.g. 'Stadtbuecherei Norderstedt' (max. length 27/35/70 chars according to different specifications)
        { variable => 'SepaDirectDebitInitiatingPartyName', mandatory => 1 },    # e.g. 'STADTBUECHEREI NORDERSTEDT'
        { variable => 'SepaDirectDebitMessageIdHeader',     mandatory => 0 }
        ,    # e.g. 'Lastschrift Stadtbuecherei-' (current date will be appended in form yyyymmdd)
        { variable => 'SepaDirectDebitRemittanceInfo', mandatory => 1 },  # e.g. 'Jahresentgelt' (max. length 140 chars)
        { variable => 'SepaDirectDebitBorrowerNoticeLettercode', mandatory => 0 }
        ,    # e.g. 'SEPA_NOTE_CHARGE'. Value '' or undef indicates that the library has deactivated the notification.
        { variable => 'SepaDirectDebitBorrowerNoticeMtt', mandatory => 0 }
        , # e.g. 'print'. Usually not existing. Set on request only for customers that ask for 'print' output of the borrower direct debit notice in any case.
        { variable => 'SepaDirectDebitAccountTypes', mandatory => 1 },    # e.g. 'ACCOUNT|ACCOUNT_RENEW|MANUAL|OVERDUE'
        { variable => 'SepaDirectDebitMinFeeSum',    mandatory => 0 }
        ,    # e.g. '5.00'. Value '' or undef results in default value 0.01.
        { variable => 'SepaDirectDebitLocalInstrumentCode', mandatory => 1 }
        , # e.g. 'CORE'. 'COR1'.'B2B' (CORE = SEPA Basis-Lastschrift (Verbraucher); COR1 entspricht CORE, jedoch mit auf 1 Bankarbeitstag reduzierter Bearbeitungszeit)
        {
            variable  => 'SepaDirectDebitPaymentInstructionFileName',
            mandatory => 1
        }    # e.g. 'pain.008.<<cc>><<yy>><<mm>><<dd>>.xml'

    );

    $self->{sepaSysPrefs} = {};
    foreach my $sepaSysPrefRecord (@sepaSysPrefRecords) {
        $self->{sepaSysPrefs}->{ $sepaSysPrefRecord->{variable} } =
            C4::Context->preference( $sepaSysPrefRecord->{variable} );
        print STDERR "Koha::SEPAPayment::checkSepaDirectDebitConfiguration() self->{sepaSysPrefs}->{"
            . $sepaSysPrefRecord->{variable} . "}:"
            . $self->{sepaSysPrefs}->{ $sepaSysPrefRecord->{variable} } . ":\n"
            if $self->{verbose} > 1;
...
sub readCashRegisterConfiguration {
    ...
    $cashRegisterConfig->{withoutCashRegisterManagement} =
        1;    # default: avoiding cash register management in Koha::Account->pay()
    $cashRegisterConfig->{cashRegisterNeedsToBeOpened} = 0;
    $cashRegisterConfig->{cash_register_manager_id} =
        '';    # borrowernumber of manager of cash register for SEPA direct debit payments

    if ( C4::Context->preference("ActivateCashRegisterTransactionsOnly") ) {
        $cashRegisterConfig->{paymentsSepaDirectDebitCashRegisterName} =
            C4::Context->preference('SepaDirectDebitCashRegisterName');
        $cashRegisterConfig->{paymentsSepaDirectDebitCashRegisterManagerCardnumber} =
            C4::Context->preference('SepaDirectDebitCashRegisterManagerCardnumber');
        print STDERR "Koha::SEPAPayment::readCashRegisterConfiguration() paymentsSepaDirectDebitCashRegisterName:"
            . (
            defined( $cashRegisterConfig->{paymentsSepaDirectDebitCashRegisterName} )
            ? $cashRegisterConfig->{paymentsSepaDirectDebitCashRegisterName}
            : 'undef'
            )
            . ": paymentsSepaDirectDebitCashRegisterManagerCardnumber:"
            . (
            defined( $cashRegisterConfig->{paymentsSepaDirectDebitCashRegisterManagerCardnumber} )
            ? $cashRegisterConfig->{paymentsSepaDirectDebitCashRegisterManagerCardnumber}
            : 'undef'
            )
            . ":\n"
        ...
        if (   length( $cashRegisterConfig->{paymentsSepaDirectDebitCashRegisterName} )
            || length( $cashRegisterConfig->{paymentsSepaDirectDebitCashRegisterManagerCardnumber} ) )
        {
            ...
            $cashRegisterConfig->{cash_register_manager} = Koha::Patrons->search(
                { cardnumber => $cashRegisterConfig->{paymentsSepaDirectDebitCashRegisterManagerCardnumber} } )->next();
            if ( $cashRegisterConfig->{cash_register_manager} ) {
                $cashRegisterConfig->{cash_register_manager_id} =
                    $cashRegisterConfig->{cash_register_manager}->borrowernumber();
                $cashRegisterConfig->{cash_register_manager_branchcode} =
                    $cashRegisterConfig->{cash_register_manager}->branchcode();
                print STDERR "Koha::SEPAPayment::readCashRegisterConfiguration() cash_register_manager_id:"
                    . (
                    defined( $cashRegisterConfig->{cash_register_manager_id} )
                    ? $cashRegisterConfig->{cash_register_manager_id}
                    : 'undef'
                    )
                    . ": cash_register_manager_branchcode:"
                    . (
                    defined( $cashRegisterConfig->{cash_register_manager_branchcode} )
                    ? $cashRegisterConfig->{cash_register_manager_branchcode}
                    : 'undef'
                    )
                    . ":\n"
                ...
                $cashRegisterConfig->{cash_register_mngmt} = C4::CashRegisterManagement->new(
                    $cashRegisterConfig->{cash_register_manager_branchcode},
                    $cashRegisterConfig->{cash_register_manager_id}
                );

                if ( $cashRegisterConfig->{cash_register_mngmt} ) {
                    my $openedCashRegister = $cashRegisterConfig->{cash_register_mngmt}
                        ->getOpenedCashRegisterByManagerID( $cashRegisterConfig->{cash_register_manager_id} );
                    if ( defined $openedCashRegister ) {
                        print STDERR "Koha::SEPAPayment::readCashRegisterConfiguration() opened cash_register_name:"
                            . (
                            defined( $openedCashRegister->{'cash_register_name'} )
                            ? $openedCashRegister->{'cash_register_name'}
                            : 'undef'
                            )
                            . ":\n"
                            if $self->{verbose} > 1;

                        if ( $openedCashRegister->{'cash_register_name'} eq
                            $cashRegisterConfig->{paymentsSepaDirectDebitCashRegisterName} )
                        {
                            $cashRegisterConfig->{cashRegisterNeedsToBeOpened} = 0;
                            $cashRegisterConfig->{cash_register_id} = $openedCashRegister->{'cash_register_id'};
                        } else {
                            $cashRegisterConfig->{cash_register_mngmt}->closeCashRegister(
                                $openedCashRegister->{'cash_register_id'},
                                $cashRegisterConfig->{cash_register_manager_id}
                            );
                        }
                    }
                    if ( $cashRegisterConfig->{cashRegisterNeedsToBeOpened} ) {
                        ...
                        $cashRegisterConfig->{cash_register_id} =
                            $cashRegisterConfig->{cash_register_mngmt}->readCashRegisterIdByName(
                            $cashRegisterConfig->{paymentsSepaDirectDebitCashRegisterName} );
                        print STDERR "Koha::SEPAPayment::readCashRegisterConfiguration() cash_register_id:"
                            . (
                            defined( $cashRegisterConfig->{cash_register_id} )
                            ? $cashRegisterConfig->{cash_register_id}
                            : 'undef'
                            )
                            . ":"
                        ...
                        if (
                            defined $cashRegisterConfig->{cash_register_id}
                            && $cashRegisterConfig->{cash_register_mngmt}->canOpenCashRegister(
                                $cashRegisterConfig->{cash_register_id}, $cashRegisterConfig->{cash_register_manager_id}
                            )
                            )
                        {
                            my $opened = $cashRegisterConfig->{cash_register_mngmt}->openCashRegister(
                                $cashRegisterConfig->{cash_register_id},
                                $cashRegisterConfig->{cash_register_manager_id}
                            );
                            if ($opened) {    # 0/1
                            ...
                            print STDERR
                                "Koha::SEPAPayment::readCashRegisterConfiguration() cash_register_mngmt->openCashRegister("
                                . $cashRegisterConfig->{cash_register_manager_branchcode} . ", "
                                . $cashRegisterConfig->{cash_register_manager_id}
                            ...
                            my $mess =
                                "Koha::SEPAPayment::readCashRegisterConfiguration(): error when trying to open cash register paymentsSepaDirectDebitCashRegisterName:$cashRegisterConfig->{paymentsSepaDirectDebitCashRegisterName}:";
...
sub getErrorMsg {
    ...
    print STDERR "Koha::SEPAPayment::getErrorMsg() returns errorMsg:"
        . ( defined( $self->{errorMsg} ) ? $self->{errorMsg} : 'undef' ) . ":\n"
        if $self->{verbose} > 1;
...
sub updateResultHash {
    ...
    print STDERR "Koha::SEPAPayment::updateResultHash(resultHashName:$resultHashName:) START currentHitBorrowernumber:"
        . ( defined($currentHitBorrowernumber) ? $currentHitBorrowernumber : 'undef' ) . ":\n"
        if $self->{verbose} > 1;
    ...
    foreach my $borrowersKey ( sort keys %{ $currentFeeHit->{borrowers} } ) {
        $self->{$resultHashName}->{$currentHitBorrowernumber}->{borrowers}->{$borrowersKey} =
            $currentFeeHit->{borrowers}->{$borrowersKey};
    }

    # accountlines keys: all selected accountlines.accountlines_id with corresponding amountoutstanding and debit_type_code
    my $currentHitAccountlinesId = $currentFeeHit->{accountlines}->{accountlines_id};
    $self->{$resultHashName}->{$currentHitBorrowernumber}->{accountlines}->{$currentHitAccountlinesId}
        ->{accountlines_id} = $currentFeeHit->{accountlines}->{accountlines_id};
    $self->{$resultHashName}->{$currentHitBorrowernumber}->{accountlines}->{$currentHitAccountlinesId}
        ->{debit_type_code} = $currentFeeHit->{accountlines}->{debit_type_code};
    $self->{$resultHashName}->{$currentHitBorrowernumber}->{accountlines}->{$currentHitAccountlinesId}
        ->{amountoutstanding} = $currentFeeHit->{accountlines}->{amountoutstanding};
    ...
    $self->{$resultHashName}->{$currentHitBorrowernumber}->{accountlinesSumAmountoutstanding} +=
        $currentFeeHit->{accountlines}->{amountoutstanding};
    ...
    if (   $self->{$resultHashName}->{$currentHitBorrowernumber}->{accountlinesMinId} == 0
        || $currentFeeHit->{accountlines}->{accountlines_id} <
        $self->{$resultHashName}->{$currentHitBorrowernumber}->{accountlinesMinId} )
    {
        $self->{$resultHashName}->{$currentHitBorrowernumber}->{accountlinesMinId} =
            $currentFeeHit->{accountlines}->{accountlines_id};
    }
    ...
    foreach my $borrowerAttributesKey ( sort keys %{ $currentFeeHit->{borrower_attributes} } ) {
        $self->{$resultHashName}->{$currentHitBorrowernumber}->{borrower_attributes}->{$borrowerAttributesKey} =
            $currentFeeHit->{borrower_attributes}->{$borrowerAttributesKey};
    }

    print STDERR
        "Koha::SEPAPayment::updateResultHash(resultHashName:$resultHashName:) END self->{$resultHashName}->{$currentHitBorrowernumber}:"
        . Dumper( $self->{$resultHashName}->{$currentHitBorrowernumber} ) . ":\n"
        if $self->{verbose} > 1;
}
...
sub renewMembershipForSepaDirectDebitPatrons {
    ...
    print STDERR "Koha::SEPAPayment::renewMembershipForSepaDirectDebitPatrons() START expiryAfterDays:"
    ...
    print STDERR "Koha::SEPAPayment::renewMembershipForSepaDirectDebitPatrons dateFrom:"
        . $dateFrom
        . ": dateUntil:"
        . $dateUntil . ":\n"
        if $self->{verbose} > 1;
    ...
    $params->{'borrower_attributes.code'}      = { "=" => 'SEPA' };
    $params->{'borrower_attributes.attribute'} = { "=" => '1' };

    print STDERR "Koha::SEPAPayment::renewMembershipForSepaDirectDebitPatrons() is calling patrons->search() params:"
        . Dumper($params) . ":\n"
        if $self->{verbose} > 1;
    ...
    print STDERR "Koha::SEPAPayment::renewMembershipForSepaDirectDebitPatrons() patronsRS.count:"
        . $patronsRS->count . ":\n"
        if $self->{verbose} > 0;
    ...
    while ( my $patronHit = $patronsRS->next() ) {
        ...
        print STDERR "Koha::SEPAPayment::renewMembershipForSepaDirectDebitPatrons renew_account for borrowernumber:"
            . $patronHit->borrowernumber
            . ": (categorycode:"
            . $patronHit->categorycode
            . ": old dateexpiry:"
            . $olddateexpiry
            . ":) has returned new dateexpiry:"
            . (
            defined($newdateexpiry)
            ? output_pref( { 'dt' => $newdateexpiry, dateformat => 'iso', dateonly => 1 } )
            : 'undef'
            )
            . ":\n"
            if $self->{verbose} > 0;
        $renewMembershipCount += 1;
        if ( defined($newdateexpiry)
            && ( output_pref( { 'dt' => $newdateexpiry, dateformat => 'iso', dateonly => 1 } ) ne $olddateexpiry ) )
        {
...
sub paySelectedFeesForSepaDirectDebitPatrons {
    ...
    my $requestedCollectionDate  = dt_from_string->add( days => $sepaDirectDebitDelayDays )
        ;    # date the direct debit will be booked by the bank in the borrowers bank account

    print STDERR
        "Koha::SEPAPayment::paySelectedFeesForSepaDirectDebitPatrons() START params->{sepaDirectDebitDelayDays}:"
        . $params->{sepaDirectDebitDelayDays}
    ...
    my $debit_type_code_sel = "'ACCOUNT','ACCOUNT_RENEW'"
        ; # In the first version of SEPA direct debit payment only membership fees (since 21.05 represented by debit_type_code 'ACCOUNT' and 'ACCOUNT_RENEW') had to be handled.
    my $sepaDirectDebitAccountTypes = $self->{sepaSysPrefs}->{SepaDirectDebitAccountTypes};
    print STDERR "Koha::SEPAPayment::paySelectedFeesForSepaDirectDebitPatrons() sepaDirectDebitAccountTypes:"
        . $sepaDirectDebitAccountTypes . ":\n"
        if $self->{verbose} > 1;
    if ($sepaDirectDebitAccountTypes) {
        $debit_type_code_sel = ''
            ; # In the second version of SEPA direct debit payment all debit_type_codes of interest have to be configured. (e.g. 'ACCOUNT|ACCOUNT_RENEW|MANUAL|OVERDUE')
        my @accountTypes = split( /\|/, $sepaDirectDebitAccountTypes );
        foreach my $accountType (@accountTypes) {
            if ($debit_type_code_sel) {
                $debit_type_code_sel .= ",";
            }
            ...
            if ( $accountType eq 'OVERDUE' ) {
                $overdueSelect =
                    " AND (a.debit_type_code != 'OVERDUE' OR ( a.debit_type_code = 'OVERDUE' AND a.status NOT IN ('UNRETURNED') )) ";
                $overdueSelectAl2 =
                    " AND (al2.debit_type_code != 'OVERDUE' OR ( al2.debit_type_code = 'OVERDUE' AND al2.status NOT IN ('UNRETURNED') )) ";
            }
        }
    }
    ...
    while ( my $openFee = $sth->fetchrow_hashref ) {
        print STDERR "Koha::SEPAPayment::paySelectedFeesForSepaDirectDebitPatrons() Dumper(openFee):"
            . Dumper($openFee) . ":\n"
            if $self->{verbose} > 1;
    ...
    foreach my $borrowernumber ( sort keys %{ $self->{inKohaSelected} } ) {
        my $kohaPaymentId =
            $self->paySelectedFeesOfPatron( $self->{inKohaSelected}->{$borrowernumber}, $requestedCollectionDate );

        $self->{inKohaSelected}->{$borrowernumber}->{accountlinesKohaPaymentId} =
            $kohaPaymentId;    # value 0 indicates failed creation of accountlines record for payment
...
sub paySelectedFeesOfPatron {
    ...
    my $kohaPaymentId = 0;
    print STDERR "Koha::SEPAPayment::paySelectedFeesOfPatron() START accountlines count:"
        ...
        . $requestedCollectionDate . ":\n"
        if $self->{verbose} > 1;
    print STDERR "Koha::SEPAPayment::paySelectedFeesOfPatron() borrowersSelectedFees:"
        . Dumper($borrowersSelectedFees) . ":\n"
        if $self->{verbose} > 1;
    ...
    my $signOk =
        1;    # $self->checkSign($borrowersSelectedFees);    # wei 10.06.2020: no checks of SEPA signature, please!
        ...
        && $borrowersSelectedFees->{accountlinesCount} > 0 )
    {
        ...
        if ( $lines[0] ) {    # found at least one accountlines record
            for ( my $i = 0 ; $i < scalar @lines ; $i += 1 ) {
                print STDERR
                    "Koha::SEPAPayment::paySelectedFeesOfPatron() Dumper(lines[$i]->{_result}->{_column_data}):"
                    . Dumper( $lines[$i]->{_result}->{_column_data} ) . ":\n"
                    if $self->{verbose} > 1;
            }
            my $descriptionText = 'Zahlung (SEPA Lastschrift)';
            my $noteText = output_pref( { 'dt' => $requestedCollectionDate, dateformat => 'iso', dateonly => 1 } )
                ;    # requested collection date (vorgesehenes Datum für Lastschrifteinzug)
            ...
            my $kohaPaymentResult = $account->pay(
                {
                    amount     => $borrowersSelectedFees->{accountlinesSumAmountoutstanding},
                    lines      => \@lines,
                    library_id => $borrowersSelectedFees->{borrowers}->{branchcode}
            ...
            if ( $kohaPaymentResult && defined( $kohaPaymentResult->{payment_id} ) && $kohaPaymentResult->{payment_id} )
            {
                $kohaPaymentId = $kohaPaymentResult->{payment_id};
            }
        }
    }
    if ($kohaPaymentId) {
        ...
        print STDERR
            "Koha::SEPAPayment::paySelectedFeesOfPatron() error: direct debit payment of selected fees for borrower:"
            . $borrowersSelectedFees->{borrowers}->{borrowernumber}
            . ": amount:"
            . $borrowersSelectedFees->{accountlinesSumAmountoutstanding}
            . ": count:"
        ...
        my $errormsg = sprintf(
            'Betrag:%.2f: der Gebühr(en):%s: konnte in Koha nicht bezahlt werden',
            $borrowersSelectedFees->{accountlinesSumAmountoutstanding}, $selectedAccountlinesIds
        );
        push @{ $borrowersSelectedFees->{errormsg} }, $errormsg;
    }
    print STDERR "Koha::SEPAPayment::paySelectedFeesOfPatron() returns kohaPaymentId:"
        . ( $kohaPaymentId ? $kohaPaymentId : 'undef' ) . ":\n"
        if $self->{verbose} > 1;
...
sub checkMandateId {
    ...
    {
        print STDERR "Koha::SEPAPayment::checkMandateId() cardnumber not defined (borrower:"
            . $borrowersSelectedFees->{borrowers}->{borrowernumber} . ":)\n"
            if $self->{verbose} > 0;
...
sub checkIban {
    ...
    if (
        !(
               defined( $borrowersSelectedFees->{borrower_attributes} )
            && defined( $borrowersSelectedFees->{borrower_attributes}->{SEPA_IBAN} )
        )
        )
    {
        print STDERR "Koha::SEPAPayment::checkIban() IBAN not defined (borrower:"
            . $borrowersSelectedFees->{borrowers}->{borrowernumber} . ":)\n"
            if $self->{verbose} > 0;
        ...
        print STDERR "Koha::SEPAPayment::checkIban() SEPA_IBAN:"
            . (
            defined( $borrowersSelectedFees->{borrower_attributes}->{SEPA_IBAN} )
            ? $borrowersSelectedFees->{borrower_attributes}->{SEPA_IBAN}
            : 'undef'
            )
            . ": countryCode:"
            . ( defined($countryCode) ? $countryCode : 'undef' )
            . ": checkSum:"
            . ( defined($checkSum) ? $checkSum : 'undef' )
            . ": BBAN:"
            . ( defined($basicBankAccountNumber) ? $basicBankAccountNumber : 'undef' ) . ":\n"
            if $self->{verbose} > 1;
        ...
        {
            print STDERR "Koha::SEPAPayment::checkIban() invalid IBAN:"
                . $borrowersSelectedFees->{borrower_attributes}->{SEPA_IBAN}
                . ": (borrower:"
                . $borrowersSelectedFees->{borrowers}->{borrowernumber} . ":)\n"
                if $self->{verbose} > 0;
            my $errormsg =
                sprintf( 'IBAN:%s: ist fehlerhaft', $borrowersSelectedFees->{borrower_attributes}->{SEPA_IBAN} );
            push @{ $borrowersSelectedFees->{errormsg} }, $errormsg;
        } else {
            $ret = 1;
        }
...
sub checkBic {
    ...
    if (
        !(
               defined( $borrowersSelectedFees->{borrower_attributes} )
            && defined( $borrowersSelectedFees->{borrower_attributes}->{SEPA_BIC} )
        )
        )
    {
        print STDERR "Koha::SEPAPayment::checkBic() BIC not defined (borrower:"
            . $borrowersSelectedFees->{borrowers}->{borrowernumber} . ":)\n"
            if $self->{verbose} > 0;
        $ret = 1;
    } else {
        ...
        if ( $bicLen == 0 || $bicLen == 8 || $bicLen == 11 ) {
            ...
            print STDERR "Koha::SEPAPayment::checkBic() invalid BIC:"
                . $borrowersSelectedFees->{borrower_attributes}->{SEPA_BIC}
                . ": (borrower:"
                . $borrowersSelectedFees->{borrowers}->{borrowernumber} . ":)\n"
                if $self->{verbose} > 0;
            my $errormsg =
                sprintf( 'BIC:%s: ist fehlerhaft', $borrowersSelectedFees->{borrower_attributes}->{SEPA_BIC} );
            push @{ $borrowersSelectedFees->{errormsg} }, $errormsg;
        }
    }
    return $ret;
...
sub printSepaNotice {
    ...
    print STDERR
        "Koha::SEPAPayment::printSepaNotice() START branchcode:$branchcode: borrowernumber:$borrowernumber: letter_code:$self->{lettercode}:\n"
        if $self->{verbose} > 1;
    ...
    my $accountlinePayment =
        $accountlines->find( { accountlines_id => $borrowersPaidFees->{accountlinesKohaPaymentId} } );
    ...
    my $send_notification = sub {
        ...
        print STDERR
            "Koha::SEPAPayment::printSepaNotice() send_notification() borrowernumber:$borrowernumber: letter_code:$letter_code: mtt:$mtt:\n"
            if $self->{verbose} > 1;
        ...
        unless ($letter) {
            warn
                "Koha::SEPAPayment::printSepaNotice(): Could not find a letter called '$letter_params{'letter_code'}' for $mtt in the '$letter_params{'module'}' module";
            return 0;
        }
        ...
        if ( $self->{nomail} ) {    # for development and debugging only
            ...
            if ( $noticeFees->checkForNoticeFeeRules() == 1 ) {
                ...
                my $noticeFeeRule = $noticeFees->getNoticeFeeRule(
                    $letter_params{branchcode}, $patron->categorycode, $mtt,
                    $letter_code
                );
    ...
    if (   $self->{sepaSysPrefs}->{SepaDirectDebitBorrowerNoticeMtt}
        && $self->{sepaSysPrefs}->{SepaDirectDebitBorrowerNoticeMtt} eq 'print' )
    {    # This system preference is set on customers request only.
...
sub writeSepaDirectDebitErrorFile {
    ...
    foreach my $borrowernumber ( sort keys %{ $self->{inKohaSelected} } ) {
        if ( exists( $self->{inKohaSelected}->{$borrowernumber}->{errormsg} )
            && defined( $self->{inKohaSelected}->{$borrowernumber}->{errormsg} ) )
        {
            ...
            if ( $errormsgcount > 0 ) {
                ...
                for ( my $i = 0 ; $i < $errormsgcount ; $i += 1 ) {
                    my $mess = sprintf(
                        "Ausweis:%s: %s  <br>\n",
                        $self->{inKohaSelected}->{$borrowernumber}->{borrowers}->{cardnumber},
                        $self->{inKohaSelected}->{$borrowernumber}->{errormsg}->[$i]
                    );
    ...
    if ( length($errormessages) > 0 ) {
        ...
        print STDERR "Koha::SEPAPayment::writeSepaDirectDebitErrorFile() will now open error output file:"
            . $paymentInstructionErrorFileName
            . ": for writing\n"
            if $self->{verbose} > 1;
        my $fh;
        my $res = open $fh, ">:encoding(UTF-8)", $paymentInstructionErrorFileName;
        print STDERR "Koha::SEPAPayment::writeSepaDirectDebitErrorFile() tried to open error output file:"
            . $paymentInstructionErrorFileName
            . ": fh:$fh: res:"
            . ( defined($res) ? $res : 'undef' ) . ":\n"
            if $self->{verbose} > 1;
        if ($res) {
            my $errorheaderline = sprintf(
                "<h3>Fehlerprotokoll SEPA-Lauf vom %s Uhr </h3>\n",
                DateTime->now( time_zone => C4::Context->tz() )->strftime('%d.%m.%Y %H:%M:%S')
            );
            $res = print $fh $errorheaderline . $errormessages;
            close $fh;
            print STDERR "Koha::SEPAPayment::writeSepaDirectDebitErrorFile() tried to write to error output file:"
                . $paymentInstructionErrorFileName
                . ": res:"
                . ( defined($res) ? $res : 'undef' ) . ":\n"
                if $self->{verbose} > 1;
            if ($res) {
                print STDERR "Koha::SEPAPayment::writeSepaDirectDebitErrorFile() error output file:"
                    . $paymentInstructionErrorFileName
                    . ": has been written\n"
                    if $self->{verbose} > 1;
                $fileWriteSccess = 1;
...
sub writeSepaDirectDebitFile {
    ...
    my ( $sec, $min, $hour, $mday, $mon, $year, $wday, $yday, $isdst ) = localtime;
    my $msgId = sprintf(
        "%s%04d%02d%02d", $self->{sepaSysPrefs}->{SepaDirectDebitMessageIdHeader}, $year + 1900, $mon + 1,
        $mday
    );
    my $creDtTm      = sprintf( "%04d-%02d-%02dT%02d:%02d:%02d", $year + 1900, $mon + 1, $mday, $hour, $min, $sec );
    my $reqdColltnDt = output_pref( { 'dt' => $requestedCollectionDate, dateformat => 'iso', dateonly => 1 } )
        ;    # requested collection date (vorgesehenes Datum für Lastschrifteinzug)
    my $remittanceInfo    = sprintf( "%-.140s", $self->{sepaSysPrefs}->{SepaDirectDebitRemittanceInfo} );
    my $success           = 0;
    my $ctrlSum           = 0.0;
    my $directDebitsCount = 0
        ; # we send only 1 <PmtInf> block, so <CstmrDrctDbtInitn><PmtInf><NbOfTxs> == <CstmrDrctDbtInitn><GrpHdr><NbOfTxs> == $directDebitsCount

    foreach my $borrowernumber ( sort keys %{ $self->{inKohaPaid} } ) {
        $ctrlSum +=
            ( sprintf( "%.2f", $self->{inKohaPaid}->{$borrowernumber}->{accountlinesSumAmountoutstanding} ) + 0.0 );
        $directDebitsCount += 1;
    }
    print STDERR
        "Koha::SEPAPayment::writeSepaDirectDebitFile(requestedCollectionDate:$requestedCollectionDate) START directDebitsCount:"
        . $directDebitsCount
        . ": ctrlSum:"
        . $ctrlSum . ":\n"
        if $self->{verbose} > 1;
    ...
    $xmlwriter->dataElement( 'Nm' => $self->{sepaSysPrefs}->{SepaDirectDebitInitiatingPartyName} )
        ;                                                          # InitiatingParty.Name
    $xmlwriter->endTag('InitgPty');
    $xmlwriter->endTag('GrpHdr');

    $xmlwriter->startTag('PmtInf');                                # PaymentInformation
    $xmlwriter->dataElement( 'PmtInfId' => $self->{sepaSysPrefs}->{SepaDirectDebitCreditorIban} )
        ;    # PaymentInformationID (we can use the IBAN for this field)
    ...
    $xmlwriter->dataElement( 'Cd' => $self->{sepaSysPrefs}->{SepaDirectDebitLocalInstrumentCode} )
        ;    # LocalInstrument.Code (CORE, COR1, B2B) (CORE und COR1: SEPA Basis-Lastschrift (Verbraucher))
    ...
    $xmlwriter->dataElement( 'BIC' => $self->{sepaSysPrefs}->{SepaDirectDebitCreditorBic} )
    ...
    $xmlwriter->dataElement( 'Id' => $self->{sepaSysPrefs}->{SepaDirectDebitCreditorId} )
        ;    # CreditorIdentification.ID.PrivateID.Other.ID (Gläubiger-Identifikationsnummer)
    ...
    foreach my $borrowernumber ( sort keys %{ $self->{inKohaPaid} } ) {
        ...
        my $debtorName =
            $self->debtorName( $borrFeesPaid->{borrowers}->{surname}, $borrFeesPaid->{borrowers}->{firstname} )
            ;    # default initialization, resulting in surname, firstname
        if ( defined( $borrFeesPaid->{borrower_attributes}->{Konto_von} )
            && length( $borrFeesPaid->{borrower_attributes}->{Konto_von} ) > 0 )
        {
            $debtorName = $self->debtorName( $borrFeesPaid->{borrower_attributes}->{Konto_von}, undef )
                ;    # if set, use borrower attribute having code 'Konto_von' as debtor name
        }
        my $dateOfSignature = $borrFeesPaid->{borrower_attributes}->{SEPA_Sign};
        $dateOfSignature = sprintf( "%04d-%02d-%02d", $year + 1900, $mon + 1, $mday )
            ; # there may be invalid entries in $borrFeesPaid->{borrower_attributes}->{SEPA_Sign}, so we ignore it and use today date
        my $bic = '';
        ...
        $xmlwriter->dataElement( 'EndToEndId' => 'NOTPROVIDED' )
            ;                                    # PaymentID.EndToEndId (from the viewpoint of the initiator)
        $xmlwriter->endTag('PmtId');

        $xmlwriter->dataElement(
            'InstdAmt', sprintf( "%.2f", $borrFeesPaid->{accountlinesSumAmountoutstanding} ),
            'Ccy' => 'EUR'
        );                                       # InstructedAmount (in currency 'EUR')
    ...
    print STDERR "Koha::SEPAPayment::writeSepaDirectDebitFile() will now open payment instruction output file:"
        . $paymentInstructionFileName
        . ": for writing\n"
        if $self->{verbose} > 1;
    my $fh;
    my $res = open $fh, ">:encoding(UTF-8)", $paymentInstructionFileName;
    print STDERR "Koha::SEPAPayment::writeSepaDirectDebitFile() tried to open payment instruction output file:"
        . $paymentInstructionFileName
        . ": fh:$fh: res:"
        . ( defined($res) ? $res : 'undef' ) . ":\n"
        if $self->{verbose} > 1;
    if ($res) {
        ...
        close $fh;
        print STDERR "Koha::SEPAPayment::writeSepaDirectDebitFile() tried to write to payment instruction output file:"
            . $paymentInstructionFileName
            . ": res:"
            . ( defined($res) ? $res : 'undef' ) . ":\n"
            if $self->{verbose} > 1;
        if ($res) {
            print STDERR "Koha::SEPAPayment::writeSepaDirectDebitFile() payment instruction output file:"
                . $paymentInstructionFileName
                . ": has been written\n"
                if $self->{verbose} > 1;
            $success = 1;
