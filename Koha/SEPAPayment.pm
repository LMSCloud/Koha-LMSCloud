...
sub _setUserenvToSepaCashRegisterManager {
    ...
    print STDERR "Koha::SEPAPayment::_setUserenvToSepaCashRegisterManager() userenv gesetzt auf "
        . "borrowernumber:" . $manager->borrowernumber . ": cardnumber:" . $manager->cardnumber . ":\n"
        if $self->{verbose} > 0;
...
sub updateResultHash {
    ...
    $self->{$resultHashName}->{$currentHitBorrowernumber}->{accountlines}->{$currentHitAccountlinesId}->{debit_type_code} = $currentFeeHit->{accountlines}->{debit_type_code};
...
sub paySelectedFeesForSepaDirectDebitPatrons {
    ...
    if ( $sepaDirectDebitAccountTypes ) {
        $debit_type_code_sel = '';    # In the second version of SEPA direct debit payment all debit_type_codes of interest have to be configured. (e.g. 'ACCOUNT|ACCOUNT_RENEW|MANUAL|OVERDUE')
...
sub writeSepaDirectDebitFile {
    ...
    $xmlwriter->dataElement(      'PmtInfId' => $self->{sepaSysPrefs}->{SepaDirectDebitCreditorIban});  # PaymentInformationID (we can use the IBAN for this field)
