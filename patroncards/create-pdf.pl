...
eval {
    $pdf_file = (
        @label_ids || @borrower_numbers
        ? "card_single_" . scalar( @label_ids || @borrower_numbers )
        : "card_batch_$batch_id"
    );
    ...
    $pdf->Compress(1)
        ;   # comment this out to debug pdf files, but be sure to uncomment it in production or you may be very sorry...
...
CARD_ITEMS:
    foreach my $item ( @{$items} ) {
        if ($item) {
            my $print_layout_xml =
                ( ( ( $cardscount % 2 == 1 ) && ($layout_back_id) ) ? dclone($layout_back_xml) : dclone($layout_xml) )
                ;    # We must have a true copy of the layout xml hash, otherwise
        ...
        PROCESS_IMAGES:
            foreach my $card_image ( sort( keys %{$images} ) ) {
                ...
                my $alt_width = ceil( $image->Get('width') )
                    ;    # the rounding up is important: Adobe reader does not handle long decimal numbers well
                ...
                $image->Resize(
                    width  => $oversize_factor * $display_width,
                    height => $oversize_factor * $display_height
                );
