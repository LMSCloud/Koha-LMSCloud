...
my $builder = sub {
    ...
    <script>
        function Click$params->{id}() {
                ...
                var branch = \$("#subfield952a select").select2().val();
                \$.ajax({
                    url: '/cgi-bin/koha/cataloguing/plugin_launcher.pl',
                    type: 'POST',
                    data: {
                        'plugin_name': 'barcode-schulen-berlin.pl',
                        'code'    : code.value,
                        'branch'  : branch
                    },
...
my $launcher = sub {
    ...
    my $prefix = C4::Context->preference('BarcodePrefix');
    if (! $code ) {	
		if ( $branch =~ /([0-9]{2})/ ) {
		    $branch = $1;
		}
    ...
    if ( $code =~ m/^$prefix[0-9]{2}$/ ) {
        ...
        if ( $av ) {
            $av->lib($av->lib + 1);
            $av->store;
            $template->param( return => $code . sprintf( '%06s', ( $av->lib ) ), );
        } else {
