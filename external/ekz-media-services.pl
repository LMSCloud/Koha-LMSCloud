...
if ( $requiredHttpSoapAction =~ /.*urn:budgetcheck.*/ ) {
    ...
    $soapResponseElement =
        C4::External::EKZ::BestellsystemWSDL::NotImplementedElement( $requiredHttpSoapAction, $soapEnvelopeHash );
}
...
sub buildSoapResponse {
    ...
    $logger->trace(
        "ekz-media-services.pl::buildSoapResponse soapResponseElements:" . Dumper(@soapResponseElements) . ":" );
...
sub buildSoapContent {
    ...
    foreach my $element (@elements) {
        $logger->trace( "ekz-media-services.pl::buildSoapContent element is a :"
                . $element
                . ": and ref gives:"
                . ref($element)
                . ":" );
        ...
        if ( ref($element) eq 'ARRAY' ) {
            ...
            if ( ref($element) eq 'REF' ) {
                ...
                if ( defined($type) && $type eq 'string' ) {    # finally at a leaf of the tree
                    ...
                    $logger->trace(
                        "ekz-media-services.pl::buildSoapContent element is no ARRAY, no REF, type is string");
