...
my ( $biblioitemnumbertagfield, $biblioitemnumbertagsubfield ) =
...
sub do_one_pass {
    if ($authorities) {
        index_records(
            'authority', $directory, $skip_export, $skip_index, $process_zebraqueue, $nosanitize,
            $do_not_clear_zebraqueue, $verbose_logging, $zebraidx_log_opt, $authorityserverdir
        );
    } else {
    ...
    if ($biblios) {
        index_records(
            'biblio', $directory, $skip_export, $skip_index, $process_zebraqueue, $nosanitize,
            $do_not_clear_zebraqueue, $verbose_logging, $zebraidx_log_opt, $biblioserverdir
        );
    } else {
...
sub zebraqueue_not_empty {
    ...
    my $query = $dbh->prepare( 'SELECT COUNT(*) FROM zebraqueue WHERE ' . $where_str );
...
sub index_records {
    my (
        $record_type,             $directory,       $skip_export,      $skip_index, $process_zebraqueue, $nosanitize,
        $do_not_clear_zebraqueue, $verbose_logging, $zebraidx_log_opt, $server_dir
    ) = @_;
    ...
    if ( $skip_export && $verbose_logging ) {
        ...
        if ($process_zebraqueue) {
            ...
            unless ($process_zebraqueue_skip_deletes) {
                ...
                $records_deleted =
                    generate_deleted_marc_records( $record_type, $entries, "$directory/del_$record_type" );
            ...
            $num_records_exported = export_marc_records_from_list(
                $record_type, $entries, "$directory/upd_$record_type",
                $records_deleted
            );
            mark_zebraqueue_batch_done($entries);
    ...
    if ($skip_index) {
        ...
        if ($process_zebraqueue) {
            do_indexing(
                $record_type, 'adelete', "$directory/del_$record_type", $reset, $noshadow, $record_fmt,
                $zebraidx_log_opt
            ) if %$records_deleted;
            do_indexing(
                $record_type, 'update', "$directory/upd_$record_type", $reset, $noshadow, $record_fmt,
                $zebraidx_log_opt
            ) if $num_records_exported;
        } else {
            do_indexing(
                $record_type, 'update', "$directory/$record_type", $reset, $noshadow, $record_fmt,
                $zebraidx_log_opt
...
sub export_marc_records_from_sth {
    ...
    while ( my ($record_number) = $sth->fetchrow_array ) {
        ...
        if ($nosanitize) {
            ...
            eval { my $doc = $tester->parse_string($marcxml); };
            if ($@) {
        ...
        if ( defined $marc ) {
            eval {
                ...
                eval { my $doc = $tester->parse_string($rec); };
                if ($@) {
                    die "invalid XML: $@";
                }
