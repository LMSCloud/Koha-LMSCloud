...
my @moduleletters = (
    { module => 'circulation', matchcodes => 'ODUE.*' }
    ,    # we might add later messages for advance notices like: DUE|DUEDGST|PREDUE|PREDUEDGST
...
sub readNoticeFeeRules {
    ...
    my $branch = shift;
    my @notice_fee_rules;
    my $query = qq{ SELECT  notice_fee_rules.*,
            ...
            WHERE notice_fee_rules.branchcode = ? };
    $query =~ s/^\s*/ /mg;
...
sub cloneNoticeFeeRules {
    ...
    while ( my $rule = $copyrules->next() ) {
        $existingRules{ $rule->categorycode() . "\t" . $rule->message_transport_type() . "\t" . $rule->letter_code() }
            = 1;
    }
    ...
    while ( my $rule = $deleterules->next() ) {
        if (
            exists(
                $existingRules{
                    $rule->categorycode() . "\t" . $rule->message_transport_type() . "\t" . $rule->letter_code()
                }
            )
            )
        {
