...
if ( $op eq "cud-export" ) {
    ...
    my $libraries = Koha::Libraries->search_filtered(
        { -or      => [ mobilebranch => undef, mobilebranch => '' ] },
        { order_by => 'branchname' }
    )->unblessed;
    ...
    if ( $record_type eq 'bibs' or $record_type eq 'auths' ) {
        ...
        unless (@record_ids) {
            if ( $record_type eq 'bibs' ) {
                ...
                my @itemtypes =
                    map { $it_map{$_} ? $_ : () } $query->multi_param('itemtype');    #Validate inputs against map
                ...
                my $biblioitems =
                    Koha::Biblioitems->search( $conditions, { join => 'items', columns => 'biblionumber' } );
        ...
        if (
            ...
            and (  $record_type eq 'db' and C4::Context->config('backup_db_via_tools')
                or $record_type eq 'conf' and C4::Context->config('backup_conf_via_tools') )
            )
        {
            ...
            unless ($successful_export) {
                ...
                $remotehost =~ s/(\n|\r)//;
                warn "A suspicious attempt was made to download the "
                    . ( $record_type eq 'db' ? 'db' : 'configuration' )
                    . "at '$filename' by someone at "
                    . $remotehost . "\n";
            }
        }
    }
