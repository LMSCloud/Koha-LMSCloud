...
if ($category) {
    ...
    if ($category_exists) {
        my $authorisedValueSearch =
            Koha::AuthorisedValues->search( { category => $category }, { order_by => ['authorised_value'] } );
...
if ( $action eq 'check_update' && $category_exists ) {
                ...
                if ( $csvline->[0] || $csvline->[1] ) {
                    ...
                    if ( !$csvline->[0] ) {
                        ...
                        if ($delete) {
                            ...
                            $lineerror =
                                { type => "csv", line => $linenumber, error => "not_exists", action => 'delete' };
                            $lineaction = {
                                do               => 'delete',
                        ...
                        foreach my $categ (@$category_values) {
                            $new = $categ
                                if ( $categ->{authorised_value} eq $csvline->[1]
                                && !( $update && $update->{id} == $categ->{id} ) );
                            $index++;
                        }
                        if ( $update && !$new ) {
                            $lineaction = {
                            ...
                            $lineerror =
                                { type => "csv", line => $linenumber, error => "not_exists", action => 'update' };
                            $lineaction = {
                                do                   => 'update',
                                old_authorised_value => $csvline->[0],
                                old_lib              => "",
    ...
    if ( $actioncount && $actioncount =~ /^\d+$/ && $actioncount > 0 ) {
        for ( my $i = 1 ; $i <= $actioncount ; $i++ ) {
            ...
            my $actionresult = {
                result   => 0,         action => $action, authorised_value => $authorised_value, lib => $lib,
                lib_opac => $lib_opac, select => $select
            };

            if ( $action eq 'delete' && $authorised_value ) {
                my $authorisedValueSearch =
                    Koha::AuthorisedValues->search( { category => $category, authorised_value => $authorised_value } );
                ...
                my $authorisedValueSearch =
                    Koha::AuthorisedValues->search( { category => $category, authorised_value => $select } );
                if ( $authorisedValueSearch->count == 1 ) {
                    while ( my $authval = $authorisedValueSearch->next ) {
                        $authval->set(
                            {
...
my @categories =
    Koha::AuthorisedValueCategories->search( { category_name => { '!=' => '' } }, { order_by => ['category_name'] } )
