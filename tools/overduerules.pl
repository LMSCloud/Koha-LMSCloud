...
if ( $op eq 'cud-save' ) {
    ...
    my $sth_insert = $dbh->prepare(
        "INSERT INTO overduerules (branchcode,categorycode, delay1,letter1,debarred1, delay2,letter2,debarred2, delay3,letter3,debarred3, delay4,letter4,debarred4, delay5,letter5,debarred5 ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
    );
    my $sth_update = $dbh->prepare(
        "UPDATE overduerules SET delay1=?, letter1=?, debarred1=?, delay2=?, letter2=?, debarred2=?, delay3=?, letter3=?, debarred3=?, delay4=?, letter4=?, debarred4=?, delay5=?, letter5=?, debarred5=? WHERE branchcode=? AND categorycode=?"
    );
    ...
    foreach my $bor ( keys %temphash ) {
        ...
        CHKDELAY: for ( my $i = 5 ; $i > 1 ; $i-- ) {
                if ( $temphash{$bor}->{ 'delay' . $i } ) {
                    for ( my $j = $i - 1 ; $j >= 1 ; $j-- ) {
                        if ( ( !$temphash{$bor}->{ 'delay' . $j } )
                            or $temphash{$bor}->{ 'delay' . $i } <= $temphash{$bor}->{ 'delay' . $j } )
                        {
...
# add a new claiming fee rule
    ...
    my $claimrule = Koha::ClaimingRules->find(
        { categorycode => $categorycode, itemtype => $itemtype, branchcode => $branchcode } );
    if ($claimrule) {
...
sub readClaimingRules {
    ...
    my $language = shift;
    my @claiming_rules;
    my $query = qq{ SELECT  claiming_rules.*,
