...
use C4::Acquisition
    qw( GetOrders GetOrder get_rounded_price GetBasket GetBasketgroup GetBasketsByBasketgroup GetBasketgroups GetBasketsByBookseller ModBasket CloseBasketgroup GetBasketGroupAsCSV DelBasketgroup ReOpenBasketgroup ModBasketgroup NewBasket NewBasketgroup GetNotClosedBasketsByBookseller GetBasketgroupsNotClosed);
use Koha::Database;
...
sub printbasketgrouppdf {
    ...
    my @valid_pdfformats =
        qw(pdfformat::layout3pages pdfformat::layout2pages pdfformat::layout3pagesfr pdfformat::layout2pagesde pdfformat::layout1page pdfformat::layout2pagesdinde pdfformat::layout2pagesdinde2);
        ...
        for my $ord (@ords) {
            ...
            if ($marcrecord) {
                if ( C4::Context->preference("marcflavour") eq 'UNIMARC' ) {
                    ...
                    if ( $marcrecord->field("001") && $marcrecord->field("001")->data() ) {
                        $cn = $marcrecord->field("001")->data();
                    }
                    if ( $marcrecord->field("003") && $marcrecord->field("003")->data() ) {
