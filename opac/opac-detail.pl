...
if ( C4::Context->preference('DivibibEnabled') && $record->field("001") && $record->field("003") ) {
    ...
    if ( $idProv eq C4::Divibib::NCIPService::DIVIBIBAGENCYID ) {
        ...
        my $useitem = undef;
        my $divitem = undef;
    SELITEM: while ( my $item = $items->next ) {
            my $booksellerid = $item->booksellerid || '';
            if ( $booksellerid eq 'Onleihe' ) {
                foreach my $ditem ( @{$divibib_items} ) {
                    $useitem = $item->unblessed;
                    $divitem = $ditem;
                    last SELITEM;
                }
            }
    ...
    while ( my $item = $items->next ) {
        ...
        if ( C4::Context->preference('DivibibEnabled') && scalar(@divibibItems) ) {
            for my $i ( 0 .. $#divibibItems ) {
                if ( $divibibItems[$i]->{itemnumber} eq $item_info->{itemnumber} ) {
                    $item_info = $divibibItems[$i];
                    delete $divibibItems[$i];
                    last;
                }
            }
        }
...
if (  !C4::Context->preference("OPACXSLTDetailsDisplay")
    || C4::Context->preference("OpacDetailAntolinLinks")
    || C4::Context->preference("OpacDetailWikipediaLinks") )
{
...
if ( C4::Context->preference("EKZCover") || C4::Context->preference("DivibibEnabled") ) {
    ...
    foreach my $tag ( $record->field('856') ) {
            ...
            )
        {
            $template->param(
                'contentsample',
                { 'link' => $tag->subfield('u'), 'type' => $record->field('337')->subfield('a') }
            );
        }
    }
...
if ( C4::Context->preference('OpacDetailVolumeDisplay') ) {
    ...
    foreach my $hostfield (
        $record->field(
            '770', '772', '773', '774', '775', '776', '777', '780', '785', '787', '800', '810', '811', '830'
        )
        )
    {
    ...
    eval {
        my ( $volError, $volumeData, $linkedRecordData ) =
            C4::VolumeData::GetVolumeData( $checkRecNum, $biblionumber, \@linkedRecords, $institutionCode );
        if ( !$volError ) {
            $template->param(
