...
if ( $level == 2 && $filter =~ /^[BCD]$/ ) {
    $sth = $dbh->prepare(
        "SELECT * FROM browser WHERE level=? AND classification NOT rlike '^(CD|DVD|BD)' AND classification like ? ORDER BY description"
    );
    $sth->execute( $level, $filter . "%" );
} else {
    if ( $level > 1 ) {
        $sth =
            $dbh->prepare("SELECT * FROM browser WHERE level=? AND classification like BINARY ? ORDER BY description");
        $sth->execute( $level, $filter . "%" );
    } elsif ($prefix) {
        $sth =
            $dbh->prepare("SELECT * FROM browser WHERE level=1 AND classification rlike '^S[:]' ORDER BY description");
        $sth->execute();
    } else {
        $sth = $dbh->prepare(
            "SELECT * FROM browser WHERE level=1 AND classification rlike '^[A-Z]' AND classification NOT IN ('CD','DVD','BD') ORDER BY description"
        );
        $sth->execute();
    }
}
...
if ( $level == 1 ) {
    if ($prefix) {
        $sth =
            $dbh->prepare("SELECT * FROM browser WHERE level=1 AND classification rlike '^M[:]' ORDER BY description");
    } else {
        $sth = $dbh->prepare(
            "SELECT * FROM browser WHERE level=1 AND classification IN ('CD','DVD','BD') ORDER BY description");
    }
    ...
    if ($prefix) {
        $sth =
            $dbh->prepare("SELECT * FROM browser WHERE level=1 AND classification rlike '^Y[:]' ORDER BY description");
    } else {
        $sth =
            $dbh->prepare("SELECT * FROM browser WHERE level=1 AND classification rlike '^[0-9]' ORDER BY description");
    }
    ...
    if ($prefix) {
        $sth =
            $dbh->prepare("SELECT * FROM browser WHERE level=1 AND classification rlike '^C[:]' ORDER BY description");
    ...
    if ($prefix) {
        $sth =
            $dbh->prepare("SELECT * FROM browser WHERE level=1 AND classification rlike '^Z[:]' ORDER BY description");
