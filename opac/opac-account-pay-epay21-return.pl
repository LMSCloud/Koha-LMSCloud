...
# Copyright (C) 2019-2021 LMSCLoud GmbH
#
...
$logger->debug("opac-account-pay-epay21-return.pl START cgi:" . Dumper($cgi) . ":");
...
if ( C4::Context->preference('Epay21PaypageOpacPaymentsEnabled') ) {
    ...
    $logger->debug("opac-account-pay-epay21-return.pl creating new C4::Epayment::Epay21 object. borrowernumberKoha:$borrowernumberKoha: amountKoha:$amountKoha: accountlinesKoha:" . Dumper(@accountlinesKoha) . ": paytypeKoha:$paytypeKoha: timeKoha:$timeKoha:");
    ...
    if ( $patron ) {
        ...
        ( $error, $errorTemplate ) = $epay21->checkOnlinePaymentStatusAndPayInKoha( $cgi );

    } else {
        my $mess = "Error: No patron found having borrowernumber:$borrowernumberKoha:";
        $logger->error("opac-account-pay-epay21-return.pl $mess");
        carp ("opac-account-pay-epay21-return.pl " . $mess . "\n");
    }
    $redirectUrl = "/cgi-bin/koha/opac-account.pl?payment=$amountKoha&payment-error=$errorTemplate";
}

$logger->debug("opac-account-pay-epay21-return.pl END error:$error: errorTemplate:$errorTemplate: redirectUrl:$redirectUrl:");
