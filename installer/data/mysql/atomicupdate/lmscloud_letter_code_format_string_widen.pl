...
return {
    ...
    description =>
        "Widen letter.code (20->50) and creator_layouts.format_string (210->1024) to match LMS-extended schema",
    up => sub {
        my ($args) = @_;
        ...
        if ( $letter_code && $letter_code !~ /^varchar\(50\)/i ) {
            $dbh->do(
                q{ALTER TABLE letter MODIFY `code` varchar(50) NOT NULL DEFAULT '' COMMENT 'unique identifier for this notice or slip'}
            );
