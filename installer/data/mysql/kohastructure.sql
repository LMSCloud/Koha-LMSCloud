...
CREATE TABLE `categories` ( -- this table shows information related to Koha patron categories
  ...
  `family_card` tinyint(1) NOT NULL default 0, -- mark a category as family card (linked borrowers via the guarantor relationship are not charged with the enrollment fee)
...
CREATE TABLE `accountlines` (
  ...
  CONSTRAINT `accountlines_ibfk_3` FOREIGN KEY (`branchcode`) REFERENCES `branches` (`branchcode`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `claiming_rules` (
  ...
  `claim_fee_level1` decimal(28,6) default NULL, -- fine amount for reaching 1st claim
  `claim_fee_level2` decimal(28,6) default NULL, -- fine amount for reaching 2nd claim
  `claim_fee_level3` decimal(28,6) default NULL, -- fine amount for reaching 3nd claim
  `claim_fee_level4` decimal(28,6) default NULL, -- fine amount for reaching 4nd claim
