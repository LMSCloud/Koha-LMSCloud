...
sub modify_acquisition_import {
    ...
    while (
        my (
            $id, $object_type, $object_number, $object_date, $rec_type, $object_item_number, $processingstate,
            $processingtime
        )
        = $sthSel->fetchrow
        )
    {
        print
            "upgrade_ekzOrderNr_for_STO::modify_acquisition_import id:$id: object_type:$object_type: object_number:$object_number: object_date:$object_date: rec_type:$rec_type: object_item_number:$object_item_number: processingstate:$processingstate: processingtime:$processingtime:\n"
            if $debug;
        print
            "upgrade_ekzOrderNr_for_STO::modify_acquisition_import object_number:$object_number: object_item_number:$object_item_number:\n"
            if $debug;
        ...
        print
            "upgrade_ekzOrderNr_for_STO::modify_acquisition_import object_number:$object_number -> $new_object_number:\n"
            if $debug;
        ...
        print
            "upgrade_ekzOrderNr_for_STO::modify_acquisition_import object_item_number:$object_item_number -> $new_object_item_number:\n"
            if $debug;

        if ( $new_object_number ne $object_number || $new_object_item_number ne $object_item_number ) {
            my $sthMod = $dbh->prepare(
                q{
            ...
            $sthMod->execute( $new_object_number, $new_object_item_number, $id )
                ;    # $new_object_item_number undef -> object_item_number = NULL
            $modifiedCount += 1;
        }
    }

    print
        "upgrade_ekzOrderNr_for_STO::modify_acquisition_import END; ekzKundenNr:$ekzKundenNr: modifiedCount:$modifiedCount:\n"
        if $debug;
...
sub modify_aqbasketgroups {
    ...
    my $modifiedCount = 0;
    print
        "upgrade_ekzOrderNr_for_STO::modify_aqbasketgroups START ekzKundenNr:$ekzKundenNr: ekzBooksellersId:$ekzBooksellersId:\n"
        if $debug;
    ...
    print
        "upgrade_ekzOrderNr_for_STO::modify_aqbasketgroups END; ekzKundenNr:$ekzKundenNr: modifiedCount:$modifiedCount:\n"
        if $debug;
...
sub modify_aqbasket {
    ...
    my $modifiedCount = 0;
    print
        "upgrade_ekzOrderNr_for_STO::modify_aqbasket START ekzKundenNr:$ekzKundenNr: ekzBooksellersId:$ekzBooksellersId:\n"
        if $debug;
    ...
    while ( my ( $basketno, $basketname, $note, $booksellerid ) = $sthSel->fetchrow ) {
        print
            "upgrade_ekzOrderNr_for_STO::modify_aqbasket basketno:$basketno: basketname:$basketname: note:$note: booksellerid:$booksellerid:\n"
            if $debug;
...
print
    "upgrade_ekzOrderNr_for_STO.pl END modifiedCountAcquisitionImport:$modifiedCountAcquisitionImport:  modifiedCountAqbasketgroups:$modifiedCountAqbasketgroups:  modifiedCountAqbasket:$modifiedCountAqbasket:\n";
