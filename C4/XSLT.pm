...
sub get_xsl_filename {
    ...
    if ( $xslfilename =~ /^\s*"?default"?\s*$/i ) {
        ...
        if ( $xslsyspref eq "XSLTDetailsDisplay" ) {
            ...
            $htdocs      = C4::Context->config('intrahtdocs');
            $theme       = C4::Context->preference("template");
            $xslfile     = C4::Context->preference('marcflavour') . "slim2intranetVolume.xsl";
        } elsif ( $xslsyspref eq "OPACXSLTVolumeDisplay" ) {
            $htdocs     = C4::Context->config('opachtdocs');
            $customdocs = C4::Context->config('opaccustomdocs');
            $theme      = C4::Context->preference("opacthemes");
            $xslfile    = C4::Context->preference('marcflavour') . "slim2OPACVolume.xsl";
        }
