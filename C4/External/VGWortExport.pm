...
sub getExportCountsPerVGWortMediaType {
    ...
    $sth->execute(
        $self->{fromDate},        $self->{toDate},       @{ $self->{itypes} },      @{ $self->{itypes} },
        $self->{useLibraryGroup}, $self->{libraryGroup}, $self->{useSingleLibrary}, $self->{singleLibrary}
    );
    ...
    while ( my $mediaTypeCount = $sth->fetchrow_hashref ) {
        ...
        if ( exists( $resultMapping->{ $mediaTypeCount->{year} }->{$mediatype} ) ) {
            ...
            $resultMapping->{ $mediaTypeCount->{year} }->{$mediatype}->{itypeCount}->{ $mediaTypeCount->{itype} } =
...
sub sorted_keys {
    ...
    if ( $name eq 'MELDUNG' )    # only this tag I care about the order;
    {
        ...
        return grep { exists $hashref->{$_} } @ordered,
            grep { not $ordered_hash{$_} } $self->SUPER::sorted_keys( $name, $hashref );
    }
