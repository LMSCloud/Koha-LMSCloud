            ...
            foreach my $offer ( @{ $data->{offers} } ) {
                if ( reftype($offer) eq 'HASH' ) {
                    if (
                        ...
                        && (
                            $self->{'searchAllCollections'}
                            || ( exists( $self->{'searchCollections'}->{ $offer->{name} } )
                                && $self->{'searchCollections'}->{ $offer->{name} } == 1 )
                        )
                        )
                    {
                        my $collection = {};
                        ...
                        $image = URI->new_abs( $image, $self->{'helimaAPIBaseURL'} )->as_string
                            if ( $image =~ m#^\s*/# );
                        push @{ $result->{collections} },
                            {
                            name        => $offer->{name},        id    => $offer->{id}, hits => $offer->{matches} + 0,
                            description => $offer->{description}, image => $image
                            };
                    }
                }
            }
            ...
            && scalar( @{ $data->{results} } ) > 0 )
        {
            ...
            foreach my $lesson ( @{ $results->{lessons} } ) {
                ...
                if ( reftype($lesson) eq 'HASH' ) {
                    for my $key (
                        'active', 'area',  'dauer', 'fach',       'id', 'image_url', 'klassenstufen', 'medium',
                        'title',  'topic', 'url',   'valid_from', 'valid_until'
                        )
                    {
