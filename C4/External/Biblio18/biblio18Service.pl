...
helper logger        => sub {
    state $logger =
        Mojo::Log->new( path => $config->{logger}->{'log_file'}, level => $config->{logger}->{'log_level'} );
};
...
get '/search' => sub {
    ...
    if ( checkAuthentication($c) ) {
        ...
        $c->logger->info( "request($rid):$ip($agent) => search result: found "
                . scalar( @{ $searchresult->{CatalogItems} } )
                . " result records" );
