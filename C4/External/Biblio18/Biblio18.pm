...
sub search {
    ...
    if (   C4::Context->preference("Biblio18QueryMaxSearchResultCount")
        && C4::Context->preference("Biblio18QueryMaxSearchResultCount") =~ /^([0-9]+)$/ )
    {
        $results_per_page = $1;
    }
    my $offset         = 0;
    ...
    if ($querystring) {
        ...
        my $authorisedValueSearch = Koha::AuthorisedValues->search(
            { category => "BIBLIO18-MediaGroupUnion-Mapping" },
            { order_by => ['authorised_value'] }
        );
...
sub formatBiblioData {
    ...
    while ( my $item = $items->next ) {
        ...
        next
            if ( $self->{parsedItemIncludeRules}
            && !$item->hidden_in_opac( { rules => $self->{parsedItemIncludeRules} } ) );

        # check specific Bilio18 item exclude rules if defined
        next
            if ( $self->{parsedItemExcludeRules}
            && $item->hidden_in_opac( { rules => $self->{parsedItemExcludeRules} } ) );
        ...
        if ( $item->damaged ) {
            ...
            my $datedue =
                C4::Circulation::CalcDateDue( $issuedate, $itype, $hold->branchcode, $hold->borrower->unblessed );
