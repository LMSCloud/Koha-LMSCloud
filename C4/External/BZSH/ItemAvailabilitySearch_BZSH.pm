...
sub new {
    ...
    my @illItemtypes = split( /\|/, C4::Context->preference("ILLItemTypes") )
        ;    # system preference that may contain a list of possible ILL item types (separated by '|')
...
sub search_title_with_best_item_status {
    ...
    if ( defined $sel_titel ) {
        ...
        eval { ( $error, $marcresults, $total_hits ) = $self->{searcher}->simple_search_compat( $query, 0, 100000 ); };
        if ( $error || $@ ) {
            warn
                "C4::External::BZSH::ItemAvailabilitySearch_BZSH::search_title_with_best_item_status() query:$query: error from simple_search_compat():$error $@";
        }

        if ( defined $error ) {
            my $log_str = sprintf(
                "ItemAvailabilitySearch_BZSH: search for sel_titel:%s: returned error:%d/%s:\n", $sel_titel,
                $error,                                                                          $error
            );
            carp $log_str;
        }
    ...
    if ( $total_hits == 0 ) {
        if ( !defined $sel_srisbn || length($sel_srisbn) == 0 ) {
            my $log_str = sprintf(
                "ItemAvailabilitySearch_BZSH: Title not found by sel_titel:%s: and sel_srisbn is empty - no 2nd search done. Returning status 0.\n",
                $sel_titel
            );
            carp $log_str;
        } else {
            ...
            if ( !defined( $selISBN[0] ) || length( $selISBN[0] ) == 0 ) {
                my $log_str = sprintf(
                    "ItemAvailabilitySearch_BZSH: ISBN not valid -> not searching for ISBN, but trying EAN:%013d:.\n",
                    $sel_srisbn
                );
                carp $log_str;
                ...
                if ( defined $error ) {
                    my $log_str = sprintf(
                        "ItemAvailabilitySearch_BZSH: search for EAN:%013d: returned error:%d/%s:\n",
                ...
                for ( my $i = 0 ; $i < 4 ; $i += 1 ) {
                    if ( defined( $selISBN[$i] ) && length( $selISBN[$i] ) > 0 ) {
                        ...
                        if ( defined $error ) {
                            my $log_str = sprintf(
                                "ItemAvailabilitySearch_BZSH: search for ISBN/EAN:%s: returned error:%d/%s:\n",
    ...
    for ( my $i = 0 ; $i < $hits and defined $marcresults->[$i] and $best_item_status != 1 ; $i++ ) {
        eval { $marcrecord = C4::Search::new_record_from_zebra( 'biblioserver', $marcresults->[$i] ) };
        ...
        if ($marcrecord) {
            ...
            $self->search_item_with_best_item_status(
                $marcrecord,       $biblionumber, \$best_item_status,
                \$best_itemnumber, \$best_biblionumber
            );
        }
    }
...
sub search_item_with_best_item_status {
    ...
    my $catalogNotLoanMatch = $self->checkCatalogCheckRules(
        C4::Context->preference("BZSHAvailabilitySetItemStatusNotLoanByCatalogData"),
        $marcrecord
    );
    ...
    if ($items_rs) {
        while ( my $item = $items_rs->next() ) {
            ...
            if ( $self->{selbranchcodecheck}
                && !defined( $self->{selbranchcodehash}->{ $itemrecord->{'homebranch'} } ) )
            {
    ...
    foreach my $checkrule ( split( /\|/, $catalogNotLoanRules ) ) {
        if ( $checkrule =~ /^\s*([0-9]{3})(\((.)(.)\))?(\$([0-9a-z]))?\s*(!=|=~|!~|=)(.*)$/ ) {
            ...
            foreach my $field ( $record->field($marcField) ) {
                $fieldcnt++;
                next
                    if ( ( $marcInd1 && $marcInd1 ne $field->indicator(1) )
                    || ( $marcInd2 && $marcInd2 ne $field->indicator(2) ) );
                my @subfields;
                if ( $marcField >= 10 ) {
