...
use C4::Acquisition
    qw( NewBasket GetBasket GetBaskets ModBasket ReopenBasket GetBasketgroupsGeneric NewBasketgroup CloseBasketgroup ReOpenBasketgroup GetOrderFromItemnumber ModOrderDeliveryNote );
...
sub readLSFromEkzWsLieferscheinList {
    ...
    $logger->info( "readLSFromEkzWsLieferscheinList() START ekzCustomerNumber:"
    ...
    $result =
        $ekzwebservice->callWsLieferscheinList( $ekzCustomerNumber, $selVon, $selBis, $selKundennummerWarenEmpfaenger );
...
sub readLSFromEkzWsLieferscheinDetail {
    ...
    $logger->info( "readLSFromEkzWsLieferscheinDetail() START ekzCustomerNumber:"
            . ( defined($ekzCustomerNumber) ? $ekzCustomerNumber : 'undef' )
            . ": selId:"
            . ( defined($selId) ? $selId : 'undef' )
            . ": selLieferscheinnummer:"
            . ( defined($selLieferscheinnummer) ? $selLieferscheinnummer : 'undef' )
            . ":" );
    $logger->trace( "readLSFromEkzWsLieferscheinDetail() START Dumper(\$refLieferscheinDetailElement):"
            . Dumper($refLieferscheinDetailElement)
            . ":" );

    my $ekzwebservice = C4::External::EKZ::lib::EkzWebServices->new();
    $result = $ekzwebservice->callWsLieferscheinDetail(
        $ekzCustomerNumber, $selId, $selLieferscheinnummer,
...
sub genKohaRecords {
    my (
        $ekzCustomerNumber, $messageID, $lieferscheinDetailElement, $lieferscheinRecord, $createdTitleRecords,
        $updatedTitleRecords
    ) = @_;
    ...
    $logger->info(
            ...
            . ( defined( $lieferscheinRecord->{'id'} ) ? $lieferscheinRecord->{'id'} : 'undef' )
            . ": Lieferscheinnummer:"
            . ( defined( $lieferscheinRecord->{'nummer'} ) ? $lieferscheinRecord->{'nummer'} : 'undef' )
            . ": teilLieferungCount:"
            . (
            defined( $lieferscheinRecord->{'teilLieferungCount'} )
            ? $lieferscheinRecord->{'teilLieferungCount'}
            : 'undef'
            )
            . ":"
    );
    ...
    try {
        ...
        my $ekzWebServicesSetItemSubfieldsWhenReceived =
            C4::Context->preference("ekzWebServicesSetItemSubfieldsWhenReceived");
        ...
        $lieferscheinDatum = DateTime->new(
            year => substr( $lsDatum, 0, 4 ), month     => substr( $lsDatum, 5, 2 ),
            day  => substr( $lsDatum, 8, 2 ), time_zone => 'local'
        );
        ...
        $logger->trace( "genKohaRecords() search delivery note message record in acquisition_import selParam:"
                . Dumper($selParam)
                . ":" );
        ...
        $logger->trace( "genKohaRecords() ref(acquisitionImportLieferschein):"
                . ref($acquisitionImportLieferschein)
                . ": ref(hit)"
                . ref($hit)
                . ":" );
        if ( defined($hit) ) {
            ...
            my $mess = sprintf(
                "genKohaRecords(): The delivery note number '%s' has already been used at %s. Processing denied.",
            ...
            $logger->trace( "genKohaRecords() Dumper(schemaResultAcquitionImport->{_column_data}):"
                    ...
                    . ":" );
            $logger->trace(
                "genKohaRecords() acquisitionImportIdLieferschein:" . $acquisitionImportIdLieferschein . ":" );
        }
        ...
        if ( !$lieferscheinNummerIsDuplicate ) {
            ...
            foreach my $teilLieferungRecord ( @{ $lieferscheinRecord->{'teilLieferungRecords'} } ) {
                ...
                $logger->trace(
                    "genKohaRecords() teilLieferungRecord gelieferteExemplare:$teilLieferungRecord->{'gelieferteExemplare'}: teilLieferung:$teilLieferungRecord->{'teilLieferung'}: auftragsPositionCount:$teilLieferungRecord->{'auftragsPositionCount'}:"
                );
                my $auftragsPosition = $teilLieferungRecord->{'auftragsPositionRecords'}->[0]
                    ;    # this array always consists of only 1 element
                $logger->trace(
                    "genKohaRecords() auftragsPosition ekzexemplarid:"
                        . (
                        defined( $auftragsPosition->{'ekzexemplarid'} )
                        ? $auftragsPosition->{'ekzexemplarid'}
                        : 'undef'
                        )
                        . ": ekzArtikelNr:$auftragsPosition->{'artikelNummer'}: isbn:$auftragsPosition->{'isbn'}: ean:$auftragsPosition->{'ean'}: kundenBestelldatum:$auftragsPosition->{'kundenBestelldatum'}:"
                );
                    ...
                    foreach my $acquisitionImportEkzExemplarIdHit ( $acquisitionImportEkzExemplarIdHits->all() ) {
                        ...
                        if ( defined($acquisitionImportTitleHit) ) {
                            ...
                            if ( defined($selBiblionumber) ) {
                                ...
                                &processItemHit(
                                    $lieferscheinNummer,                         $lieferscheinDatum, $dateTimeNow,
                                    $ekzWebServicesSetItemSubfieldsWhenReceived, $lsEkzArtikelNr,    '',
                ...
                $logger->debug(
                    "genKohaRecords() after method 1 deliveredItemsCount:$deliveredItemsCount: updOrInsItemsCount:$updOrInsItemsCount: titleHits->{'count'}:$titleHits->{'count'}: biblionumber:$biblionumber: lsEkzArtikelNr:$lsEkzArtikelNr:"
                );
                    ...
                    foreach my $acquisitionImportEkzExemplarIdHit ( $acquisitionImportEkzExemplarIdHits->all() ) {
                        ...
                        if ( defined($acquisitionImportTitleHit) ) {
                            ...
                            if ( defined($selBiblionumber) ) {
                                ...
                                &processItemHit(
                                    $lieferscheinNummer,                         $lieferscheinDatum, $dateTimeNow,
                                    $ekzWebServicesSetItemSubfieldsWhenReceived, $lsEkzArtikelNr,    $lsReferenznummer,
                                    $auftragsPosition, $acquisitionImportTitleHit,            $titleHits, $biblionumber,
                                    $acquisitionImportEkzExemplarIdHit, $emaillog,            \$updOrInsItemsCount,
                                    $ekzAqbooksellersId,                $updatedTitleRecords, $logger
                                );
                ...
                $logger->debug(
                    "genKohaRecords() after method 2 deliveredItemsCount:$deliveredItemsCount: updOrInsItemsCount:$updOrInsItemsCount: titleHits->{'count'}:$titleHits->{'count'}: biblionumber:$biblionumber: lsEkzArtikelNr:$lsEkzArtikelNr:"
                );
                ...
                #                        }
                #
                #                        &processItemHit($lieferscheinNummer, $lieferscheinDatum, $dateTimeNow, $ekzWebServicesSetItemSubfieldsWhenReceived, $lsEkzArtikelNr, '', $auftragsPosition, $acquisitionImportTitleHit, $titleHits, $biblionumber, $acquisitionImportTitleItemHit, $emaillog, \$updOrInsItemsCount, $ekzAqbooksellersId, $updatedTitleRecords, $logger);
                #                    }
                #                }
                #            }    # end method3
                ...
                if ( $updOrInsItemsCount < $deliveredItemsCount ) {
                        ...
                        || $titleHits->{'records'}->[0]->field("003")->data() ne "DE-Rt5" )
                    {
                        ...
                        for ( my $searchmode = 1 ; $searchmode <= 2 ; $searchmode *= 2 ) {
                            ...
                            $logger->info(
                                "genKohaRecords() method4: from local DB searchmode:$searchmode: titleHits->{'count'}:"
                                    . $titleHits->{'count'}
                                    . ":" );
                            if ( $titleHits->{'count'} > 0 && defined $titleHits->{'records'}->[0] ) {
                                $biblionumber = $titleHits->{'records'}->[0]->subfield( "999", "c" );
                    ...
                    $logger->info(
                        "genKohaRecords() method4: biblioExisting:$biblioExisting: biblioInserted:$biblioInserted: biblionumber:$biblionumber: lsEkzArtikelNr:$lsEkzArtikelNr:"
                    );
                    ...
                    if ( $biblioExisting || $biblioInserted ) {
                        ...
                        my $acquisitionImportTitleDeliveryRS =
                            $acquisitionImportTitleDelivery->_resultset()->create($insParam);
                        $logger->trace( "genKohaRecords() method4: acquisitionImportTitleDeliveryRS->{_column_data}:"
                                . Dumper( $acquisitionImportTitleDeliveryRS->{_column_data} )
                                . ":" );

                        # now add the items data for the new biblionumber
                        my $ekzExemplarID =
                            $ekzBestellNr . '-' . $lsEkzArtikelNr;    # dummy item number for the 'invented' order
                        ...
                        if ( length($ekzAqbooksellersId) ) {
                            ...
                            if ( @{$selbaskets} > 0 ) {
                                ...
                                $basketno = C4::Acquisition::NewBasket(
                                    ...
                                    'created by ekz LieferscheinDetail', '', undef, $branchcode, $branchcode, 0,
                                    'ordering'
                                );    # XXXWH fixed text ok?
                        ...
                        if ( defined($basketno) && $basketno > 0 ) {
                            ...
                            $orderinfo->{ecost}            = $priceInfo->{gesamtpreis_tax_included}
                                ;    #  corresponds to input field 'Budgeted cost' in UI (discounted, per item)
                        ...
                        for ( my $j = 0 ; $j < $exemplarcount ; $j++ ) {
                            ...
                            if ( defined $itemnumber && $itemnumber > 0 ) {
                                ...
                                if ( $itemHitCount > 0 && defined( $itemHit->{_column_data} ) ) {
                                    ...
                                    if ( defined($ekzWebServicesSetItemSubfieldsWhenReceived)
                                        && length($ekzWebServicesSetItemSubfieldsWhenReceived) > 0 )
                                    {
                                ...
                                my $acquisitionImportItemDeliveryRS =
                                    $acquisitionImportItemDelivery->_resultset()->create($insParam);
                                $logger->trace(
                                          "genKohaRecords() method4: acquisitionImportItemDeliveryRS->{_column_data}:"
                                        . Dumper( $acquisitionImportItemDeliveryRS->{_column_data} )
                                        . ":" );
                            ...
                            push @{ $emaillog->{'records'} },
                                [
                                $lsEkzArtikelNr,             defined $biblionumber ? $biblionumber : "no biblionumber",
                                $emaillog->{'importresult'}, $titeldata, $isbnean,     $emaillog->{'problems'},
                                $emaillog->{'importerror'},  2,          $ordernumber, $basketno
                                ];
                ...
                $logger->debug(
                    "genKohaRecords() after method 4 deliveredItemsCount:$deliveredItemsCount: updOrInsItemsCount:$updOrInsItemsCount: titleHits->{'count'}:$titleHits->{'count'}: biblionumber:$biblionumber: lsEkzArtikelNr:$lsEkzArtikelNr:"
                );
            ...
            push @{ $emaillog->{'logresult'} },
                [
                'LieferscheinDetail', $messageID, $emaillog->{'actionresult'}, $acquisitionError,
                $ekzAqbooksellersId,  undef
                ];    # arg basketno is undef, because with standing orders multiple delivery baskets are possible
            $logger->trace(
...
sub priceInfoFromMessage {
    ...
    my $ustProzentVoll =
        defined( $lieferscheinRecord->{'ustProzentVoll'} )
        ? $lieferscheinRecord->{'ustProzentVoll'}
        : &C4::External::EKZ::lib::EkzKohaRecords::defaultUstSatz('V') * 100.0;  # e.g. 19.00 for VAT rate of 19% (0.19)
    my $ustProzentHalb =
        defined( $lieferscheinRecord->{'ustProzentHalb'} )
        ? $lieferscheinRecord->{'ustProzentHalb'}
    ...
    $priceInfo->{rabatt} = "0.0"
        ; # 'rabatt' not sent in LieferscheinDetailResponseElement.auftragsPosition, so we calculate it from verkaufsPreis and nachlass (15.0 means 15 %)
    ...
    $priceInfo->{wertPositionsTeil} =
        defined( $auftragsPosition->{'wertPositionsTeil'} )
        ? $auftragsPosition->{'wertPositionsTeil'}
        : "0.00";    # info by etecture: <wertPositionsTeil> = <verkaufsPreis> - <nachlass>
    $priceInfo->{wertMehrpreise} =
    ...
    $priceInfo->{ust} =
        "0.00";      # 'ust' not sent in LieferscheinDetailResponseElement.auftragsPosition, so we will calculate it
    $priceInfo->{ustSatz} = $ustProzentHalb / 100.0
        ; # 'ustSatz' not sent in LieferscheinDetailResponseElement.auftragsPosition, so we evaluate XML element <ustProzentHalb> or <ustProzentVoll>
    $priceInfo->{mwst} =
...
sub processItemHit {
    my (
        $lieferscheinNummer, $lieferscheinDatum, $dateTimeNow, $ekzWebServicesSetItemSubfieldsWhenReceived,
        $lsArtikelNr, $lsReferenznummer, $auftragsPosition,    $acquisitionImportTitleHit, $titleHits, $biblionumber,
    ...
    # Can be used only if ekz media service is linked to Koha acquisition (via system preference ekzAqbooksellersId).
    if ( defined $itemnumber ) {
        ...
        if ( $aqorders_itemsRS && $aqorders_itemsRS->first() ) {
            ...
            { # The aqorders_items record does not exist any more if the items record meanwhile has been deleted or if ekz media service is not linked to Koha acquisition or in case of database inconsistency.
    ...
    if ( defined $titleItemObjectRS && defined $itemnumber ) {
        ...
        if ( $itemHitCount > 0 && defined( $itemHit->{_column_data} ) ) {
            ...
            if ( defined($ekzAqbooksellersId) && length($ekzAqbooksellersId) ) {
                ...
                ( $ordernumberFound, $basketnoFound ) = processItemOrder(
                    $lieferscheinNummer, $lieferscheinDatum,             $biblionumber, $itemnumber,
                    $auftragsPosition,   $acquisitionImportTitleItemHit, $logger
                );
                $logger->trace(
                    "processItemHit() processItemOrder() returned ordernumberFound:$ordernumberFound: basketnoFound:$basketnoFound:"
                );
            }
            ...
            my $resDeliveryTitle = $acquisitionImportDeliveryNoteTitle->upd_or_ins( $selParam, $updParam, $insParam )
                ;    # TODO: evaluate $resDeliveryTitle
            $logger->trace( "processItemHit() insert acquisition_import record for delivery title res:"
                    . Dumper( $resDeliveryTitle->_resultset()->{_column_data} )
                    . ":" );
            ...
            my $resDeliveryItem =
                $acquisitionImportDeliveryNoteItem->_resultset()->create($insParam);   # TODO: evaluate $resDeliveryItem
            $logger->trace( "processItemHit() insert acquisition_import record for item res:"
                    . Dumper( $resDeliveryItem->{_column_data} )
                    . ":" );
        }
    }
    ...
    {    # item successfully updated
        ...
        $emaillog->{'problems'} .= "\n" if ( $emaillog->{'problems'} );
        $emaillog->{'problems'} .=
            "ERROR: Update der Exemplardaten für EKZ ArtikelNr.: " . $lsArtikelNr . " wurde abgewiesen.\n";
        $emaillog->{'importresult'} = -1;
        $emaillog->{'importerror'}  = 1;
    ...
    push @{ $emaillog->{'records'} },
        [
        $lsArtikelNr, defined $biblionumber ? $biblionumber : "no biblionumber", $emaillog->{'importresult'},
...
sub processItemOrder {
    my (
        $lieferscheinNummer,            $lieferscheinDatum, $biblionumber, $itemnumber, $auftragsPosition,
        $acquisitionImportTitleItemHit, $logger
    ) = @_;
    ...
    $logger->info(
        "processItemOrder() Start biblionumber:$biblionumber: itemnumber:$itemnumber: acquisitionImportTitleItemHit object_number:"
            . $acquisitionImportTitleItemHit->object_number
            . ":" );
    ...
    if ($isStoOrSer) {    # it is a item of a standing or serial order
        ...
        if ( defined($aqbasket_delivery_hits) && scalar @{$aqbasket_delivery_hits} > 0 ) {
            ...
            my $aqbasket_delivery_no = &C4::Acquisition::NewBasket(
                $aqbasket_of_order->{booksellerid},  $aqbasket_of_order->{authorisedby}, $aqbasket_delivery_name,
        ...
        # close basket
        $logger->debug( "processItemOrder() is calling Koha::Acquisition::Baskets->find(basketno:"
                . $aqbasket_delivery->{basketno}
                . ")" );
        ...
        $logger->info(
            "processItemOrder() basketgroup with name:$aqbasket_delivery_name: has basketgroupid:$basketgroupid:");
    ...
    $logger->info( "processItemOrder() returns ordernumber_ret:"
            . $ordernumber_ret
            . ": and basketno_ret:"
            . $basketno_ret
            . ":" );
