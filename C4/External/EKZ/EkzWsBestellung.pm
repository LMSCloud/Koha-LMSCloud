...
sub BestelleBeiEkz {
    ...
    my $aqbooksellers_hit
        ; # We assume here exclusive use of aqbookseller 'ekz'. But each library may have a distinct acbooksellers record with name 'ekz.
    ...
    foreach my $basketgroup ( @{$basketgroupList} ) {
        $logger->debug( "BestelleBeiEkz() basketgroup->basketgroupid:"
                . $basketgroup->{basketgroupid}
                . ": ->supplyOption:"
                . $basketgroup->{supplyOption}
                . ":" );
        ...
        if ( my $aqbasketgroups_hit = $sth->fetchrow_hashref ) {
            if ( !defined($aqbooksellers_hit) ) {
                ...
                if ( !$aqbooksellers_hit ) {
                    my $mess =
                        sprintf( "aqbooksellers WHERE id = %s : Not found.", $aqbasketgroups_hit->{'booksellerid'} );
        ...
        if ( !$aqbasket_hits || scalar @{$aqbasket_hits} == 0 ) {
            ...
            foreach my $aqbasket_hit ( @{$aqbasket_hits} ) {
                ...
                $param->{rechnungsEmpfaenger}->{ekzKundenNr} =
                    $aqbooksellers_hit->{'accountnumber'};                       # rechnungsEmpfaenger is optional
                 # $param->{rechnungsEmpfaenger}->{adresseElement}->{...}     # (ekz believes that adresseElement will be ignored)

                $param->{rechnungsKopieEmpfaenger}->{ekzKundenNr} =
                    $aqbooksellers_hit->{'accountnumber'};    # rechnungsKopieEmpfaenger is optional
                ...
                $param->{quellSystem} = 'LMSCloud'
                    ; # 'quellSystem' ist optional.  Oder Angabe der Untergruppe, z.B. DKSH Rendsburg? Woher dann nehmen?
                ...
                foreach my $aqorders_hit ( @{$aqorders_hits} ) {
                    $logger->debug(
                        "BestelleBeiEkz() aqorders_hit->{ordernumber}:" . $aqorders_hit->{'ordernumber'} . ":" );
                    ...
                    my $query_aqorders_items =
                        "SELECT * FROM aqorders_items WHERE ordernumber = ? ORDER BY itemnumber ";
                    my $sth = $dbh->prepare($query_aqorders_items);
                    $sth->execute( $aqorders_hit->{'ordernumber'} );
                    my $aqorders_items_hits = $sth->fetchall_arrayref( {} );
                    $logger->info(
                        "BestelleBeiEkz() scalar aqorders_items_hits:" . scalar @{$aqorders_items_hits} . ":" );
                    foreach my $aqorders_items_hit ( @{$aqorders_items_hits} ) {
                        push @{ $aqbasketgroupRes->{aqorders}->{ $aqorders_hit->{'ordernumber'} }->{orderitemnumbers} },
                            $aqorders_items_hit->{'itemnumber'};
                    }
                    ...
                    if (   $aqorders_hit->{'orderstatus'} eq 'cancelled'
                        ...
                        || scalar
                        @{ $aqbasketgroupRes->{aqorders}->{ $aqorders_hit->{'ordernumber'} }->{orderitemnumbers} } < 1 )
                    {

                        my $mess = sprintf(
                            "warning: Koha aqorder is skipped! (ordernumber:%s: orderstatus:%s: quantity:%s: items:%s:.",
                            $aqorders_hit->{'ordernumber'}, $aqorders_hit->{'orderstatus'}, $aqorders_hit->{'quantity'},
                            scalar @{
                                $aqbasketgroupRes->{aqorders}->{ $aqorders_hit->{'ordernumber'} }->{orderitemnumbers}
                            }
                        );
                        $logger->warn("BestelleBeiEkz() error: $mess");
                    ...
                    $logger->debug( "BestelleBeiEkz() Koha::Biblios->find->metadata->record of biblionumber:"
                            . $aqorders_hit->{'biblionumber'}
                            . ":" );
                    ...
                    if ($marcrecord) {
                        ...
                        foreach my $ind2Sort ( 1, 2, 0 )
                        {    # priority: publication (1) / distribution (2) / production (0)
                    ...
                    $param->{titel}->[$iTitel]->{titelangabe}->{titelInfo}->{erscheinungsJahr} = $erscheinungsJahr
                        if $erscheinungsJahr;

                    # exemplar info  (optional)
                    $param->{titel}->[$iTitel]->{exemplar}->[0]->{lmsExemplarID} =
                        $aqorders_hit->{'ordernumber'} . '';    # force type to string
                    ...
                    $param->{titel}->[$iTitel]->{exemplar}->[0]->{konfiguration}->{warenEmpfaenger}->{ekzKundenNr} =
                        $aqbooksellers_hit->{'accountnumber'};    # warenEmpfaenger is optional
                    ...
                    if ( $haushaltsstelle || $kostenstelle ) {
                        $param->{titel}->[$iTitel]->{exemplar}->[0]->{konfiguration}->{budget}->[0]->{anteil} = 100
                            ; # Verplanung auf / Bezahlung über eine einzige Kostenstelle (Koha-Etat), keine Aufteilung auf mehrere Etats
                        $param->{titel}->[$iTitel]->{exemplar}->[0]->{konfiguration}->{budget}->[0]->{haushaltsstelle}
                            = $haushaltsstelle
                            if $haushaltsstelle;
                        $param->{titel}->[$iTitel]->{exemplar}->[0]->{konfiguration}->{budget}->[0]->{kostenstelle} =
                            $kostenstelle
                            if $kostenstelle;
                    }
                    ...
                    $param->{titel}->[$iTitel]->{exemplar}->[0]->{konfiguration}->{preis}->{rabatt} =
                        $aqorders_hit->{'discount'};
                    $param->{titel}->[$iTitel]->{exemplar}->[0]->{konfiguration}->{preis}->{fracht} =
                        0.0;    # also required, may be 0.00 of course
                    $param->{titel}->[$iTitel]->{exemplar}->[0]->{konfiguration}->{preis}->{einband} =
                        0.0;    # also required, may be 0.00 of course
                    $param->{titel}->[$iTitel]->{exemplar}->[0]->{konfiguration}->{preis}->{bearbeitung} =
                        0.0;    # also required, may be 0.00 of course
                    ...
                    $param->{titel}->[$iTitel]->{exemplar}->[0]->{konfiguration}->{preis}->{ustSatz} =
                        $aqorders_hit->{'tax_rate_on_ordering'} * 100.0;    # ustSatz==7 represents 7% GST
                    $param->{titel}->[$iTitel]->{exemplar}->[0]->{konfiguration}->{preis}->{ust} =
                        $aqorders_hit->{'tax_value_on_ordering'};
                    my $listprice = $aqorders_hit->{'listprice'};    # in the supplier's currency, price for single item
                    my $listpriceMinusDiscount = $listprice -
                        ( $listprice * ( $aqorders_hit->{'discount'} / 100.0 ) )
                        ;    # in the supplier's currency, price for single item, discout applied
                    $param->{titel}->[$iTitel]->{exemplar}->[0]->{konfiguration}->{preis}->{gesamtpreis} =
                        $listpriceMinusDiscount;
                    $logger->trace( "BestelleBeiEkz() listprice:"
                            . $listprice
                            . ": discount:"
                            . $aqorders_hit->{'discount'}
                            . ": listpriceMinusDiscount:"
                    ...
                    $gesamtpreisSum +=
                        $param->{titel}->[$iTitel]->{exemplar}->[0]->{konfiguration}->{preis}->{gesamtpreis} *
                        $param->{titel}->[$iTitel]->{exemplar}->[0]->{konfiguration}->{anzahl};
                ...
                if ( $iTitel < 1 ) {
                    my $mess = sprintf(
                        "aqbasket with basketno:%s: does not refer to any aqorder with regular orderstatus (new, ordered, etc.) and at least one item.",
                        $aqbasket_hit->{basketno}
                    );
                    $logger->warn("BestelleBeiEkz() error: $mess");
                    ...
                    $sth->finish();    # finish $query_aqbasket
                    last
                        ; # ignore further aqbasket hits in this aqbasketgroup - they must not exist, according to the specification
                }
                ...
                $logger->debug( "BestelleBeiEkz() is searching for acquisition_import_objects record with selParam:"
                        . Dumper($selParam)
                        . ":" );
                my $oldOrderHits_rs = $acquisitionImportOldOrder->_resultset()->search($selParam);

                $logger->debug( "BestelleBeiEkz() count of old acquisition_import records for same aqbasketgroupno:"
                        . $oldOrderHits_rs->count()
                        . ":" );
                while ( ( my $hit = $oldOrderHits_rs->next() ) ) {

                    # delete assigned record in acquisition_import_objects
                    my $selParam = { acquisition_import_id => $hit->{_column_data}->{'id'} };
                    $logger->debug( "BestelleBeiEkz() is searching for acquisition_import_objects record with selParam:"
                            . Dumper($selParam)
                            . ":" );
                    ...
                    $logger->debug(
                        "BestelleBeiEkz() count of old acquisition_import_objects records for this acquisition_import.id:"
                            . $oldOrderObjectsHits_rs->count()
                            . ": All of them will be deleted now." );
                    $oldOrderObjectsHits_rs->delete();

                    # delete hit record in acquisition_import
                    $logger->debug( "BestelleBeiEkz() acquisition_import record with id:"
                            . $hit->{_column_data}->{'id'}
                            . ": will be deleted now." );
                    $hit->delete();
                }
                ...
                ( $wsResult, $wsRequest, $wsResponse ) =
                    $ekzwebservice->callWsBestellung( $aqbooksellers_hit->{'accountnumber'}, $param, 1, undef )
                    ;    # not calling the webService, just generating the request string
                ...
                my $insParam                 = {
                    ...
                    vendor_id     => 'ekz',
                    object_type   => 'order',
                    object_number => 'basketno:'
                        . $aqbasketgroupRes->{basketno}
                    ,    # the basketno will be sent in BestellInfo request in XML-Element lmsBestellCode
                    object_date => DateTime::Format::MySQL->format_datetime($dateTimeNowOfBasketOrder)
                    ,    # in local time_zone
                    rec_type => 'message',

                    #object_item_number => '', # NULL
                    processingstate => 'requested',
                    processingtime  => DateTime::Format::MySQL->format_datetime($dateTimeNowOfBasketOrder)
                    ,    # in local time_zone
                ...
                $acquisitionImportIdBestellung =
                    $acquisitionImportBestellung->_resultset()->create($insParam)->get_column('id');
                ...
                foreach my $ordernumber ( sort { $a cmp $b } keys %{ $aqbasketgroupRes->{aqorders} } ) {
                    ...
                    my $insParam = {
                        ...
                        object_item_number => 'ordernumber:'
                            . $ordernumber
                        ,    # the ordernumber will be sent in BestellInfo request in XML-Element lmsExemplarID
                        processingstate => 'requested',
                        processingtime  => DateTime::Format::MySQL->format_datetime($dateTimeNowOfBasketOrder)
                        ,    # in local time_zone
                             #payload => NULL, # NULL
                    ...
                    foreach my $itemnumber ( @{ $aqbasketgroupRes->{aqorders}->{$ordernumber}->{orderitemnumbers} } ) {
                        ...
                        my $insParam = {
                            ...
                            object_item_number => 'ordernumber:'
                                . $ordernumber
                            , # the ordernumber will be sent in BestellInfo request by BestellInfo in XML-Element lmsExemplarID
                            processingstate => 'requested',
                            processingtime  => DateTime::Format::MySQL->format_datetime($dateTimeNowOfBasketOrder)
                            ,    # in local time_zone
                                 #payload => NULL, # NULL
                ...
                $logger->debug(
                    "BestelleBeiEkz() is calling C4::External::EKZ::lib::EkzWebServices->callWsBestellung() with param:"
                        . Dumper($param)
                        . ":" );
                ( $wsResult, $wsRequest, $wsResponse ) =
                    $ekzwebservice->callWsBestellung( $aqbooksellers_hit->{'accountnumber'}, $param, 1, $wsRequest )
                    ; # 1st param (ekzKundenNr) not used at the moment  - but could be used for getting name for <besteller> from systempreferences
                ###########################################################################################

                $logger->debug(
                    "BestelleBeiEkz() has called C4::External::EKZ::lib::EkzWebServices->callWsBestellung(); wsResult:"
                        . Dumper($wsResult)
                        . ":" );
                last
                    ; # ignore further aqbasket hits in this aqbasketgroup - they must not exist, according to the specification
            ...
            if ( $wsResult->{statusCode} ne 'SUCCESS' || !$wsResult->{ekzBestellNr} ) {
                my $mess =
                    "statusCode:'" . $wsResult->{statusCode} . "' statusMessage:'" . $wsResult->{statusMessage} . "'";
                ...
                my $query2_aqorders =
                    "UPDATE aqorders SET order_vendornote = IF(order_vendornote IS NULL, ?, CONCAT(order_vendornote, '\n', ?))  WHERE ordernumber = ? ";
                ...
                my $query2_aqbasket =
                    "UPDATE aqbasket SET booksellernote = IF(booksellernote IS NULL, ?, CONCAT(booksellernote, '\n', ?))  WHERE basketno = ? ";
...
sub BestelleBeiEkz_Test {
    ...
    $param->{titel}->[0]->{exemplar}->[0]->{konfiguration}->{preis}->{fracht} =
        0.0;    # also required, may be 0.00 of course
    $param->{titel}->[0]->{exemplar}->[0]->{konfiguration}->{preis}->{einband} =
        0.0;    # also required, may be 0.00 of course
    $param->{titel}->[0]->{exemplar}->[0]->{konfiguration}->{preis}->{bearbeitung} =
        0.0;    # also required, may be 0.00 of course
