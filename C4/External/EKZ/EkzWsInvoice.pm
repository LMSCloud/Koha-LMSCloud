...
use C4::Items qw( ModItemFromMarc )
    ; # additionally GetMarcItem is required here, but it is not exported by C4::Items, so we have to use it inofficially
...
sub readReFromEkzWsRechnungList {
    ...
    $logger->info( "readReFromEkzWsRechnungList() START ekzCustomerNumber:"
            . ( defined($ekzCustomerNumber) ? $ekzCustomerNumber : 'undef' )
            . ": selVon:"
            . ( defined($selVon) ? $selVon : 'undef' )
            . ": selBis:"
            . ( defined($selBis) ? $selBis : 'undef' )
            . ": selKundennummerWarenEmpfaenger:"
            . ( defined($selKundennummerWarenEmpfaenger) ? $selKundennummerWarenEmpfaenger : 'undef' )
            . ":" );

    my $ekzwebservice = C4::External::EKZ::lib::EkzWebServices->new();
    $result =
        $ekzwebservice->callWsRechnungList( $ekzCustomerNumber, $selVon, $selBis, $selKundennummerWarenEmpfaenger );
...
sub readReFromEkzWsRechnungDetail {
    ...
    $logger->info( "readReFromEkzWsRechnungDetail() START ekzCustomerNumber:"
            . ( defined($ekzCustomerNumber) ? $ekzCustomerNumber : 'undef' )
            . ": selId:"
            . ( defined($selId) ? $selId : 'undef' )
            . ": selRechnungsnummer:"
            . ( defined($selRechnungsnummer) ? $selRechnungsnummer : 'undef' )
            . ":" );
    $logger->trace( "readReFromEkzWsRechnungDetail() START \$refRechnungDetailElement:"
            . $refRechnungDetailElement
            . ": Dumper(\$refRechnungDetailElement):"
            . Dumper($refRechnungDetailElement)
            . ":" );

    my $ekzwebservice = C4::External::EKZ::lib::EkzWebServices->new();
    $result = $ekzwebservice->callWsRechnungDetail(
        $ekzCustomerNumber, $selId, $selRechnungsnummer,
        $refRechnungDetailElement
    );
...
sub genKohaRecords {
    my (
        $ekzCustomerNumber, $messageID, $rechnungDetailElement, $rechnungRecord, $createdTitleRecords,
        $updatedTitleRecords
    ) = @_;
    ...
    $logger->info(
              "genKohaRecords() START ekzCustomerNumber:"
            . ( defined($ekzCustomerNumber) ? $ekzCustomerNumber : 'undef' )
            . ": messageID:"
            . ( defined($messageID)                ? $messageID              : 'undef' ) . ": id:"
            . ( defined( $rechnungRecord->{'id'} ) ? $rechnungRecord->{'id'} : 'undef' )
            . ": Rechnungsnummer:"
            . ( defined( $rechnungRecord->{'nummer'} ) ? $rechnungRecord->{'nummer'} : 'undef' )
            . ": mehrpreisRechnung:"
            . ( defined( $rechnungRecord->{'mehrpreisRechnung'} ) ? $rechnungRecord->{'mehrpreisRechnung'} : 'undef' )
            . ": auftragsPositionCount:"
            . (
            defined( $rechnungRecord->{'auftragsPositionCount'} )
            ? $rechnungRecord->{'auftragsPositionCount'}
            : 'undef'
            )
            . ":"
    );
    ...
    try {
        ...
        my $ekzWebServicesSetItemSubfieldsWhenOrdered =
            C4::Context->preference("ekzWebServicesSetItemSubfieldsWhenOrdered");
        my $ekzWebServicesSetItemSubfieldsWhenInvoiced =
            C4::Context->preference("ekzWebServicesSetItemSubfieldsWhenInvoiced");
        ...
        $rechnungDatum = DateTime->new(
            year => substr( $reDatum, 0, 4 ), month     => substr( $reDatum, 5, 2 ),
            day  => substr( $reDatum, 8, 2 ), time_zone => 'local'
        );
        ...
        if ( defined($mehrpreisRechnung) && $mehrpreisRechnung eq 'true' ) {
            ...
            my $mess =
                sprintf( "The invoice '%s' is a so called 'Mehrpreisrechnung'. Processing denied.\n", $rechnungNummer );
            ...
            $logger->trace( "genKohaRecords() search invoice message record in acquisition_import selParam:"
                    . Dumper($selParam)
                    . ":" );
            ...
            if ( defined($hit) ) {
                ...
                my $mess = sprintf(
                    "The invoice number '%s' has already been used at %s. Processing denied.",
                ...
                $logger->trace(
                    "genKohaRecords() ref(schemaResultAcquitionImport):" . ref($schemaResultAcquitionImport) . ":" );

                #$logger->trace("genKohaRecords() Dumper(schemaResultAcquitionImport):" . Dumper($schemaResultAcquitionImport) . ":");
                $logger->trace( "genKohaRecords() Dumper(schemaResultAcquitionImport->{_column_data}):"
                        . Dumper( $schemaResultAcquitionImport->{_column_data} )
                        . ":" );
        ...
        if ( !$rechnungNummerIsDuplicate && !$rechnungIsMehrpreisRechnung ) {
            my $invoiceids = ()
                ; # if all works as designed, the referenced hash should hold exactly 1 element at the end of the loop (additional elements are stored for debugging only)
            ...
            foreach my $auftragsPosition ( @{ $rechnungRecord->{'auftragsPositionRecords'} } ) {
                ...
                $logger->trace(
                    "genKohaRecords() auftragsPosition ekzexemplarid:"
                        . (
                        defined( $auftragsPosition->{'ekzexemplarid'} ) ? $auftragsPosition->{'ekzexemplarid'}
                        : 'undef'
                        )
                        . ": referenznummer:"
                        . (
                        defined( $auftragsPosition->{'referenznummer'} ) ? $auftragsPosition->{'referenznummer'}
                        : 'undef'
                        )
                        . ": artikelNummer:$auftragsPosition->{'artikelNummer'}: isbn:$auftragsPosition->{'isbn'}: ean:$auftragsPosition->{'ean'}: exemplareBestellt:$auftragsPosition->{'exemplareBestellt'}: kundenBestelldatum:$auftragsPosition->{'kundenBestelldatum'}:"
                );
                ...
                my $invoicedItemsCount =
                    $auftragsPosition->{'exemplareBestellt'};    # This is the invoiced (=delivered) item quantity
                ...
                if (   defined( $auftragsPosition->{'ekzexemplarid'} )
                    && length( $auftragsPosition->{'ekzexemplarid'} ) > 0
                    && $updOrInsItemsCount < $invoicedItemsCount )
                {
                    ...
                    my $selParam = {
                        ...
                        rec_type           => "item",
                        processingstate    =>
                            { '-IN' => [ 'ordered', 'delivered' ] }    # AND processingstate IN ('ordered', 'delivered')
                    };
                    $logger->trace( "genKohaRecords() method1: search order item record in acquisition_import selParam:"
                            . Dumper($selParam)
                            . ":" );
                    my $acquisitionImportEkzExemplarIdHits =
                        Koha::AcquisitionImport::AcquisitionImports->new()->_resultset()->search($selParam);
                    $logger->trace( "genKohaRecords() method1: scalar acquisitionImportEkzExemplarIdHits:"
                    ...
                    foreach my $acquisitionImportEkzExemplarIdHit ( $acquisitionImportEkzExemplarIdHits->all() ) {
                        ...
                        $logger->trace( "genKohaRecords() method1: acquisitionImportEkzExemplarIdHit->{_column_data}:"
                                . Dumper( $acquisitionImportEkzExemplarIdHit->{_column_data} )
                                . ":" );
                        ...
                        my $selParam = {
                            ...
                            rec_type        => "title",
                            processingstate =>
                                'ordered' # processingstate of record with rec_type "title" and object_type => 'order' stays always in state 'ordered'
                        };
                        $logger->trace(
                                  "genKohaRecords() method1: search title order record in acquisition_import selParam:"
                                . Dumper($selParam)
                                . ":" );
                        my $acquisitionImportTitleHit =
                            Koha::AcquisitionImport::AcquisitionImports->new()->_resultset()->find($selParam)
                            ;    # unique via 'id'
                        $logger->trace( "genKohaRecords() method1: acquisitionImportTitleHit->{_column_data}::"
                                . Dumper( $acquisitionImportTitleHit->{_column_data} )
                                . ":" );
                        if ( defined($acquisitionImportTitleHit) ) {
                            ...
                            $logger->trace(
                                "genKohaRecords() method1: search title order record in acquisition_import_objects selParam:"
                                    . Dumper($selParam)
                                    . ":" );
                            ...
                            $logger->trace( "genKohaRecords() method1: titleObjectRS->{_column_data}:"
                                    . Dumper( $titleObjectRS->{_column_data} )
                                    . ":" );
                            ...
                            if ( defined($selBiblionumber) ) {
                                if ( $selBiblionumber != $biblionumber ) {
                                    ...
                                    if ( $titleHits->{'count'} > 0 && defined $titleHits->{'records'}->[0] ) {
                                        ...
                                        my $tmp_cn =
                                            ( defined( $titleHits->{'records'}->[0]->field("001") )
                                                && $titleHits->{'records'}->[0]->field("001")->data() )
                                            ? $titleHits->{'records'}->[0]->field("001")->data()
                                            : $biblionumber;
                                        my $tmp_cna =
                                            ( defined( $titleHits->{'records'}->[0]->field("003") )
                                                && $titleHits->{'records'}->[0]->field("003")->data() )
                                            ? $titleHits->{'records'}->[0]->field("003")->data()
                                            : "undef";
                                        ...
                                        $ekzKohaRecord->overwriteCatalogDataIfRequired(
                                            $ekzCustomerNumber,
                                            $biblionumber, $reqParamTitelInfo, $titleHits->{'records'}->[0],
                                            $updatedTitleRecords
                                        );
                                        ...
                                        ( $titeldata, $isbnean ) =
                                            $ekzKohaRecord->getShortISBD( $titleHits->{'records'}->[0] );
                                        push @{ $emaillog->{'records'} },
                                            [
                                            $invEkzArtikelNr,
                                            defined $biblionumber ? $biblionumber : "no biblionumber",
                                            $emaillog->{'importresult'}, $titeldata, $isbnean, $emaillog->{'problems'},
                                            $emaillog->{'importerror'},  1,          undef,    undef
                                            ];
                                        $logger->trace( "genKohaRecords() method1: emaillog->{'records'}->[0]:"
                                                . Dumper( $emaillog->{'records'}->[0] )
                                                . ":" );
                                    } else {
                                ...
                                my $invoiceid = &processItemHit(
                                    $rechnungNummer,                             $rechnungDatum,   $dateTimeNow,
                                    $ekzWebServicesSetItemSubfieldsWhenInvoiced, $invEkzArtikelNr, '', $rechnungRecord,
                                    $auftragsPosition, $acquisitionImportTitleHit,            $titleHits, $biblionumber,
                                    $acquisitionImportEkzExemplarIdHit, $emaillog,            \$updOrInsItemsCount,
                                    $ekzAqbooksellersId,                $updatedTitleRecords, $logger
                                );
                                if ($invoiceid) {
                ...
                $logger->debug(
                    "genKohaRecords() after method 1 invoicedItemsCount:$invoicedItemsCount: updOrInsItemsCount:$updOrInsItemsCount: titleHits->{'count'}:$titleHits->{'count'}: biblionumber:$biblionumber: invEkzArtikelNr:$invEkzArtikelNr:"
                );

                # method2: searching for ekzArtikelNr and referenznummer identity if ekzArtikelNr > 0 and referenznummer > 0
                #          (Typical for an item of a running standing order since 2020-09-14 or of a running serial order since 2021-01-14,
                #           when referenznummer for standing order item and serial order item was introduced by ekz.)
                if (   defined( $auftragsPosition->{'artikelNummer'} )
                    && $auftragsPosition->{'artikelNummer'} > 0
                    && defined( $auftragsPosition->{'referenznummer'} )
                    && length( $auftragsPosition->{'referenznummer'} ) > 0
                    && $updOrInsItemsCount < $invoicedItemsCount )
                {
                    ...
                    $logger->trace( "genKohaRecords() method2: search order item record in acquisition_import selParam:"
                            . Dumper($selParam)
                            . ": orderByParam:"
                            . Dumper($orderByParam)
                            . ":" );
                    ...
                    foreach my $acquisitionImportEkzExemplarIdHit ( $acquisitionImportEkzExemplarIdHits->all() ) {
                        ...
                        $logger->trace( "genKohaRecords() method2: acquisitionImportEkzExemplarIdHit->{_column_data}:"
                                . Dumper( $acquisitionImportEkzExemplarIdHit->{_column_data} )
                                . ":" );
                        ...
                        $logger->trace(
                                  "genKohaRecords() method2: search title order record in acquisition_import selParam:"
                                . Dumper($selParam)
                                . ":" );
                        my $acquisitionImportTitleHit =
                            Koha::AcquisitionImport::AcquisitionImports->new()->_resultset()->find($selParam)
                            ;    # unique via 'id'
                        $logger->trace( "genKohaRecords() method2: acquisitionImportTitleHit->{_column_data}::"
                                . Dumper( $acquisitionImportTitleHit->{_column_data} )
                                . ":" );
                        if ( defined($acquisitionImportTitleHit) ) {
                            my $selParam = {
                            ...
                            $logger->trace(
                                "genKohaRecords() method2: search title order record in acquisition_import_objects selParam:"
                                    . Dumper($selParam)
                                    . ":" );
                            my $titleObject     = Koha::AcquisitionImport::AcquisitionImportObjects->new();
                            my $titleObjectRS   = $titleObject->_resultset()->search($selParam)->first();
                            my $selBiblionumber = $titleObjectRS->get_column('koha_object_id');
                            $logger->trace( "genKohaRecords() method2: titleObjectRS->{_column_data}:"
                                    . Dumper( $titleObjectRS->{_column_data} )
                                    . ":" );
                            ...
                            if ( defined($selBiblionumber) ) {
                                if ( $selBiblionumber != $biblionumber ) {
                                    ...
                                    if ( $titleHits->{'count'} > 0 && defined $titleHits->{'records'}->[0] ) {
                                        ...
                                        $logger->trace( "genKohaRecords() method2: emaillog->{'records'}->[0]:"
                                                . Dumper( $emaillog->{'records'}->[0] )
                                                . ":" );
                                    } else {
                                        next;    # next in acquisitionImportEkzExemplarIdHits->all()
                                ...
                                my $invoiceid = &processItemHit(
                                    $rechnungNummer,                             $rechnungDatum,   $dateTimeNow,
                                    $ekzWebServicesSetItemSubfieldsWhenInvoiced, $invEkzArtikelNr, $invReferenznummer,
                                    $rechnungRecord, $auftragsPosition, $acquisitionImportTitleHit, $titleHits,
                                    $biblionumber, $acquisitionImportEkzExemplarIdHit, $emaillog, \$updOrInsItemsCount,
                                    $ekzAqbooksellersId, $updatedTitleRecords,         $logger
                                );
                                if ($invoiceid) {
                                    $invoiceids->{$invoiceid} = $invoiceid;
                ...
                $logger->debug(
                    "genKohaRecords() after method 2 invoicedItemsCount:$invoicedItemsCount: updOrInsItemsCount:$updOrInsItemsCount: titleHits->{'count'}:$titleHits->{'count'}: biblionumber:$biblionumber: invEkzArtikelNr:$invEkzArtikelNr:"
                );
                ...
                if ( $updOrInsItemsCount < $invoicedItemsCount ) {
                    $logger->debug(
                        "genKohaRecords() method4: create item for ekzArtikelNr:$auftragsPosition->{'artikelNummer'}:");

                    if (   $titleHits->{'count'} == 0
                        || !defined $titleHits->{'records'}->[0]
                        || !defined( $titleHits->{'records'}->[0]->field("001") )
                        || $titleHits->{'records'}->[0]->field("001")->data() != $auftragsPosition->{'artikelNummer'}
                        || !defined( $titleHits->{'records'}->[0]->field("003") )
                        || $titleHits->{'records'}->[0]->field("003")->data() ne "DE-Rt5" )
                    {

                        # get additional fields of title data
                        $reqParamTitelInfo->{'ekzArtikelArt'} = $auftragsPosition->{'artikelart'}
                            ; # TODO: this is not a code value as in BestellInfo, but plain text (e.g. 'Bücher' instead of 'B', so a mapping function is required
                        ...
                        if ( $auftragsPosition->{'waehrung'} eq 'EUR'
                            && defined( $auftragsPosition->{'verkaufsPreis'} ) )
                        {
                            $reqParamTitelInfo->{'preis'} = $auftragsPosition->{'verkaufsPreis'}
                                ;    # without regard to $auftragsPosition->{'nachlass'}
                        }
                        $reqParamTitelInfo->{'auflage'} =
                            $auftragsPosition->{'auflageNummer'} . $auftragsPosition->{'auflageText'};
                        $logger->debug( "genKohaRecords() method4: reqParamTitelInfo->{'ekzArtikelNr'}:"
                                . $reqParamTitelInfo->{'ekzArtikelNr'}
                                . ":" );
                        ...
                        if ( length($titleSelHashkey) > 6 && defined( $createdTitleRecords->{$titleSelHashkey} ) ) {
                            ...
                            $logger->info(
                                "genKohaRecords() method4: got used biblionumber:$biblionumber: from createdTitleRecords->{$titleSelHashkey}"
                            );
                        }
                        ...
                        for ( my $searchmode = 1 ; $searchmode <= 2 ; $searchmode *= 2 ) {
                            $logger->info( "genKohaRecords() method4: in loop searchmode:$searchmode: ekzArtikelNr:"
                                    . $reqParamTitelInfo->{'ekzArtikelNr'}
                                    . ": titleHits->{'count'}:"
                                    . $titleHits->{'count'}
                                    . ":" );
                            if ( $titleHits->{'count'} > 0 && defined $titleHits->{'records'}->[0] ) {
                                ...
                                $logger->debug(
                                    "genKohaRecords() method4: in loop last; searchmode:$searchmode: ekzArtikelNr:"
                                        . $reqParamTitelInfo->{'ekzArtikelNr'}
                                        . ": titleHits->{'count'}:"
                                        . $titleHits->{'count'}
                                        . ": biblionumber:"
                            ...
                            if ( $searchmode == 1 && !$reqParamTitelInfo->{'ekzArtikelNr'} ) {
                                $logger->debug(
                                    "genKohaRecords() method4: in loop next1; searchmode:$searchmode: ekzArtikelNr:"
                                        . $reqParamTitelInfo->{'ekzArtikelNr'}
                                        . ": titleHits->{'count'}:"
                                        . $titleHits->{'count'}
                                        . ":" );
                                next
                                    ; # ekzArtikelNr not sent in request, so search for remaining criteria (ISBN, EAN, etc.)
                            }
                            $titleHits = $ekzKohaRecord->readTitleInLocalDB( $reqParamTitelInfo, $searchmode, 1 );
                            $logger->info(
                                "genKohaRecords() method4: in loop searchmode:$searchmode: from local DB titleHits->{'count'}:"
                                    . $titleHits->{'count'}
                                    . ":" );
                            if ( $titleHits->{'count'} > 0 && defined $titleHits->{'records'}->[0] ) {
                                $biblionumber = $titleHits->{'records'}->[0]->subfield( "999", "c" );
                                $logger->debug(
                                    "genKohaRecords() method4: in loop next2; searchmode:$searchmode: ekzArtikelNr:"
                                        . $reqParamTitelInfo->{'ekzArtikelNr'}
                                        . ": titleHits->{'count'}:"
                                        . $titleHits->{'count'}
                                        . ": biblionumber:"
                            ...
                            foreach my $titleSource (@titleSourceSequence) {
                                $logger->info(
                                    "genKohaRecords() method4: searchmode:$searchmode: in loop titleSource:$titleSource:"
                                );
                                if ( $titleHits->{'count'} > 0 && defined $titleHits->{'records'}->[0] ) {
                                    last;    # title data have been found in lastly tested title source
                                }
                                ...
                                if ( $titleSource eq '_LMSC' ) {
                                    if ( $searchmode == 2 ) {
                                        ...
                                        $titleHits =
                                            $ekzKohaRecord->readTitleInLMSPool( $reqParamTitelInfo, $searchmode );
                                        $logger->info(
                                            "genKohaRecords() method4: from LMS Pool searchmode:$searchmode: titleHits->{'count'}:"
                                                . $titleHits->{'count'}
                                                . ":" );
                                    }
                                    ...
                                    if ( $searchmode == 2 ) {
                                        ...
                                        $titleHits = $ekzKohaRecord->readTitleFromEkzWsMedienDaten(
                                            $reqParamTitelInfo->{'ekzArtikelNr'} );
                                        $logger->info(
                                            "genKohaRecords() method4: from ekz Webservice 'MedienDaten' searchmode:$searchmode: titleHits->{'count'}:"
                                                . $titleHits->{'count'}
                                                . ":" );
                                    ...
                                    if ( $searchmode == 2 ) {
                                        ...
                                        $titleHits = $ekzKohaRecord->createTitleFromFields($reqParamTitelInfo)
                                            ;    # creates marc data, not a biblio DB record
                                        $logger->info(
                                            "genKohaRecords() method4: from sent titel fields searchmode:$searchmode: titleHits->{'count'}:"
                                                . $titleHits->{'count'}
                                                . ":" );
                                    }
                                    ...
                                    if ( $searchmode == 2 ) {
                                        ...
                                        $titleHits = $ekzKohaRecord->readTitleFromZ3950Target(
                                            $titleSource,
                                            $reqParamTitelInfo
                                        );
                                        $logger->info( "genKohaRecords() method4: from z39.50 search on target:"
                                                . $titleSource
                                                . ": searchmode:$searchmode: titleHits->{'count'}:"
                                                . $titleHits->{'count'}
                                                . ":" );
                                    }
                                }
                        ...
                        if ( $titleHits->{'count'} > 0 && defined $titleHits->{'records'}->[0] ) {
                            if ( $biblionumber == 0 )
                            {    # title data have been found in one of the sources, but not in local DB
                                ...
                                ( $biblionumber, $biblioitemnumber, $newrec ) = $ekzKohaRecord->addNewRecords(
                                    $titleHits,                    $volumeEkzArtikelNr,  $ekzBestellNr,
                                    $ekzWsHideOrderedTitlesInOpac, $createdTitleRecords, $titleSelHashkey
                                );
                                $logger->info( "genKohaRecords() method4: new biblionumber:"
                                        . $biblionumber
                                        . ": biblioitemnumber:"
                                        . $biblioitemnumber
                                        . ":" );
                                $logger->debug( "genKohaRecords() method4: titleHits:" . Dumper($titleHits) . ":" );
                                $logger->trace( "genKohaRecords() method4: titleSelHashkey:"
                                        . $titleSelHashkey
                                        . ": createdTitleRecords->{titleSelHashkey}->{biblionumber}:"
                                        . $createdTitleRecords->{$titleSelHashkey}->{biblionumber}
                                        . ": ->{titleHits}:"
                                        . Dumper( $createdTitleRecords->{$titleSelHashkey}->{titleHits} )
                                        . ":" );

                                if ( defined $biblionumber && $biblionumber > 0 ) {
                                    $updatedTitleRecords->{$biblionumber}->{biblionumber} = $biblionumber
                                        ; # it makes no sense to overwrite title data that have been inserted in this run
                                    $biblioInserted = 1;
                                    ...
                                    $emaillog->{'problems'} .=
                                        "ERROR: Import der Titeldaten für EKZ Artikel: $reqParamTitelInfo->{'ekzArtikelNr'} wurde abgewiesen.\n";
                                    $emaillog->{'importresult'} = -1;
                                ...
                                # Check if to reread title data via webservice MedienDaten etc. and, if required, (partially) overwrite the title record.
                                $ekzKohaRecord->overwriteCatalogDataIfRequired(
                                    $ekzCustomerNumber, $biblionumber,
                                    $reqParamTitelInfo, $titleHits->{'records'}->[0], $updatedTitleRecords
                                );
                            ...
                            push @{ $emaillog->{'records'} },
                                [
                                $reqParamTitelInfo->{'ekzArtikelNr'},
                                defined $biblionumber ? $biblionumber : "no biblionumber", $emaillog->{'importresult'},
                                $titeldata, $isbnean, $emaillog->{'problems'}, $emaillog->{'importerror'}, 1, undef,
                                undef
                                ];
                            $logger->trace( "genKohaRecords() method4: emaillog->{'records'}->[0]:"
                                    . Dumper( $emaillog->{'records'}->[0] )
                                    . ":" );
                        }
                    }
                    $logger->info(
                        "genKohaRecords() method4: biblioExisting:$biblioExisting: biblioInserted:$biblioInserted: biblionumber:$biblionumber: invEkzArtikelNr:$invEkzArtikelNr:"
                    );
                    ...
                    if ( $biblioExisting || $biblioInserted ) {
                        ...
                        if ( !defined($invEkzArtikelNr) || $invEkzArtikelNr eq '0' || $invEkzArtikelNr eq '' ) {
                            ...
                            $invEkzArtikelNr = $tmp_cna . '-' . $tmp_cn;
                            $logger->info(
                                "genKohaRecords() method4: created fallback invEkzArtikelNr:$invEkzArtikelNr:");
                        }
                        ...
                        $logger->trace( "genKohaRecords() method4: acquisitionImportTitleRS->{_column_data}:"
                                . Dumper( $acquisitionImportTitleRS->{_column_data} )
                                . ":" );
                        ...
                        $logger->trace( "genKohaRecords() method4: titleImportObjectRS->{_column_data}:"
                                . Dumper( $titleImportObjectRS->{_column_data} )
                                . ":" );
                        ...
                        my $acquisitionImportTitleInvoiceRS =
                            $acquisitionImportTitleInvoice->_resultset()->create($insParam);
                        $logger->trace( "genKohaRecords() method4: acquisitionImportTitleInvoiceRS->{_column_data}:"
                                . Dumper( $acquisitionImportTitleInvoiceRS->{_column_data} )
                                . ":" );

                        # now add the items data for the new biblionumber
                        my $ekzExemplarID =
                            $ekzBestellNr . '-' . $invEkzArtikelNr;    # dummy item number for the 'invented' order
                        my $exemplarcount = $invoicedItemsCount - $updOrInsItemsCount;
                        $logger->trace(
                            "genKohaRecords() method4: exemplar ekzExemplarID:$ekzExemplarID: exemplarcount:$exemplarcount:"
                        );
                        ...
                        if ( length($ekzAqbooksellersId) ) {
                            ...
                            if ( @{$selbaskets} > 0 ) {
                                ...
                                $logger->info(
                                    "genKohaRecords() method4: found aqbasket with basketname:$basketname: having basketno:"
                                        . $basketno
                                        . ":" );
                            } else {
                                ...
                                if ($patron) {
                                    ...
                                    $logger->info(
                                        "genKohaRecords() method4: found patron with surname = 'LCService' authorisedby:"
                                            . $authorisedby
                                            . ":" );
                                }
                                my $branchcode = $ekzKohaRecord->branchcodeFallback( '', $homebranch );
                                $basketno = C4::Acquisition::NewBasket(
                                    $ekzAqbooksellersId, $authorisedby, $basketname,
                                    'created by ekz RechnungDetail', '', undef, $branchcode, $branchcode, 0,
                                    'ordering'
                                );    # XXXWH fixed text ok?
                                $logger->trace( "genKohaRecords() method4: created new basket having basketno:"
                                        . Dumper($basketno)
                                        . ":" );
                                if ($basketno) {
                                    my $basketinfo = {};
                        ...
                        $logger->info(
                            "genKohaRecords() method4: ekzAqbooksellersId:$ekzAqbooksellersId: acquisitionError:$acquisitionError: basketno:$basketno:"
                        );
                        ...
                        if ( defined($basketno) && $basketno > 0 ) {
                            ...
                            my ( $dummy1, $dummy2, $budgetid, $dummy3 ) =
                                $ekzKohaRecord->checkAqbudget( $ekzCustomerNumber, $haushaltsstelle, $kostenstelle, 1 );
                            ...
                            $orderinfo->{listprice} = $priceInfo->{verkaufsPreis}
                                ;    # input field 'Vendor price' in UI (in foreign currency, not discounted, per item)
                            $orderinfo->{unitprice} = 0.0
                                ; #  corresponds to input field 'Actual cost' in UI (discounted) and will be initialized with budgetedcost in the GUI in 'receiving' step
                            ...
                            $orderinfo->{order_vendornote}   = sprintf(
                            ...
                            if ( $priceInfo->{nachlass} != 0.0 ) {
                                $orderinfo->{order_vendornote} .=
                                    sprintf( "Nachlass: %.2f %s\n", $priceInfo->{nachlass}, $priceInfo->{waehrung} );
                            }
                            if ( $priceInfo->{wertPositionsTeil} != 0.0 ) {
                                $orderinfo->{order_vendornote} .= sprintf(
                                    "Positionsteilwert: %.2f %s\n", $priceInfo->{wertPositionsTeil},
                                    $priceInfo->{waehrung}
                                );
                            }
                            if ( $priceInfo->{wertMehrpreise} != 0.0 ) {
                                $orderinfo->{order_vendornote} .= sprintf(
                                    "Mehrpreis: %.2f %s\n", $priceInfo->{wertMehrpreise},
                                    $priceInfo->{waehrung}
                                );
                            }
                            if ( $priceInfo->{wertBearbeitung} != 0.0 ) {
                                $orderinfo->{order_vendornote} .= sprintf(
                                    "Bearbeitungspreis: %.2f %s\n", $priceInfo->{wertBearbeitung},
                                    $priceInfo->{waehrung}
                                );
                            }
                            ...
                            $orderinfo->{orderstatus}      = 'ordered'
                                ; # This orderstatus is transient; it will immediately be updated by the following call of processItemInvoice().
                            $orderinfo->{rrp} = $priceInfo->{replacementcost_tax_included}
                                ;    #  corresponds to input field 'Replacement cost' in UI (not discounted, per item)
                            ...
                            $orderinfo->{ecost}            = $priceInfo->{gesamtpreis_tax_included}
                                ; #  corresponds to input field 'Budgeted cost' in UI (discounted, per item); discounted
                            ...
                            $orderinfo->{tax_rate_bak}       = $priceInfo->{ustSatz}
                                ;    #  corresponds to input field 'Tax rate' in UI (7% are stored as 0.07)
                            $orderinfo->{tax_rate_on_ordering}  = $priceInfo->{ustSatz};
                            $orderinfo->{tax_rate_on_receiving} = undef
                                ; # This fieldvalue is transient; it will immediately be updated by the following call of processItemInvoice().
                            $orderinfo->{tax_value_bak} =
                                $priceInfo->{ust};    #  corresponds to input field 'Tax value' in UI
                            ...
                            $orderinfo->{tax_value_on_receiving} = undef
                                ; # This fieldvalue is transient; it will immediately be updated by the following call of processItemInvoice().
                            $orderinfo->{discount} = $priceInfo->{rabatt}
                                ;    #  corresponds to input field 'Discount' in UI (5% are stored as 5.0)
                            $logger->trace( "genKohaRecords() method4: trying to create Koha order with orderinfo:"
                                    . Dumper($orderinfo)
                                    . ":" );
                        ...
                        for ( my $j = 0 ; $j < $exemplarcount ; $j++ ) {
                                ...
                                && defined( $auftragsPosition->{'verkaufsPreis'} ) )
                            {
                                ...
                                $item_hash->{replacementprice} = $priceInfo->{replacementcost_tax_included}
                                    ;    # without regard to $auftragsPosition->{'nachlass'}
                            }
                            $item_hash->{notforloan} = 0
                                ; # default initialization: 'item is invoiced' implicitly means 'item is delivered' -> can be loaned (may be overwritten via sysprefs ekzWebServicesSetItemSubfieldsWhenOrdered and ekzWebServicesSetItemSubfieldsWhenInvoiced)
                            ...
                            if ( defined $itemnumber && $itemnumber > 0 ) {
                                ...
                                $logger->trace(
                                    "genKohaRecords() method4: searched first time for itemnumber:$itemnumber: itemHitCount:$itemHitCount: itemHit->{_column_data}:"
                                        . Dumper( $itemHit->{_column_data} )
                                        . ":" );
                                if ( $itemHitCount > 0 && defined( $itemHit->{_column_data} ) ) {
                                    ...
                                    if ( defined($ekzWebServicesSetItemSubfieldsWhenOrdered)
                                        && length($ekzWebServicesSetItemSubfieldsWhenOrdered) > 0 )
                                    {
                                        my @affects = split q{\|}, $ekzWebServicesSetItemSubfieldsWhenOrdered;
                                        $logger->debug( "genKohaRecords() method4: has to do "
                                                . scalar @affects
                                                . " affects (ordering)" );
                                        if (@affects) {
                                            my $frameworkcode = C4::Biblio::GetFrameworkCode($biblionumber);
                                            my ($itemfield) =
                                                C4::Biblio::GetMarcFromKohaField( 'items.itemnumber', $frameworkcode );
                                            ...
                                            if ($item) {
                                                ...
                                                C4::Items::ModItemFromMarc(
                                                    ...
                                                    ; # $updatedTitleRecords->{$titleRecordBiblionumber} has already been set a few lines ago
                                            }
                                        }
                                    }
                                }
                                ...
                                $logger->trace(
                                    "genKohaRecords() method4: searched second time for itemnumber:$itemnumber: itemHitCount:$itemHitCount: itemHit->{_column_data}:"
                                        . Dumper( $itemHit->{_column_data} )
                                        . ":" );
                                if ( $itemHitCount > 0 && defined( $itemHit->{_column_data} ) ) {
                                    ...
                                    if ( defined($ekzWebServicesSetItemSubfieldsWhenInvoiced)
                                        && length($ekzWebServicesSetItemSubfieldsWhenInvoiced) > 0 )
                                    {
                                ...
                                $logger->trace( "genKohaRecords() method4: acquisitionImportItemRS->{_column_data}:"
                                        . Dumper( $acquisitionImportItemRS->{_column_data} )
                                        . ":" );
                                ...
                                $logger->trace( "genKohaRecords() method4: itemImportObjectRS->{_column_data}:"
                                        . Dumper( $itemImportObjectRS->{_column_data} )
                                        . ":" );
                                ...
                                if ( defined($order) ) {
                                    ...
                                    if ( defined($ekzAqbooksellersId) && length($ekzAqbooksellersId) ) {
                                        $logger->debug(
                                            "genKohaRecords() method4: is calling processItemInvoice() itemnumber:$itemnumber: ordernumber:$ordernumber: basketno:$basketno:"
                                        );
                                        ( $ordernumberFound, $basketnoFound, $invoiceid ) = processItemInvoice(
                                            $rechnungNummer,                $rechnungDatum,       $biblionumber,
                                            $itemnumber,                    $rechnungRecord,      $auftragsPosition,
                                            $acquisitionImportTitleItemHit, $updatedTitleRecords, $logger
                                        );
                                        $logger->debug(
                                            "genKohaRecords() method4: after processItemInvoice() itemnumber:$itemnumber: ordernumberFound:$ordernumberFound: basketnoFound:$basketnoFound:"
                                        );
                                        if ($invoiceid) {
                                            $invoiceids->{$invoiceid} = $invoiceid;
                                        }
                                ...
                                my $acquisitionImportItemInvoiceRS =
                                    $acquisitionImportItemInvoice->_resultset()->create($insParam);
                                $logger->trace(
                                          "genKohaRecords() method4: acquisitionImportItemInvoiceRS->{_column_data}:"
                                        . Dumper( $acquisitionImportItemInvoiceRS->{_column_data} )
                                        . ":" );
                                ...
                                $emaillog->{'problems'} .= "\n" if ( $emaillog->{'problems'} );
                                $emaillog->{'problems'} .=
                                    "ERROR: Import der Exemplardaten für EKZ Exemplar-ID: $ekzExemplarID wurde abgewiesen.\n";
                                $emaillog->{'importresult'} = -1;
                                $emaillog->{'importerror'}  = 1;
                            }
                            my $tmp_cn =
                            ...
                            my $importId =
                                ...
                                . $tmp_cn
                                . '(ControlNrId)'
                                . $tmp_cna;    # if cna = 'DE-Rt5' then this cn is the ekz article number
                            $emaillog->{'importIds'}->{$importId} = $itemnumber;
                            $logger->trace(
                                "genKohaRecords() method4: importedItemsCount:$emaillog->{'importedItemsCount'}: set next importId:"
                                    . $importId
                                    . ":" );
                            ...
                            push @{ $emaillog->{'records'} },
                                [
                                $invEkzArtikelNr,            defined $biblionumber ? $biblionumber : "no biblionumber",
                                $emaillog->{'importresult'}, $titeldata, $isbnean,          $emaillog->{'problems'},
                                $emaillog->{'importerror'},  2,          $ordernumberFound, $basketnoFound
                                ];
                            $logger->trace( "genKohaRecords() method4: emaillog->{'records'}->[0]:"
                                    . Dumper( $emaillog->{'records'}->[0] )
                                    . ":" );
                            $logger->trace( "genKohaRecords() method4: emaillog->{'records'}->[1]:"
                                    . Dumper( $emaillog->{'records'}->[1] )
                                    . ":" );
                ...
                $logger->debug(
                    "genKohaRecords() after method 4 invoicedItemsCount:$invoicedItemsCount: updOrInsItemsCount:$updOrInsItemsCount: titleHits->{'count'}:$titleHits->{'count'}: biblionumber:$biblionumber: invEkzArtikelNr:$invEkzArtikelNr:"
                );

                # create @actionresult message for log email, representing 1 title with all its processed items
                my @actionresultTit = (
                    'insertRecords',                    0, "X", "Y", $emaillog->{'processedTitlesCount'},
                    $emaillog->{'importedTitlesCount'}, $emaillog->{'foundTitlesCount'},
                    $emaillog->{'processedItemsCount'}, $emaillog->{'importedItemsCount'},
                    $emaillog->{'updatedItemsCount'},   $emaillog->{'records'}
                );
            ...
            push @{ $emaillog->{'logresult'} },
                [
                'RechnungDetail', $messageID, $emaillog->{'actionresult'}, $acquisitionError, $ekzAqbooksellersId,
                undef,            $invoiceids
                ];  # arg basketno is undef, because with standing orders multiple delivery/invoice baskets are possible
            $logger->trace(
                "genKohaRecords() Dumper(emaillog->{'logresult'}):" . Dumper( $emaillog->{'logresult'} ) . ":" );
            ...
            if ( length($ekzAqbooksellersId) && defined($basketno) && $basketno > 0 ) {
                ...
                if ($aqbasket) {
                    ...
                    # close the basket
                    $logger->debug( "genKohaRecords() is calling Koha::Acquisition::Baskets->find(basketno:"
                            . $aqbasket->{basketno}
                            . ")" );
                    ...
                    if ( !defined($aqbasketgroups) || scalar @{$aqbasketgroups} == 0 ) {
                        ...
                        $logger->trace( "genKohaRecords() created basketgroup with name:"
                                . $aqbasket->{basketname}
                                . ": having basketgroupid:$basketgroupid:" );
                    } else {
                        $basketgroupid = $aqbasketgroups->[0]->{id};
                        $logger->trace( "genKohaRecords() found basketgroup with name:"
                                . $aqbasket->{basketname}
                                . ": having basketgroupid:$basketgroupid:" );
                    }
        ...
        if ( ref($exceptionThrown) eq 'Koha::Exceptions::WrongParameter' ) {
            ...
            $logger->error( "genKohaRecords() caught generic exception:" . Dumper($exceptionThrown) . ":" )
                ;    # unbelievable: croak throws a string
        }
        ...
        if ($createdTitleRecords) {
            foreach my $titleSelHashkey ( sort keys %{$createdTitleRecords} ) {
                ...
                $logger->debug(
                    "genKohaRecords() is deleting createdTitleRecords->{$titleSelHashkey} because of database rollback and no other use of this title data"
                );
                delete $createdTitleRecords->{$titleSelHashkey}
                    ; # remove elements of createdTitleRecords inserted by current call because this transaction is rolled back
            }
        }
        if ($updatedTitleRecords) {
            foreach my $titleRecordBiblionumber ( sort keys %{$updatedTitleRecords} ) {
                ...
                $logger->debug(
                    "genKohaRecords() is deleting updatedTitleRecords->{$titleRecordBiblionumber} because of database rollback and no other use of this title/items data"
                );
                delete $updatedTitleRecords->{$titleRecordBiblionumber}
                    ; # remove elements of updatedTitleRecords inserted by current call because this transaction is rolled back
            }
        }
    ...
    if ($createdTitleRecords) {
        foreach my $titleSelHashkey ( sort keys %{$createdTitleRecords} ) {
            ...
            if ( defined $biblionumber ) {
                ...
                $logger->debug(
                          "genKohaRecords() pushed biblionumber:$biblionumber: to array biblionumbers (new length:"
                        . scalar @biblionumbers
                        . ":)." );
            }
            $createdTitleRecords->{$titleSelHashkey}->{isAlreadyCommitted} =
                1;    # mark elements of createdTitleRecords newly added by current call as committed
            $logger->debug( "genKohaRecords() has set createdTitleRecords->{$titleSelHashkey}->{isAlreadyCommitted}:"
                    . $createdTitleRecords->{$titleSelHashkey}->{isAlreadyCommitted}
                    . ":" );
        }
    }
        ...
        foreach my $titleRecordBiblionumber ( sort keys %{$updatedTitleRecords} ) {
            if ( defined $titleRecordBiblionumber ) {
                ...
                if ( grep( /^$titleRecordBiblionumber$/, @biblionumbers ) == 0 ) {
                    ...
                    $logger->debug(
                        "genKohaRecords() pushed biblionumber:$titleRecordBiblionumber: of updatedTitleRecords to array biblionumbers (new length:"
                            . scalar @biblionumbers
                            . ":)." );
                }
                $updatedTitleRecords->{$titleRecordBiblionumber}->{isAlreadyCommitted} =
                    1;    # mark elements of updatedTitleRecords newly added by current call as committed
                $logger->debug(
                    "genKohaRecords() has set updatedTitleRecords->{$titleRecordBiblionumber}->{isAlreadyCommitted}:"
                        . $updatedTitleRecords->{$titleRecordBiblionumber}->{isAlreadyCommitted}
                        . ":" );
            }
        }
    ...
    if (@biblionumbers) {
        ...
        $logger->debug(
            "genKohaRecords() is calling indexer->index_records() with biblionumbers:" . Dumper(@biblionumbers) . ":" );
        ...
        try {
            ...
            my $mess = sprintf(
                "genKohaRecords(): Exception thrown by update_index:%s:, so the index has to be rebuilt manually!!!",
                $_[0]
            );
            $logger->error($mess);
    ...
    if ( $emaillog && defined( $emaillog->{'logresult'} ) && scalar( @{ $emaillog->{'logresult'} } ) > 0 ) {
        ...
        ( $message, $subject, $haserror ) = $ekzKohaRecord->createProcessingMessageText(
            $emaillog->{'logresult'}, "headerTEXT", $emaillog->{'dt'},
            \@importIds, $rechnungNummer
        );
...
sub priceInfoFromMessage {
    ...
    my $ekzInvoiceSkipAdditionalCosts = C4::Context->preference("ekzInvoiceSkipAdditionalCosts")
        ; # 0 -> add wertMehrpreise and wertBearbeitung to wertPositionsTeil   1 -> skip wertMehrpreise and wertBearbeitung, i.e. take wertPositionsTeil only (as invoice item price)
    my $ustProzentVoll =
        defined( $rechnungRecord->{'ustProzentVoll'} )
        ? $rechnungRecord->{'ustProzentVoll'}
        : &C4::External::EKZ::lib::EkzKohaRecords::defaultUstSatz('V') * 100.0;  # e.g. 19.00 for VAT rate of 19% (0.19)
    my $ustProzentHalb =
        defined( $rechnungRecord->{'ustProzentHalb'} )
        ? $rechnungRecord->{'ustProzentHalb'}
        : &C4::External::EKZ::lib::EkzKohaRecords::defaultUstSatz('E') * 100.0;    # e.g. 7.00 for VAT rate of 7% (0.07)
    my $priceInfo = {};

    $priceInfo->{exemplareBestellt} = defined( $auftragsPosition->{'exemplareBestellt'} )
        && $auftragsPosition->{'exemplareBestellt'} != 0 ? $auftragsPosition->{'exemplareBestellt'} : "1";
    $priceInfo->{verkaufsPreis} =
        defined( $auftragsPosition->{'verkaufsPreis'} ) ? $auftragsPosition->{'verkaufsPreis'} : "0.00";
    $priceInfo->{nachlass} =
        defined( $auftragsPosition->{'nachlass'} )
        ? $auftragsPosition->{'nachlass'}
        : "0.00";    # <nachlass> for all exemplareBestellt of this <auftragsPosition> in sum.
    $priceInfo->{nachlassProExemplar} = &C4::External::EKZ::lib::EkzKohaRecords::round(
        ( $priceInfo->{nachlass} / ( $priceInfo->{exemplareBestellt} * 1.0 ) ), 2 );    # nachlass per exemplar
    $priceInfo->{rabatt} = "0.0"
        ; # 'rabatt' not sent in RechnungDetailResponseElement.auftragsPosition, so we calculate it from verkaufsPreis and nachlass (15.0 means 15 %)

    if ( $priceInfo->{verkaufsPreis} != 0.0 ) {
        $priceInfo->{rabatt} =
            ( $priceInfo->{nachlass} * 100.0 ) / ( $priceInfo->{verkaufsPreis} * $priceInfo->{exemplareBestellt} )
            ;    # (value 15.0 means 15 %)
    }
    ...
    $priceInfo->{wertPositionsTeil} =
        defined( $auftragsPosition->{'wertPositionsTeil'} ) ? $auftragsPosition->{'wertPositionsTeil'} : "0.00";
    $priceInfo->{wertMehrpreise} =
        defined( $auftragsPosition->{'wertMehrpreise'} ) ? $auftragsPosition->{'wertMehrpreise'} : "0.00";
    $priceInfo->{wertBearbeitung} =
        defined( $auftragsPosition->{'wertBearbeitung'} ) ? $auftragsPosition->{'wertBearbeitung'} : "0.00";
    $priceInfo->{waehrung} = defined( $auftragsPosition->{'waehrung'} ) ? $auftragsPosition->{'waehrung'} : "EUR";
    $priceInfo->{ust} =
        "0.00";    # 'ust' not sent in RechnungDetailResponseElement.auftragsPosition, so we will calculate it
    $priceInfo->{ustSatz} = $ustProzentHalb / 100.0
        ; # 'ustSatz' not sent in RechnungDetailResponseElement.auftragsPosition, so we evaluate XML element <ustProzentHalb> or <ustProzentVoll>
    $priceInfo->{mwst} =
        defined( $auftragsPosition->{'mwst'} ) ? $auftragsPosition->{'mwst'} : "E";    # 'E':ermässigt 'V':voll
    ...
    if ($ekzInvoiceSkipAdditionalCosts) {
        ...
        $priceInfo->{gesamtpreis_tax_included} =
            $priceInfo->{verkaufsPreis} +
            &C4::External::EKZ::lib::EkzKohaRecords::round(
            ( $priceInfo->{wertMehrpreise} + $priceInfo->{wertBearbeitung} ) / $priceInfo->{exemplareBestellt}, 2 );
    }
    ...
    if ( defined( $auftragsPosition->{'wertPositionsTeil'} ) ) {
        ...
        if ($ekzInvoiceSkipAdditionalCosts) {
            $priceInfo->{gesamtpreis_tax_included} = &C4::External::EKZ::lib::EkzKohaRecords::round(
                $priceInfo->{wertPositionsTeil} / $priceInfo->{exemplareBestellt}, 2 )
                ; # 'gesamtpreis' not sent in RechnungDetailResponseElement.auftragsPosition, so we calculate it; total for a single item
        } else {
            $priceInfo->{gesamtpreis_tax_included} = &C4::External::EKZ::lib::EkzKohaRecords::round(
                ( $priceInfo->{wertPositionsTeil} + $priceInfo->{wertMehrpreise} + $priceInfo->{wertBearbeitung} ) /
                    $priceInfo->{exemplareBestellt}, 2 )
                ; # 'gesamtpreis' not sent in RechnungDetailResponseElement.auftragsPosition, so we calculate it; total for a single item
        }
    ...
    if ( $priceInfo->{ust} == 0.0 && $priceInfo->{ustSatz} != 0.0 && $priceInfo->{ustSatz} != -1.0 )
    {             # calculate ust from ustSatz
        $priceInfo->{ust} =
            $priceInfo->{gesamtpreis_tax_included} * $priceInfo->{ustSatz} / ( 1 + $priceInfo->{ustSatz} );
        $priceInfo->{ust} = &C4::External::EKZ::lib::EkzKohaRecords::round( $priceInfo->{ust}, 2 );
    }
    if (   $priceInfo->{ustSatz} == 0.0
        && $priceInfo->{ust} != 0.0
        && $priceInfo->{gesamtpreis_tax_included} != $priceInfo->{ust} )
    {             # calculate ustSatz from ust
    ...
    $priceInfo->{gesamtpreis_tax_excluded} =
        &C4::External::EKZ::lib::EkzKohaRecords::round( $priceInfo->{gesamtpreis_tax_excluded}, 2 );

    $priceInfo->{replacementcost_tax_included} =
        $priceInfo->{verkaufsPreis};    # list price of single item in library's currency, not discounted
    $priceInfo->{replacementcost_tax_excluded} =
        $divisor == 0.0 ? 0.0 : $priceInfo->{replacementcost_tax_included} / $divisor;
    $priceInfo->{replacementcost_tax_excluded} =
        &C4::External::EKZ::lib::EkzKohaRecords::round( $priceInfo->{replacementcost_tax_excluded}, 2 );
...
sub processItemHit {
    my (
        $rechnungNummer,   $rechnungDatum,  $dateTimeNow, $ekzWebServicesSetItemSubfieldsWhenInvoiced, $reArtikelNr,
        $reReferenznummer, $rechnungRecord, $auftragsPosition, $acquisitionImportTitleHit, $titleHits, $biblionumber,
        $acquisitionImportTitleItemHit, $emaillog, $updOrInsItemsCountRef, $ekzAqbooksellersId, $updatedTitleRecords,
        $logger
    ) = @_;
    my $selParam         = '';
    my $updParam         = '';
    ...
    $logger->trace(
        "processItemHit() titleItemObjectRS->{_column_data}:" . Dumper( $titleItemObjectRS->{_column_data} ) . ":" );
    ...
    if ( defined $itemnumber ) {
        ...
        $selParam = { itemnumber => $itemnumber };
        ...
        if ( $aqorders_itemsRS && $aqorders_itemsRS->first() ) {
            ...
            if ($aqorders_itemsHit)
            {    # The aqorders_items record does not exist any more if the items record meanwhile has been deleted.
                $ordernumber = $aqorders_itemsHit->get_column('ordernumber');
                $logger->trace( "processItemHit() aqorders_itemsHit->{_column_data}:"
                        . Dumper( $aqorders_itemsHit->{_column_data} )
                        . ": ordernumber:"
                        . $ordernumber
                        . ":" );
            }
        ...
        $logger->trace(
                  "processItemHit() searched via itemnumber:$itemnumber: in table aqorders_items, found ordernumber:"
                . $ordernumber
                . ":" );
        ...
        if ($ordernumber) {
            ...
            $selParam = { ordernumber => $ordernumber };
            ...
            if ( $aqordersRS && $aqordersRS->first() ) {
                ...
                if ($aqordersHit) {                        # always true if there is no database inconsistency
                    ...
                    $logger->trace( "processItemHit() aqordersHit->{_column_data}:"
                            . Dumper( $aqordersHit->{_column_data} )
                            . ": basketno:"
                            . $basketno
                            . ":" );
                }
            }
            $logger->trace( "processItemHit() searched via ordernumber:$ordernumber: in table aqorders, found basketno:"
                    . $basketno
                    . ":" );
            ...
            if ($basketno) {
                ...
                if ( $aqbasketRS && $aqbasketRS->first() ) {
                    ...
                    if ($aqbasketHit) {                        # always true if there is no database inconsistency
                        ...
                        $logger->trace( "processItemHit() aqbasketHit->{_column_data}:"
                                . Dumper( $aqbasketHit->{_column_data} )
                                . ": booksellerid:"
                                . $booksellerid
                                . ":" );
                    }
                ...
                $logger->trace(
                          "processItemHit() searched via basketno:$basketno: in table aqbasket, found booksellerid:"
                        . $booksellerid
                        . ": for comparing with ekzAqbooksellersId:"
                        . $ekzAqbooksellersId
                        . ":" );
                ...
                if ( defined($ekzAqbooksellersId) && length($ekzAqbooksellersId) ) {
                    if ( !defined $booksellerid || $booksellerid != $ekzAqbooksellersId ) {
                        $logger->warn(
                            "processItemHit() will not use itemnumber:$itemnumber: as ordernumber:$ordernumber: leads to basketno:$basketno: but booksellerid:$booksellerid: differs from ekzAqbooksellersId:$ekzAqbooksellersId:"
                        );
                        $itemnumber = undef
                            ; # void this itemnumber - this item now belongs to another aqbookseller and so may not be used for ekz data import any more
                    }
                }
            }
        }
        $logger->trace( "processItemHit() candidate item for update has itemnumber:"
                . $itemnumber
                . ": (after order transfer check)" );
    }
    ...
    if ( defined $titleItemObjectRS && defined $itemnumber ) {
        ...
        $itemHit      = $itemHitRs->find($itemSelParam);
        $logger->trace(
            "processItemHit() searched for itemnumber:$itemnumber: itemHitCount:$itemHitCount: itemHit->{_column_data}:"
                . Dumper( $itemHit->{_column_data} )
                . ":" );
        ...
        if ( $itemHitCount > 0 && defined( $itemHit->{_column_data} ) ) {
                ...
                && length($ekzWebServicesSetItemSubfieldsWhenInvoiced) > 0 )
            {
                ...
                if (@affects) {
                    ...
                    if ($item) {
                        ...
                        $logger->trace(
                            "processItemHit() after calling C4::Items::ModItemFromMarc biblionumber:$biblionumber: itemnumber:$itemnumber:"
                        );
            ...
            $logger->trace(
                "processItemHit() after re-reading itemHit itemnumber:$itemnumber: itemHitCount:$itemHitCount: itemHit->{_column_data}:"
                    . Dumper( $itemHit->{_column_data} )
                    . ":" );
            ...
            if ( defined($ekzAqbooksellersId) && length($ekzAqbooksellersId) ) {
                ...
                # update Koha acquisition order and update/insert invoice data
                ( $ordernumberFound, $basketnoFound, $invoiceid_ret ) = processItemInvoice(
                    $rechnungNummer,   $rechnungDatum, $biblionumber, $itemnumber, $rechnungRecord,
                    $auftragsPosition, $acquisitionImportTitleItemHit, $updatedTitleRecords, $logger
                );
                $logger->trace(
                    "processItemHit() processItemInvoice() returned ordernumberFound:$ordernumberFound: basketnoFound:$basketnoFound: invoiceid_ret:$invoiceid_ret:"
                );
            } else {
                ...
                if ($item) {
                    ...
                    $logger->debug( "processItemHit() item->store() itemnumber:"
                            . $itemnumber
                            . ": gesamtpreis:"
                            . $priceInfo->{gesamtpreis_tax_included}
                            . ": replacementcost_tax_included:"
                            . $priceInfo->{replacementcost_tax_included}
                            . ":" );
                    ...
                    $logger->error(
                              "processItemHit() item not found for update of price and replacementprice! itemnumber:"
                            . $itemnumber
                            . ": gesamtpreis:"
                            . $priceInfo->{gesamtpreis_tax_included}
                            . ": replacementcost_tax_included:"
            ...
            $logger->trace(
                "processItemHit() acquisitionImportTitleItemHit->update res:" . Dumper( $res->{_column_data} ) . ":" );
            ...
            $logger->trace(
                "processItemHit() update or insert acquisition_import record for title calling Koha::AcquisitionImport::AcquisitionImports->new()->upd_or_ins(selParam, updParam, insParam) with selParam:"
                    . Dumper($selParam)
                    . ": updParam:"
                    . Dumper($updParam)
                    . ": insParam:"
            ...
            my $resInvoiceTitle = $acquisitionImportInvoiceTitle->upd_or_ins( $selParam, $updParam, $insParam )
                ;    # TODO: evaluate $resInvoiceTitle
            $logger->trace( "processItemHit() insert acquisition_import record for invoice title res:"
                    . Dumper( $resInvoiceTitle->_resultset()->{_column_data} )
                    . ":" );
            ...
            $logger->trace(
                "processItemHit() insert acquisition_import record for item calling Koha::AcquisitionImport::AcquisitionImports->new()->_resultset()->create(insParam) with insParam:"
                    . Dumper($insParam)
                    . ":" );
            my $acquisitionImportInvoiceItem = Koha::AcquisitionImport::AcquisitionImports->new();
            my $resInvoiceItem =
                $acquisitionImportInvoiceItem->_resultset()->create($insParam);    # TODO: evaluate $resInvoiceItem
            $logger->trace( "processItemHit() END insert acquisition_import record for item res:"
                    . Dumper( $resInvoiceItem->{_column_data} )
                    . ":" );
        }
    }
    ...
    if (   defined $titleItemObjectRS
        && defined $itemnumber
        && $itemHitCount > 0
        && defined( $itemHit->{_column_data} )
        && defined $res )
    {    # item successfully updated
        ...
        $emaillog->{'problems'} .= "\n" if ( $emaillog->{'problems'} );
        $emaillog->{'problems'} .=
            "ERROR: Update der Exemplardaten für EKZ ArtikelNr.: " . $reArtikelNr . " wurde abgewiesen.\n";
        $emaillog->{'importresult'} = -1;
        $emaillog->{'importerror'}  = 1;
        $emaillog->{'processedItemsCount'} += 0
            ; # The item could not be processed here in processItemHit(). But probably it soon will be processed by method4.
    }
    my $tmp_cn =
        ( defined( $titleHits->{'records'}->[0]->field("001") ) && $titleHits->{'records'}->[0]->field("001")->data() )
        ? $titleHits->{'records'}->[0]->field("001")->data()
        : $biblionumber;
    my $tmp_cna =
        ( defined( $titleHits->{'records'}->[0]->field("003") ) && $titleHits->{'records'}->[0]->field("003")->data() )
        ? $titleHits->{'records'}->[0]->field("003")->data()
        : "undef";
    my $importId =
          '(ControlNumber)'
    ...
    $emaillog->{'importIds'}->{$importId} = $itemnumber;
    $logger->trace(
        "processItemHit() updatedItemsCount:$emaillog->{'updatedItemsCount'}: set next importIds:" . $importId . ":" );

    # add result of updating item to log email
    my ( $titeldata, $isbnean ) = ( $itemnumber, '' );
    push @{ $emaillog->{'records'} },
        [
        $reArtikelNr, defined $biblionumber ? $biblionumber : "no biblionumber", $emaillog->{'importresult'},
        $titeldata, $isbnean, $emaillog->{'problems'}, $emaillog->{'importerror'}, 2, $ordernumberFound,
        $basketnoFound
        ];
...
sub processItemInvoice {
    my (
        $rechnungNummer, $rechnungDatum, $biblionumber, $itemnumber, $rechnungRecord, $auftragsPosition,
        $acquisitionImportTitleItemHit, $updatedTitleRecords, $logger
    ) = @_;
    ...
    $logger->info(
        "processItemInvoice() Start rechnungNummer:$rechnungNummer: rechnungDatum:$rechnungDatum: biblionumber:$biblionumber: itemnumber:$itemnumber: acquisitionImportTitleItemHit id:"
            . $acquisitionImportTitleItemHit->id
            . "object_item_number:"
            . $acquisitionImportTitleItemHit->object_item_number
            . ":" );
    ...
    if ( $acquisitionImportTitleItemHit->object_number =~ /^(sto|ser)\.\d+\.ID\d+/ ) {
        $isStoOrSer = 1
            ; # aqorders record of standing/serial order title has to be shifted from the general aqbaskets record of the standing/serial order to an specific one, if not done already by delivery note synchronisation
    }
    ...
    if ( !$aqbasket_of_order ) {
        $logger->error(
            "processItemInvoice() could not find aqbasket of order via basketno_ret:" . $basketno_ret . ":" );
    ...
    if (
        $isStoOrSer
        && (
            $aqbasket_of_order->{basketname} =~ /^S-sto\.\d+\.ID\d+/
            || # order is part of a standing order and has not been shifted into separate basket by delivery note synchronisation
            $aqbasket_of_order->{basketname} =~ /^F-ser\.\d+\.ID\d+/
        ) # order is part of a serial order and has not been shifted into separate basket by delivery note synchronisation
        )
    {
        ...
        $logger->debug(
            "processItemInvoice() Dumper aqbasket_of_invoice_hits:" . Dumper($aqbasket_of_invoice_hits) . ":" );
        if ( defined($aqbasket_of_invoice_hits) && scalar @{$aqbasket_of_invoice_hits} > 0 ) {
            ...
            my $aqbasket_of_invoice_no = &C4::Acquisition::NewBasket(
                $aqbasket_of_order->{booksellerid},  $aqbasket_of_order->{authorisedby}, $aqbasket_of_invoice_name,
                $aqbasket_of_order->{basketname},    "", $aqbasket_of_order->{basketcontractnumber},
                $aqbasket_of_order->{deliveryplace}, $aqbasket_of_order->{billingplace},
                $aqbasket_of_order->{is_standing},   $aqbasket_of_order->{create_items}
            );
        ...
        if ( !$aqbasket_of_invoice ) {
            $logger->error(
                      "processItemInvoice() could NOT find or create aqbasket of invoice; aqbasket_of_invoice_name:"
                    . $aqbasket_of_invoice_name
                    . ":" );
        ...
        if ( !$orderRecord ) {
            $logger->error(
                "processItemInvoice() could not find orderRecord for STO; selection params:" . Dumper($params) . ":" );
        ...
        $logger->debug( "processItemInvoice() is calling Koha::Acquisition::Baskets->find(basketno:"
                . $aqbasket_of_invoice->{basketno}
                . ")" );
        ...
        $logger->info(
            "processItemInvoice() basketgroup with name:R-$rechnungNummer: has basketgroupid:$basketgroupid:");
    ...
    my $invoices = [];
    $logger->debug(
        "processItemInvoice() is calling C4::Acquisition::GetInvoices(invoicenumber_equal:$rechnungNummer, supplierid:"
            . $aqbasket_of_order->{booksellerid}
            . ", billingdatefrom:$rechnungDatum, billingdateto:$rechnungDatum)" );
    ...
    if ( scalar @{$invoices} == 0 ) {
        $logger->debug(
            "processItemInvoice() is calling C4::Acquisition::AddInvoice(invoicenumber:$rechnungNummer: booksellerid:"
                . $aqbasket_of_order->{booksellerid}
                . ": billingdate:"
                . $rechnungDatum
                . ": shipmentdate:"
        ...
        if ( !defined $invoiceid ) {
            $logger->error(
                "processItemInvoice() could NOT create invoice via C4::Acquisition::AddInvoice(invoicenumber:$rechnungNummer: booksellerid:"
                    . $aqbasket_of_order->{booksellerid}
                    . ": billingdate:"
                    . $rechnungDatum
                    . ": shipmentdate:"
    ...
    my $order = Koha::Acquisition::Orders->find($ordernumber_ret)
        ;  # contains more fields then $orderRecord; needed for populate_with_prices_for_receiving and ModReceiveOrder()

    ### XXXWH $order->{quantityreceived} += 1; nein, das läuft über ModReceiveOrder
    $order->listprice( $priceInfo->{verkaufsPreis} )
        ;    # in supplier's currency, not discounted, per item (input field 'Vendor price' in UI)
    $order->tax_rate_on_receiving( $priceInfo->{ustSatz} )
        ;    # tax_value_on_receiving is calculated in populate_with_prices_for_receiving() based on this
    ...
    if ( $bookseller->listincgst ) {    # as far as we know this is always true for bookseller 'ekz'
        $order->unitprice( $priceInfo->{gesamtpreis_tax_included} )
            ; # discounted price per item (input field 'Actual cost' in UI / entered cost, handling etc. incl. (set to 0.0 in the phase  before receipt))
    } else {
        $order->unitprice( $priceInfo->{gesamtpreis_tax_excluded} )
            ; # discounted price per item (input field 'Actual cost' in UI / entered cost, handling etc. incl. (set to 0.0 in the phase  before receipt))
    }
    ...
    $new_order_vendornote .= sprintf(
        "Rechnung:\nVerkaufspreis: %.2f %s (Exemplare: %d)\n", $priceInfo->{verkaufsPreis},
        $priceInfo->{waehrung},                                $priceInfo->{exemplareBestellt}
    );
    if ( $priceInfo->{nachlass} != 0.0 ) {
    ...
    if ( $priceInfo->{wertPositionsTeil} != 0.0 ) {
        $new_order_vendornote .=
            sprintf( "Positionsteilwert: %.2f %s\n", $priceInfo->{wertPositionsTeil}, $priceInfo->{waehrung} );
    }
    if ( $priceInfo->{wertMehrpreise} != 0.0 ) {
        $new_order_vendornote .=
            sprintf( "Mehrpreis: %.2f %s\n", $priceInfo->{wertMehrpreise}, $priceInfo->{waehrung} );
    }
    if ( $priceInfo->{wertBearbeitung} != 0.0 ) {
        $new_order_vendornote .=
            sprintf( "Bearbeitungspreis: %.2f %s\n", $priceInfo->{wertBearbeitung}, $priceInfo->{waehrung} );
    }
    $order->order_vendornote($new_order_vendornote);
    $order->discount( $priceInfo->{rabatt} )
        ; # rabatt value of ekz quotes percents, so 15.0 means 15 %. So the value of $priceInfo->{rabatt} can be used without transformation for aqorders.discount.
    ...
    $logger->trace(
              "processItemInvoice() order->populate_with_prices_for_receiving done, order->{_result}->{_column_data}:"
            . Dumper( $order->{_result}->{_column_data} )
            . ":" );
    ...
    my ( $datereceived, $new_ordernumber ) = C4::Acquisition::ModReceiveOrder(
        {
            ...
            quantityreceived => 1
            , # we are working item by item, i.e. processItemInvoice() is called per item, even if $priceInfo->{exemplareBestellt} > 1
    ...
    $logger->trace(
        "processItemInvoice() ModReceiveOrder done, datereceived:$datereceived: new_ordernumber:$new_ordernumber: (new) order:"
            . Dumper($order)
            . ":" );
    ...
    my $item = Koha::Items->find($itemnumber);
    if ($item) {
        $item->booksellerid('ekz')
            ; # same value as in BestellInfo, probably better than the correct but 'random' $aqbasket_of_order->{booksellerid} as done by staff interface
        ...
        $item->replacementprice( $priceInfo->{replacementcost_tax_included} );
        $item->replacementpricedate( dt_from_string() );
        $logger->debug( "processItemInvoice() item->store() itemnumber:"
                . $itemnumber
                . ": gesamtpreis:"
                . $priceInfo->{gesamtpreis_tax_included}
                . ": replacementcost_tax_included:"
        ...
        $logger->error( "processItemInvoice() item not found for update of price and replacementprice! itemnumber:"
                . $itemnumber
                . ": gesamtpreis:"
                . $priceInfo->{gesamtpreis_tax_included}
                . ": replacementcost_tax_included:"
    ...
    $logger->info( "processItemInvoice() returns ordernumber_ret:"
            . $ordernumber_ret
            . ": basketno_ret:"
            . $basketno_ret
            . ": invoiceid_ret:"
