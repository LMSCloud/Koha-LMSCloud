...
our @ISA = qw(Exporter);
our @EXPORT =
    qw( getCurrentYear readSerialOrdersFromEkzWsFortsetzungList readSerialOrderFromEkzWsFortsetzungDetail genKohaRecords );
...
sub readSerialOrdersFromEkzWsFortsetzungList {
    ...
    $logger->info( "readSerialOrdersFromEkzWsFortsetzungList() START ekzCustomerNumber:"
...
sub readSerialOrderFromEkzWsFortsetzungDetail {
    ...
    my $refFortsetzungDetailElement =
        shift;    # for storing the FortsetzungDetailElement of the SOAP response body in DB table acquisition_import
    ...
    my $ekzwebservice = C4::External::EKZ::lib::EkzWebServices->new();
    $result = $ekzwebservice->callWsFortsetzungDetail(
        $ekzCustomerNumber,     $selFortsetzungsId, $selBearbeitungsGruppe,
        $selBearbeitungsNummer, $selFortsetzungsAuftragsNummer, $selMitReferenznummer, $refFortsetzungDetailElement
    );
...
sub genKohaRecords {
    my (
        $ekzCustomerNumber,   $messageID, $fortsetzungDetailElement, $serWithNewState, $lastRunDate, $todayDate,
        $createdTitleRecords, $updatedTitleRecords
    ) = @_;
    my $logger        = Koha::Logger->get( { interface => 'C4::External::EKZ::EkzWsSerialOrder' } );
    ...
    try {
        ...
        foreach my $titel ( @{$serialOrderTitles} ) {
            if ( $titel->{'BestellDatum'} && length( substr( $titel->{'BestellDatum'}, 0, 10 ) ) == 10 ) {
                my $statusDatum = substr( $titel->{'BestellDatum'}, 0, 10 )
                    ; # format yyyy-mm-ddT00:00:00.000 to yyyy-mm-dd. Regrettably there is no statusDatum as with standing orders
                    ...
                    if ( !defined($testDatum) || $@ ) {
                        ...
                        if ($lastRunDateIsSet) {
                            if (
                                ...
                                && # 'vorbereitet' || 'in nächster Lieferung' || 'Bereits geliefert' (i.e. 'prepared' || 'included in next delivery' || 'delivered')
                            ...
                            if (
                                ...
                                && # 'vorbereitet' || 'in nächster Lieferung' (i.e. 'prepared' || 'included in next delivery'
        ...
        if ($insOrUpd) {
            ...
            $ekzBestellNr = 'ser.'
                ...
                . $serWithNewState->{'fortsetzungsId'}
                ;    # FortsetzungList response contains no order number, so we create this dummy order number
            ...
            $logger->debug( "genKohaRecords() search serial order message record in acquisition_import selParam:"
                    . Dumper($selParam)
                    . ":" );
            $logger->debug( "genKohaRecords() search serial order message record in acquisition_import insParam:"
                    . Dumper($insParam)
                    . ":" );
            $logger->debug( "genKohaRecords() search serial order message record in acquisition_import updParam:"
                    . Dumper($updParam)
                    . ":" );
                ...
                if ( length($ekzAqbooksellersId) ) {
                    ...
                    if ( @{$selbaskets} > 0 ) {
                        ...
                        $basketno = C4::Acquisition::NewBasket(
                            ...
                            'created by ekz FortsetzungDetail', '', undef, $branchcode, $branchcode, 0, 'ordering'
                        );    # XXXWH fixed text ok?
                        $logger->info(
            ...
            foreach my $titel ( @{$serialOrderTitles} ) {
                ...
                $reqParamTitelInfo->{'preis'}        = $titel->{'preis'}
                    ; # listprice; $titel->{'artikelPreis'} seems to be reduced by discount (but may also contain handling charges?)
                    ...
                    foreach my $titleSource (@titleSourceSequence) {
                        ...
                        if ( $titleSource eq '_LMSC' ) {
                            ...
                            if ( $searchmode == 2 ) {
                                ...
                                $logger->info(
                                    "genKohaRecords() from sent titel fields searchmode:$searchmode: titleHits->{'count'}:"
                                        . $titleHits->{'count'}
                                        . ":" );
                            }
                ...
                if ( $biblioExisting || $biblioInserted ) {
                    ...
                    $logger->debug( "genKohaRecords() search serial order title record in acquisition_import selParam:"
                            . Dumper($selParam)
                            . ":" );
                    ...
                    if ( defined($hit) ) {
                        ...
                        my $mess = sprintf(
                            "The ekz article number '%s' has already been used in the serial order %s at %s. Processing skipped for this title in order to avoid repeated item record creation.",
                            $reqParamTitelInfo->{'ekzArtikelNr'}, $serWithNewState->{'fortsetzungsId'},
                        ...
                        next
                            ; # The ekz article number has already been used in this serial order. Skip processing of this title in order to avoid repeated item record creation.
                    ...
                    my $ekzExemplarID = $ekzBestellNr . '-'
                        . $reqParamTitelInfo->{'ekzArtikelNr'}
                        ; # FortsetzungDetail response contains no item number, so we create this dummy item number (just in case that <referenznummer> is not sent)
                    ...
                    my $rabatt = 0.0
                        ; # not sent in FortsetzungDetail response (XXXWH: perhaps could be calculated by (listprice-discountedprice)/listprice with listprice = $titel->{'preis'} and discountedprice = $titel->{'artikelPreis'}
                    ...
                    my $ustSatz     = &C4::External::EKZ::lib::EkzKohaRecords::defaultUstSatz('E')
                        ;                      # not sent in FortsetzungDetail response
                    ...
                    my $gesamtpreis =
                        defined( $titel->{'preis'} )
                        ? $titel->{'preis'}
                        : "0.00";    # normally: discounted total for a single item
                    my $reqWaehrung = 'EUR';
                    ...
                    my $listprice_tax_included =
                        $gesamtpreis - $fracht -
                        $einband -
                        $bearbeitung;    # not sent in FortsetzungDetail response, so we calculate it
                    if ( $rabatt != 0.0 ) {
                    ...
                    my @itemReferenznummer = ()
                        ; # used for generating values for acquisition_import.object_item_number of the records representing the serial order title's items (format: ser.<ekzKundenNr>.<fortsetzungsId>-<ekzArtikelNr>-<referenznummer>)
                    ...
                    my @itemOrder = ()
                        ;  # used for creating the aqorders_items records for the created aqorders record for this title
                    if ( defined($basketno) && $basketno > 0 ) {
                        ...
                        my $haushaltsstelle =
                            ...
                            : "";    # as far as known: <haushaltsstelle> is not sent in FortsetzungDetail response
            ...
            push @logresult,
                [ 'FortsetzungDetail', $messageID, \@actionresult, $acquisitionError, $ekzAqbooksellersId, $basketno ];
    ...
    if ( scalar(@logresult) > 0 && ( $cntTitlesHandled > 0 || $cntItemsHandled > 0 ) ) {
        ...
        ( $message, $subject, $haserror ) =
            $ekzKohaRecord->createProcessingMessageText( \@logresult, "headerTEXT", $dt, \@importIds, $ekzBestellNr )
