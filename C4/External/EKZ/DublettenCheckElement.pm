...
sub process {
    my ( $self, $request ) = @_
        ; # $request->{'soap:Envelope'}->{'soap:Body'} contains our deserialized DublettenCheckElement of the HTTP request

    my $soapEnvelopeHeader = $request->{'soap:Envelope'}->{'soap:Header'};
    my $soapEnvelopeBody   = $request->{'soap:Envelope'}->{'soap:Body'};

    $self->{logger}->debug( "process() START request->{'soap:Envelope'}->{'soap:Header'}:"
            . Dumper( $request->{'soap:Envelope'}->{'soap:Header'} )
            . ":" );
    $self->{logger}->debug( "process() START request->{'soap:Envelope'}->{'soap:Body'}:"
    ...
    if ( $authenticated && $ekzLocalServicesEnabled ) {
        $self->{logger}->trace(
            "process() HTTP request titel:" . $soapEnvelopeBody->{'ns2:DublettenCheckElement'}->{'titel'} . ":" );
        $self->{logger}->trace( "process() HTTP request ref(titel):"
                . ref( $soapEnvelopeBody->{'ns2:DublettenCheckElement'}->{'titel'} )
                . ":" );

        my $titeldefined =
            (      exists $soapEnvelopeBody->{'ns2:DublettenCheckElement'}
                && defined $soapEnvelopeBody->{'ns2:DublettenCheckElement'}
                && exists $soapEnvelopeBody->{'ns2:DublettenCheckElement'}->{'titel'}
                && defined $soapEnvelopeBody->{'ns2:DublettenCheckElement'}->{'titel'} );
        ...
        if ( $titeldefined && ref( $soapEnvelopeBody->{'ns2:DublettenCheckElement'}->{'titel'} ) eq 'HASH' ) {
            $titelArrayRef = [ $soapEnvelopeBody->{'ns2:DublettenCheckElement'}->{'titel'} ]
        ...
        for ( my $i = 0 ; $i < $titleCount ; $i++ ) {
            ...
            if ( $titel && defined( $titel->{'titelInfo'} ) && ref( $titel->{'titelInfo'} ) eq 'HASH' ) {
                ...
                $self->{logger}->debug(
                    "process() HTTP request ekzArtikelNr:"
                        . (
                        defined( $reqParamTitelInfo->{'ekzArtikelNr'} )
                        ? $reqParamTitelInfo->{'ekzArtikelNr'}
                        : 'undef'
                        )
                        . ":"
                );
                $self->{logger}->debug( "process() HTTP request isbn:"
                        . ( defined( $reqParamTitelInfo->{'isbn'} ) ? $reqParamTitelInfo->{'isbn'} : 'undef' )
                        . ":" );
                $self->{logger}->debug( "process() HTTP request isbn13:"
                        . ( defined( $reqParamTitelInfo->{'isbn13'} ) ? $reqParamTitelInfo->{'isbn13'} : 'undef' )
                        . ":" );
                $self->{logger}->debug( "process() HTTP request issn:"
                        . ( defined( $reqParamTitelInfo->{'issn'} ) ? $reqParamTitelInfo->{'issn'} : 'undef' )
                        . ":" );
                $self->{logger}->debug( "process() HTTP request ismn:"
                        . ( defined( $reqParamTitelInfo->{'ismn'} ) ? $reqParamTitelInfo->{'ismn'} : 'undef' )
                        . ":" );
                $self->{logger}->debug( "process() HTTP request ean:"
                        . ( defined( $reqParamTitelInfo->{'ean'} ) ? $reqParamTitelInfo->{'ean'} : 'undef' )
                        . ":" );
                $self->{logger}->debug( "process() HTTP request author:"
                        . ( defined( $reqParamTitelInfo->{'author'} ) ? $reqParamTitelInfo->{'author'} : 'undef' )
                        . ":" );
                $self->{logger}->debug( "process() HTTP request titel:"
                        . ( defined( $reqParamTitelInfo->{'titel'} ) ? $reqParamTitelInfo->{'titel'} : 'undef' )
                        . ":" );
                $self->{logger}->debug(
                    "process() HTTP request erscheinungsJahr:"
                        . (
                        defined( $reqParamTitelInfo->{'erscheinungsJahr'} )
                        ? $reqParamTitelInfo->{'erscheinungsJahr'}
                        : 'undef'
                        )
                        . ":"
                );
                ...
                $self->{logger}->debug( "process() reqEkzArtikelNr:"
                        . $reqParamTitelInfo->{'ekzArtikelNr'}
                        . ": Anzahl dublettenInfo:"
                        . scalar @dublettenInfo
                        . ":" );
                $self->{logger}->trace( "process() reqEkzArtikelNr:"
                        . $reqParamTitelInfo->{'ekzArtikelNr'}
                        . ": dublettenInfo:"
                        . @dublettenInfo
                        . ":" );
    ...
    foreach my $dublettenInfo (@dublettenInfoListe) {
        ...
        foreach my $dupExemplar ( @{ $dublettenInfo->{'exemplare'} } ) {
            $self->{logger}
                ->debug( "process() Anzahl dupExemplar->{'zweigstelle'}:" . $dupExemplar->{'zweigstelle'} . ":" );
...
sub searchDubletten {
    ...
    if ($testIt) {
        ...
        $marcresults = $ekzKohaRecord->readTitleDubletten( $reqParamTitelInfo, 7, 0 );
    }
    ...
    for ( my $i = 0 ; $i < $hits and defined $marcresults->[$i] ; $i++ ) {
        ...
        if ($marcrecord) {
            ...
            if ( !exists( $titel{'ekzArtikelNr'} ) ) {
                ...
                if ( defined( $reqParamTitelInfo->{'ekzArtikelNr'} )
                    && length( $reqParamTitelInfo->{'ekzArtikelNr'} ) > 0 )
                {
                    $ekzArtikelNr = $reqParamTitelInfo->{'ekzArtikelNr'}
                        ; # effect of returning this value: the ekz web site displays title and ISBN of this medium; otherwise these two columns stay empty
                } else {
                    ...
                    if ( $kontrollNrID eq "DE-Rt5" && defined( $marcrecord->field("001") ) ) {
                        $ekzArtikelNr =
                            $marcrecord->field("001")->data();  # the cn is a ekz article number only if cna == "DE-Rt5"
                    } else {
                        my $id = '';
                        if ( defined( $reqParamTitelInfo->{'titel'} ) && length( $reqParamTitelInfo->{'titel'} ) > 0 ) {
                            $id = $reqParamTitelInfo->{'titel'};
                        } elsif ( defined( $reqParamTitelInfo->{'isbn13'} )
                            && length( $reqParamTitelInfo->{'isbn13'} ) > 0 )
                        {
                            $id = $reqParamTitelInfo->{'isbn13'};
                        } elsif ( defined( $reqParamTitelInfo->{'isbn'} )
                            && length( $reqParamTitelInfo->{'isbn'} ) > 0 )
                        {
                ...
                $self->{logger}->debug( "searchDubletten() marcrecord->field('003'):"
                        . ( defined( $marcrecord->field("003") ) ? $marcrecord->field("003")->data() : 'undef' )
                        . ": marcrecord->field('001'):"
                        . ( defined( $marcrecord->field('001') ) ? $marcrecord->field('001') : 'undef' )
                        . ": ekzArtikelNr:"
                        . $ekzArtikelNr
                        . ":" );
            }

            $self->{logger}->debug( "searchDubletten() reqParamTitelInfo->{'ekzArtikelNr'}:"
                    . $reqParamTitelInfo->{'ekzArtikelNr'}
                    . ": duplicate candidate biblionumber:"
                    . $biblionumber
                    . ":" );
            ...
            while ( my $item = $items->next ) {
                ...
                if ( defined $marcrecord->subfield( "260", "c" ) && length $marcrecord->subfield( "260", "c" ) > 0 ) {
                    ...
                    $erscheinungsjahr =
                        defined $marcrecord->subfield( "264", "c" ) ? $marcrecord->subfield( "264", "c" ) : "";
                }
