...
sub process {
    my ( $self, $request ) = @_
        ;  # $request->{'soap:Envelope'}->{'soap:Body'} contains our deserialized BudgetCheckElement of the HTTP request
    $self->init();

    $self->{logger}->info( "process() START request:" . Dumper($request) . ":" );
    $self->{logger}->debug( "process() request->{'soap:Envelope'}->{'soap:Header'}:"
            . Dumper( $request->{'soap:Envelope'}->{'soap:Header'} )
            . ":" );
    $self->{logger}->debug( "process() request->{'soap:Envelope'}->{'soap:Body'}:"
            . Dumper( $request->{'soap:Envelope'}->{'soap:Body'} )
            . ":" );

    $self->{logger}->debug( "process() HTTP request request messageID:"
            . $request->{'soap:Envelope'}->{'soap:Body'}->{'ns2:BudgetCheckElement'}->{'messageID'}
            . ":" );
    $self->{logger}->debug( "process() HTTP request request hauptstelle:"
            . $request->{'soap:Envelope'}->{'soap:Body'}->{'ns2:BudgetCheckElement'}->{'hauptstelle'}
            . ":" );
    ...
    foreach my $tag ( keys %{ $soapEnvelopeBody->{'ns2:BudgetCheckElement'} } ) {
        $self->{logger}
            ->trace( "process() HTTP request soapEnvelopeBody->{'ns2:BudgetCheckElement'} tag:" . $tag . ":" );
    }
    ...
    if ( $authenticated && $ekzLocalServicesEnabled ) {
        $self->{logger}
            ->trace( "process() HTTP request titel:" . $soapEnvelopeBody->{'ns2:BudgetCheckElement'}->{'titel'} . ":" );
        $self->{logger}->trace( "process() HTTP request ref(titel):"
                . ref( $soapEnvelopeBody->{'ns2:BudgetCheckElement'}->{'titel'} )
                . ":" );
        my $titeldefined =
            (      exists $soapEnvelopeBody->{'ns2:BudgetCheckElement'}
                && defined $soapEnvelopeBody->{'ns2:BudgetCheckElement'}
                && exists $soapEnvelopeBody->{'ns2:BudgetCheckElement'}->{'titel'}
                && defined $soapEnvelopeBody->{'ns2:BudgetCheckElement'}->{'titel'} );
        ...
        if ( $titeldefined && ref( $soapEnvelopeBody->{'ns2:BudgetCheckElement'}->{'titel'} ) eq 'HASH' ) {
            $titelArrayRef = [ $soapEnvelopeBody->{'ns2:BudgetCheckElement'}->{'titel'} ]
        ...
        for ( my $i = 0 ; $i < $titleCount ; $i++ ) {
            ...
            $self->{logger}->debug( "process() HTTP request exemplarArrayRef:"
            ...
            $self->{logger}
                ->debug( "process() Anz. titelInfo->exemplare:" . scalar @{ $titelInfo->{'exemplare'} } . ":" );
    ...
    foreach my $haushaltsstelle ( sort { $a cmp $b } keys %{ $self->{budgetEkz} } ) {
        $self->{logger}->debug(
            "process() haushaltsstelle:$haushaltsstelle): checkResult:"
                . (
                defined( $self->{budgetEkz}->{$haushaltsstelle}->{checkResult} )
                ? $self->{budgetEkz}->{$haushaltsstelle}->{checkResult}
                : 'undef'
                )
                . ":"
        );
        if ( $self->{budgetEkz}->{$haushaltsstelle}->{checkResult} & 1 ) {
            ...
            foreach my $kostenstelle ( sort { $a cmp $b } keys %{ $self->{budgetEkz}->{$haushaltsstelle} } ) {
                $self->{logger}->debug(
                    "process() haushaltsstelle:$haushaltsstelle: kostenstelle:$kostenstelle: checkResult:"
                        . (
                        defined( $self->{budgetEkz}->{$haushaltsstelle}->{checkResult} )
                        ? $self->{budgetEkz}->{$haushaltsstelle}->{checkResult}
                        : 'undef'
                    ...
                    if ( $self->{budgetEkz}->{$haushaltsstelle}->{$kostenstelle}->{checkResult} ) {
                        foreach my $errtype ( 32, 256, 512, 1024 ) {
                            if ( $self->{budgetEkz}->{$haushaltsstelle}->{$kostenstelle}->{checkResult} & $errtype ) {
                                foreach my $ekzArtikelNr (
                                    @{
                                        $self->{budgetEkz}->{$haushaltsstelle}->{$kostenstelle}->{ekzArtikelNr}
                                            ->{$errtype}
                                    }
                                    )
                                {
                                    if ( length( $errorMessage->{$errtype} ) == 0 ) {
                                        if ( $errtype == 32 ) {
                                            $errorMessage->{$errtype} .=
                                                "Ungültige Konto-Angabe bei ekzArtikel $ekzArtikelNr";
                                        }
                                        if ( $errtype == 256 ) {
                                            $errorMessage->{$errtype} .=
                                                "Überschreitung des Kontos $haushaltsstelle/$kostenstelle durch ekzArtikel $ekzArtikelNr";
                                        }
                                        if ( $errtype == 512 ) {
                                            my $encumb =
                                                $self->{budgetKoha}
                                                ->{ $self->{budgetEkz}->{$haushaltsstelle}->{$kostenstelle}->{budgetId}
                                                }->{budget_encumb};
                                            $errorMessage->{$errtype} .= sprintf(
                                                "Überschreitung der Warnquote von $haushaltsstelle/$kostenstelle (%.0f%%) durch ekzArtikel $ekzArtikelNr",
                                                $encumb
                                            );
                                        }
                                        if ( $errtype == 1024 ) {
                                            ...
                                            $errorMessage->{$errtype} .= sprintf(
                                                "Überschreitung des Warnlimits von $haushaltsstelle/$kostenstelle (%.2f EUR) durch ekzArtikel $ekzArtikelNr",
                                                $expend
                                            );
                                        }
                                    } else {
    ...
    foreach my $titelInfo (@titelInfoListe) {
        ...
        foreach my $exemplar ( @{ $titelInfo->{'exemplare'} } ) {
            ...
            my $soapExemplar = SOAP::Data->name(
                'exemplar' => \SOAP::Data->value(
    ...
    )->SOAP::Header::attr('xmlns:ns1="http://www.ekz.de/BestellsystemWSDL"');
...
sub handleBudgetCheck {
    ...
    for ( my $i = 0 ; $i < $itemCount ; $i++ ) {
        $self->{logger}
            ->debug("handleBudgetCheck() reqEkzArtikelNr:$reqEkzArtikelNr: itemCount:$itemCount: exemplar loop i:$i:");

        my $temporaryId =
            ( defined $exemplarArrayRef->[$i]->{'temporaryId'}
                && length( $exemplarArrayRef->[$i]->{'temporaryId'} ) > 0 )
            ? $exemplarArrayRef->[$i]->{'temporaryId'}
            : "temporaryId not set";
        my $exemplarquantity = $exemplarArrayRef->[$i]->{'konfiguration'}->{'anzahl'};
        $self->{logger}->debug(
            "handleBudgetCheck() itemCount $itemCount: exemplar loop i:$i: exemplarquantity:$exemplarquantity:");
        my $inBudget =
            1;    # If the real budget check in Koha is not activated, we return that the budget is not exceeded.
        ...
        if ( defined( $self->{ekzAqbooksellersId} ) && length( $self->{ekzAqbooksellersId} ) ) {
            my $haushaltsstelle =
                  $exemplarArrayRef->[$i]->{'konfiguration'}->{'budget'}->{'haushaltsstelle'}
                ? $exemplarArrayRef->[$i]->{'konfiguration'}->{'budget'}->{'haushaltsstelle'}
                : '---';
            my $kostenstelle =
                  $exemplarArrayRef->[$i]->{'konfiguration'}->{'budget'}->{'kostenstelle'}
                ? $exemplarArrayRef->[$i]->{'konfiguration'}->{'budget'}->{'kostenstelle'}
                : '---';
            my $gesamtpreis =
                defined( $exemplarArrayRef->[$i]->{'konfiguration'}->{'preis'}->{'gesamtpreis'} )
                ? $exemplarArrayRef->[$i]->{'konfiguration'}->{'preis'}->{'gesamtpreis'}
                : '0.0.';
            $self->{logger}->debug(
                "handleBudgetCheck() exemplar loop i:$i: haushaltsstelle:$haushaltsstelle: kostenstelle:$kostenstelle: gesamtpreis:$gesamtpreis: exemplarquantity:$exemplarquantity:"
            );
            ...
            if ( $haushaltsstelle eq '---' || $kostenstelle eq '---' ) {
                ...
                if ( $haushaltsstelle eq '---' ) {
                    ...
                    push @{ $self->{budgetEkz}->{$haushaltsstelle}->{$kostenstelle}->{ekzArtikelNr}->{16} },
                        $reqEkzArtikelNr;
                }
            } else {
                my ( $budgetperiodId, $budgetperiodDescription, $budgetId, $budgetCode ) =
                    $ekzKohaRecord->checkAqbudget( $self->{hauptstelle}, $haushaltsstelle, $kostenstelle, 0 );
                ...
                if ( !defined( $self->{budgetEkz}->{$haushaltsstelle}->{$kostenstelle} ) ) {
                    ...
                    $self->{budgetEkz}->{$haushaltsstelle}->{$kostenstelle}->{budgetperiodDescription} =
                        $budgetperiodDescription;
                    ...
                    $self->{logger}->debug(
                        "handleBudgetCheck() created self->{budgetEkz}->{$haushaltsstelle}->{$kostenstelle} with budgetperiodId:$budgetperiodId: budgetperiodDescription:$budgetperiodDescription: budgetId:$budgetId: budgetCode:$budgetCode:"
                    );
                }
                ...
                if ( !( defined($budgetId) && $budgetId > 0 ) ) {
                    ...
                    push @{ $self->{budgetEkz}->{$haushaltsstelle}->{$kostenstelle}->{ekzArtikelNr}->{32} },
                        $reqEkzArtikelNr;
                } else {
                    my $unittotal =
                        $exemplarquantity * $gesamtpreis;    # discounted, tax incl.  (if aqbooksellers.invoiceincgst)
                    if ( !defined( $self->{budgetKoha}->{$budgetId} ) ) {
                        ...
                        $self->{budgetKoha}->{$budgetId}->{budget_encumb} =
                            $budget->{budget_encumb};    # quote, e.g. 90.0 represents 90% (of budget_amount)
                        $self->{budgetKoha}->{$budgetId}->{budg_encumbrance} =
                            $budget->{budget_amount} * $budget->{budget_encumb} / 100.0;
                        $self->{budgetKoha}->{$budgetId}->{budget_expend} =
                            $budget->{budget_expend};    # means, e.g. 9000.0 represents 9000.00 EUR
                        ...
                        $self->{logger}
                            ->debug( "handleBudgetCheck() created self->{budgetKoha}->{$budgetId} with budget_amount:"
                                . scalar $self->{budgetKoha}->{$budgetId}->{budget_amount}
                                . ": budg_spent:"
                                . scalar $self->{budgetKoha}->{$budgetId}->{budg_spent}
                                . ": budg_ordered:"
                                . scalar $self->{budgetKoha}->{$budgetId}->{budg_ordered}
                                . ":" );
                    }
                    if ( defined( $self->{budgetKoha}->{$budgetId}->{budget_amount} ) ) {
                        my $budget_used = $self->{budgetKoha}->{$budgetId}->{budg_spent} +
                            $self->{budgetKoha}->{$budgetId}->{budg_ordered};
                        my $budget_remaining = $self->{budgetKoha}->{$budgetId}->{budget_amount} - $budget_used;

                        $self->{logger}->debug( "handleBudgetCheck() self->{budgetKoha}->{$budgetId}->{budget_amount}:"
                                . scalar $self->{budgetKoha}->{$budgetId}->{budget_amount}
                                . ":" );
                        $self->{logger}->debug( "handleBudgetCheck() self->{budgetKoha}->{$budgetId}->{budget_encumb}:"
                                . scalar $self->{budgetKoha}->{$budgetId}->{budget_encumb}
                                . ":" );
                        $self->{logger}
                            ->debug( "handleBudgetCheck() self->{budgetKoha}->{$budgetId}->{budg_encumbrance}:"
                                . scalar $self->{budgetKoha}->{$budgetId}->{budg_encumbrance}
                                . ":" );
                        $self->{logger}->debug( "handleBudgetCheck() self->{budgetKoha}->{$budgetId}->{budget_expend}:"
                                . scalar $self->{budgetKoha}->{$budgetId}->{budget_expend}
                                . ":" );
                        $self->{logger}->debug( "handleBudgetCheck() self->{budgetKoha}->{$budgetId}->{budg_ordered}:"
                                . scalar $self->{budgetKoha}->{$budgetId}->{budg_ordered}
                                . ":" );
                        $self->{logger}->debug( "handleBudgetCheck() self->{budgetKoha}->{$budgetId}->{budg_spent}:"
                                . scalar $self->{budgetKoha}->{$budgetId}->{budg_spent}
                                . ":" );
                        $self->{logger}->debug(
                            "handleBudgetCheck() budget_used:$budget_used: budget_remaining:$budget_remaining: unittotal:$unittotal:"
                        );

                        if ( $unittotal > $budget_remaining ) {
                            $inBudget = 0;
                            $self->{budgetKoha}->{$budgetId}->{warn_remaining} =
                            ...
                            push @{ $self->{budgetEkz}->{$haushaltsstelle}->{$kostenstelle}->{ekzArtikelNr}->{256} },
                                $reqEkzArtikelNr;
                        } else {
                            if (   ( $self->{budgetKoha}->{$budgetId}->{budg_encumbrance} + 0 )
                                && ( $budget_used + $unittotal ) >
                                $self->{budgetKoha}->{$budgetId}->{budg_encumbrance} )
                            {
                                $self->{budgetKoha}->{$budgetId}->{warn_encumbrance} = sprintf(
                                    "Warning! You will exceed %s %% of your fund.",
                                    $self->{budgetKoha}->{$budgetId}->{budget_encumb}
                                );
                                ...
                                push @{ $self->{budgetEkz}->{$haushaltsstelle}->{$kostenstelle}->{ekzArtikelNr}->{512}
                                }, $reqEkzArtikelNr;
                            }
                            if (   ( $self->{budgetKoha}->{$budgetId}->{budget_expend} + 0 )
                                && ( $budget_used + $unittotal ) > $self->{budgetKoha}->{$budgetId}->{budget_expend} )
                            {
                                $self->{budgetKoha}->{$budgetId}->{warn_expenditure} = sprintf(
                                    "Warning! You will exceed maximum limit %s %s for your fund.",
                                    $self->{budgetKoha}->{$budgetId}->{budget_expend}, 'EUR'
                                );
                                ...
                                push @{ $self->{budgetEkz}->{$haushaltsstelle}->{$kostenstelle}->{ekzArtikelNr}->{1024}
                                }, $reqEkzArtikelNr;
                            }
                        }
...
sub NotImplementedElement {
    ...
    my ($request) = @_
        ; # $request->{'soap:Envelope'}->{'soap:Body'} contains our deserialized not implemented SOAP element of the HTTP request
    my $soapElementName = "";
    ...
    $logger->debug( "NotImplementedElement() HTTP request request->{'soap:Envelope'}->{'soap:Body'}:"
            . Dumper( $request->{'soap:Envelope'}->{'soap:Body'} )
            . ":" );
