...
use C4::Acquisition
    qw( NewBasket GetBasket GetBaskets ModBasket GetBasketgroupsGeneric NewBasketgroup CloseBasketgroup );
...
sub new {
    ...
    my $self = {
        ...
        'hauptstelle' =>
            undef,    # will be set later, in function process() based on ekzKundenNr in XML element 'hauptstelle'
        'homebranch' =>
            undef,    # will be set later, in function process() based on ekzKundenNr in XML element 'hauptstelle'
        'auftragsnummer' =>
            undef,    # will be set later, in function process() based on supplyOption in XML element 'auftragsnummer'
        ...
        'ekzAqbooksellersId'                        =>
            '',    # will be set later, in function process() based on ekzKundenNr in XML element 'hauptstelle'
        'ekzKohaRecordClass'  => undef,
        'createdTitleRecords' => {}
        , # for storing biblionumber and title data of newly created title records to avoid multiple creation (Zebra/ES indexer is too slow, and from 22.11 on the ES indexer is locked out by database transaction)
        'updatedTitleRecords' => {}
        , # for storing biblionumber of updated title records (even if only its items are updated) for explicit index update
...
sub init {
    ...
    $self->{ekzWebServicesSetItemSubfieldsWhenOrdered} =
        C4::Context->preference("ekzWebServicesSetItemSubfieldsWhenOrdered");
    $self->{ekzAqbooksellersId} =
        '';    # will be set later, in function process() based on ekzKundenNr in XML element 'hauptstelle'
    ...
    $self->{emaillog}->{updatedTitlesCount} =
        0;    # counts the found titles with added or updated items in this step (0/1)
...
sub process {
    ...
    $self->{logger}
        ->info( "process() messageID:" . $soapEnvelopeBody->{'ns2:BestellInfoElement'}->{'messageID'} . ":" );
    $self->{logger}
        ->info( "process() ekzBestellNr:" . $soapEnvelopeBody->{'ns2:BestellInfoElement'}->{'ekzBestellNr'} . ":" );
    $self->{logger}
        ->info( "process() hauptstelle:" . $soapEnvelopeBody->{'ns2:BestellInfoElement'}->{'hauptstelle'} . ":" );
    ...
    my $wssusername =
        defined( $soapEnvelopeHeader->{'wsse:Security'}->{'wsse:UsernameToken'}->{'wsse:Username'} )
        ? $soapEnvelopeHeader->{'wsse:Security'}->{'wsse:UsernameToken'}->{'wsse:Username'}
        : "WSS-username not defined";
    my $wsspassword =
        defined( $soapEnvelopeHeader->{'wsse:Security'}->{'wsse:UsernameToken'}->{'wsse:Password'} )
        ? $soapEnvelopeHeader->{'wsse:Security'}->{'wsse:UsernameToken'}->{'wsse:Password'}
        : "WSS-username not defined";
    ...
    my $reqEkzBestellNr =
        defined $soapEnvelopeBody->{'ns2:BestellInfoElement'}->{'ekzBestellNr'}
        && length( $soapEnvelopeBody->{'ns2:BestellInfoElement'}->{'ekzBestellNr'} )
        ? $soapEnvelopeBody->{'ns2:BestellInfoElement'}->{'ekzBestellNr'}
        : 'UNDEFINED';
    ...
    my $reqLmsBestellCode =
        defined $soapEnvelopeBody->{'ns2:BestellInfoElement'}->{'lmsBestellCode'}
        && length( $soapEnvelopeBody->{'ns2:BestellInfoElement'}->{'lmsBestellCode'} )
        ? $soapEnvelopeBody->{'ns2:BestellInfoElement'}->{'lmsBestellCode'}
        : '';
    my $zeitstempel        = $soapEnvelopeBody->{'ns2:BestellInfoElement'}->{'zeitstempel'};
    my $reqEkzBestellDatum = DateTime->new(
        year => substr( $zeitstempel, 0, 4 ), month     => substr( $zeitstempel, 5, 2 ),
        day  => substr( $zeitstempel, 8, 2 ), time_zone => 'local'
    );
    my $reqWaehrung =
        defined $soapEnvelopeBody->{'ns2:BestellInfoElement'}->{'waehrung'}
        && length( $soapEnvelopeBody->{'ns2:BestellInfoElement'}->{'waehrung'} )
        ? $soapEnvelopeBody->{'ns2:BestellInfoElement'}->{'waehrung'}
        : 'EUR';

    $self->{hauptstelle} = $soapEnvelopeBody->{'ns2:BestellInfoElement'}->{'hauptstelle'};
    $self->{homebranch} =
        $self->{ekzKohaRecordClass}->{'ekzWsConfig'}->getEkzWebServicesDefaultBranch( $self->{hauptstelle} );
    $self->{homebranch} =~ s/^\s+|\s+$//g;    # trim spaces
    $self->{logger}->info( "process() self->{homebranch}:" . $self->{homebranch} . ":" );
    $self->{ekzAqbooksellersId} =
        $self->{ekzKohaRecordClass}->{'ekzWsConfig'}->getEkzAqbooksellersId( $self->{hauptstelle} );
    $self->{ekzAqbooksellersId} =~ s/^\s+|\s+$//g;    # trim spaces
    $self->{logger}->info( "process() self->{ekzAqbooksellersId}:" . $self->{ekzAqbooksellersId} . ":" );

    $self->{auftragsnummer} =
        $soapEnvelopeBody->{'ns2:BestellInfoElement'}->{'auftragsnummer'};    # used only for DKSH at the moment
    $self->{ccode} =
        $self->{ekzKohaRecordClass}->ccodeBySupplyOption( $self->{auftragsnummer} );  # used only for DKSH at the moment
    $self->{logger}->info( "process() self->{auftragsnummer}:"
            . ( defined( $self->{auftragsnummer} ) ? $self->{auftragsnummer} : 'undef' )
            . ": ->{ccode}:"
            . ( defined( $self->{ccode} ) ? $self->{ccode} : 'undef' )
            . ":" );
    ...
    try {
        $self->{logger}
            ->info( "process() authenticated:" . $authenticated . ": reqEkzBestellNr:" . $reqEkzBestellNr . ":" );
        if ( $authenticated && $ekzLocalServicesEnabled && $reqEkzBestellNr ne 'UNDEFINED' ) {
            ...
            if ( defined($hit) ) {
                ...
                $self->{logger}->trace(
                    "process() order message ordered hit->{_column_data}:" . Dumper( $hit->{_column_data} ) . ":" );
                my $mess = sprintf(
                    "process(): The ekzBestellNr '%s' has already been used at %s. Processing denied.",
                ...
                { # it is a BestellInfo triggered by a call of ekz webservice Bestellung -> UPDATE acquisition_import* records
                    my $selParam = {
                        vendor_id     => "ekz",
                        object_type   => "order",
                    ...
                    if ( !defined($hit) ) {
                        ...
                        my $mess = sprintf(
                            "process(): The lmsBestellCode '%s' has not been found. Processing of whole ekz BestellInfo denied.",
                            $reqLmsBestellCode
                        );
                        $self->{logger}->error($mess);
                        carp "BestellInfoElement::" . $mess . "\n";
                        ...
                        my $mess = sprintf(
                            "process(): The lmsBestellCode '%s' is not unique in acquisition_imports. Processing of whole ekz BestellInfo denied.",
                            $reqLmsBestellCode
                        );
                        $self->{logger}->error($mess);
                        carp "BestellInfoElement::" . $mess . "\n";
                        ...
                        $self->{logger}->debug( "process() acquisitionImportBestellInfo hit->{_column_data}:"
                                . Dumper( $hit->{_column_data} )
                                . ":" );

                        my $updParam = {
                            object_number => $reqEkzBestellNr,
                            object_date   => DateTime::Format::MySQL->format_datetime($reqEkzBestellDatum)
                            ,    # in local time_zone
                            processingstate => "ordered",
                            processingtime  => DateTime::Format::MySQL->format_datetime( $self->{dateTimeNow} )
                            ,    # in local time_zone
                ...
                {    # it is a BestellInfo triggered by an ekz medienshop order -> INSERT acquisition_import* records
                    ...
                    $acquisitionImportIdBestellInfo =
                        $acquisitionImportBestellInfo->_resultset()->create($insParam)->get_column('id');
                }
                $self->{logger}
                    ->debug( "process() acquisitionImportIdBestellInfo:" . $acquisitionImportIdBestellInfo . ":" );
            }

            $self->{logger}->debug(
                "process() HTTP request titel::" . $soapEnvelopeBody->{'ns2:BestellInfoElement'}->{'titel'} . ":" );
            $self->{logger}->debug( "process() HTTP request ref(titel):"
                    . ref( $soapEnvelopeBody->{'ns2:BestellInfoElement'}->{'titel'} )
                    . ":" );

            # look for XML <titel> blocks
            my $titeldefined =
                (      exists $soapEnvelopeBody->{'ns2:BestellInfoElement'}
                    && defined $soapEnvelopeBody->{'ns2:BestellInfoElement'}
                    && exists $soapEnvelopeBody->{'ns2:BestellInfoElement'}->{'titel'}
                    && defined $soapEnvelopeBody->{'ns2:BestellInfoElement'}->{'titel'} );
            ...
            if ( $titeldefined && ref( $soapEnvelopeBody->{'ns2:BestellInfoElement'}->{'titel'} ) eq 'HASH' ) {
                $titelArrayRef = [ $soapEnvelopeBody->{'ns2:BestellInfoElement'}->{'titel'} ]
                    ;                  # ref to anonymous array containing the single hash reference
            } else {
                ...
                if ( $titeldefined && ref( $soapEnvelopeBody->{'ns2:BestellInfoElement'}->{'titel'} ) eq 'ARRAY' ) {
                    $titelArrayRef = $soapEnvelopeBody->{'ns2:BestellInfoElement'}->{'titel'}
                        ;              # ref to deserialized array containing the hash references
                }
            }
            $self->{logger}->debug( "process() HTTP request titel array:"
                    . Dumper($titelArrayRef)
                    . ": AnzElem:"
                    . scalar @$titelArrayRef
                    . ":" );

            my $titleCount = scalar @$titelArrayRef;
            $self->{logger}->debug( "process() HTTP HTTP titleCount:"
                    . $titleCount
                    . ": reqEkzBestellNr:"
                    . $reqEkzBestellNr
                    . ": ekzBestellNrIsDuplicate:"
                    . $ekzBestellNrIsDuplicate
                    . ":" );
                ...
                )
            {
                ...
                $self->{logger}->debug(
                    "process() title check-loop i:$i: titel->{'ekzArtikelNr'}:" . $titel->{'ekzArtikelNr'} . ":" );

                if (   !defined( $titel->{'ekzArtikelNr'} )
                    || $titel->{'ekzArtikelNr'} + 0 == 0
                    || index( $titel->{'ekzArtikelNr'}, '-' ) >= 0 )
                {
                    if ( defined( $titel->{'ekzArtikelNr'} ) ) {    # do some normalization
                        ...
                        if ( index( $titel->{'ekzArtikelNr'}, '-' ) >= 0 ) {    # probably an ISBN, so remove the '-'
                            ...
                            if ( $titel->{'ekzArtikelNr'} + 0 > 99999999 )
                            {    # it was at least an ISBN 10 (which may end with an 'X')
                                $self->{logger}->debug(
                                    "process() ekzArtikelNrNotValid:$ekzArtikelNrNotValid: now titel->{'ekzArtikelNr'}:"
                                        . $titel->{'ekzArtikelNr'}
                                        . ":" );
                    ...
                    if ( $titel && defined( $titel->{'titelInfo'} ) && ref( $titel->{'titelInfo'} ) eq 'HASH' ) {
                        ...
                        if (   $ekzArtikelNrNotValid == 1
                            && defined( $titel->{'titelInfo'}->{'isbn13'} )
                            && $titel->{'titelInfo'}->{'isbn13'}
                            && !ref( $titel->{'titelInfo'}->{'isbn13'} ) )
                        {
                            my $isbn13;
                            ...
                            if ( defined($isbn13) && $isbn13 ) {
                                ...
                                } elsif ( $isbn13Length >= 1 && $reqLmsBestellCode )
                                { # it is a BestellInfo triggered by a call of ekz webservice Bestellung -> add lacking 0 to isbn13
                                    $titel->{'ekzArtikelNr'} = ( '0' x ( 13 - $isbn13Length ) )
                                        . $isbn13;  # we know already that $isbn13Length < 13 from the comparision above
                                    $ekzArtikelNrNotValid = 0;
                                }
                            }
                        }
                        ...
                        if (   $ekzArtikelNrNotValid == 1
                            && defined( $titel->{'titelInfo'}->{'ean'} )
                            && $titel->{'titelInfo'}->{'ean'}
                            && !ref( $titel->{'titelInfo'}->{'ean'} ) )
                        {
                            my $ean;
                            ...
                            if ( defined($ean) && $ean ) {
                                ...
                                if ( $eanLength >= 13 ) {
                                    ...
                                    $ekzArtikelNrNotValid = 0;
                                } elsif ( $eanLength >= 1 && $reqLmsBestellCode )
                                { # it is a BestellInfo triggered by a call of ekz webservice Bestellung -> add lacking 0 to ean
                                    $titel->{'ekzArtikelNr'} = ( '0' x ( 13 - $eanLength ) )
                                        . $ean;    # we know already that $eanLength < 13 from the comparision above
                                    $ekzArtikelNrNotValid = 0;
                                }
                            }
                        }
                        ...
                        if (   $ekzArtikelNrNotValid == 1
                            && defined( $titel->{'titelInfo'}->{'isbn'} )
                            && $titel->{'titelInfo'}->{'isbn'}
                            && !ref( $titel->{'titelInfo'}->{'isbn'} ) )
                        {
                            my $isbn;
                            my $isbnPreferredLen = 10;
                            if ( length( $titel->{'titelInfo'}->{'isbn'} ) >= 1 ) {
                                $isbn = $titel->{'titelInfo'}->{'isbn'}
                                    ;    # the ISBN may end with 'X', we evaluate the number before this 'X'
                                $isbn =~ s/\s//g;
                                $isbn =~ tr/-//d;
                                if ( index( $isbn, 'X' ) == 9 ) {
                                    $isbnPreferredLen = 9;
                            ...
                            if ( defined($isbn) && $isbn ) {
                                ...
                                if ( $isbnLength >= $isbnPreferredLen ) {
                                    ...
                                    $ekzArtikelNrNotValid = 0;
                                } elsif ( $isbnLength >= 1 && $reqLmsBestellCode )
                                { # it is a BestellInfo triggered by a call of ekz webservice Bestellung -> add lacking 0 to isbn
                                    $titel->{'ekzArtikelNr'} =
                                        ( '0' x ( $isbnPreferredLen - $isbnLength ) )
                                        . $isbn
                                        ; # we know already that $isbnLength < $isbnPreferredLen from the comparision above
                                    $ekzArtikelNrNotValid = 0;
                                }
                            }
                        }
                    ...
                    $self->{logger}->debug( "process() title check-loop i:$i: now titel->{'ekzArtikelNr'}:"
                            . $titel->{'ekzArtikelNr'}
                            . ":" );
                    if (  !$ekzArtikelNrNotValid
                        && $titel->{'ekzArtikelNr'} > 0
                        && length( $titel->{'ekzArtikelNr'} ) >= 9 )
                    {
                ...
                $self->{logger}
                    ->info( "process() ekzArtikelNrNotValid:$ekzArtikelNrNotValid: now titel->{'ekzArtikelNr'}:"
                        . $titel->{'ekzArtikelNr'}
                        . ":" );
            }
            ...
            if (   $titleCount > 0
                && $reqEkzBestellNr ne 'UNDEFINED'
                && !$ekzBestellNrIsDuplicate
                && !$lmsBestellCodeNotFound
                && !$lmsBestellCodeNotUnique
                && !$ekzArtikelNrNotValid )
            {
                ...
                $self->{ekzAqbooksellersId} =
                    $self->{ekzKohaRecordClass}->checkEkzAqbooksellersId( $self->{ekzAqbooksellersId}, 1 );
                if ( length( $self->{ekzAqbooksellersId} ) ) {
                    if ($reqLmsBestellCode)
                    { # it is a BestellInfo triggered by a call of ekz webservice Bestellung -> search this aqbasket record by basketno
                        my $selbaskets = C4::Acquisition::GetBaskets( { 'basketno' => $reqLmsBestellCode } );
                        if ( @{$selbaskets} > 0 ) {
                            $basketno = $selbaskets->[0]->{'basketno'};
                            $self->{logger}->debug(
                                "process() searched by lmsBestellCode and found aqbasket with basketno:$basketno:");
                        } else {
                            $acquisitionError = 2;
                        }
                    } else
                    { # it is a BestellInfo triggered by an ekz medienshop order -> search by basketname or create the aqbasket record
                        ...
                        if ( @{$selbaskets} > 0 ) {
                            ...
                            if ($patron) {
                                ...
                                $self->{logger}
                                    ->info( "genKohaRecords() found patron with surname = 'LCService' authorisedby:"
                                        . $authorisedby
                                        . ":" );
                            }
                            my $branchcode = $self->{ekzKohaRecordClass}->branchcodeFallback( '', $self->{homebranch} );
                            $basketno = C4::Acquisition::NewBasket(
                                $self->{ekzAqbooksellersId},  $authorisedby, $basketname,
                                'created by ekz BestellInfo', '', undef, $branchcode, $branchcode, 0, 'ordering'
                            );    # XXXWH
                            $self->{logger}
                                ->debug( "process() created new basket having basketno:" . Dumper($basketno) . ":" );
                            if ($basketno) {
                                my $basketinfo = {};
                ...
                $self->{logger}->info( "process() ekzAqbooksellersId:"
                        . $self->{ekzAqbooksellersId}
                        . ": acquisitionError:$acquisitionError: basketno:$basketno:" );
                ...
                if ( !$acquisitionError ) {
                    ...
                    for ( my $i = 0 ; $i < $titleCount ; $i++ ) {
                        ...
                        $reqParamTitelInfo->{'ekzVerkaufsEinheitsNr'} =
                            $titel->{'titelInfo'}->{'ekzVerkaufsEinheitsNr'};
                        ...
                        $self->{logger}
                            ->debug( "process() HTTP request ref(exemplar):" . ref( $titel->{'exemplar'} ) . ":" );
                        ...
                        if ( $exemplardefined && ref( $titel->{'exemplar'} ) eq 'HASH' ) {
                            $exemplarArrayRef =
                                [ $titel->{'exemplar'} ];  # ref to anonymous array containing the single hash reference
                        } else {
                            ...
                            if ( $exemplardefined && ref( $titel->{'exemplar'} ) eq 'ARRAY' ) {
                                $exemplarArrayRef =
                                    $titel->{'exemplar'};    # ref to deserialized array containing the hash references
                            }
                        }
                        $self->{logger}->debug( "process() HTTP request exemplarArray:"
                                . Dumper($exemplarArrayRef)
                                . ": AnzElem:"
                                . scalar @{$exemplarArrayRef}
                                . ":" );
                        my @idPaarListeTmp = $self->handleTitelBestellInfo(
                            $acquisitionImportIdBestellInfo, $reqEkzBestellNr,
                            $reqEkzBestellDatum,             $reqLmsBestellCode, $reqParamTitelInfo, $exemplarArrayRef,
                            $reqWaehrung,                    $basketno,          $authorisedby
                        );    ## add or update title data and item data

                        $self->{logger}->debug( "process() Anzahl idPaarListeTmp:"
                                . scalar @idPaarListeTmp
                                . ": idPaarListeTmp:"
                                . Dumper(@idPaarListeTmp)
                                . ":" );
                    ...
                    if ( length( $self->{ekzAqbooksellersId} ) && defined($basketno) && $basketno > 0 ) {
                        ...
                        if ($aqbasket) {
                            ...
                            # close the basket
                            $self->{logger}->debug( "process() is calling Koha::Acquisition::Baskets->find(basketno:"
                                    . $aqbasket->{basketno}
                                    . ")" );
                            my $kohabasket = Koha::Acquisition::Baskets->find( $aqbasket->{basketno} );
                            if ($kohabasket) {
                                $self->{logger}
                                    ->debug( "process() is calling Koha::Acquisition::Baskets->find(basketno:"
                                        . $aqbasket->{basketno}
                                        . ")->close" );
                                $kohabasket->close;
                            }
                            ...
                            $basketgroupid = undef;
                            my $aqbasketgroups =
                                &C4::Acquisition::GetBasketgroupsGeneric( $params, { orderby => "id DESC" } );
                            $self->{logger}
                                ->debug( "process() Dumper aqbasketgroups:" . Dumper($aqbasketgroups) . ":" );
                            ...
                            if ( !defined($aqbasketgroups) || scalar @{$aqbasketgroups} == 0 ) {
                                ...
                                $self->{logger}->debug( "process() created basketgroup with name:"
                                        . $aqbasket->{basketname}
                                        . ": having basketgroupid:$basketgroupid:" );
                            } else {
                                $basketgroupid = $aqbasketgroups->[0]->{id};
                                $self->{logger}->debug( "process() found basketgroup with name:"
                                        . $aqbasket->{basketname}
                                        . ": having basketgroupid:$basketgroupid:" );
                            }
                            ...
                            if ($basketgroupid) {
                                ...
                                $self->{logger}
                                    ->debug("process() is calling CloseBasketgroup basketgroupid:$basketgroupid:");
        ...
        if ( ref($exceptionThrown) && exists( $exceptionThrown->{message} ) && $exceptionThrown->{message} ) {
            $self->{logger}
                ->error( "process() caught exception having message:" . Dumper( $exceptionThrown->{message} ) . ":" );
    ...
    $self->{logger}
        ->info( "process() Anzahl idPaarListe:" . scalar @idPaarListe . ": idPaarListe:" . Dumper(@idPaarListe) . ":" );
    ...
    if ($exceptionThrown) {
        ...
        $respStatusMessage =
              "Webservices für ekz-Anfragen sind in der Koha-Instanz "
            . C4::External::EKZ::EkzAuthentication::kohaInstanceName()
            . " nicht aktiviert.";
        ...
        $respStatusMessage =
            "Die angeforderte Bestellung ist anhand lmsBestellCode '$reqLmsBestellCode' nicht eindeutig zu identifizieren.";
        ...
        if ( $acquisitionError == 2 ) {
            $respStatusMessage =
                "Der Bestellkorb kann anhand von lmsBestellCode '$reqLmsBestellCode' nicht identifiziert werden.";
        } else {
    ...
    foreach my $idPaar (@idPaarListe) {
        ...
        my $soapIdPaar = SOAP::Data->name(
            'idPaar' => \SOAP::Data->value(
    ...
    push @{ $self->{emaillog}->{logresult} },
        [
        'BestellInfo', $soapEnvelopeBody->{'ns2:BestellInfoElement'}->{'messageID'},
        $self->{emaillog}->{actionresult}, $acquisitionError, $self->{ekzAqbooksellersId}, $basketno
        ];
    $self->{logger}->info(
        "process() VVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVV"
    );
    $self->{logger}
        ->info( "process() Dumper(self->{emaillog}->{logresult}):" . Dumper( $self->{emaillog}->{logresult} ) . ":" );
    $self->{logger}->info(
        "process() ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^"
    );
    ...
    if ( $respStatusCode eq 'ERROR' ) {
        ...
        if ( $self->{createdTitleRecords} ) {
            foreach my $titleSelHashkey ( sort keys %{ $self->{createdTitleRecords} } ) {
                ...
                if ( defined $biblionumber ) {
                    ...
                    $self->{logger}->debug(
                        "process() pushed biblionumber:$biblionumber: of createdTitleRecords to array biblionumbers (new length:"
        ...
        if ( $self->{updatedTitleRecords} ) {
            foreach my $titleRecordBiblionumber ( sort keys %{ $self->{updatedTitleRecords} } ) {
                if ( defined $titleRecordBiblionumber ) {
                    $self->{logger}
                        ->debug( "process() an updated title has biblionumber:" . $titleRecordBiblionumber . ":" );
                    if ( grep( /^$titleRecordBiblionumber$/, @biblionumbers ) == 0 ) {
                        push @biblionumbers, $titleRecordBiblionumber;
                        $self->{logger}->debug(
                            "process() pushed biblionumber:$titleRecordBiblionumber: of updatedTitleRecords to array biblionumbers (new length:"
                                . scalar @biblionumbers
                                . ":)." );
                    }
                }
        ...
        if (@biblionumbers) {
            ...
            $self->{logger}->debug(
                "process() is calling indexer->index_records() with biblionumbers:" . Dumper(@biblionumbers) . ":" );
            ...
            try {
                ...
                my $mess = sprintf(
                    "handleTitelBestellInfo(): Exception thrown by update_index:%s:, so the index has to be rebuilt manually!!!",
                    $_[0]
                );
                $self->{logger}->error($mess);
                carp "BestellInfoElement::" . $mess . "\n";
        ...
        if ( scalar @{ $self->{emaillog}->{logresult} } > 0 ) {   # RG 31.03.2020: send e-mail also if reqLmsBestellCode
            ...
            my ( $message, $subject, $haserror ) = $self->{ekzKohaRecordClass}->createProcessingMessageText(
                $self->{emaillog}->{logresult}, "headerTEXT", $self->{emaillog}->{dt},
                \@importIds, $reqEkzBestellNr
            );    # we use ekzBestellNr as part of importID in MARc field 025.a: (EKZImport)$importIDs[0]
            $self->{ekzKohaRecordClass}
                ->sendMessage( $soapEnvelopeBody->{'ns2:BestellInfoElement'}->{'hauptstelle'}, $message, $subject );
        }
    }
    ...
    )->SOAP::Header::attr('xmlns:ns2="http://www.ekz.de/BestellsystemWSDL"');
...
sub handleTitelBestellInfo {
    my (
        $self,              $acquisitionImportIdBestellInfo, $reqEkzBestellNr, $reqEkzBestellDatum, $reqLmsBestellCode,
        $reqParamTitelInfo, $exemplare,                      $reqWaehrung,     $basketno,           $authorisedby
    ) = @_;
    ...
    $self->{logger}->info(
        "handleTitelBestellInfo() Start reqEkzBestellNr:$reqEkzBestellNr: reqEkzBestellDatum:$reqEkzBestellDatum: reqWaehrung:$reqWaehrung: basketno:$basketno:"
    );

    # step 1: find or create biblio record
    if ($reqLmsBestellCode)
    {    # it is a BestellInfo triggered by a call of ekz webservice Bestellung -> fetch the biblionumber via aqorders
        if (   defined($exemplare)
            && defined( $exemplare->[0] )
            && defined( $exemplare->[0]->{'lmsExemplarID'} )
            && length( $exemplare->[0]->{'lmsExemplarID'} ) > 0 )
        {
            $reqLmsExemplarID = $exemplare->[0]->{'lmsExemplarID'};    # lmsExemplarID contains aqorders.ordenumber
        } else {
            my $mess = sprintf(
                "handleTitelBestellInfo(): No lmsExemplarID sent for lmsBestellCode '%s'. Processing of whole ekz BestellInfo denied.",
                $reqLmsBestellCode
            );
            $self->{logger}->error($mess);
            carp "BestellInfoElement::" . $mess . "\n";
            ...
            Koha::Exceptions::WrongParameter->throw(
                error => sprintf(
                    "Der BestellInfo-Request mit lmsBestellCode '%s' enthält das Feld lmsExemplarID nicht, somit kann der Titelsatz nicht identifiziert werden. Abbruch der Verarbeitung der gesamten ekz BestellInfo.\n",
                    $reqLmsBestellCode
                ),
            );
        }
        ...
        if ( !$aqordersHit ) {
            my $mess = sprintf(
                "handleTitelBestellInfo(): The Koha order with aqorders.ordernumber '%s' has not been found. Processing of whole ekz BestellInfo denied.",
                $reqLmsExemplarID
            );
            $self->{logger}->error($mess);
            carp "BestellInfoElement::" . $mess . "\n";
            ...
            Koha::Exceptions::WrongParameter->throw(
                error => sprintf(
                    "Die Koha-Bestellung mit aqorders.ordernumber '%s' (lmsExemplarID) wurde nicht gefunden. Abbruch der Verarbeitung der gesamten ekz BestellInfo.",
                    $reqLmsExemplarID
                ),
            );
        }
        ...
        if ( !$titleHits || $titleHits->{'count'} != 1 || !defined $titleHits->{'records'}->[0] ) {
            my $mess = sprintf(
                "handleTitelBestellInfo(): The Koha biblio record with biblionumber '%s' has not been found. Processing of whole ekz BestellInfo denied.",
                $biblionumber
            );
            $self->{logger}->error($mess);
            carp "BestellInfoElement::" . $mess . "\n";
            ...
            Koha::Exceptions::WrongParameter->throw(
                error => sprintf(
                    "Die Koha-Biblo-Daten mit biblionumber '%s' wurden nicht gefunden. Abbruch der Verarbeitung der gesamten ekz BestellInfo.",
                    $biblionumber
                ),
            );
        }
        ...
        if ( length($titleSelHashkey) > 6 && defined( $self->{createdTitleRecords}->{$titleSelHashkey} ) ) {
            ...
            $self->{logger}->info(
                "handleTitelBestellInfo() got used biblionumber:$biblionumber: from self->{createdTitleRecords}->{$titleSelHashkey}"
            );
        }
        ...
        for ( my $searchmode = 1 ; $searchmode <= 2 ; $searchmode *= 2 ) {
            $self->{logger}->info( "handleTitelBestellInfo() in loop searchmode:$searchmode: ekzArtikelNr:"
                    . $reqParamTitelInfo->{'ekzArtikelNr'}
                    . ": titleHits->{'count'}:"
                    . $titleHits->{'count'}
                    . ":" );
            if ( $titleHits->{'count'} > 0 && defined $titleHits->{'records'}->[0] ) {
                $self->{logger}->debug( "handleTitelBestellInfo() in loop last; searchmode:$searchmode: ekzArtikelNr:"
                        . $reqParamTitelInfo->{'ekzArtikelNr'}
                        . ": titleHits->{'count'}:"
                        . $titleHits->{'count'}
                        . ": biblionumber:"
            ...
            if ( $searchmode == 1 && !$reqParamTitelInfo->{'ekzArtikelNr'} ) {
                $self->{logger}->debug( "handleTitelBestellInfo() in loop next1; searchmode:$searchmode: ekzArtikelNr:"
                        . $reqParamTitelInfo->{'ekzArtikelNr'}
                        . ": titleHits->{'count'}:"
                        . $titleHits->{'count'}
                        . ":" );
            ...
            $self->{logger}
                ->info( "handleTitelBestellInfo() from local DB searchmode:$searchmode: titleHits->{'count'}:"
                    . $titleHits->{'count'}
                    . ":" );
            if ( $titleHits->{'count'} > 0 && defined $titleHits->{'records'}->[0] ) {
                $biblionumber = $titleHits->{'records'}->[0]->subfield( "999", "c" );
                $self->{logger}->debug( "handleTitelBestellInfo() in loop next2; searchmode:$searchmode: ekzArtikelNr:"
                        . $reqParamTitelInfo->{'ekzArtikelNr'}
                        . ": titleHits->{'count'}:"
                        . $titleHits->{'count'}
                        . ": biblionumber:"
            ...
            foreach my $titleSource (@titleSourceSequence) {
                $self->{logger}
                    ->info("handleTitelBestellInfo() searchmode:$searchmode: in loop titleSource:$titleSource:");
                if ( $titleHits->{'count'} > 0 && defined $titleHits->{'records'}->[0] ) {
                    last;    # title data have been found in lastly tested title source
                }
                ...
                if ( $titleSource eq '_LMSC' ) {
                    if ( $searchmode == 2 ) {
                        ...
                        $self->{logger}->info(
                                  "handleTitelBestellInfo() from LMS Pool searchmode:$searchmode: titleHits->{'count'}:"
                                . $titleHits->{'count'}
                                . ":" );
                    }
                    ...
                    if ( $searchmode == 2 ) {
                        ...
                        $titleHits = $self->{ekzKohaRecordClass}
                            ->readTitleFromEkzWsMedienDaten( $reqParamTitelInfo->{'ekzArtikelNr'} );
                        $self->{logger}->info(
                            "handleTitelBestellInfo() from ekz Webservice 'MedienDaten' searchmode:$searchmode: titleHits->{'count'}:"
                                . $titleHits->{'count'}
                                . ":" );
                    ...
                    if ( $searchmode == 2 ) {
                        ...
                        $titleHits = $self->{ekzKohaRecordClass}->createTitleFromFields($reqParamTitelInfo)
                            ;    # creates marc data, not a biblio DB record
                        $self->{logger}->info(
                            "handleTitelBestellInfo() from sent titelinfo fields searchmode:$searchmode: titleHits->{'count'}:"
                                . $titleHits->{'count'}
                                . ":" );
                    }
                    ...
                    if ( $searchmode == 2 ) {
                        ...
                        $titleHits =
                            $self->{ekzKohaRecordClass}->readTitleFromZ3950Target( $titleSource, $reqParamTitelInfo );
                        $self->{logger}->info( "handleTitelBestellInfo() from z39.50 search on target:"
                                . $titleSource
        ...
        if ( $titleHits->{'count'} > 0 && defined $titleHits->{'records'}->[0] ) {
            if ( $biblionumber == 0 ) {    # title data have been found in one of the sources, but not in local DB
                ...
                ( $biblionumber, $biblioitemnumber, $newrec ) = $self->{ekzKohaRecordClass}->addNewRecords(
                    $titleHits,                            $volumeEkzArtikelNr,          $reqEkzBestellNr,
                    $self->{ekzWsHideOrderedTitlesInOpac}, $self->{createdTitleRecords}, $titleSelHashkey
                );
                $self->{logger}->info( "handleTitelBestellInfo() new biblionumber:"
                        . $biblionumber
                        . ": biblioitemnumber:"
                        . $biblioitemnumber
                        . ":" );
                $self->{logger}->debug( "handleTitelBestellInfo() titleHits:" . Dumper($titleHits) . ":" );
                $self->{logger}->trace( "handleTitelBestellInfo() titleSelHashkey:"
                        . $titleSelHashkey
                        . ": self->{createdTitleRecords}->{titleSelHashkey}->{biblionumber}:"
                        . $self->{createdTitleRecords}->{$titleSelHashkey}->{biblionumber}
                        . ": ->{titleHits}:"
                        . Dumper( $self->{createdTitleRecords}->{$titleSelHashkey}->{titleHits} )
                        . ":" );
                ...
                if ( defined $biblionumber && $biblionumber > 0 ) {
                    ...
                    $self->{emaillog}->{problems} .=
                          "ERROR: Import der Titeldaten für ekz Artikel: "
                        . $reqParamTitelInfo->{'ekzArtikelNr'}
                        . " wurde abgewiesen.\n";
                    $self->{emaillog}->{importresult} = -1;
    ...
    if ( $titleHits->{'count'} > 0 && defined $titleHits->{'records'}->[0] ) {
        ...
        push @{ $self->{emaillog}->{records} },
            [
            $reqParamTitelInfo->{'ekzArtikelNr'}, defined $biblionumber ? $biblionumber : "no biblionumber",
            $self->{emaillog}->{importresult},    $titeldata, $isbnean, $self->{emaillog}->{problems},
            $self->{emaillog}->{importerror},     1,          undef,    undef
            ];
    }
    ...
    if ( $biblioExisting || $biblioInserted ) {
        ...
        {   # it is a BestellInfo triggered by a call of ekz webservice Bestellung -> UPDATE acquisition_import* records
            ...
            $self->{logger}->debug(
                "handleTitelBestellInfo() ref(acquisitionImportTitleHit):" . ref($acquisitionImportTitleHit) . ":" );
            if ( !defined($acquisitionImportTitleHit) ) {
                ########## $lmsBestellCodeNotFound = 1;
                my $mess = sprintf(
                    "handleTitelBestellInfo(): The acquisition_import title record having lmsBestellCode '%s' and lmsExemplarID '%s' has not been found. Processing of whole ekz BestellInfo denied.",
                ...
                Koha::Exceptions::WrongParameter->throw(
                    error => sprintf(
                        "Der acquisition_import Titel-Datensatz mit lmsBestellCode '%s' und lmsExemplarID '%s' wurde nicht gefunden. Abbruch der Verarbeitung der gesamten ekz BestellInfo.\n",
                        $reqLmsBestellCode, $reqLmsExemplarID
                    ),
                );
                ...
                $self->{logger}->debug( "handleTitelBestellInfo() acquisitionImportTitleHit->{_column_data}:"
            ...
            $self->{logger}->debug( "handleTitelBestellInfo() acquisitionImportTitleRS->{_column_data}:"
                    . Dumper( $acquisitionImportTitleRS->{_column_data} )
                    . ":" );
            $self->{logger}
                ->debug( "handleTitelBestellInfo() acquisitionImportIdTitle:" . $acquisitionImportIdTitle . ":" );
            ...
            $self->{logger}->debug( "handleTitelBestellInfo() titleImportObjectRS->{_column_data}:"
        ...
        for ( my $i = 0 ; $i < $itemCount ; $i++ ) {
            my $ekzExemplarID =
                ( defined $exemplare->[$i]->{'ekzExemplarID'} && length( $exemplare->[$i]->{'ekzExemplarID'} ) > 0 )
                ? $exemplare->[$i]->{'ekzExemplarID'}
                : "ekzExemplarID not set";
            my $exemplar_anzahl = $exemplare->[$i]->{'konfiguration'}->{'anzahl'};
            $self->{logger}->debug(
                "handleTitelBestellInfo() exemplar itemCount:$itemCount: loop i:$i: exemplar_anzahl:$exemplar_anzahl:");

            if ( $exemplar_anzahl + 0 < 1 ) {
                my $mess = sprintf(
                    "handleTitelBestellInfo(): The BestellInfo-request having ekzBestellNr '%s' and lmsBestellCode '%s' has invalid value ('%s') in field <anzahl>. Processing of whole ekz BestellInfo denied.",
                ...
                Koha::Exceptions::WrongParameter->throw(
                    error => sprintf(
                        "Der BestellInfo-Request mit ekzBestellNr '%s' und lmsBestellCode '%s' enthaelt im Feld (Exemplar-)Anzahl einen unzulaessigen Wert ('%s'). Abbruch der Verarbeitung der gesamten ekz BestellInfo.\n",
                        $reqEkzBestellNr, $reqLmsBestellCode, $exemplar_anzahl
                    ),
                );
            }
            ...
            $self->{logger}
                ->debug( "handleTitelBestellInfo() exemplare->[$i]->{'konfiguration'}->{'ExemplarFelderElement'}:"
                    . $exemplare->[$i]->{'konfiguration'}->{'ExemplarFelderElement'}
                    . ":" );
            $self->{logger}
                ->debug( "handleTitelBestellInfo() ref exemplare->[$i]->{'konfiguration'}->{'ExemplarFelderElement'}:"
                    . ref( $exemplare->[$i]->{'konfiguration'}->{'ExemplarFelderElement'} )
                    . ":" );
            ...
            if (   defined( $exemplare->[$i]->{'konfiguration'}->{'ExemplarFelderElement'} )
                && ref( $exemplare->[$i]->{'konfiguration'}->{'ExemplarFelderElement'} )
                && defined( $exemplare->[$i]->{'konfiguration'}->{'ExemplarFelderElement'}->{'ExemplarFeldElement'} )
                && ref( $exemplare->[$i]->{'konfiguration'}->{'ExemplarFelderElement'} ) )
            {
                if (
                    ref( $exemplare->[$i]->{'konfiguration'}->{'ExemplarFelderElement'}->{'ExemplarFeldElement'} ) eq
                    'HASH' )
                {
                    $exemplarfelderArrayRef =
                        [ $exemplare->[$i]->{'konfiguration'}->{'ExemplarFelderElement'}->{'ExemplarFeldElement'} ]
                        ;    # ref to anonymous array containing the single hash reference
                } else {

                    # if there are sent more than one ExemplarFeldElement blocks, they are delivered here as array ref
                    if (
                        ref( $exemplare->[$i]->{'konfiguration'}->{'ExemplarFelderElement'}->{'ExemplarFeldElement'} )
                        eq 'ARRAY' )
                    {
                        $exemplarfelderArrayRef =
                            $exemplare->[$i]->{'konfiguration'}->{'ExemplarFelderElement'}->{'ExemplarFeldElement'}
            ...
            $self->{logger}->debug( "handleTitelBestellInfo() HTTP request ExemplarFeldElement Anz.Elem.:"
                    . scalar @$exemplarfelderArrayRef
                    . ": exemplarfelderArrayRef:"
                    . Dumper($exemplarfelderArrayRef)
                    . ":" );
            ...
            for ( my $j = 0 ; $j < $exemplarfeldercount ; $j++ ) {
                $self->{logger}->debug( "handleTitelBestellInfo() HTTP request ExemplarFeldElement[$j] name:"
                        . $exemplarfelderArrayRef->[$j]->{'name'}
                        . ": inhalt:"
                        . $exemplarfelderArrayRef->[$j]->{'inhalt'}
                        . ":" );
            ...
            $self->{logger}
                ->debug( "handleTitelBestellInfo() vor checkbranchcode zweigstellencode:" . $zweigstellencode . ":" );
            if ( !$self->{ekzKohaRecordClass}->checkbranchcode($zweigstellencode) ) {
                $zweigstellencode = '';
            }
            $self->{logger}
                ->debug( "handleTitelBestellInfo() nach checkbranchcode zweigstellencode:" . $zweigstellencode . ":" );
            ...
            my $rabatt =
                defined( $exemplare->[$i]->{'konfiguration'}->{'preis'}->{'rabatt'} )
                ? $exemplare->[$i]->{'konfiguration'}->{'preis'}->{'rabatt'}
                : "0.0";
            my $fracht =
                defined( $exemplare->[$i]->{'konfiguration'}->{'preis'}->{'fracht'} )
                ? $exemplare->[$i]->{'konfiguration'}->{'preis'}->{'fracht'}
                : "0.00";
            my $einband =
                defined( $exemplare->[$i]->{'konfiguration'}->{'preis'}->{'einband'} )
                ? $exemplare->[$i]->{'konfiguration'}->{'preis'}->{'einband'}
                : "0.00";
            my $bearbeitung =
                defined( $exemplare->[$i]->{'konfiguration'}->{'preis'}->{'bearbeitung'} )
                ? $exemplare->[$i]->{'konfiguration'}->{'preis'}->{'bearbeitung'}
                : "0.00";
            my $ustSatz =
                defined( $exemplare->[$i]->{'konfiguration'}->{'preis'}->{'ustSatz'} )
                ? $exemplare->[$i]->{'konfiguration'}->{'preis'}->{'ustSatz'} / 100.0
                : &C4::External::EKZ::lib::EkzKohaRecords::defaultUstSatz('E');
            my $ust =
                defined( $exemplare->[$i]->{'konfiguration'}->{'preis'}->{'ust'} )
                ? $exemplare->[$i]->{'konfiguration'}->{'preis'}->{'ust'}
                : "0.00";
            my $gesamtpreis =
                defined( $exemplare->[$i]->{'konfiguration'}->{'preis'}->{'gesamtpreis'} )
                ? $exemplare->[$i]->{'konfiguration'}->{'preis'}->{'gesamtpreis'}
                : "0.00";    # total for a single item, discounted, incl. additional costs
            ...
            my $listpriceDiscounted_tax_included =
                $gesamtpreis - $fracht -
                $einband -
                $bearbeitung;    # not sent in LieferscheinDetailResponseElement, so we calculate it
            my $listprice_tax_included = $listpriceDiscounted_tax_included;
            if ( $rabatt != 0.0 ) {
                my $divisor = 1.0 - ( $rabatt / 100.0 );
                $listprice_tax_included =
                      $divisor == 0.0
                    ? $listprice_tax_included
                    : $listprice_tax_included /
                    $divisor;    # list price of single item in vendor's currency, not discounted
            ...
            my $replacementcost_tax_included = $listprice_tax_included
                ; # list price of single item in library's currency, not discounted (at the moment no exchange rate calculation implemented)
            my $replacementcost_tax_excluded = $listprice_tax_excluded
                ; # list price of single item in library's currency, not discounted, tax excluded (at the moment no exchange rate calculation implemented)
            ...
            if ( $reqLmsBestellCode || ( defined($basketno) && $basketno > 0 ) ) {
                ...
                my $haushaltsstelle =
                    defined( $exemplare->[$i]->{'konfiguration'}->{'budget'}->{'haushaltsstelle'} )
                    ? $exemplare->[$i]->{'konfiguration'}->{'budget'}->{'haushaltsstelle'}
                    : "";
                my $kostenstelle =
                    defined( $exemplare->[$i]->{'konfiguration'}->{'budget'}->{'kostenstelle'} )
                    ? $exemplare->[$i]->{'konfiguration'}->{'budget'}->{'kostenstelle'}
                    : "";

                my ( $dummy1, $dummy2, $budgetid, $dummy3 ) = $self->{ekzKohaRecordClass}
                    ->checkAqbudget( $self->{hauptstelle}, $haushaltsstelle, $kostenstelle, 1 );
                ...
                $budgetedcost_tax_excluded =
                    &C4::External::EKZ::lib::EkzKohaRecords::round( $budgetedcost_tax_excluded, 2 );
                ...
                $orderinfo->{listprice} = $listprice_tax_included
                ...
                $orderinfo->{rrp}              = $replacementcost_tax_included
                ...
                $orderinfo->{ecost}            = $budgetedcost_tax_included
                ...
                $orderinfo->{tax_rate_bak} =
                    $ustSatz;    #  corresponds to input field 'Tax rate' in UI (7% are stored as 0.07)
                ...
                if ($reqLmsBestellCode) {
                    ...
                    if ( $orderinfo->{quantity} != $aqordersHit->get_column('quantity') ) {
                        my $mess = sprintf(
                            "handleTitelBestellInfo(): ekz-Exemplaranzahl (%s) != Koha aqorders.quantity (%s). Processing of whole ekz BestellInfo denied.",
                        ...
                        Koha::Exceptions::WrongParameter->throw(
                            error => sprintf(
                                "ekz-Exemplaranzahl (%s) != Koha aqorders.quantity (%s). Abbruch der Verarbeitung der gesamten ekz BestellInfo.\n",
                                $orderinfo->{quantity}, $aqordersHit->get_column('quantity')
                            ),
                        );
                    }
                    if ( $orderinfo->{currency} ne $aqordersHit->get_column('currency') ) {
                        my $mess = sprintf(
                            "handleTitelBestellInfo(): ekz-waehrung (%s) != Koha aqorders.currency (%s). Processing of whole ekz BestellInfo denied.",
                        ...
                        Koha::Exceptions::WrongParameter->throw(
                            error => sprintf(
                                "ekz-waehrung (%s) != Koha aqorders.currency (%s). Abbruch der Verarbeitung der gesamten ekz BestellInfo.\n",
                                $orderinfo->{currency}, $aqordersHit->get_column('currency')
                            ),
                        );
                    }
            ...
            if ($reqLmsBestellCode) {
                ...
                my $selParam   = {
                    ...
                    object_number => 'basketno:' . $reqLmsBestellCode,    # reqLmsBestellCode is an aqbasket.basketno
                     #object_number => 'test basketno:' . $reqLmsBestellCode,    # reqLmsBestellCode is an aqbasket.basketno XXXWH    # for tests only
                    rec_type           => "item",
                    object_item_number => 'ordernumber:'
                        . $reqLmsExemplarID,    # reqLmsExemplarID is an aqorders.ordernumber
                    processingstate => "requested"
                };
                my $acquisitionImportIdItem;
                my $acquisitionImportItem = Koha::AcquisitionImport::AcquisitionImports->new();
                my $acquisitionImportItemRS =
                    $acquisitionImportItem->_resultset()->search($selParam);  # we have to update all items of the order
                if ( !defined($acquisitionImportItemRS) ) {
                    $self->{logger}->info( "handleTitelBestellInfo() ref(acquisitionImportItemRS):"
                            . ref($acquisitionImportItemRS)
                            . ":" );
                    my $mess = sprintf(
                        "handleTitelBestellInfo(): No acquisition_import item record having lmsBestellCode '%s' and lmsExemplarID '%s' in processing state 'prepared' has been found. Processing of whole ekz BestellInfo denied.",
                        $reqLmsBestellCode, $reqLmsExemplarID
                    );
                    $self->{logger}->error($mess);
                    carp "BestellInfoElement::" . $mess . "\n";
                    ...
                    Koha::Exceptions::WrongParameter->throw(
                        error => sprintf(
                            "Es wurde kein acquisition_import Exemplar-Datensatz mit lmsBestellCode '%s' und lmsExemplarID '%s' im Prozeß-Status 'prepared' gefunden. Abbruch der Verarbeitung der gesamten ekz BestellInfo.\n",
                            $reqLmsBestellCode, $reqLmsExemplarID
                        ),
                    );
                    ...
                    while ( my $acquisitionImportItemHit = $acquisitionImportItemRS->next() ) {
                        ...
                        $self->{logger}->debug( "handleTitelBestellInfo() acquisitionImportItemHit->{_column_data}:"
                                . Dumper( $acquisitionImportItemHit->{_column_data} )
                                . ":" );
                        ...
                        $self->{logger}->debug(
                            "handleTitelBestellInfo() acquisitionImportIdItem:" . $acquisitionImportIdItem . ":" );
                        ...
                        if ( !$itemImportObjectHit ) {
                            my $mess = sprintf(
                                "handleTitelBestellInfo(): The Koha item with acquisition_import_id '%s' has not been found. Processing of whole ekz BestellInfo denied.",
                            ...
                            $self->{emaillog}->{problems} .= "\n" if ( $self->{emaillog}->{problems} );
                            $self->{emaillog}->{problems} .=
                                "ERROR: Update der Exemplardaten für ekz Exemplar-ID: $ekzExemplarID (itemnumber:$itemnumber) wurde abgewiesen.\n";
                            $self->{emaillog}->{importresult} = -1;
                            $self->{emaillog}->{importerror}  = 1;

                            Koha::Exceptions::WrongParameter->throw(
                                error => sprintf(
                                    "Das Koha-Exemplar mit acquisition_import_id '%s' wurde nicht gefunden. Abbruch der Verarbeitung der gesamten ekz BestellInfo.",
                                    $acquisitionImportIdItem
                                ),
                            );
                        }
                        $self->{logger}->info( "handleTitelBestellInfo() itemImportObjectHit->{_column_data}:"
                                . Dumper( $itemImportObjectHit->{_column_data} )
                                . ":" );
                        ...
                        if ($item) {
                            ...
                            $self->{logger}->debug( "handleTitelBestellInfo() item->store() itemnumber:"
                                    . $itemnumber
                                    . ": gesamtpreis:"
                                    . $gesamtpreis
                            ...
                            $self->{updatedTitleRecords}->{$titleRecordBiblionumber}->{biblionumber} =
                                $titleRecordBiblionumber;
                        } else {
                            $self->{logger}->error(
                                "handleTitelBestellInfo() item not found for update of price and replacementprice! itemnumber:"
                                    . $itemnumber
                                    . ": gesamtpreis:"
                                    . $gesamtpreis
                                    . ": replacementcost_tax_included:"
                        ...
                        # positive message for log
                        $self->{emaillog}->{importresult} = 2
                            ; # -1: error   0:init value (undefined)   1: title inserted   2: title (or its items) updated
                        ...
                        push @{ $self->{emaillog}->{records} },
                            ...
                            $self->{emaillog}->{importresult}, $titeldata, $isbnean,     $self->{emaillog}->{problems},
                            $self->{emaillog}->{importerror},  2,          $ordernumber, $basketno
                            ];
                    }
                }
            } else {
                ...
                if (   length($zweigstellencode) == 0
                    && defined $self->{homebranch}
                    && length( $self->{homebranch} ) > 0 )
                {
                    $zweigstellencode = $self->{homebranch};
                    if ( !$self->{ekzKohaRecordClass}->checkbranchcode($zweigstellencode) ) {
                        $zweigstellencode = '';
                ...
                for ( my $j = 0 ; $j < $exemplar_anzahl ; $j++ ) {
                    ...
                    if ( $self->{ccode} ) {
                        $item_hash->{ccode} =
                            $self->{ccode};   # DKSH only; got from <auftragsnummer> via authorised_value_category CCODE
                    }
                    ...
                    $self->{logger}->info( "handleTitelBestellInfo() importedItemsCount:"
                            . $self->{emaillog}->{importedItemsCount}
                            . ": set next importId:"
                            . $importId
                            . ":" );
                    ...
                    if ( defined $itemnumber && $itemnumber > 0 ) {
                        ...
                        if ( defined( $self->{ekzWebServicesSetItemSubfieldsWhenOrdered} )
                            && length( $self->{ekzWebServicesSetItemSubfieldsWhenOrdered} ) > 0 )
                        {
                            ...
                            if (@affects) {
                                ...
                                if ($item) {
                                    ...
                                    C4::Items::ModItemFromMarc(
                                        ...
                                        ; # $self->{updatedTitleRecords}->{$titleRecordBiblionumber} has already been set a few lines ago
                                }
                            }
                        }
                        ...
                        $self->{logger}->debug( "handleTitelBestellInfo() acquisitionImportItemRS->{_column_data}:"
                        ...
                        $self->{logger}->debug( "handleTitelBestellInfo() itemImportObjectRS->{_column_data}:"
                    ...
                    push @{ $self->{emaillog}->{records} },
                        [
                        $reqParamTitelInfo->{'ekzArtikelNr'},
                        defined $biblionumber ? $biblionumber : "no biblionumber", $self->{emaillog}->{importresult},
                        $titeldata,   $isbnean, $self->{emaillog}->{problems}, $self->{emaillog}->{importerror}, 2,
                        $ordernumber, $basketno
                        ];
    ...
    push @actionresult,
        [
        'insertRecords',                          0, "X", "Y", $self->{emaillog}->{processedTitlesCount},
        $self->{emaillog}->{importedTitlesCount}, $self->{emaillog}->{updatedTitlesCount},
        $self->{emaillog}->{processedItemsCount}, $self->{emaillog}->{importedItemsCount},
        $self->{emaillog}->{updatedItemsCount},   $self->{emaillog}->{records}
        ];
