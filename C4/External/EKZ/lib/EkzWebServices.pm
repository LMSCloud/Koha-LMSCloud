...
use constant EKZWSURL =>
    'https://medienwelten.ekz.de/epmw-2.0/services/BestellsystemService';    # explicitly not a system preference
...
sub new {
    ...
    $self->{'ekzKundenNrWSMD'} =
        defined( $configWsMedienDaten{'ekzKundenNr'} ) ? $configWsMedienDaten{'ekzKundenNr'} : 'UNDEFINED';
    $self->{'passwortWSMD'} =
        defined( $configWsMedienDaten{'passwort'} ) ? $configWsMedienDaten{'passwort'} : 'UNDEFINED';
    $self->{'lmsNutzerWSMD'} =
        defined( $configWsMedienDaten{'lmsNutzer'} ) ? $configWsMedienDaten{'lmsNutzer'} : 'UNDEFINED';
...
sub getEkzKundenNr {
    ...
    $self->{'logger'}->trace( "getEkzKundenNr(ekzCustomerNumber:"
...
sub getEkzPasswort {
    ...
    $self->{'logger'}->trace( "getEkzPasswort(ekzCustomerNumber:"
            . ( defined($ekzCustomerNumber) ? $ekzCustomerNumber : 'undef' )
            . ") returns ret:$ret:" );
    return $ret;
}
...
sub getEkzLmsNutzer {
    ...
    $self->{'logger'}->trace( "getEkzLmsNutzer(ekzCustomerNumber:"
            . ( defined($ekzCustomerNumber) ? $ekzCustomerNumber : 'undef' )
            . ") returns ret:$ret:" );
    return $ret;
}
...
sub getEkzProcessingNoticesEmailAddress {
    ...
    $self->{'logger'}->trace( "getEkzProcessingNoticesEmailAddress(ekzCustomerNumber:"
            . ( defined($ekzCustomerNumber) ? $ekzCustomerNumber : 'undef' )
            . ") returns ret:$ret:" );
    return $ret;
}
...
sub getEkzWebServicesDefaultBranch {
    ...
    $self->{'logger'}->trace( "getEkzWebServicesDefaultBranch(ekzCustomerNumber:"
            . ( defined($ekzCustomerNumber) ? $ekzCustomerNumber : 'undef' )
            . ") returns ret:$ret:" );
    return $ret;
}
...
sub getEkzAqbooksellersId {
    ...
    $self->{'logger'}->trace( "getEkzAqbooksellersId(ekzCustomerNumber:"
            . ( defined($ekzCustomerNumber) ? $ekzCustomerNumber : 'undef' )
            . ") returns ret:$ret:" );
    return $ret;
}
...
sub callWsMedienDaten {
    ...
    $self->{'logger'}->debug(
        "callWsMedienDaten() START ekzArtikelNr:" . ( defined($ekzArtikelNr) ? $ekzArtikelNr : 'undef' ) . ":" );

    my $xmlwriter =
        XML::Writer->new( OUTPUT => 'self', NEWLINES => 0, DATA_MODE => 1, DATA_INDENT => 2, ENCODING => 'utf-8' );
    ...
    $xmlwriter->startTag(
        'wsse:UsernameToken',
        'wsu:Id' => 'UsernameToken-3d1e2053-4b6d-41c0-bb25-ba7ab39ce6dc'
    );    # it seems that we can use a constant non varying UUID here
    ...
    if ( $soapResponse->is_success ) {
        ...
        foreach my $titelnode ( $titelnodes->get_nodelist() ) {
            ...
            foreach my $child ( $titelnode->childNodes() ) {
                ...
                if ( $child->nodeName eq 'ekzArtikelNr' ) {
                    if ( $child->textContent eq $ekzArtikelNr ) {
                        ...
                        $self->{'logger'}
                            ->trace( "callWsMedienDaten() Dumper(datenSatzNodes):" . Dumper($datenSatzNodes) . ":" );

                        my $datenSatzNode = $datenSatzNodes->[0];
                        if ( defined $datenSatzNode && defined $datenSatzNode->textContent ) {
                            my $marc21XmlData =
                                decode_base64( $datenSatzNode->textContent );    # this is the MARC record 'collection'
                            ...
                            if ( defined($marc21XmlData) && length($marc21XmlData) > 0 ) {
                                ...
                                $self->{'logger'}
                                    ->trace( "callWsMedienDaten() collectionRoot:" . Dumper($collectionRoot) . ":" );
                                ...
                                $self->{'logger'}->trace(
                                    "callWsMedienDaten() collectionRecords:" . Dumper($collectionRecords) . ":" );

                                foreach my $collectionRecordnode ( $collectionRecords->get_nodelist() ) {
                                    $self->{'logger'}->trace( "callWsMedienDaten() collectionRecordnode->nodeName:"
                                            . $collectionRecordnode->nodeName
                                            . ":" );
                                    foreach my $recordChild ( $collectionRecordnode->childNodes() ) {
                                        $self->{'logger'}->trace( "callWsMedienDaten() recordChild->nodeName:"
                                                . $recordChild->nodeName
                                                . ":" );
                                    }
                                    if ( $collectionRecordnode->nodeName eq 'record' ) {
                                        $self->{'logger'}->trace( "callWsMedienDaten() collectionRecordnode:"
                                                . Dumper($collectionRecordnode)
                                                . ":" );
                                        my $marc21XmlRecord = $collectionRecordnode->toString;
                                        $self->{'logger'}
                                            ->debug( "callWsMedienDaten() marc21XmlRecord:" . $marc21XmlRecord . ":" );
                                        if ( defined($marc21XmlRecord) && length($marc21XmlRecord) > 0 ) {
                                            my $marcrecord;
                                            eval {
                                                $marcrecord =
                                                    MARC::Record::new_from_xml( $marc21XmlRecord, "utf8", 'MARC21' );
                                            };
                                            if ($@) {
                                                my $mess =
                                                    "error in MARC::Record::new_from_xml:$@:\nmarc21XmlData:$marc21XmlRecord";
                                            ...
                                            if ($marcrecord) {
                                                ...
                                                $self->{'logger'}->trace(
                                                    "callWsMedienDaten() Dumper(result->{'records'}->[$result->{'count'}]):"
                                                        . Dumper( $result->{'records'}->[ $result->{'count'} ] )
                                                        . ":" );
                                                $result->{'count'} += 1;
                                            }
                                        }
            ...
            if ( $result->{'count'} > 0 )
            {    #  $result may contain more than 1 record, e.g. in case of titles of a serial or continuation
                last;
            }
        }
    }
...
sub callWsStoList {
    ...
    $self->{'logger'}->info( "callWsStoList() START ekzCustomerNumber:"
            . ( defined($ekzCustomerNumber) ? $ekzCustomerNumber : 'undef' )
            . ": selJahr:"
            . ( defined($selJahr) ? $selJahr : 'undef' )
            . ": selStoId:"
            . ( defined($selStoId) ? $selStoId : 'undef' )
            . ": selMitTitel:"
            . ( defined($selMitTitel) ? $selMitTitel : 'undef' )
            . ": selMitKostenstellen:"
            . ( defined($selMitKostenstellen) ? $selMitKostenstellen : 'undef' )
            . ": selMitEAN:"
            . ( defined($selMitEAN) ? $selMitEAN : 'undef' )
            . ": selStatusUpdate:"
            . ( defined($selStatusUpdate) ? $selStatusUpdate : 'undef' )
            . ": selErweitert:"
            . ( defined($selErweitert) ? $selErweitert : 'undef' )
            . ": selMitReferenznummer:"
            . ( defined($selMitReferenznummer) ? $selMitReferenznummer : 'undef' )
            . ":" );
    ...
    if ( defined $selStoId && length($selStoId) > 0 ) {
        $xmlwriter->dataElement( 'stoID' => $selStoId )
            ; # <!--Optional. Wird eine StoID übergeben, werden die Infos nur für diese Sto übermittelt, ansonsten für alle StOs des angefragten Jahres-->
    }
    if ( defined $selMitTitel && length($selMitTitel) > 0 ) {
        $xmlwriter->dataElement( 'mitTitel' => $selMitTitel )
            ; # <!--Wird hier true übergeben, dann werden die Titel in der Antwort Nachricht mit übermittelt. Bei false werden nur die StO Varianten geliefert (für eine übersicht und gezielte Abfragen)-->
    }
    if ( defined $selMitKostenstellen && length($selMitKostenstellen) > 0 ) {
        $xmlwriter->dataElement( 'mitKostenstellen' => $selMitKostenstellen )
            ;    # <!-- entscheidet, ob die Kostenstellen zu den Titeln geliefert werden sollen, OPTIONAL →
    }
    if ( defined $selMitEAN && length($selMitEAN) > 0 ) {
        $xmlwriter->dataElement( 'mitEAN' => $selMitEAN )
            ;    # <!-- entscheidet, ob die ggf. vorhandene EAN zu den Titeln geliefert werden soll OPTIONAL -->
    }
    if ( defined $selStatusUpdate && length($selStatusUpdate) > 0 ) {
        $xmlwriter->dataElement( 'statusUpdate' => $selStatusUpdate )
            ;    # <!-- liefert nur Titel, deren Status sich seit dem dd.mm.yyyy geändert haben, OPTIONAL -->
    }
    if ( defined $selErweitert && length($selErweitert) > 0 ) {
        $xmlwriter->dataElement( 'erweitert' => $selErweitert )
            ;    # <!—Steuerung, ob statusdatum und anzahl in Antwort geliefert wird, OPTIONAL -->
    }
    if ( defined $selMitReferenznummer && length($selMitReferenznummer) > 0 ) {
        $xmlwriter->dataElement( 'mitReferenznummer' => $selMitReferenznummer )
            ; # <!-- entscheidet, ob die Referenznummern (und deren Exemplaranzahl) zu den Titeln geliefert werden sollen, OPTIONAL -->
    }
    ...
    if ( $soapResponse->is_success ) {
        ...
        foreach my $stoNode ( $stoNodes->get_nodelist() ) {
                        ...
                        if ( $titelChild->nodeName eq 'kostenstelle' ) {    # may be sent multiple times
                            ...
                            foreach my $referenznummerChild ( $titelChild->childNodes() ) {
                                if ( $referenznummerChild->nodeName !~ /^#/ ) {
                                    $referenznummerRecord->{ $referenznummerChild->nodeName } =
                                        $referenznummerChild->textContent;
                                }
                            }
                            if ( !exists( $titelRecord->{ $titelChild->nodeName } ) ) {
                                $titelRecord->{ $titelChild->nodeName } = [];
            ...
            $self->{'logger'}->trace( "callWsStoList() Dumper(result->{'standingOrderRecords'}->[i]):"
                    . Dumper( $result->{'standingOrderRecords'}->[ $result->{'standingOrderCount'} - 1 ] )
                    . ":" );
        }
    }
...
sub callWsFortsetzungList {
    ...
    my $result = {
        'fortsetzungStatusCount'   => 0,
        'fortsetzungStatusRecords' => {}
    };

    $self->{'logger'}->info( "callWsFortsetzungList() START ekzCustomerNumber:"
            . ( defined($ekzCustomerNumber) ? $ekzCustomerNumber : 'undef' )
            . ": selVon:"
    ...
    $self->{'logger'}
        ->debug( "callWsFortsetzungList() soapResponse->is_success:" . Dumper( $soapResponse->is_success ) . ":" );

    if ( $soapResponse->is_success ) {
        $self->{'logger'}->debug(
            "callWsFortsetzungList() Dumper(soapResponse->content):" . Dumper( $soapResponse->content ) . ":" );
        my $parser = XML::LibXML->new;
        my $dom    = $parser->parse_string( $soapResponse->content );
        ...
        $self->{'logger'}->debug(
            "callWsFortsetzungList() Dumper(fortsetzungStatusNodes):" . Dumper($fortsetzungStatusNodes) . ":" );
        ...
        foreach my $fortsetzungStatusNode ( $fortsetzungStatusNodes->get_nodelist() ) {
            $self->{'logger'}->trace(
                "callWsFortsetzungList() fortsetzungStatusNode->nodeName:" . $fortsetzungStatusNode->nodeName . ":" );
            my $fortsetzungStatusRecord = ();
            foreach my $fortsetzungStatusChild ( $fortsetzungStatusNode->childNodes() )
            {    # <status>, <fortsetzungVariante>
                $self->{'logger'}->trace( "callWsFortsetzungList() fortsetzungStatusChild->nodeName:"
                        . $fortsetzungStatusChild->nodeName
                        . ":" );
                ...
                if ( $fortsetzungStatusChild->nodeName !~ /^#/ ) {
                    if ( $fortsetzungStatusChild->nodeName eq 'status' ) {
                        $fortsetzungStatusRecord->{ $fortsetzungStatusChild->nodeName } =
                            $fortsetzungStatusChild->textContent;
                    } elsif ( $fortsetzungStatusChild->nodeName eq 'fortsetzungVariante' ) {
                        my $fortsetzungVarianteRecord = ();
                        foreach my $fortsetzungVarianteChild ( $fortsetzungStatusChild->childNodes() )
                        {    # <artikelArt>, <fortsetzungRubrik>
                            $self->{'logger'}->trace( "callWsFortsetzungList() fortsetzungVarianteChild->nodeName:"
                                    . $fortsetzungVarianteChild->nodeName
                                    . ":" );
                            $self->{'logger'}
                                ->trace( "callWsFortsetzungList() ref(fortsetzungVarianteChild->textContent):"
                                    . ref( $fortsetzungVarianteChild->textContent )
                                    . ":" );
                            if ( $fortsetzungVarianteChild->nodeName eq 'artikelArt' ) {
                                $fortsetzungVarianteRecord->{ $fortsetzungVarianteChild->nodeName } =
                                    $fortsetzungVarianteChild->textContent;
                            } elsif ( $fortsetzungVarianteChild->nodeName eq 'fortsetzungRubrik' ) {
                                my $fortsetzungRubrikRecord = ();
                                foreach my $fortsetzungRubrikChild ( $fortsetzungVarianteChild->childNodes() )
                                {    # <rubrik>, <fortsetzungTitel>
                                    $self->{'logger'}
                                        ->trace( "callWsFortsetzungList() fortsetzungRubrikChild->nodeName:"
                                            . $fortsetzungRubrikChild->nodeName
                                            . ":" );
                                    if ( $fortsetzungRubrikChild->nodeName eq 'rubrik' ) {
                                        $fortsetzungRubrikRecord->{ $fortsetzungRubrikChild->nodeName } =
                                            $fortsetzungRubrikChild->textContent;
                                    } elsif ( $fortsetzungRubrikChild->nodeName eq 'fortsetzungTitel' ) {
                                        my $fortsetzungTitelRecord = ();
                                        foreach my $fortsetzungTitelChild ( $fortsetzungRubrikChild->childNodes() )
                                        {    # <artikelnum>, <artikelname> usw.
                                            $self->{'logger'}
                                                ->trace( "callWsFortsetzungList() fortsetzungTitelChild->nodeName:"
                                                    . $fortsetzungTitelChild->nodeName
                                                    . ":" );
                                            $fortsetzungTitelRecord->{ $fortsetzungTitelChild->nodeName } =
                                                $fortsetzungTitelChild->textContent;
                                        }
                                        push @{ $fortsetzungRubrikRecord->{ $fortsetzungRubrikChild->nodeName } },
                                            $fortsetzungTitelRecord;
                                    }
                                }
                                push @{ $fortsetzungVarianteRecord->{ $fortsetzungVarianteChild->nodeName } },
                                    $fortsetzungRubrikRecord;
                            }
                        }
                        push @{ $fortsetzungStatusRecord->{ $fortsetzungStatusChild->nodeName } },
                            $fortsetzungVarianteRecord;
                    }
                }
            }
            $result->{'fortsetzungStatusRecords'}->{ $fortsetzungStatusRecord->{'status'} } =
                $fortsetzungStatusRecord;    # 'inProgress', 'canceled', 'finished'
            $result->{'fortsetzungStatusCount'} += 1;
            $self->{'logger'}->trace(
                "callWsFortsetzungList() Dumper(result->{'fortsetzungStatusRecords'}->{$fortsetzungStatusRecord->{'status'}}):"
                    . Dumper( $result->{'fortsetzungStatusRecords'}->{ $fortsetzungStatusRecord->{'status'} } )
                    . ":" );
        }
    }
...
sub callWsFortsetzungDetail {
    ...
    my $selMitReferenznummer          = shift;    # optional
    my $refFortsetzungDetailElement   = shift
        ; # for storing the read FortsetzungDetailResponseElement of the SOAP response body in DB table acquisition_import
    ...
    $self->{'logger'}->info( "callWsFortsetzungDetail() START ekzCustomerNumber:"
            . ( defined($ekzCustomerNumber) ? $ekzCustomerNumber : 'undef' )
            . ": selFortsetzungsId:"
            . ( defined($selFortsetzungsId) ? $selFortsetzungsId : 'undef' )
            . ": selBearbeitungsGruppe:"
            . ( defined($selBearbeitungsGruppe) ? $selBearbeitungsGruppe : 'undef' )
            . ": selBearbeitungsNummer:"
            . ( defined($selBearbeitungsNummer) ? $selBearbeitungsNummer : 'undef' )
            . ": selFortsetzungsAuftragsNummer:"
            . ( defined($selFortsetzungsAuftragsNummer) ? $selFortsetzungsAuftragsNummer : 'undef' )
            . ": selMitReferenznummer:"
            . ( defined($selMitReferenznummer) ? $selMitReferenznummer : 'undef' )
            . ":" );
    ...
    if ( defined $selMitReferenznummer && length($selMitReferenznummer) > 0 ) {
        $xmlwriter->dataElement( 'mitReferenznummer' => $selMitReferenznummer )
            ; # optional (Entscheidet, ob die Referenznummern (und deren Exemplaranzahl) zu den Titeln geliefert werden sollen.)
    }
    ...
    $self->{'logger'}->trace( "callWsFortsetzungDetail() Dumper(\$refFortsetzungDetailElement):"
            . Dumper($refFortsetzungDetailElement)
            . ":" );
    ...
    if ( $soapResponse->is_success ) {
        ...
        foreach my $responseNode ( $responseNodes->get_nodelist() ) {
            $self->{'logger'}
                ->trace( "callWsFortsetzungDetail() responseNode->nodeName:" . $responseNode->nodeName . ":" );
            if ( $responseNode->nodeName eq 'fortsetzungDetailStatus' ) {
                my $fortsetzungDetailStatusRecord = ();
                foreach my $fortsetzungDetailStatusChild ( $responseNode->childNodes() )
                {    # <status>, <jahresSummen>, <erfuellungsGrad>, <detail>
                    $self->{'logger'}->trace( "callWsFortsetzungDetail() fortsetzungDetailStatusChild->nodeName:"
                            . $fortsetzungDetailStatusChild->nodeName
                            . ":" );
                    ...
                    if ( $fortsetzungDetailStatusChild->nodeName !~ /^#/ ) {
                        if ( $fortsetzungDetailStatusChild->nodeName eq 'jahresSummen' ) {
                            ...
                            foreach my $jahresSummenChild ( $fortsetzungDetailStatusChild->childNodes() ) {
                                ...
                                $self->{'logger'}->trace( "callWsFortsetzungDetail() jahresSummenChild->nodeName:"
                                        . $jahresSummenChild->nodeName
                                        . ": jahresSummenChild->textContent:"
                                        . $jahresSummenChild->textContent
                                        . ":" );
                                $jahresSummenRecord->{ $jahresSummenChild->nodeName } = $jahresSummenChild->textContent;
                            }
                            push @{ $fortsetzungDetailStatusRecord->{ $fortsetzungDetailStatusChild->nodeName } },
                                $jahresSummenRecord;
                            ...
                            foreach my $detailChild ( $fortsetzungDetailStatusChild->childNodes() ) {
                                ...
                                $self->{'logger'}->trace( "callWsFortsetzungDetail() detailChild->nodeName:"
                                        . $detailChild->nodeName
                                        . ": detailChild->textContent:"
                                        . $detailChild->textContent
                                        . ":" );
                                if ( $detailChild->nodeName eq 'referenznummer' ) {
                                    ...
                                    foreach my $referenznummerChild ( $detailChild->childNodes() ) {
                                        ...
                                        if ( $referenznummerChild->nodeName !~ /^#/ ) {
                                            $self->{'logger'}
                                                ->trace( "callWsFortsetzungDetail() referenznummerChild->nodeName:"
                                                    . $referenznummerChild->nodeName
                                                    . ": referenznummerChild->textContent:"
                                                    . $referenznummerChild->textContent
                                                    . ":" );
                                            $referenznummerRecord->{ $referenznummerChild->nodeName } =
                            ...
                            $fortsetzungDetailStatusRecord->{ $fortsetzungDetailStatusChild->nodeName } =
                                $fortsetzungDetailStatusChild->textContent;
                        }
                    }
                }
                $fortsetzungRecord->{fortsetzungDetailStatusRecords}->{ $fortsetzungDetailStatusRecord->{status} }
                    ->{ $responseNode->nodeName } = $fortsetzungDetailStatusRecord;
        ...
        $self->{'logger'}->trace( "callWsFortsetzungDetail() Dumper(result->{'fortsetzungRecords'}->[i]):"
                . Dumper( $result->{'fortsetzungRecords'}->[ $result->{'fortsetzungCount'} - 1 ] )
                . ":" );
    }
...
sub callWsLieferscheinList {
    ...
    $self->{'logger'}->info( "callWsLieferscheinList() START ekzCustomerNumber:"
            . ( defined($ekzCustomerNumber) ? $ekzCustomerNumber : 'undef' )
            . ": selVon:"
            . ( defined($selVon) ? $selVon : 'undef' )
            . ": selBis:"
    ...
    if ( $soapResponse->is_success ) {
        ...
        $self->{'logger'}
            ->trace( "callWsLieferscheinList() Dumper(lieferscheinNodes):" . Dumper($lieferscheinNodes) . ":" );

        foreach my $lieferscheinNode ( $lieferscheinNodes->get_nodelist() ) {
            $self->{'logger'}
                ->trace( "callWsLieferscheinList() lieferscheinNode->nodeName:" . $lieferscheinNode->nodeName . ":" );
            my $lieferscheinRecord = ();
            foreach my $lieferscheinChild ( $lieferscheinNode->childNodes() )
            {    # <id> <nummer> <datum> sind hier interessant
                $self->{'logger'}->trace(
                    "callWsLieferscheinList() lieferscheinChild->nodeName:" . $lieferscheinChild->nodeName . ":" );
            ...
            $self->{'logger'}->trace( "callWsLieferscheinList() Dumper(result->{'lieferscheinRecords'}->[i]):"
                    . Dumper( $result->{'lieferscheinRecords'}->[ $result->{'lieferscheinCount'} - 1 ] )
                    . ":" );
        }
    }
...
sub callWsLieferscheinDetail {
    ...
    my $refLieferscheinDetailElement = shift
        ; # for storing the read LieferscheinDetailResponseElement of the SOAP response body in DB table acquisition_import
    ...
    $self->{'logger'}->info( "callWsLieferscheinDetail() START ekzCustomerNumber:"
            . ( defined($ekzCustomerNumber) ? $ekzCustomerNumber : 'undef' )
            . ": selId:"
    ...
    $self->{'logger'}->trace( "callWsLieferscheinDetail() Dumper(\$refLieferscheinDetailElement):"
    ...
    if ( $soapResponse->is_success ) {
        ...
        $self->{'logger'}
            ->trace( "callWsLieferscheinDetail() Dumper(lieferscheinNodes):" . Dumper($lieferscheinNodes) . ":" );

        foreach my $lieferscheinNode ( $lieferscheinNodes->get_nodelist() ) {
            $self->{'logger'}
                ->trace( "callWsLieferscheinDetail() lieferscheinNode->nodeName:" . $lieferscheinNode->nodeName . ":" );
            my $lieferscheinRecord = { 'teilLieferungCount' => 0, 'teilLieferungRecords' => [] };
            foreach my $lieferscheinChild ( $lieferscheinNode->childNodes() )
            {    # <id> <nummer> <datum> <teilLieferung> are of interest
                $self->{'logger'}->trace(
                    "callWsLieferscheinDetail() lieferscheinChild->nodeName:" . $lieferscheinChild->nodeName . ":" );
                ...
                if ( $lieferscheinChild->nodeName eq 'teilLieferung' ) {
                        ...
                        if ( $teilLieferungChild->nodeName eq 'auftragsPosition' ) {
                            ...
                            foreach my $auftragsPositionChild ( $teilLieferungChild->childNodes() ) {
                                if ( $auftragsPositionChild->nodeName !~ /^#/ ) {
                                    $auftragsPositionRecord->{ $auftragsPositionChild->nodeName } =
                                        $auftragsPositionChild->textContent;
                                }
                            }
                            ...
                            if ( $teilLieferungChild->nodeName !~ /^#/ ) {
                                $teilLieferungRecord->{ $teilLieferungChild->nodeName } =
                                    $teilLieferungChild->textContent;
                            }
                        }
                    }
                    ...
                    foreach my $rechnungsAnschriftChild ( $lieferscheinChild->childNodes() ) {
                        if ( $rechnungsAnschriftChild->nodeName !~ /^#/ ) {
                            $rechnungsAnschriftRecord->{ $rechnungsAnschriftChild->nodeName } =
                                $rechnungsAnschriftChild->textContent;
                        }
                    }
            ...
            push @{ $result->{'lieferscheinRecords'} }, $lieferscheinRecord;
            $result->{'lieferscheinCount'} += 1;
            $self->{'logger'}->trace( "callWsLieferscheinDetail() Dumper(result->{'lieferscheinRecords'}->[i]):"
                    . Dumper( $result->{'lieferscheinRecords'}->[ $result->{'lieferscheinCount'} - 1 ] )
                    . ":" );
        }
    }
...
sub callWsRechnungList {
    ...
    $self->{'logger'}->info( "callWsRechnungList() START ekzCustomerNumber:"
            . ( defined($ekzCustomerNumber) ? $ekzCustomerNumber : 'undef' )
            . ": selVon:"
            . ( defined($selVon) ? $selVon : 'undef' )
            . ": selBis:"
    ...
    if ( $soapResponse->is_success ) {
        ...
        foreach my $rechnungNode ( $rechnungNodes->get_nodelist() ) {
            ...
            foreach my $rechnungChild ( $rechnungNode->childNodes() ) {    # <id> <nummer> <datum> sind hier interessant
                $self->{'logger'}
                    ->trace( "callWsRechnungList() rechnungChild->nodeName:" . $rechnungChild->nodeName . ":" );
            ...
            $self->{'logger'}->trace( "callWsRechnungList() Dumper(result->{'rechnungRecords'}->[i]):"
                    . Dumper( $result->{'rechnungRecords'}->[ $result->{'rechnungCount'} - 1 ] )
                    . ":" );
        }
    }
...
sub callWsRechnungDetail {
    ...
    $self->{'logger'}->info( "callWsRechnungDetail() START ekzCustomerNumber:"
            . ( defined($ekzCustomerNumber) ? $ekzCustomerNumber : 'undef' )
            . ": selId:"
            . ( defined($selId) ? $selId : 'undef' )
            . ": selRechnungsnummer:"
    ...
    $self->{'logger'}->trace(
        "callWsRechnungDetail() Dumper(\$refRechnungDetailElement):" . Dumper($refRechnungDetailElement) . ":" );
    ...
    if ( $soapResponse->is_success ) {
        ...
        foreach my $rechnungNode ( $rechnungNodes->get_nodelist() ) {    # regularly there is only one rechnung node
            $self->{'logger'}
                ->trace( "callWsRechnungDetail() rechnungNode->nodeName:" . $rechnungNode->nodeName . ":" );
            my $rechnungRecord = { 'auftragsPositionCount' => 0, 'auftragsPositionRecords' => [] };
            foreach my $rechnungChild ( $rechnungNode->childNodes() )
            {    # <id> <nummer> <datum> <auftragsPosition> are of interest
                $self->{'logger'}
                    ->trace( "callWsRechnungDetail() rechnungChild->nodeName:" . $rechnungChild->nodeName . ":" );
            ...
            push @{ $result->{'rechnungRecords'} }, $rechnungRecord;
            $result->{'rechnungCount'} += 1;
            $self->{'logger'}->trace( "callWsRechnungDetail() Dumper(result->{'rechnungRecords'}->[i]):"
                    . Dumper( $result->{'rechnungRecords'}->[ $result->{'rechnungCount'} - 1 ] )
                    . ":" );
        }
    }
...
sub callWsBestellung {
    ...
    my $splitOperationMode = shift
        ; # if 0: build request and call webservice;   if 1: build request if ! preparedRequest , call webservice if preparedRequest
    ...
    $self->{'logger'}->info( "callWsBestellung() START ekzCustomerNumber:"
            . ( defined($ekzCustomerNumber) ? $ekzCustomerNumber : 'undef' )
            . ": param:"
            . ( defined($param) ? Dumper($param) : 'undef' )
            . ": splitOperationMode:"
            . ( defined($splitOperationMode) ? $splitOperationMode : 'undef' )
            . ":" );
    $self->{'logger'}->trace( "callWsBestellung() START preparedRequest:"
            . ( defined($preparedRequest) ? Dumper($preparedRequest) : 'undef' )
            . ":" );
    ...
    if ( $splitOperationMode && $preparedRequest ) {
        ...
        $xmlwriter->dataElement( 'wsse:Username' => 'bob' );
        $xmlwriter->dataElement( 'wsse:Password', 'bobPW', 'Type' =>
        ...
        if ( $param->{rechnungsEmpfaenger} ) {
            ...
            $xmlwriter->startTag('rechnungsEmpfaenger')
                ;    # not required (ekz believes that it is useless except for subfield ekzKundenNr)
            $xmlwriter->dataElement( 'ekzKundenNr' => $param->{rechnungsEmpfaenger}->{ekzKundenNr} )
                if $param->{rechnungsEmpfaenger}->{ekzKundenNr};    # not required
            $xmlwriter->dataElement( 'lmsKundenNr' => $param->{rechnungsEmpfaenger}->{lmsKundenNr} )
                if $param->{rechnungsEmpfaenger}->{lmsKundenNr};    # not required
            if ( $param->{rechnungsEmpfaenger}->{adresseElement} ) {
                $xmlwriter->startTag('adresseElement');                             # not required
                $xmlwriter->dataElement( 'name1' => $param->{rechnungsEmpfaenger}->{adresseElement}->{name1} )
                    if $param->{rechnungsEmpfaenger}->{adresseElement}->{name1};    # formally not required
                $xmlwriter->dataElement( 'name2' => $param->{rechnungsEmpfaenger}->{adresseElement}->{name2} )
                    if $param->{rechnungsEmpfaenger}->{adresseElement}->{name2};    # formally not required
                $xmlwriter->dataElement( 'name3' => $param->{rechnungsEmpfaenger}->{adresseElement}->{name3} )
                    if $param->{rechnungsEmpfaenger}->{adresseElement}->{name3};    # formally not required
                $xmlwriter->dataElement( 'strasse' => $param->{rechnungsEmpfaenger}->{adresseElement}->{strasse} )
                    if $param->{rechnungsEmpfaenger}->{adresseElement}->{strasse};    # formally not required
                $xmlwriter->dataElement( 'ort' => $param->{rechnungsEmpfaenger}->{adresseElement}->{ort} )
                    if $param->{rechnungsEmpfaenger}->{adresseElement}->{ort};        # formally not required
                $xmlwriter->dataElement( 'plz' => $param->{rechnungsEmpfaenger}->{adresseElement}->{plz} )
                    if $param->{rechnungsEmpfaenger}->{adresseElement}->{plz};        # formally not required
                $xmlwriter->dataElement( 'land' => $param->{rechnungsEmpfaenger}->{adresseElement}->{land} )
                    if $param->{rechnungsEmpfaenger}->{adresseElement}->{land};       # formally not required
        ...
        if ( $param->{rechnungsKopieEmpfaenger} ) {
            ...
            $xmlwriter->startTag('rechnungsKopieEmpfaenger')
                ;    # not required (ekz believes that it is useless except for subfield ekzKundenNr)
            $xmlwriter->dataElement( 'ekzKundenNr' => $param->{rechnungsKopieEmpfaenger}->{ekzKundenNr} )
                if $param->{rechnungsKopieEmpfaenger}->{ekzKundenNr};    # not required
            $xmlwriter->dataElement( 'lmsKundenNr' => $param->{rechnungsKopieEmpfaenger}->{lmsKundenNr} )
                if $param->{rechnungsKopieEmpfaenger}->{lmsKundenNr};    # not required
            if ( $param->{rechnungsKopieEmpfaenger}->{adresseElement} ) {
                $xmlwriter->startTag('adresseElement');                                  # not required
                $xmlwriter->dataElement( 'name1' => $param->{rechnungsKopieEmpfaenger}->{adresseElement}->{name1} )
                    if $param->{rechnungsKopieEmpfaenger}->{adresseElement}->{name1};    # formally not required
                $xmlwriter->dataElement( 'name2' => $param->{rechnungsKopieEmpfaenger}->{adresseElement}->{name2} )
                    if $param->{rechnungsKopieEmpfaenger}->{adresseElement}->{name2};    # formally not required
                $xmlwriter->dataElement( 'name3' => $param->{rechnungsKopieEmpfaenger}->{adresseElement}->{name3} )
                    if $param->{rechnungsKopieEmpfaenger}->{adresseElement}->{name3};    # formally not required
                $xmlwriter->dataElement( 'strasse' => $param->{rechnungsKopieEmpfaenger}->{adresseElement}->{strasse} )
                    if $param->{rechnungsKopieEmpfaenger}->{adresseElement}->{strasse};    # formally not required
                $xmlwriter->dataElement( 'ort' => $param->{rechnungsKopieEmpfaenger}->{adresseElement}->{ort} )
                    if $param->{rechnungsKopieEmpfaenger}->{adresseElement}->{ort};        # formally not required
                $xmlwriter->dataElement( 'plz' => $param->{rechnungsKopieEmpfaenger}->{adresseElement}->{plz} )
                    if $param->{rechnungsKopieEmpfaenger}->{adresseElement}->{plz};        # formally not required
                $xmlwriter->dataElement( 'land' => $param->{rechnungsKopieEmpfaenger}->{adresseElement}->{land} )
                    if $param->{rechnungsKopieEmpfaenger}->{adresseElement}->{land};       # formally not required
                $xmlwriter->endTag('adresseElement');                                      # not required
            }
            $xmlwriter->endTag('rechnungsKopieEmpfaenger');                                # not required
        }

        $xmlwriter->dataElement( 'kundenBestellNotiz' => $param->{kundenBestellNotiz} )
            if $param->{kundenBestellNotiz};                                               # not required
        $xmlwriter->dataElement( 'rfidDatenModell' => $param->{rfidDatenModell} )
            if $param->{rfidDatenModell};                                                  # not required
        $xmlwriter->dataElement( 'auftragsnummer' => $param->{auftragsnummer} )
            if $param->{auftragsnummer};                                                   # not required
        ...
        for ( my $iTitel = 0 ; $iTitel < scalar @{ $param->{titel} } ; $iTitel += 1 ) {
            ...
            $xmlwriter->dataElement(
                'ekzArtikelArt' => $param->{titel}->[$iTitel]->{titelangabe}->{titelInfo}->{ekzArtikelArt} )
                if $param->{titel}->[$iTitel]->{titelangabe}->{titelInfo}->{ekzArtikelArt};
            $xmlwriter->dataElement( 'author' => $param->{titel}->[$iTitel]->{titelangabe}->{titelInfo}->{author} )
                if $param->{titel}->[$iTitel]->{titelangabe}->{titelInfo}->{author};
            $xmlwriter->dataElement( 'titel' => $param->{titel}->[$iTitel]->{titelangabe}->{titelInfo}->{titel} )
                if $param->{titel}->[$iTitel]->{titelangabe}->{titelInfo}->{titel};
            $xmlwriter->dataElement( 'isbn13' => $param->{titel}->[$iTitel]->{titelangabe}->{titelInfo}->{isbn13} )
                if $param->{titel}->[$iTitel]->{titelangabe}->{titelInfo}->{isbn13};
            $xmlwriter->dataElement( 'verlag' => $param->{titel}->[$iTitel]->{titelangabe}->{titelInfo}->{verlag} )
                if $param->{titel}->[$iTitel]->{titelangabe}->{titelInfo}->{verlag};
            $xmlwriter->dataElement(
                'erscheinungsJahr' => $param->{titel}->[$iTitel]->{titelangabe}->{titelInfo}->{erscheinungsJahr} )
                if $param->{titel}->[$iTitel]->{titelangabe}->{titelInfo}->{erscheinungsJahr};
            $xmlwriter->endTag('titelInfo');
            $xmlwriter->endTag('titelangabe');

            $xmlwriter->dataElement(
                'dublettenCheckAusgefuehrt' => $param->{titel}->[$iTitel]->{dublettenCheckAusgefuehrt}
                ? $param->{titel}->[$iTitel]->{dublettenCheckAusgefuehrt}
                : 'true'
            );
            $xmlwriter->dataElement(
                'datensatz' => $param->{titel}->[$iTitel]->{datensatz}
                ? $param->{titel}->[$iTitel]->{datensatz}
                : 'false'
            );
            $xmlwriter->dataElement(
                'datensatzOnly' => $param->{titel}->[$iTitel]->{datensatzOnly}
                ? $param->{titel}->[$iTitel]->{datensatzOnly}
                : 'false'
            );
            ...
            for ( my $iEx = 0 ; $iEx < scalar @{ $param->{titel}->[$iTitel]->{exemplar} } ; $iEx += 1 ) {
                ...
                $xmlwriter->dataElement(
                    'lmsExemplarID' => $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{lmsExemplarID} )
                    if $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{lmsExemplarID};
                $xmlwriter->startTag('konfiguration');
                $xmlwriter->dataElement(
                    'anzahl' => $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{anzahl} + 0 );

                if ( $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{besteller} ) {
                    $xmlwriter->dataElement(
                        'ekzKundenNr' => $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{besteller} )
                        ;    # not required
                }
                ...
                if ( $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{warenEmpfaenger} ) {
                    ...
                    $xmlwriter->startTag('warenEmpfaenger')
                        ;    # not required (ekz believes that it is useless except for subfield ekzKundenNr)
                    $xmlwriter->dataElement( 'ekzKundenNr' =>
                            $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{warenEmpfaenger}
                            ->{ekzKundenNr} )
                        if $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{warenEmpfaenger}
                        ->{ekzKundenNr};    # not required
                    $xmlwriter->dataElement( 'lmsKundenNr' =>
                            $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{warenEmpfaenger}
                            ->{lmsKundenNr} )
                        if $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{warenEmpfaenger}
                        ->{lmsKundenNr};    # not required
                    if ( $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{warenEmpfaenger}
                        ->{adresseElement} )
                    {
                        $xmlwriter->startTag('adresseElement');    # not required
                        $xmlwriter->dataElement( 'name1' =>
                                $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{warenEmpfaenger}
                                ->{adresseElement}->{name1} )
                            if $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{warenEmpfaenger}
                            ->{adresseElement}->{name1};           # formally not required
                        $xmlwriter->dataElement( 'name2' =>
                                $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{warenEmpfaenger}
                                ->{adresseElement}->{name2} )
                            if $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{warenEmpfaenger}
                            ->{adresseElement}->{name2};           # formally not required
                        $xmlwriter->dataElement( 'name3' =>
                                $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{warenEmpfaenger}
                                ->{adresseElement}->{name3} )
                            if $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{warenEmpfaenger}
                            ->{adresseElement}->{name3};           # formally not required
                        $xmlwriter->dataElement( 'strasse' =>
                                $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{warenEmpfaenger}
                                ->{adresseElement}->{strasse} )
                            if $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{warenEmpfaenger}
                            ->{adresseElement}->{strasse};         # formally not required
                        $xmlwriter->dataElement(
                            'ort' => $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{warenEmpfaenger}
                                ->{adresseElement}->{ort} )
                            if $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{warenEmpfaenger}
                            ->{adresseElement}->{ort};             # formally not required
                        $xmlwriter->dataElement(
                            'plz' => $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{warenEmpfaenger}
                                ->{adresseElement}->{plz} )
                            if $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{warenEmpfaenger}
                            ->{adresseElement}->{plz};             # formally not required
                        $xmlwriter->dataElement(
                            'land' => $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{warenEmpfaenger}
                                ->{adresseElement}->{land} )
                            if $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{warenEmpfaenger}
                            ->{adresseElement}->{land};            # formally not required
                        $xmlwriter->endTag('adresseElement');      # not required
                    }
                ...
                for (
                    my $iBudget = 0 ;
                    $iBudget < scalar @{ $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{budget} } ;
                    $iBudget += 1
                    )
                {
                    $xmlwriter->startTag('budget');
                    $xmlwriter->dataElement( 'anteil' =>
                            $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{budget}->[$iBudget]
                            ->{anteil} );
                    $xmlwriter->dataElement( 'haushaltsstelle' =>
                            $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{budget}->[$iBudget]
                            ->{haushaltsstelle} );
                    $xmlwriter->dataElement( 'kostenstelle' =>
                            $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{budget}->[$iBudget]
                            ->{kostenstelle} );
                ...
                $xmlwriter->dataElement(
                    'rabatt' => $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{preis}->{rabatt} );
                $xmlwriter->dataElement(
                    'fracht' => sprintf(
                        "%.2f", $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{preis}->{fracht}
                    )
                );    # also required, may be 0.00 of course
                $xmlwriter->dataElement(
                    'einband' => sprintf(
                        "%.2f", $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{preis}->{einband}
                    )
                );    # also required, may be 0.00 of course
                $xmlwriter->dataElement(
                    'bearbeitung' => sprintf(
                        "%.2f", $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{preis}->{bearbeitung}
                    )
                );    # also required, may be 0.00 of course
                $xmlwriter->dataElement(
                    'ustSatz' => $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{preis}->{ustSatz} );
                $xmlwriter->dataElement(
                    'ust' => sprintf(
                        "%.2f", $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{preis}->{ust}
                    )
                );
                $xmlwriter->dataElement(
                    'gesamtpreis' => sprintf(
                        "%.2f", $param->{titel}->[$iTitel]->{exemplar}->[$iEx]->{konfiguration}->{preis}->{gesamtpreis}
                    )
                );
    ...
    if ( !$splitOperationMode || $preparedRequest ) {
        ...
        if ( $soapResponse->is_success ) {
            ...
            $self->{'logger'}
                ->trace( "callWsBestellung() Dumper(ekzBestellNrNodes):" . Dumper($ekzBestellNrNodes) . ":" );
    ...
    $self->{'logger'}->info( "callWsBestellung() result->{statusCode}:"
            . $result->{'statusCode'}
            . ": ->{statusMessage}:"
...
sub doQuery {
    ...
    my $soapEnvelopeAsOctets = Encode::encode( 'UTF-8', $soapEnvelope, Encode::FB_CROAK )
        ; # 'encode' required for avoiding error: HTTP::Message content must be bytes at /usr/share/perl5/HTTP/Request/Common.pm line 94.

    my $soapResponse = $self->{'ua'}->post(
        $self->{'url'}, 'Content-Type' => 'text/xml; charset="utf-8"', 'SOAPAction' => $soapAction,
        Content => $soapEnvelopeAsOctets
    );
...
sub genZeitstempel {
    ...
    my $tstime = sprintf( "%02d:%02d:%02d.%03d", $hour,        $min,     $sec, ( $t - int($t) ) * 1000 )
        ;    # calculate milli seconds without rounding
...
sub genTransactionId {
    ...
    my $transactionID =
        sprintf( "%d%s%06d", $timeOfDay->[0], $decimalSeparator, $timeOfDay->[1] );    # seconds.microseconds
...
sub getLastRunDate {
    ...
    if ( length($ekzWSLastRunDateSysPrefName) > 0 ) {
        ...
        $logger->trace( "getLastRunDate() ekzWSName:"
                . ( defined($ekzWSName) ? $ekzWSName : 'undef' )
                . " ekzWSLastRunDateSysPrefName:"
                . ( defined($ekzWSLastRunDateSysPrefName) ? $ekzWSLastRunDateSysPrefName : 'undef' )
                . " ekzWsLastRunDate:"
                . ( defined($ekzWsLastRunDate) ? $ekzWsLastRunDate : 'undef' )
                . ":" );
        if (   defined($ekzWsLastRunDate)
            && length($ekzWsLastRunDate) > 0
            && $ekzWsLastRunDate !~ /^\d\d\d\d-\d\d-\d\d$/ )
        {
            my $mess =
                  "got invalid ekzWsLastRunDate value:"
                . $ekzWsLastRunDate
                . ": by ekzWSLastRunDateSysPrefName:"
                . $ekzWSLastRunDateSysPrefName . ":";
            $logger->warn("getLastRunDate() $mess");
            croak "EkzWebServices::getLastRunDate() " . $mess;
        }
        if ( defined($ekzWsLastRunDate) && length($ekzWsLastRunDate) > 0 && $dateForm eq 'E' )
        {    # transform it into european form dd.mm.yyyy
            $ekzWsLastRunDate =
                  substr( $ekzWsLastRunDate, 8, 2 ) . '.'
                . substr( $ekzWsLastRunDate, 5, 2 ) . '.'
                . substr( $ekzWsLastRunDate, 0, 4 );
        }
    }
...
sub setLastRunDate {
    ...
    if ( length($ekzWSLastRunDateSysPrefName) > 0 ) {
        ...
        C4::Context->set_preference(
            $ekzWSLastRunDateSysPrefName,                            $ekzWsLastRunDate->ymd,
            "Date of last execution of ekz web service $ekzWSName.", "Free"
        );    # store the date in american form yyyy-mm-dd
    }
}
