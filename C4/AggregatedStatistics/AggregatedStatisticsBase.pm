...
sub new {
    ...
    my $self  = {
        ...
        'statisticstypedesignation' =>
            'GeneralAggregatedStatisticsType',    # default dummy, has to be updated in derived classes
    ...
    print STDERR "AggregatedStatisticsBase::new statisticstype:", $self->{'statisticstype'},
        ": statisticstypedesignation:", $self->{'statisticstypedesignation'}, ": id:", $self->{'id'}, ": name:",
        $self->{'name'}, ": description:", $self->{'description'}, ": startdate:", $self->{'startdate'}, ": enddate:",
        $self->{'enddate'}, ": op:", $self->{'op'}, ":\n"
        if $debug;
...
sub add_form {
    my $self = shift;
    print STDERR
        "AggregatedStatisticsBase::add_form statisticstype:$self->{'statisticstype'}:  statisticstypedesignation:$self->{'statisticstypedesignation'}: name:$self->{'name'}:\n"
        if $debug;
...
sub readbyname {
    my $self = shift;
    print STDERR
        "AggregatedStatisticsBase::readbyname statisticstype:$self->{'statisticstype'}:  statisticstypedesignation:$self->{'statisticstypedesignation'}: name:$self->{'name'}:\n"
        if $debug;
    my $found = 0;
    ...
    if ( $self->{'name'} ) {
        ...
        print STDERR "AggregatedStatisticsBase::readbyname count aggregatedStatistics:",
...
sub add_validate {
    ...
    $self->{'startdateDB'}    = output_pref(
        { dt => dt_from_string( scalar $input->param('startdate') ), dateformat => 'iso', dateonly => 1 } );
    $self->{'enddateDB'} =
    ...
    print STDERR "AggregatedStatisticsBase::add_validate  op:", $self->{'op'}, ": statisticstype:",
        $self->{'statisticstype'}, ": name:", $self->{'name'}, ": id:", $self->{'id'}, ":\n"
        if $debug;
    ...
    my $aggregatedStatistics = C4::AggregatedStatistics::CreateAggregatedStatistics( \%param )
        ;    # updates a record if found via id or via type & name
...
sub delete {
    ...
    print STDERR "AggregatedStatisticsBase::delete statisticstype:", $self->{'statisticstype'}, ": name:",
...
sub getadditionalparameters {
    ...
    my $additionalparameters =
        {};    # for AggregatedStatisticsBase there are no additional aggregated_statistics_parameters
...
sub readAll {
    ...
    my $aggregatedStatistics =
        C4::AggregatedStatistics::GetAggregatedStatistics( { type => $self->{'statisticstype'} } );
...
sub readAggregatedStatisticsParametersValue {
    ...
    my $name = shift;
    print STDERR
        "C4::AggregatedStatistics::AggregatedStatisticsBase::readAggregatedStatisticsParametersValue Start self->id:",
        $self->{'id'}, ": name:$name:\n"
        if $debug;
    my %param;
    my $value;

    if ( defined( $self->{'id'} ) && length( $self->{'id'} ) > 0 )
    {    # this should always be the case / only for safety's sake
        if ( defined($name) ) {    # this should always be the case / only for safety's sake
            ...
            print STDERR
                "C4::AggregatedStatistics::AggregatedStatisticsBase::readAggregatedStatisticsParametersValue statistics_id:",
                $self->{'id'}, ": name:$name:   count aggregatedStatisticsParameters:",
...
sub saveAggregatedStatisticsValue {
    ...
    print STDERR "C4::AggregatedStatistics::AggregatedStatisticsBase::saveAggregatedStatisticsValue Start name:",
...
sub readAggregatedStatisticsValue {
    my $self = shift;
    my ( $name, $type, $hit ) = @_;
    print STDERR "C4::AggregatedStatistics::AggregatedStatisticsBase::readAggregatedStatisticsValue Start self->id:",
        $self->{'id'}, ": name:", $name, ":\n"
        if $debug;
    my %param;
    ...
    {    # this should always be the case / only for safety's sake
        if ( defined($name) ) {    # this should always be the case / only for safety's sake
            ...
            if ( defined( $aggregatedStatisticsValuesRS->_resultset()->first() ) ) {
                ...
                $param{'type'} =
                    defined($type) ? $type : $aggregatedStatisticsValuesRS->_resultset()->first()->get_column('type');
            }
        }
    }
    $$hit = \%param;
...
sub delAggregatedStatisticsValue {
    my $self = shift;
    my ($name) = @_;
    print STDERR "C4::AggregatedStatistics::AggregatedStatisticsBase::delAggregatedStatisticsValue Start self->id:",
        $self->{'id'}, ": name:", $name, ":\n"
        if $debug;
    my %param;
