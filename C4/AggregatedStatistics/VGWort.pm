...
sub add_form {
    ...
    ( $self->{'categories'}, $self->{'branchloop'}, $self->{'itemtypes'} ) =
        $self->read_categories_branches_and_itemtypes($itypemapping);
...
sub eval_form {
    ...
    if ( $aggregatedStatisticsId && $aggregatedStatisticsId =~ /^[0-9]+$/ ) {
        ...
        my $aggregatedStatisticsParameters =
            C4::AggregatedStatistics::GetAggregatedStatisticsParameters( { statistics_id => $aggregatedStatisticsId } );
        ...
        my $aggregatedStatistics =
            C4::AggregatedStatistics::GetAggregatedStatistics( { id => $aggregatedStatisticsId } );
        ...
        if ( $fromDate && $toDate ) {
            my $exporter =
                C4::External::VGWortExport->new( $fromDate, $toDate, $mediaTypeMapping, $libraryGroup, $library );
