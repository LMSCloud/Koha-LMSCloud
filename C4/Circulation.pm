...
sub transferbook {
    ...
    # will be assigned with the bookmobile branch if it is a bookmobile station
...
print STDERR "C4::Circulation::CanBookBeIssued barcode:$barcode: duedate:$duedate:\n";
...
print STDERR "C4::Circulation::AddIssue barcode:$barcode: datedue:$datedue: issuedate:$issuedate:\n";
...
print STDERR "C4::Circulation::AddIssue itisanillitem:$itisanillitem: barcode:$barcode: datedue:$datedue: issuedate:$issuedate: shippingBackRequired:$shippingBackRequired:\n";
            }
...
sub GetLoanLength {
    ...
    # if the branch is a bookmobile station, we apply rules of the
    # bookmobile branch before we using general rules
    my $mobilebranch = Koha::Libraries->get_effective_branch($branchcode);
    if ( $mobilebranch ne $branchcode ) {
        $sth->execute( $borrowertype, $itemtype, $mobilebranch );
        $loanlength = $sth->fetchrow_hashref;
        return $loanlength
          if defined($loanlength) && defined $loanlength->{issuelength};

        $sth->execute( $borrowertype, '*', $mobilebranch );
        $loanlength = $sth->fetchrow_hashref;
        return $loanlength
          if defined($loanlength) && defined $loanlength->{issuelength};

        $sth->execute( '*', $itemtype, $mobilebranch );
        $loanlength = $sth->fetchrow_hashref;
        return $loanlength
          if defined($loanlength) && defined $loanlength->{issuelength};

        $sth->execute( '*', '*', $mobilebranch );
        $loanlength = $sth->fetchrow_hashref;
        return $loanlength
          if defined($loanlength) && defined $loanlength->{issuelength};
    }
    ...
    if ( $mobilebranch ne $branchcode ) {
        push @attempts, (
            ['SELECT holdallowed, returnbranch, hold_fulfillment_policy
                FROM branch_item_rules
                WHERE branchcode = ?
                  AND itemtype = ?', $mobilebranch, $itemtype],
            ['SELECT holdallowed, returnbranch, hold_fulfillment_policy
                FROM default_branch_circ_rules
                WHERE branchcode = ?', $mobilebranch]
        );
    }
...
sub AddReturn {
    ...
    my $origbranch = $branch;
    
    # If the branch is is a bookmobile station, the book need to be returned to the bookmobile branch 
    # and not the station
    $branch = Koha::Libraries->get_effective_branch($branch);
    ...
    # check if it is an ILL item    TODO: needs rework, we will shift this action in the backend ship-back-method in the future
print STDERR "C4::Circulation::AddReturn barcode:$barcode: return_date:$return_date:\n";
    ...
    if ( $itisanillitem && $illrequest ) {
        ...
        if ( $res ne '1' || $error) {
            warn "ERROR when deleting ILL title $item->{'biblionumber'} ($error) or ILL item $item->{'itemnumber'} ($res)";
            print "Content-Type: text/html\n\n<html><body><h3>ERROR when deleting ILL title $item->{'biblionumber'} ($error) or ILL item $item->{'itemnumber'} ($res)</h3></body></html>";
	        exit;
        }
print STDERR "C4::Circulation::AddReturn itisanillitem:$itisanillitem: barcode:$barcode: return_date:$return_date: res:$res:\n";
    }
...
END_SQL
    ...
    foreach my $upcoming ( @$upcoming_dues ) {
        ...
        $upcoming->{branchemail} = $branchmail{$upcoming->{branchcode}} if (defined($branchmail{$upcoming->{branchcode}}));
    }
...
sub CanBookBeRenewed {
    ...
    $branchcode = _GetCircControlBranch( $item, $patron->unblessed ) if (! C4::Context->preference('UseIssuingBranchConditionsForRenewals') );
    my $issuing_rule = Koha::IssuingRules->get_effective_issuing_rule(
        {   categorycode => $patron->categorycode,
            itemtype     => $item->{itype},
            branchcode   => $branchcode
...
sub SetDueDateOfItems {
    ...
    my $today_iso = dt_from_string(output_pref( { dt => dt_from_string, dateonly => 1, dateformat => 'iso' }),'iso');
    ...
    $selparams->{categorycode} = $params->{libraryCategory} if ( defined($params->{libraryCategory}) && $params->{libraryCategory} ne '*' );
    $selparams->{branchcode} = $params->{branchcode} if ( defined($params->{branchcode}) && $params->{branchcode} ne '*' );
...
sub AddRenewal {
    ...
    unless ($datedue) {
        ...
        my $usebranch = $issue->branchcode;
        $usebranch = _GetCircControlBranch($item, $patron_unblessed) if (! C4::Context->preference('UseIssuingBranchConditionsForRenewals') );
        $datedue =  CalcDateDue($datedue, $itemtype, $usebranch, $patron_unblessed, 'is a renewal');
    }
...
sub GetSoonestRenewDate {
    ...
    if ( defined $issuing_rule and defined $issuing_rule->norenewalbefore
        and $issuing_rule->norenewalbefore ne "" )
    {
        my $soonestrenewal =
...
sub GetIssuingCharges {
    ...
    if ( my $item_data = $sth->fetchrow_hashref ) {
        ...
        my $discount_rules;
        # if the branch is a mobilebook station we do check whether there is
        # a rule specifically for the station
        # if not we continue thge check with the related mobilebook branch
        if ( $mobilebranch ne $branch ) {
            my $discount_query = q|SELECT rentaldiscount,
                ...
                AND issuingrules.branchcode = ?|;
            my $discount_sth = $dbh->prepare($discount_query);
            $discount_sth->execute( $borrowernumber, $item_type, $branch );
            $discount_rules = $discount_sth->fetchall_arrayref({});
            $branch = $mobilebranch;
        }
        
        if ( (! defined($discount_rules)) or (! @{$discount_rules}) ) {
            my $discount_query = q|SELECT rentaldiscount,
                issuingrules.itemtype, issuingrules.branchcode
                FROM borrowers
                LEFT JOIN issuingrules ON borrowers.categorycode = issuingrules.categorycode
        ...
        if ( defined($discount_rules) and @{$discount_rules} ) {
    ...
    my $accountline = Koha::Account::Line->new(
        {
            ...
            accounttype       => 'Rent',
            date              => \'NOW()',
            branchcode        => Koha::Libraries->get_effective_branch($checkout->branchcode),
        }
    )->store();
...
print STDERR "C4::Circulation::checkIfIllItem illItemtype:$illItemtype: item->{itype}:$item->{itype}:\n";
            if ( $illItemtype eq $item->{'itype'} ) {
                $itisanillitem = 1;
                last;
            }
        }
print STDERR "C4::Circulation::checkIfIllItem itisanillitem:$itisanillitem: item->itemnumber:$item->{itemnumber}: item->barcode:$item->{barcode}: item->{'biblionumber'}:$item->{'biblionumber'}:\n";
        if ( $itisanillitem ) {
            eval {
