...
sub getCurrencyFormatterData {
    ...
    return [
        $currency, currency_symbol($currency), decimal_precision($currency), decimal_separator($currency),
    ...
    foreach my $borrowernumber ( keys %borrowers ) {
        if ($borrowernumber) {
            ...
            if ( $borrowers{$borrowernumber}->{firstname} ) {
                $borrowers{$borrowernumber}->{fullname} =
...
sub getOpenedCashRegister {
    ...
    if ( !C4::Context->preference('PermitConcurrentCashRegisterUsers') ) {
        ...
        if ( my $row = $sth->fetchrow_hashref ) {
            my $cashreg = Koha::CashRegister::CashRegisterDefinitions->find( { id => $row->{id} } );
            $sth->finish();
            return $cashreg;
        }
        $sth->finish();
...
sub getOpenedCashRegisterByManagerID {
    ...
    if ( !C4::Context->preference('PermitConcurrentCashRegisterUsers') ) {
        my $cash_register = Koha::CashRegister::CashRegisterDefinitions->search( { manager_id => $loggedinuser } );
...
sub registerPayment {
    ...
    if ($cashreg) {
        return $self->addCashRegisterTransaction(
            $cashreg->id(), 'PAYMENT', $manager_id, '', $amount, '',
            $accountlines_no
        );
    }
...
sub registerReversePayment {
    ...
    if ($cashreg) {
        return $self->addCashRegisterTransaction(
            $cashreg->id(), 'REVERSE_PAYMENT', $manager_id, '', ( $amount * -1 ),
            '', $accountlines_no
        );
    }
...
sub registerCreditPayout {
    ...
    if ($cashreg) {
        return $self->addCashRegisterTransaction(
            $cashreg->id(), 'CREDIT_PAYOUT', $manager_id, '', ( $amount * -1 ),
            '', $accountlines_no
        );
    }
...
sub canCloseCashRegister {
    ...
    my $sth = $dbh->prepare($query);
    $sth->execute(
        $cash_register_id, $concurrentBookingsEnabled, $borrowerid, $concurrentBookingsEnabled,
        $borrowerid
    );
    my $result = false;
    if ( my $row = $sth->fetchrow_hashref ) {
...
sub getPermittedCashRegisters {
    my $self          = shift;
    my $loggedinuser  = shift;
    my $cash_register = Koha::CashRegister::CashRegisterManagers->search( { manager_id => $loggedinuser } );
...
sub loadCashRegister {
    ...
    my $cash_register = Koha::CashRegister::CashRegisterDefinitions->find( { id => $cash_register_id } );
    if ($cash_register) {
...
sub getAllCashRegisters {
    ...
    foreach my $cashreg ( Koha::CashRegister::CashRegisterDefinitions->search( {} )->as_list ) {
        ...
        foreach my $manager (@cash_register_managers) {
            if ( $cashreg->id() == $manager->cash_register_id()
                && exists( $self->{managers}->{ $manager->manager_id() } ) )
            {
...
sub saveCashRegister {
    ...
    if ($cash_register_id) {
        $cash_register = Koha::CashRegister::CashRegisterDefinitions->find( { id => $cash_register_id } );
        if ($cash_register) {
    ...
    if ($cash_register) {
        ...
        $cash_register = Koha::CashRegister::CashRegisterDefinitions->find( { id => $id } );

        if ($id) {
            my @saved_managers =
                Koha::CashRegister::CashRegisterManagers->search( { cash_register_id => $id } )->as_list;
...
sub closeCashRegister {
    ...
    if ($cashreg) {
        if ( ( $cashreg->manager_id() && $borrowerid == $cashreg->manager_id() )
            || C4::Context->preference('PermitConcurrentCashRegisterUsers') )
        {

            my $cashmans =
                Koha::CashRegister::CashRegisterManagers->search( { cash_register_id => $cash_register_id } );
...
sub getLastBooking {
    ...
    if ( $row = $sth->fetchrow_hashref ) {
        ...
        if ( $row->{manager_id} ) {
            ...
            if ($row_bor) {
                $row->{manager_name} =
                    length( $row_bor->{firstname} )
                    ? $row_bor->{firstname} . ' ' . $row_bor->{surname}
                    : $row_bor->{surname};
            }
...
sub getBookingsSinceLastOpening {
    ...
    my $query = q{
        SELECT  a.id, a.cash_register_account_id, a.cash_register_id, a.manager_id, a.booking_time, 
                ...
                l.credit_type_code, l.debit_type_code, l.payment_type, l.interface, l.note as accountlines_note, 
    ...
    while ( my $row = $sth->fetchrow_hashref ) {
        ...
        foreach my $borr (
            { id => 'manager_id',     desig_name => 'manager_name', desig_isdel => 'manager_is_deleted' },
            { id => 'borrowernumber', desig_name => 'patron_name',  desig_isdel => 'patron_is_deleted' }
            )
        {
            if ( $row->{ $borr->{id} } ) {
...
sub getLastBookingsFromTo {
    ...
    my $query = q{
        SELECT  a.id, a.cash_register_account_id, a.cash_register_id, a.manager_id, l.borrowernumber, a.booking_time, 
                a.accountlines_id, a.current_balance, a.action, a.booking_amount, a.description, a.reason, 
                l.credit_type_code, l.debit_type_code, l.payment_type, l.interface, l.note as accountlines_note, 
                l.description as accountlines_description, m.title as title, l.borrowernumber
        FROM  cash_register_account a
        LEFT JOIN accountlines l ON a.accountlines_id = l.accountlines_id
        LEFT JOIN items i ON i.itemnumber = l.itemnumber
        LEFT JOIN biblio m ON i.biblionumber = m.biblionumber
    ...
    return (
        \@result, output_pref( { dt => dt_from_string($date_from), dateformat => 'iso', dateonly => 0 } ),
        output_pref( { dt => dt_from_string($date_to), dateformat => 'iso', dateonly => 0 } )
    );
}
...
sub getCashRegisterPaymentAndDepositOverview {
    ...
    my $query = q{
        SELECT c.action, sum(c.booking_amount) as booking_amount, c.reason, l.credit_type_code
        ...
        GROUP BY c.action, c.reason, l.credit_type_code
       };
    $query =~ s/^\s+/ /mg;
    my $sth = $dbh->prepare($query);
    $sth->execute(
...
sub getFinesOverview {
    ...
    if ($branchcode) {
        $branchselect =
            ...
            . $dbh->quote($branchcode) . " )";
        $cashregisterselect =
              "AND (r.no_branch_restriction = 1 OR br.branchcode = "
            . $dbh->quote($branchcode)
            . " OR br.mobilebranch = "
            . $dbh->quote($branchcode) . " )";
    }
    ...
    # get fines overview by item type
    if ( $type eq 'finesoverview' ) {

        # query of paid fines
        # the 1st to the 3rd SELECT are used to search fines that are paid with a normal payment actions
        #     SELECT 1 joins items
        #     SELECT 2 joins deleteditems
        #     SELECT 3 finds accountlines whithout a match to items or deleteditems
        # the 4th to the 6th SELECT is used to search new payments of reversed payments (should be only availaible
        #     older accountlines older to version 18.11)
        #     This one is tricky because new payments may be partial payments
        #     SELECT 4 joins items
        #     SELECT 5 joins deleteditems
        #     SELECT 6 finds accountlines whithout a match to items or deleteditems
        # the 7th to the 9th SELECT is used to search reversed reversed payments so that they are payments again
        ...
        my $query = qq{
            ...
            WHERE      c.branchcode = br.branchcode
                   ...
                   AND ao.amount <> 0.00
                   AND ao.type = 'APPLY'
                   AND a.debit_type_code <> 'VOID'
            GROUP BY 
                   a.debit_type_code, c.credit_type_code, i.itype
            UNION ALL
            SELECT 
                   a.debit_type_code,
                   c.credit_type_code,
            ...
            WHERE      c.branchcode = br.branchcode
                   ...
                   AND a.debit_type_code <> 'VOID'
            GROUP BY 
                   a.debit_type_code, c.credit_type_code, itemtype
            UNION ALL
            SELECT 
                   a.debit_type_code,
                   c.credit_type_code,
            ...
            WHERE      c.branchcode = br.branchcode
                   ...
                   AND c.credit_type_code = 'PAYMENT'
            GROUP BY 
                   a.debit_type_code, c.credit_type_code
            UNION ALL
            SELECT 
                   a.debit_type_code,
                   c.credit_type_code,
            ...
            UNION ALL
            SELECT 
                   'PaymentOfPaidBackPayment' AS debit_type_code,
                   'PaymentOfPaidBackPayment' AS credit_type_code,
                   SUM(ao.amount) * -1 AS amount,
                   COUNT(*) AS count,
                   '' AS itemtype
            FROM   branches br, account_offsets o, account_offsets ao, accountlines c, accountlines u, accountlines a
            ...
            UNION ALL
            SELECT 
                   'PartialPaymentOfPaidBackPayment' AS debit_type_code,
                   'PartialPaymentOfPaidBackPayment' AS credit_type_code,
            ...
            UNION ALL
            SELECT 
                   'PartialPaymentReverseOfPaidBackPayment' as debit_type_code,
                   'PartialPaymentReverseOfPaidBackPayment' AS credit_type_code,
                   ao.amount AS amount,
                   1 AS count,
                   "" AS itemtype
        ...
        while ( my $row = $sth->fetchrow_hashref ) {
            ...
            if ( !exists( $result->{data}->{paid}->{$debit_type}->{ $row->{itemtype} } ) ) {
                $result->{data}->{paid}->{$debit_type}->{ $row->{itemtype} } = {
                    amount                 => $amount,
                    count                  => $row->{count},
                ...
                $result->{data}->{paid}->{$debit_type}->{ $row->{itemtype} }->{amount} += $amount;
                $result->{data}->{paid}->{$debit_type}->{ $row->{itemtype} }->{count}  += $row->{count};
                $result->{data}->{paid}->{$debit_type}->{ $row->{itemtype} }->{fines_amount} +=
                    sprintf( '%.2f', $result->{data}->{paid}->{$debit_type}->{ $row->{itemtype} }->{amount} );
                $result->{data}->{paid}->{$debit_type}->{ $row->{itemtype} }->{fines_amount_formatted} =
                    $self->formatAmountWithCurrency(
                    $result->{data}->{paid}->{$debit_type}->{ $row->{itemtype} }->{amount} );
            }

            # Add values to summary calculation grouped by account type
            if ( !exists( $result->{data}->{paidtype}->{$debit_type} ) ) {
                $result->{data}->{paidtype}->{$debit_type} = {
                    amount                 => $amount,
                    count                  => $row->{count},
                    itemtype               => $row->{itemtype},
                    itemtypedescription    => $itemtypes{ $row->{itemtype} },
                ...
                $result->{data}->{paidtype}->{$debit_type}->{amount} += $amount;
                $result->{data}->{paidtype}->{$debit_type}->{count}  += $row->{count};
                $result->{data}->{paidtype}->{$debit_type}->{fines_amount} +=
                    sprintf( '%.2f', $result->{data}->{paidtype}->{$debit_type}->{amount} );
                $result->{data}->{paidtype}->{$debit_type}->{fines_amount_formatted} =
                    $self->formatAmountWithCurrency( $result->{data}->{paidtype}->{$debit_type}->{amount} );
            }

            # Add values to summary calculation grouped by account if there exists an debit type to account mapping
            ...
            if ( !exists( $result->{data}->{account}->{$mapped}->{$account} ) ) {
                ...
                $result->{data}->{account}->{$mapped}->{$account}->{fines_amount} =
                    sprintf( '%.2f', $result->{data}->{account}->{$mapped}->{$account}->{amount} );
                $result->{data}->{account}->{$mapped}->{$account}->{fines_amount_formatted} =
                    $self->formatAmountWithCurrency( $result->{data}->{account}->{$mapped}->{$account}->{amount} );
            }

            if ( !exists( $result->{data}->{mapaccount}->{paid}->{$mapped}->{$account} ) ) {
                $result->{data}->{mapaccount}->{paid}->{$mapped}->{$account} =
                    { amount => 0.0, count => 0, '0.00', '0.00' };
            }
            $result->{data}->{mapaccount}->{paid}->{$mapped}->{$account}->{amount} += $amount;
            $result->{data}->{mapaccount}->{paid}->{$mapped}->{$account}->{count}  += $row->{count};
            $result->{data}->{mapaccount}->{paid}->{$mapped}->{$account}->{fines_amount} =
                sprintf( '%.2f', $result->{data}->{mapaccount}->{paid}->{$mapped}->{$account}->{amount} );
            $result->{data}->{mapaccount}->{paid}->{$mapped}->{$account}->{fines_amount_formatted} =
                $self->formatAmountWithCurrency(
                $result->{data}->{mapaccount}->{paid}->{$mapped}->{$account}->{amount} );
        ...
        $result->{sum}->{paid}->{fines_amount_formatted} =
            $self->formatAmountWithCurrency( $result->{sum}->{paid}->{amount} );

        $sth->finish;

        # Select reverse payments with 7 UNION selects
        # The first 4 are for backward compatibility with older versions of Koha where the Reverse Payment was possible as action
        #     SELECT 1 joins items (Koha version < 21.05)
        #     SELECT 2 joins deleteditems (Koha version < 21.05)
        #     SELECT 3 finds reversed payments whithout a match to items or deleteditems (Koha version < 21.05)
        #     SELECT 4 finds reversed payments of paid back payments (Koha version < 21.05)
        #     SELECT 5 REFUNDS joins items (Koha version >= 21.05)
        #     SELECT 6 REFUNDS joins deleteditems (Koha version >= 21.05)
        #     SELECT 7 REFUNDS whithout a match to items or deleteditems (Koha version >= 21.05)
        ...
        $query = qq{
            ...
            UNION ALL
            SELECT 
                   'ReversedPaymentOfPaidBackPayment' as debit_type_code,
                   'ReversedPaymentOfPaidBackPayment' AS credit_type_code,
                   SUM(o.amount) AS amount,
                   COUNT(*) AS count,
                   '' AS itemtype
            FROM   branches br, account_offsets o, account_offsets ao, accountlines c, accountlines a
            ...
            FROM   accountlines a
                   JOIN items AS i USING (itemnumber)
                   JOIN account_offsets ao ON (ao.debit_id = a.accountlines_id AND ao.type = 'APPLY' AND ao.amount <> 0.00)
                   ...
                   JOIN account_offsets aocredit ON (aocredit.credit_id = aovoid.credit_id AND aocredit.type = 'APPLY')
                   JOIN accountlines r ON (aocredit.debit_id = r.accountlines_id AND r.debit_type_code = 'VOID' AND r.status = 'REFUNDED')
                   JOIN account_offsets ro ON (ro.debit_id = r.accountlines_id AND ro.type = 'APPLY')
                   JOIN accountlines c ON (ro.credit_id = c.accountlines_id AND c.credit_type_code = 'REFUND')
                   JOIN account_offsets co ON (co.credit_id = c.accountlines_id AND co.debit_id IS NULL),
                   branches br
            WHERE      a.credit_type_code IS NULL
                   AND c.branchcode = br.branchcode
                   AND $dateselect $branchselect
            GROUP BY 
                   a.debit_type_code, c.credit_type_code, c.accountlines_id, i.itype
            UNION All
            SELECT 
                   a.debit_type_code,
                   c.credit_type_code,
            ...
            WHERE      a.credit_type_code IS NULL
                   ...
                   AND NOT EXISTS (SELECT 1 FROM deleteditems WHERE a.itemnumber = deleteditems.itemnumber)
            GROUP BY 
                   a.debit_type_code, c.credit_type_code, c.accountlines_id
           };
        $query =~ s/^\s+/ /mg;
        ...
        while ( my $row = $sth->fetchrow_hashref ) {
            ...
            # Add values to summary calculation grouped by account type and item type
            if ( !exists( $result->{data}->{reversed}->{$debit_type}->{ $row->{itemtype} } ) ) {
                $result->{data}->{reversed}->{$debit_type}->{ $row->{itemtype} } = {
                    amount                 => $amount,
                    count                  => $row->{count},
                    itemtype               => $row->{itemtype},
                    itemtypedescription    => $itemtypes{ $row->{itemtype} },
                ...
                $result->{data}->{reversed}->{$debit_type}->{ $row->{itemtype} }->{amount} += $amount;
                $result->{data}->{reversed}->{$debit_type}->{ $row->{itemtype} }->{count}  += $row->{count};
                $result->{data}->{reversed}->{$debit_type}->{ $row->{itemtype} }->{fines_amount} +=
                    sprintf( '%.2f', $result->{data}->{reversed}->{$debit_type}->{ $row->{itemtype} }->{amount} );
                $result->{data}->{reversed}->{$debit_type}->{ $row->{itemtype} }->{fines_amount_formatted} =
                    $self->formatAmountWithCurrency(
                    $result->{data}->{reversed}->{$debit_type}->{ $row->{itemtype} }->{amount} );
            }

            # Add values to summary calculation grouped by account type
            if ( !exists( $result->{data}->{reversedtype}->{$debit_type} ) ) {
                $result->{data}->{reversedtype}->{$debit_type} = {
                    amount                 => $amount,
                    count                  => $row->{count},
                    itemtype               => $row->{itemtype},
                    itemtypedescription    => $itemtypes{ $row->{itemtype} },
                ...
                $result->{data}->{reversedtype}->{$debit_type}->{amount} += $amount;
                $result->{data}->{reversedtype}->{$debit_type}->{count}  += $row->{count};
                $result->{data}->{reversedtype}->{$debit_type}->{fines_amount} +=
                    sprintf( '%.2f', $result->{data}->{reversedtype}->{$debit_type}->{amount} );
                $result->{data}->{reversedtype}->{$debit_type}->{fines_amount_formatted} =
                    $self->formatAmountWithCurrency( $result->{data}->{reversedtype}->{$debit_type}->{amount} );
            }
            ...
            if ( !exists( $result->{data}->{mapaccount}->{reversed}->{$mapped}->{$account} ) ) {
                $result->{data}->{mapaccount}->{reversed}->{$mapped}->{$account} =
                    { amount => 0.0, count => 0, '0.00', '0.00' };
            }
            ...
            $result->{data}->{mapaccount}->{reversed}->{$mapped}->{$account}->{fines_amount} =
                sprintf( '%.2f', $result->{data}->{mapaccount}->{reversed}->{$mapped}->{$account}->{amount} );
            $result->{data}->{mapaccount}->{reversed}->{$mapped}->{$account}->{fines_amount_formatted} =
                $self->formatAmountWithCurrency(
                $result->{data}->{mapaccount}->{reversed}->{$mapped}->{$account}->{amount} );
        ...
        $result->{sum}->{reversed}->{fines_amount_formatted} =
            $self->formatAmountWithCurrency( $result->{sum}->{reversed}->{amount} );
        ...
        $result->{sum}->{overall}->{fines_amount_formatted} =
            $self->formatAmountWithCurrency( $result->{sum}->{overall}->{amount} );

        $result->{sum}->{mapped}->{fines_amount} =
            sprintf( '%.2f', exists( $result->{sum}->{mapped}->{amount} ) ? $result->{sum}->{mapped}->{amount} : 0.0 );
        $result->{sum}->{mapped}->{fines_amount_formatted} = $self->formatAmountWithCurrency(
            exists( $result->{sum}->{mapped}->{amount} ) ? $result->{sum}->{mapped}->{amount} : 0.0 );

        $result->{sum}->{unmapped}->{fines_amount} = sprintf(
            '%.2f',
            exists( $result->{sum}->{unmapped}->{amount} ) ? $result->{sum}->{unmapped}->{amount} : 0.0
        );
        $result->{sum}->{unmapped}->{fines_amount_formatted} = $self->formatAmountWithCurrency(
            exists( $result->{sum}->{unmapped}->{amount} ) ? $result->{sum}->{unmapped}->{amount} : 0.0 );
        ...
        $result->{sum}->{fines}->{fines_amount_formatted} =
            $self->formatAmountWithCurrency( $result->{sum}->{fines}->{amount} );

        $query = qq{
            SELECT c.action AS action,
                   c.reason AS reason,
                   l.credit_type_code AS credit_type_code,
        ...
        while ( my $row = $sth->fetchrow_hashref ) {
            ...
            $credit_type_code = $credit_types{$credit_type_code} if ( exists( $credit_types{$credit_type_code} ) );

            $reason = $authValuesAll->{$action}->{$reason} if ( exists( $authValuesAll->{$action}->{$reason} ) );
            if ( !exists( $result->{data}->{cashregister}->{$cashreg}->{$action}->{$reason}->{$credit_type_code} ) ) {
                $result->{data}->{cashregister}->{$cashreg}->{$action}->{$reason}->{$credit_type_code} = {
                    amount                 => $amount,
                    count                  => $row->{count},
                ...
                $result->{data}->{cashregister}->{$cashreg}->{$action}->{$reason}->{$credit_type_code}->{amount} +=
                    $amount;
                $result->{data}->{cashregister}->{$cashreg}->{$action}->{$reason}->{$credit_type_code}->{count} +=
                    $row->{count};
                $result->{data}->{cashregister}->{$cashreg}->{$action}->{$reason}->{$credit_type_code}->{fines_amount}
                    += sprintf(
                    '%.2f',
                    $result->{data}->{cashregister}->{$cashreg}->{$action}->{$reason}->{$credit_type_code}->{amount}
                    );
                $result->{data}->{cashregister}->{$cashreg}->{$action}->{$reason}->{$credit_type_code}
                    ->{fines_amount_formatted} = $self->formatAmountWithCurrency(
                    $result->{data}->{cashregister}->{$cashreg}->{$action}->{$reason}->{$credit_type_code}->{amount} );
            }
        ...
        # Get a summary of payments by payment type: 'Pay', 'Pay00', 'Pay01', 'Pay02', 'Pay03'
        # in order to calculate cash and card payments
        $query = qq{
            SELECT 
                   IFNULL(a.payment_type,'CASH') AS payment_type,
                   SUM(ao.amount) AS amount,
                   COUNT(*) AS count
            ...
            WHERE      a.branchcode = br.branchcode
                   ...
                   AND ao.debit_id = c.accountlines_id
                   AND c.debit_type_code <> 'VOID'
                   AND a.credit_type_code = 'PAYMENT'
                   AND ao.amount <> 0.00
        ...
        while ( my $row = $sth->fetchrow_hashref ) {
            ...
            $result->{sum}->{cashtype}->{$paytype}->{$what}->{amount} += $amount;
            $result->{sum}->{cashtype}->{$paytype}->{$what}->{count}  += $row->{count};
            $result->{sum}->{cashtype}->{$paytype}->{$what}->{bookings_found} = 1;
            $result->{sum}->{cashtype}->{$paytype}->{bookings_found} = 1;
        }
        $sth->finish;

        push( @ptypes, 'sum' );
        foreach my $type (@ptypes) {
            $result->{sum}->{cashtype}->{$type}->{payment}->{payment_amount} =
                sprintf( '%.2f', $result->{sum}->{cashtype}->{$type}->{payment}->{amount} || 0.0 );
            $result->{sum}->{cashtype}->{$type}->{payment}->{payment_amount_formatted} =
                $self->formatAmountWithCurrency( $result->{sum}->{cashtype}->{$type}->{payment}->{amount} );
            $result->{sum}->{cashtype}->{$type}->{payout}->{payout_amount} =
                sprintf( '%.2f', $result->{sum}->{cashtype}->{$type}->{payout}->{amount} || 0.0 );
            $result->{sum}->{cashtype}->{$type}->{payout}->{payout_amount_formatted} =
                $self->formatAmountWithCurrency( $result->{sum}->{cashtype}->{$type}->{payout}->{amount} );
        }

        # Calculate account management fees grouped by borrowers town and borrower type
        # It uses 8 selects to calculate the sum by type and city
        #       SELECT 1 calculates account management fees of active patrons with debit_type_code 'ACCOUNT'
        #       SELECT 2 calculates account management fees of deleted patrons with debit_type_code 'ACCOUNT'
        my %accoutfeegroups = (
        ...
        foreach my $accoutfeegroup ( keys %accoutfeegroups ) {
            ...
            while ( my $row = $sth->fetchrow_hashref ) {
                ...
                $result->{data}->{accountfee}->{$accoutfeegroup}->{$paytype}->{$description}->{fines_amount} = sprintf(
                    '%.2f',
                    $result->{data}->{accountfee}->{$accoutfeegroup}->{$paytype}->{$description}->{amount}
                );
                $result->{data}->{accountfee}->{$accoutfeegroup}->{$paytype}->{$description}->{fines_amount_formatted}
                    = $self->formatAmountWithCurrency(
                    $result->{data}->{accountfee}->{$accoutfeegroup}->{$paytype}->{$description}->{amount} );
            ...
            $result->{sum}->{accountfee}->{$accoutfeegroup}->{fines_amount} =
                sprintf( '%.2f', $result->{sum}->{accountfee}->{$accoutfeegroup}->{amount} );
            $result->{sum}->{accountfee}->{$accoutfeegroup}->{fines_amount_formatted} =
                $self->formatAmountWithCurrency( $result->{sum}->{accountfee}->{$accoutfeegroup}->{amount} );
        }
        ...
        while ( my $row = $sth->fetchrow_hashref ) {
            ...
            $credit_type = $credit_types{$credit_type} if ( exists( $credit_types{$credit_type} ) );

            # read required record from borrowers or deletedborrowers for getting manager_name and patron_name
            foreach my $borr (
                {
                    id          => 'manager_id', desig_name => 'manager_name', desig_cardno => 'manager_cardnumber',
                    desig_isdel => 'manager_is_deleted'
                },
                {
                    id          => 'borrowernumber', desig_name => 'patron_name', desig_cardno => 'cardnumber',
                    desig_isdel => 'patron_is_deleted'
                }
                )
            {
                if ( $row->{ $borr->{id} } ) {
            ...
            $result->{data}->[ $rownum++ ] = {
                ...
                debit_type_code        => $row->{debit_type_code},
                credit_type_code       => $row->{debit_type_code},
                amount                 => $amount,
                fines_amount           => sprintf( '%.2f', $amount ),
                fines_amount_formatted => $self->formatAmountWithCurrency($amount),
        ...
        $sth_bor->finish();
        $sth->finish();
    } elsif ( $type eq 'paymentsbyday'
        || $type eq 'paymentsbymanager'
        || $type eq 'paymentsbytype'
        || $type eq 'payoutbytype' )
    {
        ...
        if ( $type ne 'payoutbytype' ) {
            $query = qq{
                 ...
                 WHERE      a.branchcode = br.branchcode
                        AND ao.type IN ('APPLY','REVERSED')
                        AND $dateselect $branchselect
                        AND c.debit_type_code <> 'VOID'
                        AND a.credit_type_code IN ('CREDIT','PAYMENT')
                 GROUP BY
                 ...
                 SELECT 
                        2 as entrytype,
                        DATE(ao.created_on) as date,
                        c.credit_type_code as credit_type_code,
                        SUM(ao.amount) as amount,
                        c.borrowernumber as borrowernumber,
                        c.description as description,
                        c.manager_id as manager_id,
                 ...
                 WHERE      a.branchcode = br.branchcode
                        ...
                        AND $dateselect $branchselect
                 GROUP BY
                        ao.created_on, c.credit_type_code, c.borrowernumber, c.description, c.manager_id 
                 UNION ALL } . $query;
        }
        ...
        while ( my $row = $sth->fetchrow_hashref ) {
            ...
            # check whether we can deliver the description of a reason from authorised
            # values if defined
            if ( defined( $row->{reason} )
                && exists( $authValuesAll->{ $row->{credit_type_code} }->{ $row->{reason} } ) )
            {
                $row->{reason} = $authValuesAll->{ $row->{credit_type_code} }->{ $row->{reason} };
            } elsif ( $row->{entrytype} == 2 ) {
                $credit_type = $credit_types{$credit_type} if ( exists( $credit_types{$credit_type} ) );
            }
        ...
        my $query = qq{
            ...
            WHERE   a.debit_type_code IS NOT NULL
                AND a.date >= ? and a.date <= ?
                AND a.debit_type_code NOT IN ('VOID','PAYOUT')
...
sub getCashTransactionOverviewByBranch {
    ...
    my $cash_register = Koha::CashRegister::CashRegisterDefinitions->search( { branchcode => $branchcode } );
    ...
    while ( my $row = $sth->fetchrow_hashref ) {
        ...
        my $type    = 'unassigned';
        if ( $paytype eq 'CASH' || $paytype eq 'SIP00' ) {
            $type = 'cash';
        } elsif ( $paytype eq 'SIP01' || $paytype eq 'SIP02' ) {
            $type = 'card';
        }
        my $what = 'payout';
        if ( $amount >= 0.00 ) {
    ...
    foreach my $type (@ptypes) {
        ...
        $result->{$type}->{payment}->{booking_amount_formatted} =
            $self->formatAmountWithCurrency( $result->{$type}->{payment}->{amount} );
        $result->{$type}->{payout}->{booking_amount} = sprintf( '%.2f', $result->{$type}->{payout}->{amount} || 0.0 );
        $result->{$type}->{payout}->{booking_amount_formatted} =
            $self->formatAmountWithCurrency( $result->{$type}->{payout}->{amount} );
    }
    $result->{sum}->{starting_balance}->{booking_amount} =
        sprintf( '%.2f', $result->{sum}->{starting_balance}->{amount} );
    $result->{sum}->{starting_balance}->{booking_amount_formatted} =
        $self->formatAmountWithCurrency( $result->{sum}->{starting_balance}->{amount} );
    $result->{sum}->{final_balance}->{booking_amount} = sprintf( '%.2f', $result->{sum}->{final_balance}->{amount} );
    $result->{sum}->{final_balance}->{booking_amount_formatted} =
        $self->formatAmountWithCurrency( $result->{sum}->{final_balance}->{amount} );
...
sub addCashRegisterTransaction {
    ...
    while ( $trials < 11 ) {
        ...
        my $params = {
            cash_register_id         => $cash_register_id,
            cash_register_account_id => $lastTransaction->{cash_register_account_id} +
                1,    # This may clash with a concurrent insert, but there are further attempts.
