...
sub checkin {
    ...
    if ($item) {
        ...
        if ( $checkinOpts && exists( $checkinOpts->{disabled_itypes_for_checkins} ) ) {
            if (   $checkinOpts->{disabled_itypes_for_checkins}
                && $item->itemtype
                && exists( $checkinOpts->{disabled_itypes_for_checkins}->{ $item->itemtype } ) )
            {
                $circ->alert(1);
                $circ->alert_type(91);
                $circ->ok(0);
        ...
        if ( $checkinOpts && exists( $checkinOpts->{disabled_ccodes_for_checkins} ) ) {
            if (   $checkinOpts->{disabled_ccodes_for_checkins}
                && defined( $item->{'collection_code'} )
                && exists( $checkinOpts->{disabled_ccodes_for_checkins}->{ $item->{'collection_code'} } ) )
            {
                $circ->alert(1);
                $circ->alert_type(93);
                $circ->ok(0);
        ...
        if ( $checkinOpts && $checkinOpts->{disable_checkins_with_holds} ) {
            ...
            if ( $pending && ref($pending) eq 'ARRAY' && scalar(@$pending) ) {
                my $bibitem_queue =
                    [ ( grep { !( defined $_->{itemnumber} and $_->{itemnumber} ne $item->{itemnumber} ) } @{$pending} )
                    ];
                if ( $bibitem_queue && ref($bibitem_queue) eq 'ARRAY' && scalar(@$bibitem_queue) ) {
                    $circ->alert(1);
                    $circ->alert_type(92);
                    $circ->ok(0);
...
sub fee_debit {
    my (
        $self,      $patron_id,   $patron_pwd, $fee_amount, $fee_account_id, $fee_type, $product_identifier, $fee_id,
        $prod_code, $fee_comment, $trans_id,   $currency
    ) = @_;
    ...
    $trans->charge(
        $patron->{borrowernumber}, $fee_amount, $fee_type, $product_identifier, $fee_id, $prod_code,
        $fee_comment
    );
