...
sub charge {
    ...
    my $authorisedValueSearch = Koha::AuthorisedValues->search(
        { category => "DEBIT_TYPE_SIP2_MAPPED" },
    ...
    } elsif ( exists( $feeTypesSIPMapped->{$prod_code} )
        && exists( $feeTypes->{ $feeTypesSIPMapped->{$prod_code}->[0] } ) )
    {
        ...
        if (   exists( $feeTypesSIPMapped->{$fee_type} )
            && exists( $feeTypes->{ $feeTypesSIPMapped->{$fee_type}->[0] } ) )
        {
            $debit_type  = $feeTypesSIPMapped->{$fee_type}->[0];
            $description = $feeTypesSIPMapped->{$fee_type}->[1];
        } elsif ( exists( $feeTypesSIPMapped->{ $fee_type . $product_identifier } )
            && exists( $feeTypes->{ $feeTypesSIPMapped->{ $fee_type . $product_identifier }->[0] } ) )
        {
