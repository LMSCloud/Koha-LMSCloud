    ...
    (FEE_DEBIT) => {
        ...
        protocol => {
            2 => {
                ...
                template_len => 25,
                fields       => [
                    (FID_FEE_AMT), (FID_FEE_ACCOUNT_ID), (FID_INST_ID), (FID_PATRON_ID), (FID_TERMINAL_PWD),
                    (FID_PATRON_PWD), (FID_PRODUCT_CODE), (FID_FEE_ID), (FID_FEE_COMMENT), (FID_TRANSACTION_ID)
                ],
            }
        }
    },
...
sub build_patron_status {
    ...
    if ($patron) {
        ...
        if (   C4::Context->preference('SIPVendorDialect')
            ...
            && $patron->dateexpiry )
        {
            $resp .= maybe_add( FID_EXPIRATION, $patron->dateexpiry . "    235900", $server );
        }
...
sub handle_fee_debit {
    ...
    $status = $ils->fee_debit(
        $patron_id, $patron_pwd, $fee_amount,  $fee_account_id, $fee_type, $product_identifier,
        $fee_id,    $prod_code,  $fee_comment, $trans_id,       $currency
    );
...
sub handle_item_information {
    ...
    if ( !defined($item) ) {
        ...
        if (   C4::Context->preference('SIPVendorDialect')
            && C4::Context->preference('SIPVendorDialect') eq 'EasyCheck' )
        {
            $resp .= maybe_add( FID_ITEM_PROPS, $item->issue_renewals, $server );
        } else {
            $resp .= maybe_add( FID_ITEM_PROPS, $item->sip_item_properties, $server );
        }
