...
sub requestItem {
    ...
    return (
        undef, 0, "Incomplete request data. Item order user id not provided: ItemId='$itemId' userId='$userId'",
        undef
    );
}
            ...
            foreach my $biblioItem ( @{$data} ) {
                if ( $divibibItem->{'ItemId'} eq $recordToDivibibId{ $biblioItem->{'biblionumber'} } ) {
                    ...
                    if ( $divibibItem->{'DatePlaced'} =~ /^(\d\d\d\d-\d\d-\d\d)T(\d\d:\d\d:\d\d)/ ) {
                        ...
                        my $mess =
                            "C4::Divibib::NCIPService::getPendingIssues(userid: $userId) got divibibItem->{'DatePlaced'}:"
                            . Dumper( $divibibItem->{'DatePlaced'} )
                    ...
                    if ( $divibibItem->{'DateDue'} =~ /^(\d\d\d\d-\d\d-\d\d)T(\d\d:\d\d:\d\d)/ ) {
                        ...
                        my $mess =
                            "C4::Divibib::NCIPService::getPendingIssues(userid: $userId) got divibibItem->{'DateDue'}:"
                            . Dumper( $divibibItem->{'DateDue'} )
...
sub _fetch_divibib_data {
    ...
    my $response = $self->{'ua'}->post(
        $self->{'url'}, Content_Type => 'application/xml', Accept => 'application/xml',
        Content => $cmd->getXML()
    );
    ...
    if ( $response->is_success ) {
        ...
        carp "C4::Divibib::NCIPService error calling command: "
            . $cmd->getXML()
            . "\nResult: "
    ...
    warn "could not retrieve " . $self->{'url'}
        unless ( $response->content && $response->is_success && $cmd->getResponseOk() );
    return $response;
}
