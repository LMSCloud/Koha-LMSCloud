...
sub GetNotClosedBasketsByBookseller {
    my ( $booksellerid, $extra ) = @_;
    my $query =
        "SELECT * FROM aqbasket WHERE booksellerid=? AND NOT EXISTS (SELECT 1 FROM aqbasketgroups WHERE aqbasket.basketgroupid = aqbasketgroups.id AND aqbasketgroups.closed=1)";
    if ($extra) {
        if ( $extra->{groupby} ) {
            $query .= " GROUP by $extra->{groupby}";
        }
