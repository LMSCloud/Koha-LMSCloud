...
## Please see file perltidy.ERR
        ...
        WRAP_LINES:
            ...
            $line =~ s/^\s+//g;    # strip naughty leading spaces
            ( $Tx, $Tw ) = text_alignment(
                $origin_llx, $self->{'width'}, $llx_text_attr * $self->{'unitvalue'}, $string_width,
                $line,       $text_attribs->{'text_alignment'}
            );
            push @lines, { line => $line, Tx => $Tx, Ty => $Ty, Tw => $Tw };
        }
        ...
        if ( ++$whileintext == 1000 ) {
            warn
                "Patroncard.pm => draw_text: Leaving outer while draw_text with whileintext ($whileintext): text is: @$text\n";
            last;
        }
    }
}
