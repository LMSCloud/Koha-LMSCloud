...
sub Z3950SearchGeneral {
    ...
    foreach my $server (@servers) {
        ...
        $oResult[$s] =
            ...
            : $oConnection[$s]->search( new ZOOM::Query::CQL( _translate_query( $server, $squery ) ) );
        $s++;
    }
    my $xslh = Koha::XSLT::Base->new;
            ...
            if ($error) {
                ...
                for ( $i = 0 ; $i < $numresults ; $i++ ) {
                    if ( $oResult[$k]->record($i) ) {
                        undef $error;
                        ( $marcrecord, $error ) =
                            _handle_one_result_marcrecord( $oResult[$k]->record($i), $servers[$k], $xslh );
                        if ($marcrecord) {
                            $$result->{'count'} += 1;
                        ...
                        push @errconn,
                            {
                            'server' => $servers[$k]->{servername}, error => ( ( $oConnection[$k]->error_x() )[0] ),
                            seq      => $i + 1
                            };
                    }
                }    # for $i < $numresults
