...
sub ModItemFromMarc {
    ...
    # to prevent it being blanked by set_or_blank
    $item->{onloan}           = $item_object->onloan if ( $item_object->onloan && !defined $item->{onloan} );
    $item->{datelastborrowed} = $item_object->datelastborrowed
        if ( $item_object->datelastborrowed && !defined $item->{datelastborrowed} );
    $item->{datelastseen} = $item_object->datelastseen
        if ( $item_object->datelastseen && !defined $item->{datelastseen} );
    ...
    if ( exists( $filter->{conjunction} ) ) {
        ...
        if ( ( 0 < grep { $_ eq $field } @columns ) or ( substr( $field, 0, 5 ) eq 'marc:' ) ) {
            ...
            if ( ref $query eq 'ARRAY' ) {
                ...
                if ( defined( $filter->{'handleNullLikeValue'} ) )
                { # $filter->{'handleNullLikeValue'} specifies that a items record where items.$column IS NULL should be treated like a items record where items.$column = $filter->{'handleNullLikeValue'} (e.g. 0, or '')
                    ...
                    {
                        $where_fragment = {
                            str => "NOT ( NOT ( $column $op ?) AND ( $column IS NOT NULL ) )"
                            ,  # more efficient than the equivalent and intuitive "( $column $op ? OR $column IS NULL )"
                            args => [$query],
                        };
                    } else {
                        $where_fragment = {
