...
sub searchResults {
    ...
    if ( defined( $search_context->{'view'} ) && $search_context->{'view'} =~ /^(opac|intranet)volume$/ ) {
        $search_context->{'interface'} = $1;
        ...
        foreach my $field (@fields) {
            ...
            # items not on loan, but still unavailable ( lost, withdrawn, damaged )
                ...
                # item is available
                    ...
                    $available_items->{$prefix}->{itemnumbers} .= '|'
                        if ( exists( $available_items->{$prefix}->{itemnumbers} ) );
            ...
            $oldbiblio->{XSLTResultsRecord} = XSLTParse4Display(
                {
                    ...
                    record         => $marcrecord,
                    xsl_syspref    => ( $is_opac ? 'OPAC' : '' ) . $sysprefname,
                    fix_amps       => 1,
