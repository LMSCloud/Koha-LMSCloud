...
<xsl:stylesheet version="1.0"
  ...
  exclude-result-prefixes="marc str">
    ...
    <xsl:template match="marc:record">
        ...
        <xsl:variable name="IncludeAdditionalMARCFields" select="marc:sysprefs/marc:syspref[@name='IncludeAdditionalMARCFieldsInStaffDetailView']"/>
    ...
    <xsl:call-template name="show-series">
        ...
        <xsl:if test="marc:datafield[@tag=72]">
            <span class="results_summary interest_group">
            ...
            <xsl:for-each select="marc:datafield[@tag=773]">
                <xsl:if test="@ind1 !=1">
                    ...
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            <a>
                                <xsl:attribute name="href">/cgi-bin/koha/catalogue/search.pl?q=ti,phr:<xsl:value-of select="str:encode-uri(translate(marc:subfield[@code='t'], '()', ''), true())"/><xsl:if test="marc:subfield[@code='a']">+AND+name:<xsl:value-of select="str:encode-uri(translate(marc:subfield[@code='a'], '()', ''), true())"/></xsl:if></xsl:attribute>
                                <xsl:value-of select="$f773"/>
                            </a>
                        </xsl:otherwise>
                    </xsl:choose>
