...
function hideColumn(num) {
    ...
    $(
        "#itemst td:nth-child(" + hide + "),#itemst th:nth-child(" + hide + ")"
    ).toggle();
    ...
    if (valCookie) {
        ...
        if (!found) {
            ...
            Cookies.set("showColumns", cookieString, {
                expires: date,
                path: "/",
                sameSite: "Lax",
            });
...
function showColumn(num) {
    ...
    // hide header and cells matching the index
    $(
        "#itemst td:nth-child(" + show + "),#itemst th:nth-child(" + show + ")"
    ).toggle();
    // set or modify cookie with the hidden column's index
    if (valCookie) {
        valCookie = valCookie.split("/");
...
$(document).ready(function () {
    ...
    $("#clearonloanbutton").click(function () {
        $("#itemst input[name='itemnumber'][data-is-onloan='1']").each(
            function () {
                $(this).prop("checked", false);
            }
        );
