<template>
  <fieldset v-if="available_fields.length" class="rows" id="additional_fields">
      ...
      >
        ...
        >
          <li>
            <label
              :for="
                `additional_field_` + available_field.extended_attribute_type_id
              "
            ...
            <v-select
              ...
              "
              :options="
                av_options[available_field.authorised_value_category_name]
              "
            />
          </li>
        </template>
          ...
          >
            ...
            <template v-if="available_field.repeatable">
              <a
                ...
                @click="cloneField(available_field, current, $event)"
              >
  ...
  watch: {
    current_additional_fields_values: {
      ...
      handler(current_additional_fields_values) {
        this.updateParentAdditionalFieldValues(current_additional_fields_values)
      },
    },
        ...
        client_av.values
          ...
          .then(av_categories => {
            av_cat_array.forEach(av_cat => {
              ...
              this.av_options[av_cat] = av_match.authorised_values.map(av => ({
                value: av.value,
            ...
            available_fields.forEach(available_field => {
              ...
              let existing_field_values = this.additional_field_values.filter(
                afv =>
                  afv.field_id == available_field.extended_attribute_type_id &&
                  afv.value
              )
              ...
              if (existing_field_values.length) {
                existing_field_values.forEach(existing_field_value => {
                  let label = ""
                  if (available_field.authorised_value_category_name) {
                    let av_value = this.av_options[
                      available_field.authorised_value_category_name
                    ].filter(
                      av_option => av_option.value == existing_field_value.value
                    )
                    label = av_value.length ? av_value[0].label : ""
                  }
                  this.current_additional_fields_values[
  ...
  methods: {
    ...
    updateParentAdditionalFieldValues: function (
      ...
      Object.keys(current_additional_fields_values).forEach(field_id => {
        if (
          !Array.isArray(current_additional_fields_values[field_id]) &&
        ...
        if (current_additional_fields_values[field_id]) {
          let new_extended_attributes = current_additional_fields_values[
            field_id
          ].map(value => ({
            field_id: field_id,
            value: value.value,
