...
export default {
  ...
  beforeCreate() {
    ...
    this.embed =
      this.report_type === "yearly" ? ["erm_usage_yuses"] : ["erm_usage_muses"]
  ...
  methods: {
    buildColumnArray(report_display, params, data_type) {
      ...
      report_display !== "usage_data_provider" &&
        column_set.unshift({
          title: __(data_type.charAt(0).toUpperCase() + data_type.slice(1)),
          data: data_type,
          searchable: true,
          orderable: true,
        })
      ...
      if (yearly_filter) {
        months_data.forEach(month => {
          column_set.push({
            ...
            render: function (data, type, row, meta) {
              const find_correct_month = row.erm_usage_muses.filter(
            ...
            selected_months.forEach(month => {
              if (month.active) {
                ...
                column_set.push({
                  ...
                  render: function (data, type, row, meta) {
                    const filtered_by_year = row.erm_usage_muses.filter(
                      item => item.year === parseInt(year)
                    )
                    const find_correct_month = filtered_by_year.filter(
                      item => item.month === month.value
                    )
                    const data_available = find_correct_month.length
          ...
          years.forEach(year => {
            ...
            column_set.push({
              ...
              render: function (data, type, row, meta) {
                ...
                const usage = find_usage ? find_usage.totalcount : "-"
                return usage
              },
              searchable: true,
              orderable: true,
            ...
            if (access_types && access_types.length > 0) {
              access_types.forEach(access => {
                column_set.push({
                  title: __(access),
                  render: function (data, type, row, meta) {
                    const filterByType = row.erm_usage_muses.filter(
                      item => item.access_type === access
                    )
                    const filterByMetric = filterByType.filter(
                      item => item.metric_type === metric
                    )
                    const period_total = filterByMetric.reduce((acc, item) => {
              ...
              column_set.push({
                title: __(metric),
                render: function (data, type, row, meta) {
                  const filterByMetric = row.erm_usage_muses.filter(
                    item => item.metric_type === metric
                  )
                  const period_total = filterByMetric.reduce((acc, item) => {
                    return acc + item.usage_count
    ...
    tableURL(year, params) {
      ...
      if (filter_required) {
        ...
        const url = this.buildFilteredQuery(query, time_period_columns, year)
