    ...
    <form @submit="buildCustomReport($event)" class="custom_usage_report">
      <fieldset class="rows custom_report_builder">
        <div class="">
          ...
          <ol>
            <li>
              <label for="interval">{{ $__("Data display") }}:</label>
              <v-select
                id="interval"
                ...
                :options="[
                  ...
                  },
                  {
                    ...
                    description: $__('By month with period total'),
                  },
                  {
                    value: 'yearly',
                  ...
                  },
                  {
                    ...
                    description: $__('By data provider totals'),
                  },
                ]"
            ...
            <li>
              ...
              <v-select
                ...
                label="name"
                :reduce="provider => provider.erm_usage_data_provider_id"
                :options="usage_data_provider_list"
                @update:modelValue="setReportTypesAndResetFilterData($event)"
                multiple
              />
            </li>
            <li>
              <label for="report_types">{{ $__("Choose report") }}:</label>
              <v-select
                id="report_type"
                ...
                @update:modelValue="setMetricTypesAndProviderList($event)"
            ...
            <li>
              <label for="metric_types">{{ $__("Choose metric type") }}:</label>
              <v-select
                id="metric_types"
                ...
                multiple
                :disabled="this.metric_types_options.length === 0"
              />
            </li>
            <li>
              <label for="access_types">{{ $__("Choose access type") }}:</label>
              <v-select
                id="access_types"
                ...
                multiple
                :disabled="this.access_types_options.length === 0"
              />
            </li>
            <li>
              ...
              <input
                ...
                :placeholder="$__('Please enter year in format YYYY')"
                required
              />
              <span class="required">{{ $__("Required") }}</span>
            </li>
            ...
            <li>
              ...
              <v-select
                ...
                :placeholder="$__('Type at least two characters to search')"
              />
            </li>
          </ol>
        </div>
        <div class="">
          ...
          <ol class="checkbox_options">
            <li
              v-for="(prop, key, index) in title_property_column_options"
              v-bind:key="index"
              class="checkbox_options"
            >
              <label :for="prop.name" class="checkbox">{{ prop.name }}:</label>
              <input
                type="checkbox"
                :name="prop.name"
                :id="prop.name"
                :checked="true"
                v-model="title_property_column_options[key].active"
                :disabled="!prop.used_by.includes(this.data_type)"
              />
            </li>
          </ol>
        </div>
        <div
          v-if="query.data_display && query.data_display.includes('monthly')"
        >
          <h3>{{ $__("Select date display method") }}</h3>
          <div class="date_display">
            <label for="yearly_filter_required" class="date_display_title"
              >{{ $__("Display data filtered by year") }}:</label
            >
            <label for="yearly_filter_required_yes" class="date_display_radio">
              <input
                type="radio"
            ...
            <label for="yearly_filter_required_no" class="date_display_radio">
              <input
                type="radio"
                name="yearly_or_monthly"
        ...
        <div v-if="!yearly_filter_required">
          ...
          <ol>
            <li
              v-for="(year, key, index) in time_period_columns_builder"
      ...
      <div class="save_report">
        <input
          ...
          class="year_input"
        />
        <button @click="saveToDefaultReports($event)" class="btn btn-default">
          {{ $__("Save report") }}
        </button>
      </div>
    </form>
...
export default {
  setup() {
    ...
    const { setConfirmationDialog, setMessage, setError } = inject("mainStore")
  ...
  data() {
    return {
      ...
      metric_types_matrix: {
        ...
        Unique_Item_Investigations: ["PR", "DR", "TR", "TR_B3", "TR_J3", "IR"],
        ...
        Unique_Title_Requests: ["PR", "PR_P1", "DR", "TR", "TR_B1", "TR_B3"],
  ...
  methods: {
      ...
      await client[`usage_${report_type}s`].getAll(query).then(
        response => {
          ...
          const removedDuplicates = [
            ...new Map(
              addNewData.map(object => [object[`${report_type}_id`], object])
            ).values(),
          ]
    ...
    monthSelector(e) {
      if (!this.query.start_year || !this.query.end_year) {
        alert("Please select a start and end year before choosing this option")
        return
      }
      if (e.target.id === "yearly_filter_required_no") {
        ...
        const months = this.determineMonths(this.query, years, this.months_data)
    ...
    buildMonthlyUrlQuery(query, time_period_columns, data_display, db_table) {
      let url
      let prefix
      switch (data_display) {
        case "monthly":
      ...
      const queryArray = years.map(year => {
        ...
        if (usage_data_providers) {
          queryByYear[`${prefix}.usage_data_provider_id`] = usage_data_providers
        }
    ...
    setMetricTypesAndProviderList(e) {
      ...
      if (report_type) {
        ...
        for (const metric in this.metric_types_matrix) {
          if (this.metric_types_matrix[metric].includes(report_type, false)) {
            const av_metric_type = av_type.find(type => metric === type.value)
        ...
        this.usage_data_provider_list = this.usage_data_providers.filter(
          provider => {
        ...
        const title_properties = Object.keys(this.title_property_column_options)
        title_properties.shift() // Remove the first column as this is a default
        title_properties.forEach(prop => {
          if (
            !this.title_property_column_options[prop].used_by.includes(
              this.data_type
            )
          ) {
    ...
    setReportTypesAndResetFilterData(providers) {
      ...
      providers.forEach(id => {
        ...
        single_report_types.forEach(type => {
          const report_type = this.av_report_types.find(rt => rt.value === type)
    ...
    validateFormAndCreateUrlParams() {
      ...
      if (!valid_start_year || !valid_end_year) {
        this.setError(this.$__("Please enter a year with the format YYYY"))
        return
      }
      ...
      if (!metric_types || (metric_types && metric_types.length === 0)) {
        const final_metric_types = this.metric_types_options.map(metric => {
          return metric.value
        })
      ...
      if (
        ...
        const final_access_types = this.access_types_options.map(access => {
          return access
        })
