<template>
  <div>
    <fieldset>
      ...
      />
      {{ $__("Publication type") }}:
      <select id="publication_type_filter" v-model="filters.publication_type">
      ...
      <input
        ...
        :value="$__('Submit')"
      />
      <span v-if="cannot_search">{{ $__("Please enter a search term") }}</span>
    </fieldset>
    ...
    >
      <div v-if="local_title_count !== undefined && local_title_count !== null">
  ...
  data: function () {
    ...
    return {
      ...
      tableOptions: {
        ...
        filters_options: {
          1: () => this.map_av_dt_filter("av_title_publication_types"),
        },
        actions: { 0: ["show"] },
        default_filters: {
  ...
  methods: {
    ...
    filter_table: async function () {
      if (this.filters.publication_title.length) {
        ...
        if (this.$refs.table) {
          this.$refs.table.redraw("/api/v1/erm/eholdings/ebsco/titles")
        }
                ...
                ...(this.filters.publication_title
                  ? {
                      "me.publication_title": {
                        like: "%" + this.filters.publication_title + "%",
                      },
                    }
                  : {}),
                ...(this.filters.publication_type
                  ? {
                      "me.publication_type": this.filters.publication_type,
                    }
                  : {}),
              }
            : undefined
      ...
      return [
        {
          ...
          render: function (data, type, row, meta) {
            let node =
              ...
              row.title_id +
              '" class="show">' +
              escape_str(`${row.publication_title} (#${row.title_id})`) +
              "</a>"
            if (row.is_selected) {
              node +=
                " " +
        ...
        },
        {
          ...
          render: function (data, type, row, meta) {
            ...
            return (
              (print_identifier
                ? escape_str(__("ISBN (Print): %s").format(print_identifier))
                : "") +
              (online_identifier
                ? escape_str(__("ISBN (Online): %s").format(online_identifier))
                : "")
            )
          },
        },
