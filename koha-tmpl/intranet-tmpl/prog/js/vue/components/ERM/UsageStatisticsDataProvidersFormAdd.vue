    ...
    <div class="page-section">
      <form @submit="onSubmit($event)">
        ...
        <fieldset class="rows">
          <ol>
            <li>
              <label class="required" for="usage_data_provider_name"
              ...
              <button
                v-if="
                  manual_form && !usage_data_provider.erm_usage_data_provider_id
                "
                type="button"
                style="margin-left: 1em"
            ...
            <li>
              <label class="required" for="usage_data_provider_report_types"
                >{{ $__("Report types") }}:
              </label>
              <v-select
                id="report_type"
                ...
                multiple
                :disabled="!selected_provider && !manual_form"
                :required="!usage_data_provider.report_types.length"
              >
        ...
        <fieldset class="rows credentials">
          <ol class="credentials_form">
            <li>
              <label
                :class="required_fields.includes('URL') ? 'required' : ''"
              ...
              <input
                ...
                :disabled="!selected_provider && !manual_form"
              />
              <span class="required" v-if="required_fields.includes('URL')"
                >{{ $__("Required") }}
              </span>
            </li>
            <li>
              <label
                :class="required_fields.includes('Release') ? 'required' : ''"
              ...
              <input
                ...
                :disabled="!selected_provider && !manual_form"
              />
              <span class="required" v-if="required_fields.includes('Release')"
                >{{ $__("Required") }}
              </span>
            </li>
            <li>
              <label
                :class="required_fields.includes('Customer') ? 'required' : ''"
              ...
              <input
                ...
                :disabled="!selected_provider && !manual_form"
              />
              <span class="required" v-if="required_fields.includes('Customer')"
                >{{ $__("Required") }}
              </span>
            </li>
            <li>
              <label
                :class="required_fields.includes('Requestor') ? 'required' : ''"
              ...
              <input
                ...
                :required="required_fields.includes('Requestor')"
                :disabled="!selected_provider && !manual_form"
              />
              <span
                class="required"
            ...
            <li>
              <label
                ...
                :class="required_fields.includes('API') ? 'required' : ''"
                >{{ $__("API key") }}:
              </label>
              <input
              ...
              />
              <span class="required" v-if="required_fields.includes('API')"
                >{{ $__("Required") }}
              </span>
            </li>
            <li>
          ...
          <ol class="credentials_form" v-if="sushi_service">
            ...
            <li>
              <label for="api_key_info">{{ $__("API key info") }}: </label>
  ...
  methods: {
    async getDataProvider(erm_usage_data_provider_id) {
      ...
      client.usage_data_providers.get(erm_usage_data_provider_id).then(
        usage_data_provider => {
          ...
          this.usage_data_provider.report_types = this.formatReportTypes(
        ...
        registry_data => {
          ...
          if (caller === "edit" && registry_data.length > 0) {
            this.selected_provider = registry_data.find(
              platform => platform.name === this.usage_data_provider.name
            )
    ...
    async getSushiService(url) {
      ...
      client.sushi_service.getAll({ url }).then(
        sushi_service => {
          const { url, api_key_required, requestor_id_required } = sushi_service
          ...
          requestor_id_required && this.required_fields.push("Requestor")
    ...
    formatReportTypes(reportTypes) {
      const dataType = typeof reportTypes === "string" ? "string" : "array"
    ...
    onSubmit(e) {
      ...
      if (erm_usage_data_provider_id) {
          ...
          .then(
            success => {
              ...
              this.$router.push({
                ...
                params: {
                  usage_data_provider_id: erm_usage_data_provider_id,
                },
              })
            },
            error => {}
        ...
        client.usage_data_providers.create(usage_data_provider).then(
          success => {
            erm_usage_data_provider_id = success.erm_usage_data_provider_id
