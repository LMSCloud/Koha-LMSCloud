      ...
      <form @submit="onSubmit($event)">
        <fieldset class="rows">
          <ol>
            ...
            <li>
              <label for="agreement_vendor_id">{{ $__("Vendor") }}:</label>
              <FormSelectVendors
            ...
            <li>
              ...
              <v-select
                ...
                :disabled="agreement.status == 'closed' ? false : true"
              />
            </li>
            <li>
              ...
              >
              <label class="radio" for="agreement_is_perpetual_yes"
                >{{ $__("Yes") }}:
                <input
                  type="radio"
                  name="is_perpetual"
        ...
        <AgreementLicenses
          ...
          :av_agreement_license_location="av_agreement_license_location"
        />
...
export default {
  ...
  methods: {
    ...
    checkForm(agreement) {
      ...
      const duplicate_related_agreement_ids = related_agreement_ids.filter(
        (id, i) => related_agreement_ids.indexOf(id) !== i
      )

      if (duplicate_related_agreement_ids.length) {
        errors.push(
          this.$__("An agreement is used as relationship several times")
        )
      }

      if (
        agreement_licenses.filter(al => al.status == "controlling").length > 1
      ) {
      ...
      agreement.user_roles.forEach((user, i) => {
        if (user.patron_str === "") {
          errors.push(
            this.$__("Agreement user %s is missing a user").format(i + 1)
          )
        }
      })
    ...
    onSubmit(e) {
      ...
      agreement.periods = agreement.periods.map(
        ({ agreement_id, agreement_period_id, ...keepAttrs }) => keepAttrs
      )
      ...
      agreement.agreement_licenses = agreement.agreement_licenses.map(
        ({ license, agreement_id, agreement_license_id, ...keepAttrs }) =>
          keepAttrs
      )

      agreement.agreement_relationships = agreement.agreement_relationships.map(
