<template>
  <fieldset class="rows" id="agreement_relationships">
    ...
    >
      <legend>
        ...
        <a href="#" @click.prevent="deleteRelationship(counter)"
          ><i class="fa fa-trash"></i> {{ $__("Remove this relationship") }}</a
        >
      </legend>
      <ol>
        <li>
          <label :for="`related_agreement_id_${counter}`" class="required"
    ...
    >
    <span v-else>{{ $__("There are no other agreements created yet") }}</span>
  </fieldset>
</template>
