<template>
  <fieldset class="rows" id="resources">
    ...
    >
      <legend>
        ...
        <a href="#" @click.prevent="deletePackage(counter)"
          ><i class="fa fa-trash"></i> {{ $__("Remove from this package") }}</a
        >
      </legend>
      <ol>
        <li>
          <label :for="`resource_package_id_${counter}`" class="required"
            >{{ $__("Package") }}:
          </label>
        ...
        <li>
          <label :for="`ended_on_${counter}`">{{ $__("End date") }}:</label>
          <flat-pickr
    ...
    <a v-if="packages.length" class="btn btn-default" @click="addPackage"
      ><font-awesome-icon icon="plus" /> {{ $__("Add to another package") }}</a
    >
