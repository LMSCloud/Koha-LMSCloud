  ...
  methods: {
    async getCounterFiles() {
      ...
      await client.counter_files
        .count({
          usage_data_provider_id: this.$route.params.usage_data_provider_id,
        })
        .then(
          count => {
    ...
    delete_counter_file(counter_log, dt, event) {
      this.setConfirmationDialog(
        {
          title: this.$__("Are you sure you want to remove this file?"),
        ...
        () => {
          const client = APIClient.erm
          client.counter_files.delete(counter_log.counter_files_id).then(
            success => {
              this.setMessage(
                this.$__("File %s deleted").format(counter_log.filename),
                true
              )
              dt.draw()
            },
