<template>
  ...
  <div v-else>
    ...
    <form v-else class="default-report" @submit="displayDefaultReport($event)">
      ...
      if (id) {
        ...
        await client.default_usage_reports.delete(id).then(
          success => {
            ...
            this.default_usage_reports = this.default_usage_reports.filter(
              report => report.erm_default_usage_report_id !== id
            )
        ...
        this.setError(this.$__("No report was selected - could not delete"))
      }
    },
