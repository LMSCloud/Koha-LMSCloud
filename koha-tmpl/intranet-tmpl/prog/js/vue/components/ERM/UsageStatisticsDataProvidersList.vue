      ...
      if (!usage_data_provider.active) {
        this.setConfirmationDialog(
          ...
          () => {
            ...
            const client = APIClient.erm
            client.usage_data_providers.update(usage_data_provider, id).then(
              success => {
                this.setMessage(
                  this.$__("Updated status for usage data provider %s").format(
                    name
                  ),
                  true
                )
          ...
          callback_result => {
            ...
            client.usage_data_providers
              .process_SUSHI_response(id, {
                begin_date: callback_result.inputs.find(input => {
      ...
      return [
        {
          ...
          render: function (data, type, row, meta) {
            return (
              ...
              '" class="show">' +
              escape_str(`${row.name} (#${row.erm_usage_data_provider_id})`) +
              "</a>"
            )
          },
        },
        ...
        },
        {
          ...
          data: "active",
          render: function (data, type, row, meta) {
            const status = row.active ? __("Active") : __("Inactive")
            return status
          },
          searchable: false,
          orderable: true,
