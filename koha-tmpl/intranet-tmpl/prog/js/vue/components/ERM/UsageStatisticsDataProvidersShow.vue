  ...
  <div v-else id="usage_data_providers_show">
    ...
    <div id="usage_data_providerstabs" class="toptabs numbered">
      <ul class="nav nav-tabs">
        ...
        <li v-for="(item, i) in available_data_types" class="nav-item" :key="i">
          <a
            href="#"
            class="nav-link"
            :data-content="item.data_type"
            @click="change_tab_content"
            v-bind:class="tab_content === item.data_type ? 'active' : ''"
            >{{ item.tab_name }}</a
          >
        </li>
        <li class="nav-item">
    ...
    <div class="tab-content">
      <div v-if="tab_content === 'detail'" class="usage_data_provider_detail">
      ...
      <template v-for="(item, i) in available_data_types">
        <div v-if="tab_content === item.data_type" :key="i">
          <UsageStatisticsProviderDataList :data_type="item.data_type" />
        </div>
      </template>
  ...
  methods: {
    ...
    delete_usage_data_provider: function (
      ...
      this.setConfirmationDialog(
        ...
        () => {
          const client = APIClient.erm
          client.usage_data_providers.delete(usage_data_provider_id).then(
            success => {
              this.setMessage(
                this.$__("Usage data provider %s deleted").format(
                  usage_data_provider_name
                ),
                true
              )
