...
export default {
  ...
  data: function () {
    this.filters.by_expired = this.$route.query.by_expired === "true" || false
    ...
    return {
      ...
      tableOptions: {
        ...
        filters_options: {
          ...
          4: () => this.map_av_dt_filter("av_agreement_closure_reasons"),
          5: [
            { _id: 0, _str: this.$__("No") },
            { _id: 1, _str: this.$__("Yes") },
          ],
          6: () => this.map_av_dt_filter("av_agreement_renewal_priorities"),
        },
        actions: {
          0: ["show"],
          "-1": this.embedded
    ...
    next(vm => {
      vm.getAgreementCount().then(() =>
        vm
          ...
          .then(() =>
            vm.getSearchableAVOptions().then(() => (vm.initialized = true))
          )
      )
    })
  },
  methods: {
      ...
      await client.additional_fields.getAll("agreement").then(
        searchable_additional_fields => {
          this.searchable_additional_fields =
            searchable_additional_fields.filter(field => field.searchable)
        },
        error => {}
      )
    },
      ...
      await client_av.values
        ...
        .then(av_categories => {
          av_cat_array.forEach(av_cat => {
            ...
            this.searchable_av_options[av_cat] = av_match.authorised_values.map(
              av => ({
                value: av.value,
      ...
      this.setConfirmationDialog(
        ...
        () => {
          ...
          client.agreements.delete(agreement.agreement_id).then(
            success => {
              this.setMessage(
                this.$__("Agreement %s deleted").format(agreement.name),
                true
              )
              dt.draw()
            },
    ...
    filter_table: async function () {
      if (!this.embedded) {
        if (this.filters.by_expired && !this.filters.max_expiration_date) {
    ...
    getTableColumns: function () {
      ...
      return [
        ...
        },
        {
          ...
          orderable: true,
          render: function (data, type, row, meta) {
            return escape_str(row.is_perpetual ? __("Yes") : __("No"))
          },
        },
        {
