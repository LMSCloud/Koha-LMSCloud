<template>
            ...
            <fieldset>
              <ol>
                <li class="form-group form-row">
                  <label class="required col-form-label" for="train_list"
                ...
                >
                  <template #search="{ attributes, events }">
                    <input
                      :required="!train_id_selected_for_copy"
                      class="vs__search"
                      v-bind="attributes"
                      v-on="events"
                    />
  ...
  <div v-else id="trains_show">
    <Toolbar>
      ...
      >
      <a v-if="!train.closed_on" class="btn btn-default" @click="closeTrain"
        ><font-awesome-icon icon="remove" /> {{ $__("Close") }}</a
      >
      <a v-else-if="!train.sent_on" class="btn btn-default" @click="sendTrain"
    ...
    <div>
      <fieldset class="rows">
        <ol>
          ...
          <li>
            <label>{{ $__("Status for item added to this train") }}:</label>
            <span>{{
          ...
          >
            ...
            <div class="attributes_values">
              <span
                ...
                v-for="(attribute, counter_attribute) in item.attributes"
  ...
  methods: {
      ...
      await client.trains.get(train_id).then(
        train => {
          ...
          let display_table = this.train.items.every(
            item => item.processing_id == this.train.default_processing_id
          )
          if (display_table) {
            this.item_table.data = []
            this.train.items.forEach(item => {
              let item_row = {}
              this.train.default_processing.attributes.forEach(attribute => {
                item_row[attribute.processing_attribute_id] = item.attributes
                  .filter(
                    a =>
    ...
    deleteTrain: function (train) {
      this.setConfirmationDialog(
        {
          title: this.$__("Are you sure you want to remove this train?"),
          message: train.name,
    ...
    removeItem(train_item_id) {
      this.setConfirmationDialog(
        {
          title: this.$__("Are you sure you want to remove this item?"),
          accept_label: this.$__("Yes, remove"),
          cancel_label: this.$__("No, do not remove"),
        },
        () => {
          const client = APIClient.preservation
          client.train_items.delete(this.train.train_id, train_item_id).then(
            success => {
    ...
    printSelected() {
      window.open(
        "/cgi-bin/koha/preservation/print_slip.pl?%s_blank".format(
          this.selected_items.map(id => "train_item_id=" + id).join("&")
        )
      )
    },
        ...
        drawCallback: function (settings) {
          ...
          $.each($(this).find("td.checkboxes"), function (index, e) {
            ...
            let checkbox = createVNode("input", {
              ...
              value: train_item_id,
              onChange: e => {
                updateSelectedItems(e.target.checked, train_item_id)
              },
            })
