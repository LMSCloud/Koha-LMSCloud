            ...
            <fieldset>
              <ol>
                <li class="form-group form-row">
                  <label class="required col-form-label" for="barcode_list"
        ...
        <form @submit="addItemsToTrain($event)">
          ...
          <div class="modal-body position-relative overflow-visible">
            <fieldset>
              <ol>
                <li class="form-group form-row">
                  <label class="required col-form-label" for="train_id"
                  ...
                  >
                    <template #search="{ attributes, events }">
                      <input
                        :required="!train_id_selected_for_add"
                        class="vs__search"
                        v-bind="attributes"
                        v-on="events"
                      />
  ...
  <div v-else id="waiting-list">
    <Toolbar>
      <a
        ...
        ><font-awesome-icon icon="plus" /> {{ $__("Add to waiting list") }}</a
      >
      <a
        ...
        {{ $__("Add last %s items to a train").format(last_items.length) }}</a
      >
    </Toolbar>
...
export default {
  ...
  methods: {
        ...
        result => {
          if (result.length) {
            if (result.length != items.length) {
              ...
              this.setMessage(
                this.$__("%s new items added.").format(result.length),
                true
              )
            }
            this.last_items = result
            if (this.$refs.table) {
              this.$refs.table.redraw("/api/v1/preservation/waiting-list/items")
            } else {
    ...
    doShow: function (biblio, dt, event) {
      ...
      location.href =
        "/cgi-bin/koha/catalogue/detail.pl?biblionumber=" + biblio.biblio_id
    },
