    ...
    <div v-if="train_item">
      <form @submit="onSubmit($event)">
        <fieldset class="rows">
          <ol>
            <li>
              <label for="itemnumber">{{ $__("Itemnumber") }}:</label>
            ...
            >
              ...
              <span v-else-if="attribute.type == 'free_text'">
                <input :id="`attribute_${counter}`" v-model="attribute.value" />
              </span>
              ...
              <a
                v-if="
                  ...
                  attributes[counter + 1].processing_attribute_id !=
                    attribute.processing_attribute_id
                "
                class="btn btn-link"
                @click="addAttribute(attribute.processing_attribute_id)"
                ><font-awesome-icon icon="plus" /> {{ $__("Add") }}</a
              >
              <a v-else class="btn btn-link" @click="removeAttribute(counter)"
                ><font-awesome-icon icon="minus" /> {{ $__("Remove") }}</a
              >
            </li>
          </ol>
        </fieldset>
    ...
    next(vm => {
      if (to.params.train_item_id) {
        vm.train = vm
          ...
          .then(() =>
            vm
              .getTrainItem(to.params.train_id, to.params.train_item_id)
              .then(() =>
                vm.refreshAttributes().then(() => (vm.initialized = true))
              )
          )
      } else {
        vm.train = vm
      ...
      this.processing.attributes.forEach(attribute => {
        ...
        if (!apply_default_value) {
          this.train_item.attributes
            .filter(
              a =>
                a.processing_attribute_id == attribute.processing_attribute_id
            )
            .forEach(a => values.push(a.value))
        } else if (attribute.type == "db_column") {
          values.push(this.default_values[attribute.processing_attribute_id])
        } else {
          values.push("")
        }
      ...
      client_av.values
        ...
        .then(av_categories => {
          av_cat_array.forEach(av_cat => {
            ...
            this.av_options[av_cat] = av_match ? av_match.authorised_values : []
          })
        })
        .then(() => this.loaded())
    },
