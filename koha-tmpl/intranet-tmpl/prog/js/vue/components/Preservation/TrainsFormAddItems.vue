  ...
  <div v-else id="trains_add_items">
    ...
    <form @submit="onSubmit($event)">
      <fieldset class="rows">
        <ol>
          <li>
            <label for="itemnumbers">{{ $__("Itemnumbers") }}:</label>
          ...
          <li
            ...
            v-bind:key="counter"
          >
            <label :for="`attribute_${counter}`">{{ attribute.name }}: </label>
    ...
    onSubmit(e) {
      ...
      client.train_items.createAll(train_items, this.train.train_id).then(
        result => {
          if (result.length) {
            this.setMessage(
              this.$__("%s items have been added to train %s.").format(
                result.length,
                this.train.train_id
              )
            )
            ...
            this.setMessage(this.$__("No items have been added to the train."))
          }
        },
        error => {}
      )
