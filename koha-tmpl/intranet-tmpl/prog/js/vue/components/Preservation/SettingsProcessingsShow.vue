      ...
      <fieldset class="rows">
        ...
        <ol v-if="processing.attributes.length">
          ...
          >
            ...
            <span v-else>{{ $__("Unknown") }} - {{ attribute.type }}</span>
          </li>
        </ol>
        <span v-else>
          {{ $__("There are no attributes defined for this processing.") }}
        </span>
      </fieldset>
      <fieldset class="action">
        <router-link :to="{ name: 'Settings' }" role="button" class="cancel">{{
          $__("Close")
        }}</router-link>
      </fieldset>
    </div>
  ...
  methods: {
    ...
    doDelete: function () {
      this.setConfirmationDialog(
        {
          title: this.$__("Are you sure you want to remove this processing?"),
        ...
        () => {
          const client = APIClient.preservation
          client.processings.delete(this.processing.processing_id).then(
            success => {
              this.setMessage(
                this.$__("Processing %s deleted").format(this.processing.name),
                true
              )
