    ...
    <div>
      ...
      >
        <fieldset class="rows">
          ...
          <ol>
            <li>
              <label class="required" for="not_for_loan_waiting_list_in"
                >{{ $__("Status for item added to waiting list") }}:</label
              >
              <v-select
              ...
              >
                <template #search="{ attributes, events }">
                  <input
                    :required="!config.settings.not_for_loan_waiting_list_in"
                    class="vs__search"
                    v-bind="attributes"
                    v-on="events"
                  />
            ...
            <li>
              <label for="not_for_loan_default_train_in"
                >{{ $__("Default status for item added to train") }}:</label
              >
              <v-select
                id="not_for_loan_default_train_in"
                v-model="config.settings.not_for_loan_default_train_in"
                label="description"
                :reduce="av => av.value"
                :options="av_notforloan"
              />
        ...
        <fieldset class="action">
          ...
          <router-link :to="{ name: 'Home' }" role="button" class="cancel">{{
            $__("Cancel")
          }}</router-link>
        </fieldset>
      </form>
