<template>
  <fieldset class="step-block">
    ...
    <div class="calendar-legend">
      ...
      {{ $__("Booked") }}
      <span class="booking-marker-dot booking-marker-dot--lead ms-3"></span>
      {{ $__("Lead Period") }}
      <span class="booking-marker-dot booking-marker-dot--trail ms-3"></span>
      {{ $__("Trail Period") }}
      <span
        class="booking-marker-dot booking-marker-dot--checked-out ms-3"
      ></span>
      {{ $__("Checked Out") }}
      <span class="booking-marker-dot booking-marker-dot--holiday ms-3"></span>
      {{ $__("Closed") }}
      <span
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const constraintHelpText = computed(() => {
      ...
      const baseMessages = {
        issuelength: props.maxBookingPeriod
          ? $__("Booking period limited to issue length (%s days)").format(
        ...
        issuelength_with_renewals: props.maxBookingPeriod
          ...
          : $__("Booking period limited to issue length with renewals"),
    ...
    const debouncedExtendHolidays = debounce(
      (libraryId, visibleStart, visibleEnd) => {
