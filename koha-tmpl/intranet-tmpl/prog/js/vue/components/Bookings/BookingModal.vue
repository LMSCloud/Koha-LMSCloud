      ...
      <div class="modal-content">
        ...
        <div class="modal-body booking-modal-body">
          ...
          >
            ...
            />
            <hr v-if="showAdditionalFields && modalState.hasAdditionalFields" />
        ...
        <div class="modal-footer">
          <div class="d-flex gap-2">
            ...
            <button class="btn btn-secondary ms-2" @click.prevent="handleClose">
              {{ $__("Cancel") }}
            </button>
          </div>
        </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const formPrefilterValid = computed(() => {
      ...
      const hasTypeOrItem = !!bookingItemId.value || !!bookingItemtypeId.value
    ...
    watch(
      ...
      async open => {
        ...
        try {
          ...
          bookingItemId.value =
            props.itemId != null
              ? normalizeIdType(bookableItems.value?.[0]?.item_id, props.itemId)
              : null
          if (props.itemtypeId) {
    ...
    watch(
      ...
      ([availableItems, patron, pickupLibrary, itemtypeId, isDataReady]) => {
        ...
        const pickupLocationsReady =
          !pickupLibrary ||
          (!loading.value.pickupLocations && pickupLocations.value.length > 0)
