      ...
      <fieldset class="rows">
        <ol>
          <li>
            ...
            <input id="name" v-model="record_source.name" required />
            <span class="required">{{ $__("Required") }}</span>
          </li>
          <li>
            <label for="can_be_edited"> {{ $__("Can be edited") }}: </label>
            <input
              id="can_be_edited"
              type="checkbox"
  ...
  methods: {
    ...
    onSubmit(e) {
      ...
      if (this.record_source_id) {
        ...
        response = client.record_sources.create(this.record_source).then(
          success => {
