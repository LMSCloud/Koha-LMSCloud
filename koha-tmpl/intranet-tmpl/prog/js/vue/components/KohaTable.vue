  ...
  data() {
    ...
    if (this.table_settings) {
      ...
      if (
        ...
        this.options.pageLength = this.table_settings.default_display_length
      }
      if (
        this.table_settings.hasOwnProperty("default_sort_order") &&
        this.table_settings.default_sort_order != null
      ) {
        this.options.order = [[this.table_settings.default_sort_order, "asc"]]
      }
    }
      ...
      if (this.searchable_additional_fields.length) {
        this.searchable_additional_fields.forEach(searchable_field => {
          ...
          this.tableColumns.push({
            ...
            relatedKey: "field_id",
            relatedValue: searchable_field.extended_attribute_type_id,
          ...
          if (searchable_field.authorised_value_category_name) {
            ...
            this.filters_options[this.tableColumns.length - 1] = options
          }
        })
      }
          ...
          render: (data, type, row) => {
            ...
            this.actions["-1"].forEach(action => {
              if (typeof action === "object") {
                ...
                if (typeof action[action_name].should_display === "function") {
                  should_display = action[action_name].should_display(row)
                }
          ...
          dt.column(col_id)
            ...
            .each(function (idx) {
              ...
              actions.forEach(action => {
                let action_name =
                  typeof action === "object" ? Object.keys(action)[0] : action
