if (RFIDWebService === undefined)
    var RFIDWebService = {
        ...
        Init: function (
            ...
            var initialized = window.sessionStorage.getItem(
                "RFIDWebServiceInitialized"
            );
            if (!initialized) {
                reInit = true;
            }
                ...
                if (status == "ok") {
                    var data = JSON.parse(
                        window.sessionStorage.getItem(
                            "RFIDWebServiceStatusInfo"
                        )
                    );
                    if (
                        data &&
                        data.responseData &&
                        data.responseData.vendorName
                    ) {
        ...
        ClearRFIDWebServiceSession: function () {
            if (window.sessionStorage) {
                if (
                    window.sessionStorage.hasOwnProperty("RFIDWebServiceStatus")
                ) {
                ...
                if (
                    window.sessionStorage.hasOwnProperty(
                        "RFIDWebServiceCheckinItemCount"
                    )
                ) {
                    window.sessionStorage.removeItem(
                        "RFIDWebServiceCheckinItemCount"
                ...
                if (
                    window.sessionStorage.hasOwnProperty(
                        "RFIDWebServiceCheckoutItemCount"
                    )
                ) {
                    window.sessionStorage.removeItem(
                        "RFIDWebServiceCheckoutItemCount"
        ...
        InitializeRFIDWebService: function (rfidWebServiceURL) {
            ...
            if (!checkServiceAvailabilityURL) {
                checkServiceAvailabilityURL =
                    window.sessionStorage.getItem("RFIDWebServiceURL");
            }
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data) {
                            ...
                            var previousStatus = window.sessionStorage.getItem(
                                "RFIDWebServiceStatus"
                            );
                            var showError = false;
                            if (
                                data.requestSuccess &&
                            ...
                            if (
                                ...
                                RFIDWebService.DisplayRFIDServiceErrorMessage(
                                    data.errorCode,
                                    data.errorMessage
                                );
                                console.log(
                                    "RFIDWebService ServiceInfo failed"
                                );
                            }
                        }
                    },
        ...
        EnableRfidCheckinCheckout: function () {
            ...
            if (enabled && enabled == "ok") {
                ...
                if (
                    $("#circ_circulation #circ_circulation_issue #barcode")
                        .length
                ) {
                ...
                if (findborrower.length) {
                    findborrower.keydown(function (event) {
                        if (event.which == RFIDWebService.actionKey) {
                            var barvalue = $(
                                "#patronsearch #findborrower"
                            ).val();
                            if (barvalue == "" || barvalue == " ") {
                                event.preventDefault();
                                RFIDWebService.ReadTagsAndStartAction(
                                    "borrower"
                                );
                            }
                        }
        ...
        CheckLastCheckoutAndContinue: function () {
            var barcode = window.sessionStorage.getItem(
                "RFIDWebServiceCheckoutItem"
            );
            ...
            if (barcode) {
                if ($(".lastcheckoutbarcode").length > 0) {
                    var barcodes = $(".lastcheckoutbarcode")
                        .map(function () {
                            return $(this).text();
                        })
                        .get();
                    ...
                    if (
                        $(
                            "#circ_circulation #circ_circulation_issue #barcode:enabled"
                        ).length &&
                        ...
                        RFIDWebService.UpdateActionStatusOfItem(
                            barcode,
                            "RFIDWebServiceCheckedOutItems",
                            "failed",
                            "checkout"
                        ...
                        if ($("#circ_impossible").length >= 1) {
                            ...
                            RFIDWebService.DisplayRemainingItemBarcodes(
                                "checkout"
                            );
                        }
                    }
        ...
        CheckLastCheckinAndContinue: function () {
            ...
            if (barcode) {
                if ($(".lastcheckinbarcode").length > 0) {
                    var barcodes = $(".lastcheckinbarcode")
                        .map(function () {
                            return $(this).text();
                        })
                        .get();
                ...
                let itemCount = window.sessionStorage.getItem(
                    "RFIDWebServiceCheckinItemCount"
                );
                let lastCheckinItems = [];
                let lastCheckinTxt = window.sessionStorage.getItem(
                    "RFIDWebServiceCheckedInItems"
                );
                if (lastCheckinTxt) {
                ...
                if (
                    ...
                    lastCheckinItems.length > 0
                ) {
                    RFIDWebService.DisplayRemainingItemBarcodes("checkin");
                    if (
                        $(RFIDWebService.checkinBlockingSelector).length < 1 &&
                        itemCount > 0
                    ) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.itemResult && data.itemResult.length == 1) {
                        if (
                            data.itemResult[0].requestSuccess &&
                            data.itemResult[0].requestSuccess == true
                        ) {
                            RFIDWebService.UpdateActionStatusOfItem(
                                barcode,
                                "RFIDWebServiceCheckedOutItems",
                            ...
                            var itemCount = window.sessionStorage.getItem(
                                "RFIDWebServiceCheckoutItemCount"
                            );

                            if (
                                $(RFIDWebService.checkoutBlockingSelector)
                                    .length < 1
                            ) {
                        ...
                        if (
                            data.itemResult[0].requestSuccess &&
                            data.itemResult[0].requestSuccess == false
                        ) {
                            RFIDWebService.UpdateActionStatusOfItem(
                                barcode,
                                "RFIDWebServiceCheckedOutItems",
                            ...
                            if (
                                data.itemResult[0].errorCode &&
                                data.itemResult[0].errorMessage
                            ) {
                                RFIDWebService.DisplayRFIDServiceErrorMessage(
                                    data.itemResult[0].errorCode,
                                    data.itemResult[0].errorMessage,
                                ...
                                console.log(
                                    "RFIDWebService CheckoutItems returns error " +
                                        data.itemResult[0].errorCode +
                                        ": " +
                                        data.itemResult[0].errorMessage
                                );
                                ...
                                console.log(
                                    "RFIDWebService CheckoutItems returned error for barcode " +
                                        barcode,
                                    data
                                );
                            }
                        ...
                        console.log(
                            "RFID CheckoutItems request returned without a result for barcode " +
                                barcode
                        );
                    }
                },
                error: function (data) {
                    console.log(
                        "RFIDWebService CheckoutItems for barcode" +
                            barcode +
                            " calling error " +
                            data
                    );
        ...
        LockItemBarcode: function (barcode) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.itemResult && data.itemResult.length == 1) {
                        if (
                            ...
                            if (
                                $(RFIDWebService.checkinBlockingSelector)
                                    .length < 1
                            ) {
                                if (itemCount && itemCount >= 1) {
                                    RFIDWebService.CheckinNextItem("checkin");
                                ...
                                $("#hold-found2,#hold-found1").on(
                                    ...
                                    function (e) {
                                        ...
                                        if (itemCount && itemCount > 0) {
                                            RFIDWebService.CheckinNextItem(
                                                "checkin"
                                            );
                                        }
                                    }
                        ...
                        if (
                            ...
                            if (
                                ...
                                console.log(
                                    "RFIDWebService CheckinItems returns error " +
                                        data.itemResult[0].errorCode +
                                        ": " +
                                        data.itemResult[0].errorMessage
                                );
                        ...
                        console.log(
                            "RFID CheckinItems request returned without a result for barcode " +
                                barcode
                        );
                    }
                },
                error: function (data) {
                    console.log(
                        "RFIDWebService CheckinItems calling error " + data
                    );
                },
            });
        },

        UpdateActionStatusOfItem: function (
            barcode,
            itemList,
            resultStatus,
            context
                    ...
                    success: function (data) {
                        if (data && data.requestSuccess) {
                            if (data.requestSuccess == false) {
                                ...
                                for (
                                    var i = 0;
                                    i < data.responseData.items.length;
                                    i++
                                ) {
                                    var barcode =
                                        data.responseData.items[i].itemID;
                                    var deleted =
                                        data.responseData.items[i].deleted;
                                    var isUserCard =
                                        data.responseData.items[i].isUserCard;
                                    var numberOfParts =
                                        data.responseData.items[i]
                                            .numberOfParts;
                                    var partNumber =
                                        data.responseData.items[i].partNumber;
                                    var checkoutAllowed =
                                        data.responseData.items[i]
                                            .checkoutAllowed;
                                    var checkedOut =
                                        data.responseData.items[i].checkedOut;
                                    var errorCode =
                                        data.responseData.items[i].errorCode;
                                    ...
                                    if (
                                        barcode &&
                                        items.includes(barcode, 0) == false
                                    ) {
                                        if (
                                            context == "checkout" &&
                                            checkoutAllowed &&
                                ...
                                if (
                                    !context.startsWith("checkin") &&
                                    checkoutNotAllowedItems.length >= 1
                                ) {
                                    var msg =
                                        checkoutNotAllowedItems.join(", ");
                                    RFIDWebService.DisplayRFIDServiceErrorMessage(
                                        "CHECKOUT_NOT_ALLOWED_ITEMS",
                                        msg
                                    );
                                    window.sessionStorage.setItem(
                                        "RFIDWebServiceJustCalling",
        ...
        StartRFIDServerCheckin: function () {
            ...
            var busy = window.sessionStorage.getItem(
                "RFIDWebServiceJustCalling"
            );
        ...
        SwitchPatron: function (barcode) {
            location.replace(
                "/cgi-bin/koha/circ/circulation.pl?findborrower=" +
        ...
        CheckoutNextItem: function () {
            ...
            if (itemCount > 0) {
                var items = JSON.parse(
                    window.sessionStorage.getItem("RFIDWebServiceCheckoutItems")
                );
        ...
        CheckinNextItem: function (context) {
            ...
            if (itemCount > 0) {
                var items = JSON.parse(
                    window.sessionStorage.getItem("RFIDWebServiceCheckinItems")
                );
        ...
        DisplayRemainingItemBarcodes: function (context) {
            if (context == "checkin") {
                let statusText =
                    '<div class="rfid-batch-media" style="display: block; border-bottom: 0.8em solid #eee; background: #fff; padding: 1em 1em 1em;">' +
                ...
                for (let i = 0; i < processedItems.length; i++) {
                    ...
                    statusText +=
                        '<span class="rfid-batch-media-next">' +
                        processedItems[i].media;
                    if (processedItems[i].status == "success") {
                        statusText +=
                            ' <i class="fa fa-check" aria-hidden="true" style="color: forestgreen"></i><span>';
                    } else {
                        statusText +=
                            ' <i class="fa fa-ban" aria-hidden="true" style="color: mediumvioletred"></i><span>';
                    }
                ...
                if (barcode) {
                    ...
                    statusText +=
                        '<span class="rfid-batch-media-next">' + barcode;
                    statusText +=
                ...
                if (itemCount > 0) {
                    ...
                    statusText +=
                        '<span class="rfid-batch-media-next">' +
                        items.join(
                            ' <i class="fa fa-hourglass-half" aria-hidden="true"></i></span> | <span class="rfid-batch-media-next">'
                        ) +
                ...
                let statusText =
                    '<div class="rfid-batch-media" style="display: block; border-top: 2px solid #eee; background: #fff; margin-top: 1em; padding: 1em 1em 1em;">' +
                    '<span class="rfid-batch-media-label">' +
                    this.messageServiceProvider.GetText("mediabatch") +
                    "</span>";
        ...
        DisplayRFIDServiceErrorMessage: function (
            errorCode,
            errorMessage,
            errorData
        ) {
            ...
            if (this.messageServiceProvider) {
                ...
                messages =
                    this.messageServiceProvider.GetRFIDErrorDialogNames();
                console.log(
                    "vendor: " +
                        vendor +
                        ";errorCode: " +
                ...
                text = this.messageServiceProvider.GetErrorMessage(
                    vendor,
                    errorCode,
                    errorMessage,
                    errorData
        ...
        DisplayRFIDServiceStatus: function () {
            ...
            if (this.messageServiceProvider) {
                messages =
                    this.messageServiceProvider.GetRFIDStatusDialogNames();
            }
            text = "";
            var data = JSON.parse(
                window.sessionStorage.getItem("RFIDWebServiceStatusInfo")
            );
            if (data) {
                text += "<table>";
                if (data.requestSuccess && data.requestSuccess == true) {
                    ...
                    if (data.responseData) {
                        ...
                        if (data.responseData.readerModel) {
                            text +=
                                "<tr><td>" +
                                messages.readerModel +
                                ": </td><td>" +
                                data.responseData.readerModel +
                                "</td></tr>";
                        }
                        if (data.responseData.serviceVersion) {
                            text +=
                                ...
                                ": </td><td>" +
                                data.responseData.serviceVersion +
                                "</td></tr>";
                        }
                        ...
                        if (data.responseData.libraryId) {
                            text +=
                                ...
                                messages.libraryId +
                                ": </td><td>" +
                                data.responseData.libraryId +
                                "</td></tr>";
                        }
                        if (data.responseData.backwardCompatibility) {
                            text +=
                                "<tr><td>" +
                                messages.backwardCompatibility +
                                ": </td><td>" +
                                data.responseData.backwardCompatibility +
                                "</td></tr>";
                        }
                    }
                } else {
