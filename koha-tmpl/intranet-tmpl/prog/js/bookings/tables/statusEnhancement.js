 ...
 */
export function enhanceStatusFilter(
    dataTable,
    tableElement,
    additionalFilters,
    filterManager
    ...
    if (
        ...
        statusDropdown.append(
            `<option value="active-pending">${__("Active")} ${__("and")} ${__("Pending")}</option>`
        );
    ...
    if (!filterManager.statusFilterFunction) {
        ...
        filterManager.statusFilterFunction = function () {
            const selectedValue =
                getSelectValueString(statusDropdown) ||
        ...
        if (
            ...
            /** @type {any} */ (additionalFilters)["-and"] =
                filterManager.statusFilterFunction;
        }
    }
