...
*/
function getIssuingStatsForBiblios(
    biblionumbers,
    yearscount,
    ignoredItypes,
    textEntry,
            ...
            $(".issuingstats").each(function (index) {
                ...
                for (var i = 0; i < itemnumbers.length; i++) {
                    if (itemstats[itemnumbers[i]]) {
                        found++;
                        sumIssues += parseInt(
                            itemstats[itemnumbers[i]].sumIssues
                        );
                        if (i == 0) {
                            combinedstats =
                                itemstats[itemnumbers[i]].issuestats;
                        } else {
                            for (
                                var k = 0;
                                k < itemstats[itemnumbers[i]].issuestats.length;
                                k++
                            ) {
                                var year =
                                    "" +
                                    itemstats[itemnumbers[i]].issuestats[k]
                                        .year;
                                var sumIssuesYear = parseInt(
                                    itemstats[itemnumbers[i]].issuestats[k]
                                        .sumIssues
                                );
                                var statfound = false;
                ...
                if (found > 0) {
                    ...
                    var displayText =
                        textEntry + ": " + sumIssues + " (" + textTotal + ")";
                    for (stat of combinedstats) {
                        displayText +=
                            ", " + stat.sumIssues + " (" + stat.year + ")";
                    }
