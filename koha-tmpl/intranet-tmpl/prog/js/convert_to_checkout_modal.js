(() => {
    ...
    function handleShowBsModal(e) {
        ...
        document.getElementById("convert_borrowernumber").value =
            borrowernumber;
        document.getElementById("convert_barcode").value = barcode;
        document.getElementById("convert_duedatespec").value =
            duedatespec || "";

        document.getElementById("convert_title").textContent = title || "";
        document.getElementById("convert_patron").textContent = patron || "";
        document.getElementById("convert_cardnumber").textContent = cardnumber
            ? `(${cardnumber})`
            : "";
        document.getElementById("convert_barcode_display").textContent =
            barcode || "";
        document.getElementById("convert_itemid").textContent = itemid
            ? `(${itemid})`
            : "";

        const formattedDueDate =
            duedatespec && typeof $date === "function"
                ? $date(duedatespec)
                : duedatespec || "";
        document.getElementById("convert_due_date").textContent =
            formattedDueDate;
