var CoverFlowByQuery = (function () {
    ...
    function CoverFlowByQuery({ createLcfInstance, removeChildNodes, loaded }) {
        ...
        const lmsCoverFlowByQuery = document.querySelectorAll(
            ".lmscoverflow-byquery"
        );
        ...
        const loadNewShelfBrowserItems = async (
            portionData,
            buttonDirection
        ) => {
            ...
            if (buttonDirection === "left" && offset > 0) {
                const resultPrevious = fetchItemData(
                    "/cgi-bin/koha/svc/coverflowbyquery?query=" +
                        encodeURIComponent(query),
                    offset - 10,
                    10
                );
                resultPrevious.then(result =>
                    extendCurrentCoverFlow(result, true, buttonDirection)
                );
            } else if (
                buttonDirection === "right" &&
                totalCount > offset + countItems
            ) {
        ...
        const LMSCoverFlowByQuery = () => {
            lmsCoverFlowByQuery.forEach(node => {
                node.addEventListener("click", e => {
                    ...
                    const result = fetchItemData(
                        "/cgi-bin/koha/svc/coverflowbyquery?query=",
                        0,
                        10
                    );
                    result.then(result => {
                        const shelfBrowserLoaded =
