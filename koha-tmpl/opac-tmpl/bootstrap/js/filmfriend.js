    ...
    if (facetData.length > 0) {
        ...
        for (var i = 0; i < facetData.length; i++) {
            if (facetData[i].searchType) {
                ...
                var facetElementName = setFilmfriendCollectionName(
                    facetData,
                    i
                );
                hrefElement.setAttribute(
                    "href",
                    "javascript:getFilmfriendResult(" + i + ",0)"
                );
...
function showFilmfriendResult(facetID) {
    ...
    $(".encyclopediaprovider").html(
        ' <a class="external-offer-link" href="' +
            filmfriendData.results[facetID].searchAtFilmFriend +
            '"  target="' +
            targetLinkFilmfriend +
            '">' +
            "filmfriend</a> "
    );
    $(".encyclopediasearchhitcount").html(
        " " + filmfriendData.results[facetID].numFound + " "
    );
    ...
    $("#showCatalogHitList").attr(
        "href",
        "javascript:showCatalogHitListFilmfriend()"
    );
    ...
    const lmse = new LMSEllipsis({
        ...
        lines: 2,
        explanations: {
            collapsed: readMoreFilmfriend,
            expanded: readLessFilmfriend,
        },
    });
    lmse.truncate();
}
function getFilmfriendPagination(facetID, maxHitCount) {
    ...
    if (prev_page_offset > 0 && last_page > 2) {
        paginationText +=
            '<li class="page-item"><a href="javascript:getFilmfriendResult(' +
            facetID +
            "," +
            prev_page_offset +
            ')" class="page-link">' +
            prevPageTextFilmfriend +
            "</a></li>";
    }
    if (current_page > 1) {
        paginationText +=
...
function generateFilmfriendEntryPerson(facetID, entryID) {
    var name =
        filmfriendData.results[facetID].hitList[entryID].firstName +
        " " +
        filmfriendData.results[facetID].hitList[entryID].lastName;
    ...
    */
    ...
    txtElement.setAttribute(
        "href",
        filmfriendData.results[facetID].hitList[entryID].filmfriendLink
    );
    ...
    if (filmfriendData.results[facetID].hitList[entryID].description) {
        ...
        txtElement.setAttribute(
            "class",
            "results_summary summary description truncable-txt"
        );
        txtElement.setAttribute("style", "font-size: 100%");
        txtElement.innerHTML =
            filmfriendData.results[facetID].hitList[entryID].description;
        colElement.appendChild(txtElement);
    }
    rowElement.appendChild(colElement);
...
function generateFilmfriendEntryMovie(facetID, entryID) {
    ...
    if (filmfriendData.results[facetID].hitList[entryID].thumbnail) {
        ...
        linkElement.setAttribute(
            "alt",
            filmfriendData.results[facetID].hitList[entryID].title
        );
        linkElement.setAttribute(
            "title",
            filmfriendData.results[facetID].hitList[entryID].title
        ...
        imageElement.setAttribute(
            "src",
            filmfriendData.results[facetID].hitList[entryID].thumbnail
        );
    ...
    colElement.appendChild(txtElement);
    if (
        (filmfriendData.results[facetID].hitList[entryID].regie &&
            filmfriendData.results[facetID].hitList[entryID].regie.length >
                0) ||
        (filmfriendData.results[facetID].hitList[entryID].actors &&
            filmfriendData.results[facetID].hitList[entryID].actors.length > 0)
    ) {
        ...
        if (filmfriendData.results[facetID].hitList[entryID].actors) {
            for (
                var s = 0;
                s <
                filmfriendData.results[facetID].hitList[entryID].actors.length;
                s++
            ) {
                if (x++ > 0) {
                    var sep = document.createElement("span");
                ...
                personNameLink.setAttribute(
                    "href",
                    filmfriendData.results[facetID].hitList[entryID].actors[s]
                        .filmfriendLink
                );
                personNameLink.setAttribute("target", targetLinkFilmfriend);
                personNameLink.setAttribute("class", "external-offer-link");
    ...
    var addCnt = 0;
    if (
        filmfriendData.results[facetID].hitList[entryID].genre &&
        filmfriendData.results[facetID].hitList[entryID].genre.length > 0
    ) {
        if (addCnt > 0) {
            var sep = document.createElement("span");
            sep.appendChild(document.createTextNode(" | "));
        ...
        for (
            var s = 0;
            s < filmfriendData.results[facetID].hitList[entryID].genre.length;
            s++
        ) {
            ...
            elAdd.appendChild(
                document.createTextNode(
                    filmfriendData.results[facetID].hitList[entryID].genre[s]
                )
            );
            if (s > 0) {
    ...
    if (filmfriendData.results[facetID].hitList[entryID].releaseYear) {
        ...
        elAdd.appendChild(
            document.createTextNode(
                filmfriendData.results[facetID].hitList[entryID].releaseYear
            )
        );
    ...
    if (
        filmfriendData.results[facetID].hitList[entryID].production &&
        filmfriendData.results[facetID].hitList[entryID].production.length > 0
    ) {
        if (addCnt > 0) {
            var sep = document.createElement("span");
            sep.appendChild(document.createTextNode(" | "));
        ...
        for (
            var s = 0;
            s <
            filmfriendData.results[facetID].hitList[entryID].production.length;
            s++
        ) {
            var elAdd = document.createElement("span");
    ...
    if (filmfriendData.results[facetID].hitList[entryID].fsk) {
        ...
        elAdd.appendChild(
            document.createTextNode(
                "FSK " + filmfriendData.results[facetID].hitList[entryID].fsk
            )
        );
        additionalInfo.appendChild(elAdd);
        addCnt++;
    ...
    if (filmfriendData.results[facetID].hitList[entryID].duration) {
        ...
        elAdd.appendChild(
            document.createTextNode(
                filmfriendData.results[facetID].hitList[entryID].duration
            )
        );
        additionalInfo.appendChild(elAdd);
        addCnt++;
    ...
    if (
        filmfriendData.results[facetID].hitList[entryID].audio &&
        filmfriendData.results[facetID].hitList[entryID].audio.length > 0
    ) {
        if (addCnt > 0) {
            var sep = document.createElement("span");
            sep.appendChild(document.createTextNode(" | "));
        ...
        for (
            var s = 0;
            s < filmfriendData.results[facetID].hitList[entryID].audio.length;
            s++
        ) {
            var elAdd = document.createElement("span");
            ...
            elAdd.appendChild(
                document.createTextNode(
                    filmfriendData.results[facetID].hitList[entryID].audio[s]
                )
            );
            if (s > 0) {
                elList.appendChild(document.createTextNode(", "));
    ...
    if (
        filmfriendData.results[facetID].hitList[entryID].subtitle &&
        filmfriendData.results[facetID].hitList[entryID].subtitle.length > 0
    ) {
        if (addCnt > 0) {
            var sep = document.createElement("span");
            sep.appendChild(document.createTextNode(" | "));
        ...
        for (
            var s = 0;
            s <
            filmfriendData.results[facetID].hitList[entryID].subtitle.length;
            s++
        ) {
            var elAdd = document.createElement("span");
            ...
            elAdd.appendChild(
                document.createTextNode(
                    filmfriendData.results[facetID].hitList[entryID].subtitle[s]
                )
            );
            if (s > 0) {
                elList.appendChild(document.createTextNode(", "));
                ...
                elList.appendChild(
                    document.createTextNode(subtitleLanguageFilmfriend)
                );
            }
            elList.appendChild(elAdd);
        }
    ...
    if (filmfriendData.results[facetID].hitList[entryID].synopsis) {
        ...
        txtElement.setAttribute(
            "class",
            "results_summary summary truncable-txt"
        );
        txtElement.setAttribute("style", "font-size: 100%");
        txtElement.innerHTML = filmfriendData.results[facetID].hitList[
            entryID
        ].synopsis.replace(/(?:\r\n|\r|\n)+/g, "<br>");
        colElement.appendChild(txtElement);
    }
