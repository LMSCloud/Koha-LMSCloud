    ...
    class LMSEllipsis {
        constructor(args) {
            ...
            this.init = () => {
                ...
                const triggerCSS =
                    ".lmsellipsis-trigger { cursor: pointer; color: #0174AD; } .lmsellipsis-trigger:hover { text-decoration: underline; }";
        ...
        static calculateElementProperties(element) {
            const [lineQuantity, lineHeight] =
                LMSEllipsis.calculateLineQuantity(element);
            const [elementHeight, elementWidth] =
                LMSEllipsis.calculateElementDimensions(element);
        ...
        static generateSubstringArray(element) {
            ...
            elementClone.setAttribute(
                "style",
                "hyphens: auto; width: auto; position: absolute; white-space: wrap;"
            );
        ...
        static isCollapsed(element) {
            ...
            for (let idx = 0; idx < element.childNodes.length; idx += 1) {
                if (
                    element.childNodes[idx].className === "lmsellipsis-postfix"
                ) {
                    isCollapsed = true;
                    break;
                }
        ...
        fitSubstrings(args) {
            const { substrings, element, elementWidth, explanations } = args;
            ...
            for (let idx = 0; idx < substrings.length; idx += 1) {
                if (linesToProcess > 1) {
                    ruler.innerHTML = LMSEllipsis.buildStringFromArr(
                        substrings.slice(previousLineLastIdx, idx)
                    );
                }
                if (linesToProcess === 1) {
                    ruler.innerHTML = LMSEllipsis.buildStringFromArr(
                ...
                if (linesToProcess === 0) {
                    fittedString = LMSEllipsis.buildStringFromArr(
                        substrings.slice(0, idx - 1)
                    );
                    break;
                }
                if (ruler.scrollWidth + previousLineLeftSpace > elementWidth) {
                    while (
                        ruler.scrollWidth + previousLineLeftSpace >
                        elementWidth
                    ) {
                        ruler.innerHTML = LMSEllipsis.buildStringFromArr(
                            substrings.slice(previousLineLastIdx, (idx -= 1))
                        );
                    }
                    linesToProcess -= 1;
        ...
        truncate() {
            const truncate = (element, tags) => {
                ...
                const { elementWidth, lineQuantity } =
            ...
            const hide = (element, tags) => {
                ...
                toggler.addEventListener("keypress", e => {
                    if (e.code === "Enter" || e.code === "Space") {
                        handleInteraction();
                    }
                });
