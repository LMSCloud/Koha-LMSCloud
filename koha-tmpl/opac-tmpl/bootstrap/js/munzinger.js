    ...
    $.ajax({
        ...
        success: function (data) {
            if (
                ...
                munzingerData.results.categories[facetID] =
...
function showMunzingerFacetEntries(facetData, query) {
    ...
    for (var i = 0; i < facetData.categorycount; i++) {
        ...
        hrefElement.setAttribute(
            "href",
            "javascript:getMunzingerResult(" + i + ",0)"
        );
...
function showMunzingerResult(facetID) {
    ...
    for (
        var i = 0;
        i < munzingerData.results.categories[facetID].hits.length;
        i++
    ) {
    ...
    if (pagination.length == 0) {
        $("#encyclopediaheader").html(
            '<strong><span class="encyclopediasource"></span></strong> <span class="encyclopediasourceaction"></span>'
        );
    } else {
        $("#encyclopediaheader").html(
            '<div class="container-fluid"><div class="row"><div class="col-sm-6"><strong><span class="encyclopediasource"></span> <span class="encyclopediasourceaction"></span></strong></div><div class="col-sm-6">' +
                pagination +
                "</div></div></div>"
        );
    }
    $(".encyclopediasource").html(
        munzingerData.results.categories[facetID].name
    );
    if (canShowMunzingerHitWords > 0) {
        $(".encyclopediasourceaction").html(
            '<a href="#" onclick="try{$(\'.munzingerHitFoundDisplay\').toggle()}catch(e){}return false" title="' +
                munzingerHitShowHide +
    ...
    $(".encyclopediaprovider").html(
        ' <a class="external-offer-link" href="' +
            munzingerData.results.searchmunzinger +
            '" target="_blank">' +
            "Munzinger</a> "
    );
    $(".encyclopediasearchhitcount").html(
        " " + munzingerData.results.categories[facetID].count + " "
    );
    ...
    $("#showCatalogHitList").attr(
        "href",
        "javascript:showCatalogHitListMunzinger()"
    );
}
...
function getMunzingerPagination(facetID, maxHitCount) {
    ...
    paginationText =
        '<div id="top-pages" class="right-align"><nav class="pagination pagination-sm noprint"><ul class="pagination">';
    ...
    if (prev_page_offset > 0 && last_page > 2) {
        paginationText +=
            '<li class="page-item"><a href="javascript:getMunzingerResult(' +
            facetID +
            "," +
            prev_page_offset +
            ')" class="page-link">' +
            prevPageTextMunzinger +
            "</a></li>";
    }
    if (current_page > 1) {
        paginationText +=
...
function generateMunzingerEntry(facetID, entryID) {
    ...
    if (munzingerData.results.categories[facetID].hits[entryID].date) {
        dateElement.textContent =
            munzingerData.results.categories[facetID].hits[entryID].date + " ";
    }
    ...
    txtElement.setAttribute(
        "href",
        munzingerData.results.categories[facetID].hits[entryID].link
    );
    if (
        munzingerData.results.categories[facetID].hits[entryID].title.length > 0
    ) {
        txtElement.textContent =
            munzingerData.results.categories[facetID].hits[entryID].title;
    }
    ...
    txtElement.innerHTML =
        munzingerData.results.categories[facetID].hits[entryID].text;
    txtDivElement.appendChild(txtElement);
    colElement.appendChild(txtDivElement);
    if (
        munzingerData.results.categories[facetID].hits[entryID].words.length >
            0 &&
        ...
        hitTxtElement.setAttribute(
            "class",
            "results_summary munzingerHitFoundDisplayName"
        );
        hitTxtElement.innerHTML =
            munzingerHitFieldname +
            munzingerData.results.categories[facetID].hits[entryID].words;
