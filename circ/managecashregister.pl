...
if ( $op eq 'open' && $status eq 'open' && $cash_register_id ) {
    ...
    ( $bookingstats, $journalfrom, $journalto ) =
        $cash_management->getCashTransactionOverviewByBranch( $branch, $journalfrom, $journalto );
    ( $finesstats, $journalfrom, $journalto ) = $cash_management->getCashRegisterPaymentAndDepositOverview(
        $cash_register->{cash_register_id}, $journalfrom,
        $journalto
    );
    ...
    ( $cashregstat, $journalfrom, $journalto ) =
        $cash_management->getCashRegisterPaymentAndDepositOverview( $cashreg, $journalfrom, $journalto );
...
if ($cash_register) {
    ...
    $cash_register_info =
        $cash_management->getCashRegisterHandoverInformationByLastOpeningAction( $cash_register->{cash_register_id} );
    if ( $cash_register->{cash_register_branchcode} ne $branch
        && !$cash_register->{cash_register_no_branch_restriction} )
    {
        $wrongBranch = 1;
    }
    if ( $status eq 'manage' && $manageaction eq 'journal' ) {
        ...
        if ($allFromOpening) {
            ...
            ( $trans, $journalfrom, $journalto ) =
                $cash_management->getLastBookingsFromTo( $cash_register->{cash_register_id}, $journalfrom, $journalto );
        }
        @transactions = @$trans;
    }
}
