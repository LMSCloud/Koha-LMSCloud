...
my $updatecharges_permissions =
      ( $writeoff_individual || $type eq 'WRITEOFF' )     ? 'writeoff'
    : ( $cancel_individual   || $type eq 'CANCELLATION' ) ? 'cancel_fee'
...
if ( $total_paid and $total_paid ne '0.00' ) {
    ...
    } elsif ( $pay_individual && !$checkCashRegisterOk ) {

        # the cash register was closed between rendering the form and the
        # submit; re-render, checkCashRegisterFailed hides the payment form
        $template->param( total_paid => $total_paid );
    } else {
        my $url;
        my $pay_result;
