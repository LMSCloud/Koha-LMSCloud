...
subtest "AddReturn logging on statistics table (item-level_itypes=1)" => sub {
    ...
    $record->append_fields( MARC::Field->new( $tagfield, '', '', $tagsubfield => $blevel_itemtype ) );
...
subtest 'Handle ids duplication' => sub {
    ...
    my $account_lines =
        Koha::Account::Lines->search( { borrowernumber => $patron->borrowernumber, issue_id => $issue_id } );
    ...
    is(
        $account_lines->count, 0,
        'No account lines should exist for this issue_id, patron should not have been charged'
    );
