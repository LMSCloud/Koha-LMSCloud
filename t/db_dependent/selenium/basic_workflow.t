...
SKIP: {
    ...
    $driver->get(
        $base_url . '/members/memberentry.pl?op=add_form&amp;categorycode=' . $sample_data->{category}{categorycode} );
    ...
    $borrowernumber = $dbh->selectcol_arrayref(
        q|SELECT borrowernumber FROM borrowers WHERE userid=?|, {},
        $sample_data->{patron}{userid}
    )->[0];
    ...
    for my $biblionumber (@biblionumbers) {
        ...
        my $inputs = $driver->find_child_elements(
            $form,
            '/.//*[not(self::node()[contains(@style,"display:none")])]/*[@type="text"]'
        );
        for my $input (@$inputs) {
            ...
            if (
                $id =~ m|^tag_952_subfield_g|       # price
                or $id =~ m|^tag_952_subfield_v|
                )                                   # replacementprice
            {
                $v = '42';                          # It's a price
            } elsif (
    ...
    $driver->find_element('//form[@id="hold-request-form"]//input[@type="radio"]')
        ->click;    # Item level, there is only 1 item per bib so we are safe
...
sub cleanup {
    ...
    for my $i ( 1 .. $number_of_biblios_to_insert ) {
        $dbh->do(
            qq|DELETE items, biblio FROM biblio INNER JOIN items ON biblio.biblionumber = items.biblionumber WHERE biblio.title = "test biblio$i"|
        );
    }
    ...
    $dbh->do(
        q|DELETE FROM circulation_rules WHERE categorycode=? AND itemtype=? AND branchcode=?|, undef,
        $sample_data->{issuingrule}{categorycode}, $sample_data->{issuingrule}{itemtype},
        $sample_data->{issuingrule}{branchcode}
    );
}
