...
subtest 'Test merge A1 to A2 (within same authtype)' => sub {
    ...
    my $rv = C4::AuthoritiesMarc::merge(
        { mergefrom => $authid2, MARCfrom => $auth2, mergeto => $authid1, MARCto => $auth1 } );
...
subtest 'Test merge A1 to modified A1, test strict mode' => sub {
    ...
    $MARC1->append_fields(
        MARC::Field->new( '609', '', '', 'a' => 'Bruce Lee', 'b' => 'Should be cleared too', '9' => $authid1 ) );
    $MARC1->append_fields(
        MARC::Field->new(
            '609', '', '', 'a' => 'Bruce Lee', 'c' => 'This is a duplicate to be removed in strict mode',
            '9' => $authid1
        )
    );
    ...
    t::lib::Mocks::mock_preference( 'AuthorityMergeMode', 'loose' );
    my $rv = C4::AuthoritiesMarc::merge(
        { mergefrom => $authid1, MARCfrom => $auth1old, mergeto => $authid1, MARCto => $auth1new } );
    ...
    is(
        $biblio1->field(109)->subfield('b'), $MARC1->field(109)->subfield('b'),
        'Subfield not overwritten in loose mode'
    );
...
subtest 'Test merge A1 to B1 (changing authtype)' => sub {
    ...
    my $retval = C4::AuthoritiesMarc::merge(
        { mergefrom => $authid1, MARCfrom => $auth1, mergeto => $authid2, MARCto => $auth2 } );
...
subtest "Test some specific postponed merge issues" => sub {
    ...
    merge(
        {
            mergefrom     => $id + 1, MARCfrom => $oldauthmarc, mergeto => $id, MARCto => $authmarc,
    ...
    $biblio = Koha::Biblios->find($biblionumber)->metadata->record;
    is(
        $biblio->subfield( '109', '9' ), $id,
        'If the 109 is no longer present, another modify merge would not bring it back'
    );
...
subtest "Graceful resolution of missing reporting tag" => sub {
    ...
    merge(
        {
            mergefrom     => $id1, MARCfrom => $authmarc, mergeto => $id2, MARCto => $authmarc,
            biblionumbers => [$biblionumber]
        }
    );
    $biblio = Koha::Biblios->find($biblionumber)->metadata->record;
...
subtest 'merge should not reorder too much' => sub {
    ...
    my $biblio = MARC::Record->new;
    $biblio->append_fields(
        MARC::Field->new( '109', '', '', 9 => $id, i => 'in front', a => 'aa', c => 'after controlled block' )
        ,    # this field shows the old situation when $9 is the first subfield
        MARC::Field->new( '609', '', '', i => 'in front', a => 'aa', c => 'after controlled block', 9 => $id )
        ,    # here $9 is already the last one
    );
    my ($biblionumber) = C4::Biblio::AddBiblio( $biblio, '' );
    ...
    merge(
        {
            mergefrom     => $id, MARCfrom => $authmarc, mergeto => $id, MARCto => $authmarc,
            biblionumbers => [$biblionumber]
        }
    );
...
subtest 'Test how merge handles controlled indicators' => sub {
    ...
    $authmarc->append_fields(
        MARC::Field->new( '008', ( ' ' x 11 ) . 'r' ),    # thesaurus code
    ...
    # Test $2 removal now
    $authmarc->field('008')->update( ( ' ' x 11 ) . 'a' );    # LOC, no $2 needed
