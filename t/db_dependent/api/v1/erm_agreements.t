...
subtest 'list() tests' => sub {
    ...
    $t->get_ok("//$userid:$password@/api/v1/erm/agreements")->status_is(200)->json_is( [] );
    ...
    $t->get_ok("//$userid:$password@/api/v1/erm/agreements")->status_is(200)->json_is( [ $agreement->to_api ] );
    ...
    $t->get_ok("//$userid:$password@/api/v1/erm/agreements?max_expiration_date=19-03-2021")->status_is(400)->json_is(
        "/errors" => [
            {
    ...
    # Unauthorized access
    $t->get_ok("//$unauth_userid:$password@/api/v1/erm/agreements")->status_is(403);
...
subtest 'get() tests' => sub {
    ...
    $t->get_ok( "//$userid:$password@/api/v1/erm/agreements/"
            . $agreement->agreement_id => { 'x-koha-embed' => 'periods,user_roles,agreement_licenses' } )
        ->status_is(200)
    ...
    # Unauthorized access
    $t->get_ok( "//$unauth_userid:$password@/api/v1/erm/agreements/" . $agreement->agreement_id )->status_is(403);

    # Attempt to get non-existent agreement
    my $agreement_to_delete = $builder->build_object( { class => 'Koha::ERM::Agreements' } );
...
subtest 'add() tests' => sub {
    ...
    my $agreement = {
        ...
        is_perpetual     => Mojo::JSON->true,
        renewal_priority => undef,
        license_info     => "Agreement license_info",
    };

    # Unauthorized attempt to write
    $t->post_ok( "//$unauth_userid:$password@/api/v1/erm/agreements" => json => $agreement )->status_is(403);
...
subtest 'update() tests' => sub {
    ...
    my $agreement_id = $builder->build_object( { class => 'Koha::ERM::Agreements' } )->agreement_id;

    # Unauthorized attempt to update
    $t->put_ok( "//$unauth_userid:$password@/api/v1/erm/agreements/$agreement_id" => json =>
    ...
    $t->put_ok( "//$userid:$password@/api/v1/erm/agreements/$non_existent_id" => json => $agreement_with_updated_field )
        ->status_is(404);

    # Wrong method (POST)
    $agreement_with_updated_field->{agreement_id} = 2;

    $t->post_ok( "//$userid:$password@/api/v1/erm/agreements/$agreement_id" => json => $agreement_with_updated_field )
        ->status_is(404);
