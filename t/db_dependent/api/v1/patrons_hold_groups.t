...
subtest 'add() tests' => sub {
    ...
    my $nonexistent_hold_id = Koha::Holds->search( {}, { order_by => { -desc => 'reserve_id' }, rows => 1 } )->next;
    $nonexistent_hold_id = $nonexistent_hold_id ? $nonexistent_hold_id->reserve_id + 1 : 999999;

    $t->post_ok( "//$userid:$password@/api/v1/patrons/"
            . $patron->id
            . "/hold_groups" => json => { hold_ids => [$nonexistent_hold_id] } )
        ->status_is( 400, 'REST3.2.1' )
        ->json_is(
        {
            error      => "One or more holds do not exist: $nonexistent_hold_id",
            error_code => "HoldDoesNotExist",
            hold_ids   => [$nonexistent_hold_id]
        }
        );
