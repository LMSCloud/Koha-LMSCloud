...
if ($do_it) {
    ...
    for my $thisccode (@$ccodes) {
        my %row = (
            value       => $thisccode->{authorised_value},
    ...
    for my $thisloc (@$shelvinglocs) {
        my %row = (
            value       => $thisloc->{authorised_value},
    ...
    my $patron_categories =
        Koha::Patron::Categories->search_with_library_limits( {}, { order_by => ['categorycode'] } );
...
sub calculate {
    ...
    for ( $i = 1 ; $i <= $line ; $i++ ) {
        ...
        foreach my $col (@loopcol) {
            ...
            if (@loopcol) {
                $value = $table[$i]->{
                    ( ( $col->{coltitle} eq "NULL" ) or ( $col->{coltitle} eq "Global" ) )
                    ? "zzEMPTY"
                    : $col->{coltitle}
                }->{'name'};
                $count = $table[$i]->{
